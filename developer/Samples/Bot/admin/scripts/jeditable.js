(function($)
  {
    $.fn.editable = function(target, options) {
      ...
      var onerror = settings.onerror||reset; if(settings.tooltip) {
      ...
      settings.autowidth = 'auto'==settings.width; settings.autoheight = 'auto'==settings.height; return this.each(function()
        {
          ...
          var savedheight = $(self).height(); $(this).data('event.editable', settings.event); if(!$.trim($(this).html())) {
          ...
          $(this).bind(settings.event, function(e)
            {
              ...
              $(self).html('');
              var form = $('<form />'); if(settings.cssclass) {
              ...
              var input_content; if(settings.loadurl) {
                ...
                loaddata[settings.id] = self.id; if($.isFunction(settings.loaddata)) {
                ...
                input_content = decodeHtml(self.revert);
              }
                  ...
                  e.preventDefault(); if(false!==onsubmit.apply(form,[settings, self])) {
                    if(false!==submit.apply(form,[settings, self])) {
                      if($.isFunction(settings.target)) {
                        ...
                        submitdata[settings.name] = input.val(); submitdata[settings.id] = self.id; if($.isFunction(settings.submitdata)) {
    ...
    name: 'value', id: 'id', type: 'text', width: 'auto', height: 'auto', event: 'click.editable', onblur: 'cancel', loadtype: 'GET', loadtext: 'Loading...', placeholder: '', loaddata: {
      }
      , submitdata: {
      }
      , ajaxoptions: {
...
/*
   * function decodeHtml
   * This function replaces the HTML "less-than" and "greater-than" entities (&lt;. &gt;)
   * with "tag brackets" (e.g. < and > ), so that AIML tags can be edited.
   * No other decoding is attempted.
   *
   * @param text
   * @return (string) out
   */
...
/*
   * function encodeHtml
   * This function only "encodes" "tag brackets" (e.g. < and > ), so that AIML tags can be
   * displayed correctly. No other encoding is attempted.
   *
   * @param text
   * @return (string) out
   */
