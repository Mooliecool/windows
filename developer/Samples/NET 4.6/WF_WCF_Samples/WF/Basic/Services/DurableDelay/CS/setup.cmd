...
echo ------------------------------------------------------------------
echo   Preparing your system for the Durable Delay Sample
echo ------------------------------------------------------------------
echo.
...
sqlcmd -S localhost\SQLExpress -E -d DefaultSampleStore -i "%frameworkdir%%FrameworkVersion%\sql\en\SqlWorkflowInstanceStoreSchema.sql" -o setup-sqlcmd.log
If ERRORLEVEL 1 goto sqlerror

echo Creating Persistence Logic ...
sqlcmd -S localhost\SQLExpress -E -d DefaultSampleStore -i "%frameworkdir%%FrameworkVersion%\sql\en\SqlWorkflowInstanceStoreLogic.sql" -o setup-sqlcmd.log
If ERRORLEVEL 1 goto sqlerror
