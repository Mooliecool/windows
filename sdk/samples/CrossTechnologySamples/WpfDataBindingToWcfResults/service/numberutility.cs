...
namespace NumberUtilities {
  static class NumberConverter {
    ...
    public static string ConvertNumberToWords(double number) {
      ...
      if( negative ) { wholePart = 0 - wholePart; }
      ...
      while( wholePart != 0 ) {
        ...
        if( threeDigits > 99 ) {
          ...
          threeDigits %= 100;
          if( threeDigits != 0 ) { threeDigitsEnglish.Append(' '); }
        }
        ...
        if( threeDigitsEnglish.Length != 0 ) {
          ...
          if( threeDigitsEnglish.Length != 0 ) { threeDigitsEnglish.Append(' '); }
          threeDigitsEnglish.Append(_thousands[thousand]);
        }

        if( english.Length != 0 ) { threeDigitsEnglish.Append(", "); }
      ...
      for( int i = numberString.Length - 1; i != dot; --i ) {
        ...
        if( (digit == '0') && !includeZeros ) { continue; }
        includeZeros = true;
        if( decimalEnglish.Length != 0 ) { decimalEnglish.Insert(0, ' '); }
      ...
      if( negative ) { english.Insert(0, "negative "); }
