...
#Region "IWSTrust Members"
        Public Function Issue(ByVal request As Message) As Message Implements IWSTrust.Issue
            ...
            Try
                ...
                Dim rst As RequestSecurityToken = RequestSecurityToken.CreateFrom(request.GetReaderAtBodyContents())

                ' Check that is really is an Issue request
                If rst.RequestType Is Nothing OrElse rst.RequestType <> Constants.Trust.RequestTypes.Issue Then
                    Throw New InvalidOperationException(rst.RequestType)
                End If

                ' Create an RSTR object
                Dim rstr As RequestSecurityTokenResponse = Issue(rst)

                ' Create response message
                Dim response As Message = Message.CreateMessage(request.Version, Constants.Trust.Actions.IssueReply, rstr)
...
#End Region
        ...
        Private Function Issue(ByVal rst As RequestSecurityToken) As RequestSecurityTokenResponse
        ...
        Private Shared Function DetermineEndpointUri(ByVal rst As RequestSecurityToken) As Uri
            ...
            Return DirectCast(IIf(epr Is Nothing, Nothing, epr.Uri), Uri)

        End Function

#Region "Session key methods"

        Private Shared Function GetSenderEntropy(ByVal rst As RequestSecurityToken) As Byte()
        ...
        Private Shared Function GetSessionKey(ByVal rst As RequestSecurityToken, ByVal rstr As RequestSecurityTokenResponse) As Byte()
...
#End Region
        ...
        Private Shared Function DetermineEncryptingToken(ByVal rst As RequestSecurityToken) As SecurityToken
            ...
            Dim encryptingTokenSubjectName As String = CStr(IIf((uri Is Nothing), "localhost", uri.DnsSafeHost))
