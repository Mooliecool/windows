...
window.onload = function loadXMLHttp() {
    ...
    ShowFirstOption(ddlCountry); // Add "Please Select" option in the top of country DropDownList 
...
function BindDdlRegion() {
    if (xmlHttp.readyState == 4) {
        if (xmlHttp.status == 200) {
            ...
            addOption(ddlRegion, result);   // Add options to region dropdownlist basing on response 
...
function BindDdlCity() {
    if (xmlHttp.readyState == 4) {
        if (xmlHttp.status == 200) {
            var result = xmlHttp.responseText;
            addOption(ddlCity, result);   // Add options to city dropdownlist basing on response 
            EnableSubmitButton(false);  // Enable submit button
...
function addOption(selectbox, strResult) {
    ...
    ShowFirstOption(selectbox); // Add "Please Select" option in the top of country DropDownList 
}
...
// Add "Please Select" option in the top of country DropDownList 
function ShowFirstOption(selectbox) {
    var optnFirst = document.createElement("OPTION");
    optnFirst.text = "Please Select";
    optnFirst.value = "-1";
