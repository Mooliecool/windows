<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="Default.aspx.cs" Inherits="CSAzureWebRoleBackendProcessing.WebRole.Default" %>
<%@ Import Namespace="CSAzureWebRoleBackendProcessing.Common" %>
...
<asp:UpdatePanel ID="UpdatePanelResult" runat="server">
    <ContentTemplate>
        <asp:DataList ID="dlResult" runat="server">
            <ItemTemplate>
                <asp:Image ID="imgProcessing" runat="server" ImageUrl="~/Images/Processing.gif" Visible='<%# !((WordEntry)Container.DataItem).IsProcessed %>' />
                <asp:Image ID="imgDone" runat="server" ImageUrl="~/Images/Done.gif" Visible='<%# ((WordEntry)Container.DataItem).IsProcessed %>' />
                <%# Eval("Content") %><hr />
            </ItemTemplate>
        </asp:DataList>
    </ContentTemplate>
