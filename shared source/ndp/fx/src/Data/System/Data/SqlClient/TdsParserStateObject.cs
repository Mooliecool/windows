...
namespace System.Data.SqlClient {
    ...
    sealed internal class TdsParserStateObject {
        ...
        internal void Cancel(int objectID) {
            ...
            try {
                ...
                while ((!hasLock) && (_parser.State != TdsParserState.Closed) && (_parser.State != TdsParserState.Broken)) {
                    ...
                    if (hasLock) { // Lock for the time being - since we need to synchronize the attention send.
        ...
        internal bool Deactivate() {
            ...
            try {
                ...
                if (state != TdsParserState.Broken && state != TdsParserState.Closed) {
                    ...
                    if (HasOpenResult) { // SQL BU DT 383773 - need to decrement openResultCount for all pending operations.
        ...
        internal bool SetPacketSize(int size) {
            ...
            if (_inBuff == null || _inBuff.Length != size) { // We only check _inBuff, since two buffers should be consistent.
        ...
        internal bool TryReadPlpBytes(ref byte[] buff, int offst, int len, out int totalBytesRead) {
            ...
            while (bytesLeft > 0) {
                ...
                if (_longlenleft == 0) { // Read the next chunk or cleanup state if hit the end
                    if (!TryReadPlpLength(false, out ignored)) {
                        return false;
                    }
                }
        ...
        internal void ReadSniSyncOverAsync() {
            ...
            try {
                ...
                if (_parser.MARSOn) { // Only take reset lock on MARS and Async.
                    CheckSetResetConnectionState(error, CallbackType.Read);
                }

                if (TdsEnums.SNI_SUCCESS == error) { // Success - process results!
                    Debug.Assert(ADP.PtrZero != readPacket, "ReadNetworkPacket cannot be null in syncronous operation!");
        ...
        internal void ReadSni(TaskCompletionSource<object> completion) {
            ...
            // We must check after assigning _networkPacketTaskSource to avoid ----s with
            ...
            try {
                ...
                else if (TdsEnums.SNI_SUCCESS_IO_PENDING != error) { // FAILURE!
                    Debug.Assert(IntPtr.Zero == readPacket, "unexpected readPacket without corresponding SNIPacketRelease");
                    ReadSniError(this, error);
#if DEBUG
                ...
                else if (msecsRemaining == 0) { // Got IO Pending, but we have no time left to wait
        ...
        private void ReadSniError(TdsParserStateObject stateObj, UInt32 error) {
            ...
            if (TdsEnums.SNI_WAIT_TIMEOUT == error) {
                ...
                if (_internalTimeout) { // This is now our second timeout - time to give up.
                    fail = true;
                }
                else {                      
        ...
        public void ReadAsyncCallback(IntPtr key, IntPtr packet, UInt32 error) { // Key never used.
