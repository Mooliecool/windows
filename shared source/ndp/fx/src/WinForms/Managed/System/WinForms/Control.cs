...
#pragma warning restore 618
        ...
        private void SetParentHandle(IntPtr value) {
            ...
            if (IsHandleCreated) {
                ...
                if (parentHandle != value || (parentHandle == IntPtr.Zero && !topLevel)) {
                    ...
                    // See VSWhidbey 531685: The handle was previously parented to the parking window. Its TopLevel property was 
        ...
        private void WmGetObject(ref Message m) {
            ...
            if (ctrlAccessibleObject != null) {
                // SECREVIEW : Asserting here is safe, the interface is used to get the LRESULT from the accessible object
                //             into the Message.Result.
                //
                IntSecurity.UnmanagedCode.Assert();
                try {
            ...
            if (intAccessibleObject != null) {
                ...
                IntPtr punkAcc;
                try {
                    // Sanity check: For security reasons, it is 
                    ...
                    if (iacc == null) {
                        ...
                        // SECREVIEW : The following assert is required because the interop call below will call into an interface (punkAcc)
                        //             which demands UnmanagedCode permission.  We make sure we are calling the right into the right interface
                        //             (see Sanity-Check comment above) and the call is safe since we are just getting the LRESULT from the 
                        //             Acc interface.  So this assert is safe.
                        //             
                        IntSecurity.UnmanagedCode.Assert();
                        try {
        ...
        private void WmDestroy(ref Message m) {
            ...
            // Disabling the assert for M2 VSWhidbey 122139
            ...
            {
                // VSWhidbey 339087. Raise the MouseLeave event for the control below the mouse
