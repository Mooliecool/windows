...
inline int IsRidType(ULONG ix) {LEAF_CONTRACT;  return ix <= iRidMax; }
inline int IsCodedTokenType(ULONG ix) {LEAF_CONTRACT;  return (ix >= iCodedToken) && (ix <= iCodedTokenMax); }
inline int IsRidOrToken(ULONG ix) {LEAF_CONTRACT; return ix <= iCodedTokenMax; }
inline int IsHeapType(ULONG ix) {LEAF_CONTRACT;  return ix >= iSTRING; }
inline int IsFixedType(ULONG ix) {LEAF_CONTRACT;  return (ix < iSTRING) && (ix > iCodedTokenMax); }
#endif
