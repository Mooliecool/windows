...
void SlashesToDots(__inout __inout_z char* pStr, int iLen=-1);
void SlashesToDots(__inout __inout_z WCHAR* pStr, int iLen=-1);

// From current value to legacy.
void DotsToSlashes(__inout __inout_z char* pStr, int iLen=-1);
void DotsToSlashes(__inout __inout_z WCHAR* pStr, int iLen=-1);

#define SLASHES2DOTS_NAMESPACE_BUFFER_UTF8(fromptr, toptr) \
    long __l##fromptr = (fromptr) ? (long) (strlen(fromptr) + 1) : 0; \
...
#define SLASHES2DOTS_NAMESPACE_BUFFER_UNICODE(fromptr, toptr) \
    long __l##fromptr = (fromptr) ? ((long) wcslen(fromptr) + 1) : 0; \
    CQuickBytes __CQuickBytes##fromptr; \
    if ( __l##fromptr) { \
