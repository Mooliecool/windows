...
[System.Runtime.InteropServices.ComVisible(true)]
    public class ASCIIEncoding : Encoding
    {
        ...
        internal override unsafe int GetByteCount(char* chars, int charCount, EncoderNLS encoder)
        {
            ...
            BCLDebug.Assert(charCount >= 0, "[ASCIIEncoding.GetByteCount]count is negative");
            BCLDebug.Assert(chars != null, "[ASCIIEncoding.GetByteCount]chars is null");

            // Assert because we shouldn't be able to have a null encoder.
            BCLDebug.Assert(encoderFallback != null, "[ASCIIEncoding.GetByteCount]Attempting to use null fallback encoder");
...
//                    this.EncodingName, encoder.Fallback.GetType()));
            ...
            if (charLeftOver > 0)
            {
                BCLDebug.Assert(Char.IsHighSurrogate(charLeftOver), "[ASCIIEncoding.GetByteCount]leftover character should be high surrogate");
                BCLDebug.Assert(encoder != null, "[ASCIIEncoding.GetByteCount]Expected encoder");
        ...
        {
            // Just need to ASSERT, this is called by something else internal that checked parameters already
            BCLDebug.Assert(bytes != null, "[ASCIIEncoding.GetBytes]bytes is null");
            BCLDebug.Assert(byteCount >= 0, "[ASCIIEncoding.GetBytes]byteCount is negative");
            BCLDebug.Assert(chars != null, "[ASCIIEncoding.GetBytes]chars is null");
            BCLDebug.Assert(charCount >= 0, "[ASCIIEncoding.GetBytes]charCount is negative");

            // Assert because we shouldn't be able to have a null encoder.
            BCLDebug.Assert(encoderFallback != null, "[ASCIIEncoding.GetBytes]Attempting to use null encoder fallback");
...
//                        this.EncodingName, encoder.Fallback.GetType()));
                    ...
                    chars < charEnd)
            {
                ...
                if (bytes >= byteEnd)
                {
                    ...
                    {
                        BCLDebug.Assert(chars > charStart || bytes == byteStart,
        ...
        internal override unsafe int GetCharCount(byte* bytes, int count, DecoderNLS decoder)
        {
            ...
            BCLDebug.Assert(bytes != null, "[ASCIIEncoding.GetCharCount]bytes is null");
            BCLDebug.Assert(count >= 0, "[ASCIIEncoding.GetCharCount]byteCount is negative");
        ...
        {
            // Just need to ASSERT, this is called by something else internal that checked parameters already
            BCLDebug.Assert(bytes != null, "[ASCIIEncoding.GetChars]bytes is null");
            BCLDebug.Assert(byteCount >= 0, "[ASCIIEncoding.GetChars]byteCount is negative");
            BCLDebug.Assert(chars != null, "[ASCIIEncoding.GetChars]chars is null");
            BCLDebug.Assert(charCount >= 0, "[ASCIIEncoding.GetChars]charCount is negative");
            ...
            while (bytes < byteEnd)
            {
                ...
                if (b >= 0x80)
                {
                    ...
                    if (!fallbackBuffer.InternalFallback(byteBuffer, bytes, ref chars))
                    {
                        ...
                        BCLDebug.Assert(bytes > byteStart || chars == charStart,
