...
namespace System.Text
{
    ...
    //
    [Serializable()] internal class DBCSCodePageEncoding : BaseCodePageEncoding, ISerializable
    {
        ...
        protected override unsafe void LoadManagedCodePage()
        {
            ...
            BCLDebug.Assert(bytesUnknown == 0x3f,
            ...
            {
                BCLDebug.Assert(((*mapCodePageCached == this.dataTableCodePage && this.bFlagDataTable) ||
            ...
            while (bytePosition < 0x10000)
            {
                ...
                {
                    // Lead byte mark
                    BCLDebug.Assert(bytePosition < 0x100, "[DBCSCodePageEncoding.LoadManagedCodePage]expected lead byte to be < 0x100");
                    useBytes = bytePosition;
                    // input stays 0xFFFE
                }
        ...
        protected unsafe override void ReadBestFitTable()
        {
            ...
            lock(InternalSyncObject)
            {
                ...
                if (arrayUnicodeBestFit == null)
                {
                    ...
                    if (bOutOfOrder)
                    {
                        BCLDebug.Assert((arrayTemp.Length / 2) < 20,
        ...
        internal override unsafe int GetByteCount(char* chars, int count, EncoderNLS encoder)
        {
            ...
            BCLDebug.Assert(count >= 0, "[DBCSCodePageEncoding.GetByteCount]count is negative");
            BCLDebug.Assert(chars != null, "[DBCSCodePageEncoding.GetByteCount]chars is null");

            // Assert because we shouldn't be able to have a null encoder.
            BCLDebug.Assert(encoderFallback != null, "[DBCSCodePageEncoding.GetByteCount]Attempting to use null fallback");
            ...
            if (charLeftOver > 0)
            {
                BCLDebug.Assert(Char.IsHighSurrogate(charLeftOver), "[DBCSCodePageEncoding.GetByteCount]leftover character should be high surrogate");
        ...
        {
            // Just need to ASSERT, this is called by something else internal that checked parameters already
            BCLDebug.Assert(bytes != null, "[DBCSCodePageEncoding.GetBytes]bytes is null");
            BCLDebug.Assert(byteCount >= 0, "[DBCSCodePageEncoding.GetBytes]byteCount is negative");
            BCLDebug.Assert(chars != null, "[DBCSCodePageEncoding.GetBytes]chars is null");
            BCLDebug.Assert(charCount >= 0, "[DBCSCodePageEncoding.GetBytes]charCount is negative");

            // Assert because we shouldn't be able to have a null encoder.
            BCLDebug.Assert(encoderFallback != null, "[DBCSCodePageEncoding.GetBytes]Attempting to use null encoder fallback");
...
//            BCLDebug.Assert((encoder == null || encoder.m_throwOnOverflow) &&
...
//                "[DBCSEncoding.GetBytes]Expected empty fallback buffer at end if not converting");
        ...
        internal override unsafe int GetCharCount(byte* bytes, int count, DecoderNLS baseDecoder)
        {
            ...
            BCLDebug.Assert(bytes != null, "[DBCSCodePageEncoding.GetCharCount]bytes is null");
            BCLDebug.Assert(count >= 0, "[DBCSCodePageEncoding.GetCharCount]byteCount is negative");
            ...
            if (decoder != null && decoder.bLeftOver > 0)
            {
                ...
                if (count == 0)
                {
                    ...
                    BCLDebug.Assert(fallbackBuffer == null,
            ...
            BCLDebug.Assert(decoder == null || !decoder.m_throwOnOverflow ||
                !decoder.InternalHasFallbackBuffer || decoder.FallbackBuffer.Remaining == 0,
        ...
        {
            // Just need to ASSERT, this is called by something else internal that checked parameters already
            BCLDebug.Assert(bytes != null, "[DBCSCodePageEncoding.GetChars]bytes is null");
            BCLDebug.Assert(byteCount >= 0, "[DBCSCodePageEncoding.GetChars]byteCount is negative");
            BCLDebug.Assert(chars != null, "[DBCSCodePageEncoding.GetChars]chars is null");
            BCLDebug.Assert(charCount >= 0, "[DBCSCodePageEncoding.GetChars]charCount is negative");
            ...
            while (bytes < byteEnd)
            {
                ...
                if (c == UNKNOWN_CHAR_FLAG && iBytes != 0)
                {
                    ...
                    if (!fallbackBuffer.InternalFallback(byteBuffer, bytes, ref chars))
                    {
                        ...
                        BCLDebug.Assert(bytes >= byteStart + byteBuffer.Length, 
