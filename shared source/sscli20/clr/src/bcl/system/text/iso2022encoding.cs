    ...
    **
    ...
    internal class ISO2022Encoding : DBCSCodePageEncoding
    {
        ...
        internal ISO2022Encoding(SerializationInfo info, StreamingContext context) : base(info, context)
        {
            ...
            BCLDebug.Assert(false, "Didn't expect to make it to DBCSCodePageEncoding serialization constructor");
        ...
        protected unsafe override String GetMemorySectionName()
        {
            ...
            switch (this.CodePage)
            {
                ...
                case 52936:
                    ...
                    break;
                default:
                    BCLDebug.Assert(false, "[ISO2022Encoding.GetMemorySectionName] Don't expect to get here for code page " + this.CodePage);
        ...
        internal override unsafe int GetByteCount(char* chars, int count, EncoderNLS baseEncoder)
        {
            ...
            BCLDebug.Assert(count >= 0, "[ISO2022Encoding.GetByteCount]count is negative");
            BCLDebug.Assert(chars != null, "[ISO2022Encoding.GetByteCount]chars is null");
        ...
        {
            // Just need to ASSERT, this is called by something else internal that checked parameters already
            BCLDebug.Assert(chars != null, "[ISO2022Encoding.GetBytes]chars is null");
            BCLDebug.Assert(byteCount >= 0, "[ISO2022Encoding.GetBytes]byteCount is negative");
            BCLDebug.Assert(charCount >= 0, "[ISO2022Encoding.GetBytes]charCount is negative");

            // Assert because we shouldn't be able to have a null encoder.
            BCLDebug.Assert(encoderFallback != null, "[ISO2022Encoding.GetBytes]Attempting to use null encoder fallback");
...
*/
        ...
        internal override unsafe int GetCharCount(byte* bytes, int count, DecoderNLS baseDecoder)
        {
            ...
            BCLDebug.Assert(bytes != null, "[ISO2022Encoding.GetCharCount]bytes is null");
            BCLDebug.Assert(count >= 0, "[ISO2022Encoding.GetCharCount]byteCount is negative");
        ...
        {
            // Just need to ASSERT, this is called by something else internal that checked parameters already
            BCLDebug.Assert(bytes != null, "[ISO2022Encoding.GetChars]bytes is null");
            BCLDebug.Assert(byteCount >= 0, "[ISO2022Encoding.GetChars]byteCount is negative");
            BCLDebug.Assert(charCount >= 0, "[ISO2022Encoding.GetChars]charCount is negative");
  ...
  //                  iCount = GetCharsCP50227CN( bytes, byteCount, chars, charCount, decoder);
    //                break;
                case 52936:
                    ...
                    break;
                default:
                    BCLDebug.Assert(false, "[ISO2022Encoding.GetChars] had unexpected code page");
                    break;
            }
                                                  ...
                                                  byte* bytes, int byteCount, ISO2022Encoder encoder)
        {
            ...
            if (encoder != null)
            {
                ...
                if (charLeftOver > 0)
                {
                    BCLDebug.Assert(Char.IsHighSurrogate(charLeftOver), "[ISO2022Encoding.GetBytesCP5022xJP]leftover character should be high surrogate");
                ...
                if (bLeadByte == LEADBYTE_HALFWIDTH)
                {
                    ...
                    if (currentMode != ISO2022Modes.ModeHalfwidthKatakana)
                    {
                        ...
                        else
                        {
                            ...
                            BCLDebug.Assert(CodePage == 50221, "[ISO2022Encoding.GetBytesCP5022xJP]Expected Code Page 50221");
                                                    ...
                                                    byte* bytes, int byteCount, ISO2022Encoder encoder)
        {
            ...
            if (encoder != null)
            {
                ...
                if (charLeftOver > 0)
                {
                    BCLDebug.Assert(Char.IsHighSurrogate(charLeftOver), "[ISO2022Encoding.GetBytesCP50225KR]leftover character should be high surrogate");
            ...
            if (bytes != null && encoder != null)
            {
                ...
                if (!encoder.MustFlush || encoder.charLeftOver != (char)0)
                {
                    ...
                    BCLDebug.Assert(!encoder.MustFlush || !encoder.m_throwOnOverflow,
                                           ...
                                           byte* bytes, int byteCount, ISO2022Encoder encoder)
        {
            ...
            if (encoder != null)
            {
                ...
                if (charLeftOver > 0)
                {
                    BCLDebug.Assert(Char.IsHighSurrogate(charLeftOver), "[ISO2022Encoding.GetBytesCP52936]leftover character should be high surrogate");
        ...
        {
            BCLDebug.Assert(count > 0, "[ISO2022Encoding.DecrementEscapeBytes]count > 0");
        ...
        private unsafe int GetCharsCP52936(byte* bytes, int byteCount,
                                                char* chars, int charCount, ISO2022Decoder decoder)
        {
            BCLDebug.Assert(byteCount >=0, "[ISO2022Encoding.GetCharsCP52936]count >=0");
            BCLDebug.Assert(bytes!=null, "[ISO2022Encoding.GetCharsCP52936]bytes!=null");
            ...
            while (buffer.MoreData || byteLeftOver >= 0)
            {
                ...
                {
                    // Should be ModeHZ
                    BCLDebug.Assert(currentMode == ISO2022Modes.ModeHZ, "[ISO2022Encoding.GetCharsCP52936]Expected ModeHZ");
                    char cm;
