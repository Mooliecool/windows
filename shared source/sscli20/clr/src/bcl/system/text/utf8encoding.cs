...
[System.Runtime.InteropServices.ComVisible(true)]
    public class UTF8Encoding : Encoding
    {
        ...
        internal override unsafe int GetByteCount(char *chars, int count, EncoderNLS baseEncoder)
        {
            ...
            BCLDebug.Assert(count >=0, "[UTF8Encoding.GetByteCount]count >=0");
                ...
                if (pSrc >= pEnd) {
                    ...
                    if (ch == 0) {
                        ...
                        if (fallbackBuffer != null && fallbackBuffer.bFallingBack) {
                            BCLDebug.Assert(ch >= 0xD800 && ch <= 0xDBFF,
...
//                            - (CharUnicodeInfo.HIGH_SURROGATE_START << 10) );
                                                ...
                                                byte* bytes, int byteCount, EncoderNLS baseEncoder)
        {
            ...
            BCLDebug.Assert(byteCount >=0, "[UTF8Encoding.GetBytes]byteCount >=0");
            BCLDebug.Assert(charCount >=0, "[UTF8Encoding.GetBytes]charCount >=0");
            BCLDebug.Assert(bytes!=null, "[UTF8Encoding.GetBytes]bytes!=null");
            ...
            EncodeChar:
                ...
                if (pTarget > pAllocatedBufferEnd - bytesNeeded) {
                    ...
                    BCLDebug.Assert(pSrc >= chars || pTarget == bytes,
                ...
                BCLDebug.Assert(pTarget <= pAllocatedBufferEnd, "[UTF8Encoding.GetBytes]pTarget <= pAllocatedBufferEnd");
            ...
            if (encoder != null)
            {
                BCLDebug.Assert(!encoder.MustFlush || ch == 0,
        ...
        {
            BCLDebug.Assert(count >=0, "[UTF8Encoding.GetCharCount]count >=0");
            BCLDebug.Assert(bytes!=null, "[UTF8Encoding.GetCharCount]bytes!=null");
            ...
            for (;;)
            {
                ...
                if ((ch & FinalByte) == 0) {
                    BCLDebug.Assert( (ch & (SupplimentarySeq | ThreeByteSeq)) != 0,
            ...
            // Shouldn't have anything in fallback buffer for GetCharCount
            // (don't have to check m_throwOnOverflow for count)
            BCLDebug.Assert(fallback == null || fallback.Remaining == 0,
        ...
        {
            BCLDebug.Assert(chars!=null, "[UTF8Encoding.GetChars]chars!=null");
            BCLDebug.Assert(byteCount >=0, "[UTF8Encoding.GetChars]count >=0");
            BCLDebug.Assert(charCount >=0, "[UTF8Encoding.GetChars]charCount >=0");
            BCLDebug.Assert(bytes!=null, "[UTF8Encoding.GetChars]bytes!=null");
            ...
            InvalidByteSequence:
                ...
                if (!FallbackInvalidByteSequence(ref pSrc, ch, fallback, ref pTarget))
                {
                    ...
                    BCLDebug.Assert(pSrc >= bytes || pTarget == chars,
            ...
            EncodeChar:
                ...
                *pTarget = (char)ch;
                ...
                BCLDebug.Assert(pTarget <= pAllocatedBufferEnd, "[UTF8Encoding.GetChars]pTarget <= pAllocatedBufferEnd");
            ...
            if (baseDecoder != null)
            {
                ...
                BCLDebug.Assert(!baseDecoder.MustFlush || ch == 0 || !baseDecoder.m_throwOnOverflow,
