...
//
const BYTE NativeCompareInfo::pXWDrop[] =                  // values to drop from XW
{
    0xc6,                         // weight 4
    0x03,                         // weight 5
    0xe4,                         // weight 6
...
const BYTE NativeCompareInfo::pXWSeparator[] =             // separator values for XW
{
    0xff,                         // weight 4
    0x02,                         // weight 5
    0xff,                         // weight 6
...
NativeCompareInfo::NativeCompareInfo(int nLcid, SortingTable* pSortingFile):
    m_hSortKey(NULL), m_IfReverseDW(FALSE), m_IfCompression(FALSE),
    m_IfDblCompression(FALSE), m_pNext(NULL)
{
    CONTRACTL {
        NOTHROW;
        GC_NOTRIGGER;
        MODE_COOPERATIVE;
        PRECONDITION(CheckPointer(pSortingFile));
    } CONTRACTL_END;
    m_nLcid = nLcid;
...
NativeCompareInfo::~NativeCompareInfo()
{
    ...
    // Hmm, this was #ifdef'd out, do we wanna do this or not?
    // Why not close the m_hSortKey ASAP (it'll stay open as long as its mapped anyway) - shawnste
    if (m_pSortKey != NULL && m_hSortKey != NULL) {
    ...
    int* WhichJamo)
{
    CONTRACTL {
        ...
        GC_NOTRIGGER;
        MODE_COOPERATIVE;
        PRECONDITION(CheckPointer(ppString1));
        PRECONDITION(CheckPointer(ctr1));
        PRECONDITION(cchCount1 >= 0);
        PRECONDITION(CheckPointer(pWeight1));
        PRECONDITION(CheckPointer(ppString2));
        PRECONDITION(CheckPointer(ctr2));
        PRECONDITION(cchCount2 >= 0);
        PRECONDITION(CheckPointer(uw1));
        PRECONDITION(CheckPointer(uw2));
        PRECONDITION(CheckPointer(pWeight2));
        PRECONDITION(CheckPointer(pState));
        PRECONDITION(CheckPointer(WhichJamo));
    } CONTRACTL_END;
    ...
    // Compare Jamo units (either a single Jamo or a valid old Hangul Jamo composition) 
    // until we run out of Jamo units in either string.
    ...
    for (;;)
    {
        if (IsJamo(**ppString1))
        {
            ...
            {                
                if ((oldHangulsFound1 = (DWORD) MapOldHangulSortKey(*ppString1, *ctr1, &UW, JamoWeight1)) > 0)
                {
                    ...
                    *ppString1 += (oldHangulsFound1 - 1);     // We always increment ppString1/ctr1 at the end of the loop, so we subtract 1.
        ...
        if (*pWeight1 == CMP_INVALID_WEIGHT)
        {
            ...
            // so that the string comparison can restart within the loop of CompareString().
            //
    ...
    int cchCount2)
{
    ...
    //
    // We have already validated pString1 and pString2 in COMNlsInfo::CompareString().
    //
    ...
    while ((cchCount1 != 0) && (cchCount2 != 0))
    {
        ...
        if (Weight1 != Weight2)
        {
            ...
            //
            if ((uw1 != uw2) ||
                (sm1 == FAREAST_SPECIAL) || // Slow path desirable for Kana here (special weights)
                (sm1 == JAMO_SPECIAL)    || // Slow path needed for Jamo (shifting weights)
                (sm1 == EXTENSION_A))       // Slow path also needed for Extension A (shifting weights)
            {
                //
            ...
            if (State & (STATE_DW | STATE_REVERSE_DW))
            {
                ...
                if (dw1 != dw2)
                {
                    ...
                    // Hence cchCount should be greater than 1.
                    while (cchCount1 > 1)
                    {
    ...
    int cchCount2)
{
    ...
    //
    // We have already validated pString1/pString2 in COMNlsInfo::CompareString().
    _ASSERTE(pString1 != NULL && pString2 != NULL);
            ...
            NOT_END_STRING(ctr2, pString2, cchCount2) )
    {
        ...
        //
        if ((Weight1 != Weight2) ||
            (GET_SCRIPT_MEMBER(&Weight1) == FAREAST_SPECIAL) || // Slow path desirable for Kana here (special weights)
            (GET_SCRIPT_MEMBER(&Weight1) == JAMO_SPECIAL) ||    // Slow path needed for Jamo (shifting weights)
            (GET_SCRIPT_MEMBER(&Weight1) == EXTENSION_A))       // Slow path also needed for Extension A (shifting weights)
        {
            //
...
void NativeCompareInfo::GET_FAREAST_WEIGHT(DWORD& wt, WORD& uw, DWORD mask, LPCWSTR pBegin, LPCWSTR pCur, DWORD& ExtraWt)
{
    CONTRACTL {
        ...
        GC_NOTRIGGER;
        MODE_COOPERATIVE;
        PRECONDITION(CheckPointer(pBegin));
        PRECONDITION(CheckPointer(pCur));
    } CONTRACTL_END;
    ...
    int cbDest)
{
    CONTRACTL {
        ...
        PRECONDITION(CheckPointer(pDest, NULL_OK));
        PRECONDITION(cchSrc >= 0);
        PRECONDITION(cbDest >= 0);
    } CONTRACTL_END;
...
void NativeCompareInfo::GetCompressionWeight(
    ...
    CONTRACTL {
        ...
        GC_NOTRIGGER;
        MODE_COOPERATIVE;
        PRECONDITION(CheckPointer(pSortkey1));
        PRECONDITION(CheckPointer(pSortkey2));
        PRECONDITION(CheckPointer(pString1End));
        PRECONDITION(CheckPointer(pString2End));
    } CONTRACTL_END;
...
/*============================IndexOfString============================
** Skip unsortable code points of the specified string and return the first sortable string pointer.
** 
**  Parameters:
**      pStr    The string to be scanned.
**      pStrEnd The end of the string to be scanned.
**      fIgnoreNonSpace flag to indicate if the non-space characters should be skipped.
**      fIgnoreSymbols  flag to indicate if the symbol characters should be skipped.
**
**  Returns:
**      A string pointer to the first sortable string.  If there are no sortable characters in pStr, 
**      the return value will be pStrEnd+1.
**
==============================================================================*/

LPCWSTR NativeCompareInfo::SkipUnsortable(LPCWSTR pStr, LPCWSTR pStrEnd, BOOL fIgnoreNonSpace, BOOL fIgnoreSymbols)
{    
    CONTRACTL {
        NOTHROW;
        GC_NOTRIGGER;
        MODE_COOPERATIVE;
        PRECONDITION(CheckPointer(pStr));
        PRECONDITION(CheckPointer(pStrEnd));
    } CONTRACTL_END;
    
    while (pStr <= pStrEnd) {
        BOOL bContinue = FALSE;
        DWORD dwWeight = GET_DWORD_WEIGHT(m_pSortKey, *pStr);
...
int NativeCompareInfo::IndexOfString(
    ...
    CONTRACTL {
        ...
        GC_NOTRIGGER;
        MODE_COOPERATIVE;
        PRECONDITION(CheckPointer(pString1));
        PRECONDITION(CheckPointer(pString2));
        PRECONDITION(CheckPointer(m_pSortKey));        
    } CONTRACTL_END;
    ...
    int cExpChar1 = 0, cExpChar2 = 0;       // Counter for expansion characters.
    ...
    // Pre-scan the target string so that we always start searching for the first sortable character.
    pString2 = SkipUnsortable(pString2, pString2End, fIgnoreNonSpace, fIgnoreSymbols);
    if (pString2 > pString2End)
    {
        // There is no sortable characters in the target string. Always returns 0 in this case.
        // This is for case which IndexOf("ABC", "\xffff\xfff"). This will return nStartIndex just like IndexOf("ABC", "").
        return (nStartIndex);
    }
    ...
    for (; pString1 <= pString1SearchEnd; pString1++) {
        ...
        // Pre-scan the source string so that we always start seraching for the first sortable character.
        // This will advance pString1 so that the inner loop starts at the first sortable character.
        LPCWSTR pSearchStr1 = pString1 = SkipUnsortable(pString1, pString1SearchEnd, fIgnoreNonSpace, fIgnoreSymbols);
        if (pString1 > pString1SearchEnd)
        {
            // If we reach for the end of source string, and we know that the target string is not empty, so
            // there is no chance for a match.  -1 is returned.
            return (-1);
        }
        ...
        while ((pSearchStr2 <= pString2End) && (pSearchStr1 <= pString1End)) {
            ...
            if (m_IfCompression) {
                ...
                GetCompressionWeight(&sortKey1, pSearchStr1, pString1End, &sortKey2, pSearchStr2, pString2End);
            ...
            // 4. Hangul Jamo are involved.
            // 5. Extension A is involved.
            ...
            sm1  = sortKey1.UW.SM_AW.Script;
            sm2  = sortKey2.UW.SM_AW.Script;
            
            // Reset extra weight
            dwExtraWt1 = dwExtraWt2 = 0;

            // 
            // First take care of all of the special processing cases where there is more involved than just
            // looking at the SM. This includes FAREAST_SPECIAL, JAMO_SPECIAL, and EXTENSION_A.
            //
            
            if ((sm1 == JAMO_SPECIAL) || (sm2 == JAMO_SPECIAL))
            {
                //
                // The Jamo diff code thinks in terms of an entirely different set of
                // parameters, as called from CompareString/LongCompareString. We must
                // fake those parameters now.
                //
                LPCWSTR pStr1 = pSearchStr1;
                LPCWSTR pStr2 = pSearchStr2;
                WORD uw1 = sortKey1.UW.Unicode;
                WORD uw2 = sortKey2.UW.Unicode;
                int cchCount1 = pString1SearchEnd - pSearchStr1;
                int cchCount2 = pString2End - pSearchStr2;
                LPCWSTR pLastJamo = pStr1;
                DWORD State = STATE_JAMO_WEIGHT;  // Enable WhichJamo checking
                int WhichJamo = 0;

                fContinue = FindJamoDifference(
                    &pStr1, &cchCount1, 0, &(*pdwWeight1), 
                    &pStr2, &cchCount2, 0, &(*pdwWeight2), 
                    &pLastJamo,
                    &uw1, &uw2, 
                    &State,
                    &WhichJamo);

                // If WhichJamo is not 0 or CSTR_EQUAL, it means at least one of
                // the strings had valid Jamo constructions and they were not the 
                // same, or they were just not really valid Jamo.
                if((WhichJamo != CSTR_EQUAL) &&
                    (WhichJamo != 0))
                {
                    goto NextCharInString1;
                }

                // Do all of the necessary fixups  
                pSearchStr1 = pStr1;
                pSearchStr2 = pStr2;
                sortKey1 = *((PSORTKEY)pdwWeight1);
                sortKey2 = *((PSORTKEY)pdwWeight2);

                // re-get the script member
                sm1  = sortKey1.UW.SM_AW.Script;
                sm2  = sortKey2.UW.SM_AW.Script;
            }

            if ((sm1 == EXTENSION_A) || (sm2 == EXTENSION_A))
            {
                //
                // If chars are in Extension A and are otherwise unassigned, 
                // then SM is in AW, AW is in DW, and DW is MIN_DW.
                //
                if (((sm1 == EXTENSION_A) ^ (sm2 == EXTENSION_A)) ||
                    (MAKE_UNICODE_WT(sortKey1.UW.SM_AW.Alpha, sortKey1.Diacritic) != 
                    (MAKE_UNICODE_WT(sortKey2.UW.SM_AW.Alpha, sortKey2.Diacritic))))
                {
                    // They are either not both Extension A ideographs, or they are two 
                    // different Extension A ideographs. Either way, move along...
                    goto NextCharInString1;
                }
                else
                {
                    // Same character, we still have a match; move to next char in pString1.
                    goto Advance;
                }
            }            
            ...
            // We do not mask until after checking Jamo/Extension A, to avoid doing do inappropriately.
            ...
            *pdwWeight2 &= dwMask; // Sortkey weight for characters in the second string.
            ...
            // Resync the sortKey values -- they are what is mainly used after the Kana code.
            sortKey1 = *((PSORTKEY)pdwWeight1);
            sortKey2 = *((PSORTKEY)pdwWeight2);
...
int NativeCompareInfo::LastIndexOfString(LPCWSTR pString1, LPCWSTR pString2, int nStartIndex, int nEndIndex, int nLength2, DWORD dwFlags, int* pnMatchEndIndex) {
    CONTRACTL {
        ...
        PRECONDITION(CheckPointer(pString1));
        PRECONDITION(CheckPointer(pString2));        
        PRECONDITION(CheckPointer(pnMatchEndIndex));
        PRECONDITION(CheckPointer(m_pSortKey));        
    } CONTRACTL_END;
    ...
    for (pString1 = pString1End; pString1 >= pString1Start; pString1--) {
        ...
        while ((pSearchStr2 <= pString2End) && (pSearchStr1 <= pString1End)) {
            ...
            if ((sm1 == JAMO_SPECIAL) || (sm2 == JAMO_SPECIAL))
            {
                ...
                WORD uw1 = sortKey1.UW.Unicode;
                WORD uw2 = sortKey2.UW.Unicode;
                int cchCount1 = pString1End - pSearchStr1;
                int cchCount2 = pString2End - pSearchStr2;
                LPCWSTR pLastJamo = pStr1;
                DWORD State = STATE_JAMO_WEIGHT;  // Enable WhichJamo checking
                int WhichJamo = 0;
            ...
            // as EXPANSION) and IgnoreSymbols (letters can NOT be symbols).
            // Mask the diacritic/case/width/Kana type using the pattern stored in dwMask.
            // We do not mask until after checking Jamo and Extension A to avoid inappropriate masking.
            //
...
BOOL NativeCompareInfo::IsSuffix(LPCWSTR pSource, int nSourceLen, LPCWSTR pSuffix, int nSuffixLen, DWORD dwFlags) {
    CONTRACTL {
        ...
        GC_NOTRIGGER;
        MODE_COOPERATIVE;
        PRECONDITION(CheckPointer(pSource));
        PRECONDITION(nSourceLen >= 0);
        PRECONDITION(CheckPointer(pSuffix));        
        PRECONDITION(nSuffixLen >= 0);
    } CONTRACTL_END;
...
BOOL NativeCompareInfo::IsPrefix(LPCWSTR pSource, int nSourceLen, LPCWSTR pPrefix, int nPrefixLen, DWORD dwFlags) {
    CONTRACTL {
        ...
        PRECONDITION(CheckPointer(pSource));
        PRECONDITION(nSourceLen >= 0);
        PRECONDITION(CheckPointer(pPrefix));        
        PRECONDITION(nPrefixLen >= 0);
    } CONTRACTL_END;
...
/*================================IsSortable================================
**Action: The native implementation for CompareInfo.IsSortable().
**Returns: 
**  TRUE if every character in the string is defined in the sorting table.
**Arguments:
**  pString    The string to test
**  nStringLen The length of the string
**Exceptions: OutOfMemoryException if we run out of memory.
** 
**NOTE NOTE: The way this method works:
**
**          1) Iterate through every character in the string.
**          2) If a character is unsortable and is not in the small list that we consider defined, return FALSE.
**          3) If a character is from the PUA, return FALSE.
**          4) If a character is an unpaired surrogate code unit, return FALSE.
**
**          If the entire string is iterated without FALSE being returned, then the method will return TRUE.
==============================================================================*/

BOOL NativeCompareInfo::IsSortable(LPCWSTR pString, int nStringLen) {
    CONTRACTL {
        GC_NOTRIGGER;
        MODE_COOPERATIVE;
        SO_TOLERANT;
        PRECONDITION(CheckPointer(pString));
        PRECONDITION(nStringLen >= 0);
    } CONTRACTL_END;
    ...
    while(ich < nStringLen) {
        ...
        if (m_pSortKey[wch].UW.SM_AW.Script  == UNSORTABLE) {
            //
    ...
    // If we made it this far, we succeeded, unless the string is of zero length.
    return(((0 == nStringLen) ? FALSE : TRUE));
}
