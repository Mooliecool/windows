                       ...
                       bool isDeref)
{
    ...
    if (!isDeref && CorTypeInfo::IsObjRef(eltType))
    {
    ...
    else if (CorTypeInfo::IsPrimitiveType(eltType))
    {
        otherFlags |= CLRDATA_VALUE_IS_PRIMITIVE;
    }
    ...
    for (ULONG32 i = 0; i < m_numLocs; i++)
    {
        ...
        else
        {
            ...
            status = m_dac->m_target->
                ReadVirtual(TO_CDADDR(loc->addr), buffer, loc->size,
                            &done);
            if (status != S_OK)
            {
                return status;
    ...
    EX_TRY
    {
        ...
        for (ULONG32 i = 0; i < m_numLocs; i++)
        {
            ...
            else
            {
                ...
                status = m_dac->m_target->
                    WriteVirtual(TO_CDADDR(loc->addr), buffer, loc->size,
                                 &done);
                if (status != S_OK)
                {
                    goto Exit;
...
{
    // XXX drewb - Obsolete method, never implemented.
    return E_UNEXPECTED;
}
...
ClrDataValue::GetRefAssociatedValue(IXCLRDataValue** assocValue)
{
    ...
    // XXX drewb - Good way to get the right size for everything?
    ...
    /* [out] */ IXCLRDataTypeInstance **assocType)
{
    ...
    EX_TRY
    {
        ...
        {
            ArrayBase* arrayBase = PTR_ArrayBase(m_baseAddr);
    ...
    /* [out] */ IXCLRDataValue **value)
{
    ...
    EX_TRY
    {
        ...
        arrayBase = PTR_ArrayBase(m_baseAddr);
        ...
        dim = baseRank;
        offs = TO_CDADDR(arrayBase->GetDataPtr());
                               ...
                               mdFieldDef *tokenRet)
{
    ...
    if ((status = ConvertUtf8(fieldDesc->GetName(),
                              nameBufRetLen, nameLenRet, nameBufRet)) != S_OK)
    {
        return status;
    }
    ...
    {
        baseAddr = TO_CDADDR
            (fieldDesc->GetStaticAddressHandle
             ((void*)fieldDesc->GetBaseInDomain(appDomain)));
    }
    else
    {
        baseAddr = TO_CDADDR(fieldDesc->GetAddress((void*)objBase));
    }
    ...
    /* [out] */ IXCLRDataTypeDefinition **base)
{
    ...
    EX_TRY
    {
        ...
        if (m_typeHandle.IsNull())
        {
            ...
            m_module->GetMDImport()->GetTypeDefProps(m_token, &attr, &token);
        }
        else
        {
            ...
            token = typeHandle.GetClass()->GetCl();
        }
    ...
    /* [out] */ ULONG32* rank)
{
    ...
    EX_TRY
    {
        ...
        {
            EEClass* pClass = m_typeHandle.GetClass();
            ...
            else
            {
                ArrayClass* arrayClass =
                    PTR_ArrayClass(PTR_HOST_TO_TADDR(pClass));
                *rank = arrayClass->GetRank();
                status = S_OK;
            }
        }
    }
