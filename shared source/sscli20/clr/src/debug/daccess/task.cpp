...
// XXX drewb - Why aren't these extra MD APIs in a header?
    ...
    /* [size_is][in] */ BYTE context[  ])
{
    ...
    if (!CheckContextSizeForInBuffer(contextSize, context))
    {
        return E_INVALIDARG;
    }
...
HRESULT 
ClrDataTask::RequestIsCantStop(
    /* [in] */ ULONG32 inBufferSize,
    /* [size_is][in] */ BYTE *inBuffer,
    /* [in] */ ULONG32 outBufferSize,
    /* [size_is][out] */ BYTE *outBuffer)
{
    // Is this thread in the runtime (ie, can't stop)?    
    // Look at the GC mode:
    // if in cooperative mode --> Yes
    // if in preemptive, look at top frame
    //    if IsTransitionToNativeFrame() --> No
    //    else --> Yes

    // Validate params.
    // Input: Nothing.
    // Output: a DacpIsCantStop structure.
    if ((inBufferSize != 0) ||
        (inBuffer != NULL) ||
        (outBufferSize != sizeof(DacpIsCantStop)) ||
        (outBuffer == NULL))
    {
        return E_INVALIDARG;
    }

    DacpIsCantStop * pOut = reinterpret_cast<DacpIsCantStop *> (outBuffer);

    if (m_thread == NULL)
    {
        // No EE thread, we're definintely not in the runtime.
        pOut->fIsCantStop = FALSE;
    }
    else if (m_thread->m_fPreemptiveGCDisabled)
    {
        // Cooperative
        pOut->fIsCantStop = TRUE;
    }
    else
    {
        // We're in preemptive.
        Frame * pFrame = m_thread->GetFrame();
        if ((pFrame == NULL) || (pFrame == FRAME_TOP))
        {
            // preemptive + no frame, not in runtime,
            pOut->fIsCantStop = FALSE;
        } 
        else if (pFrame->IsTransitionToNativeFrame())
        {
            // We've gone to native code, definitely outside of the runtime.
            pOut->fIsCantStop = FALSE;
        }
        else
        {
            // We're in preemptive, but we're still in the runtime.
            // (maybe we'll blocked for something).
            pOut->fIsCantStop = TRUE;
        }
    }
    return S_OK;    
    ...
    /* [size_is][out] */ BYTE *outBuffer)
{
    ...
    EX_TRY
    {
        ...
        case DACTASKPRIV_REQUEST_IS_CANTSTOP:
            status = RequestIsCantStop(inBufferSize, inBuffer,
    ...
    EX_TRY
    {
        ...
        if (rawName)
        {
            ...
            else
            {
                if (nameLen)
                {
                    *nameLen = wcslen((PCWSTR)rawName) + 1;
                }
                              ...
                              OUT PVOID* iface)
{
    ...
    {
        return GetMdInterface(interfaceId,
                              (IUnknown**)&m_mdImport,
                              iface);
    }
    else
    ...
    /* [size_is][out] */ WCHAR name[  ])
{
    ...
    EX_TRY
    {
        ...
        //
        if (m_module->GetFile()->GetPath().DacGetUnicode(bufLen, name, &_nameLen) &&
            (name[0] || m_module->GetFile()->GetModuleFileNameHint().DacGetUnicode(bufLen, name, &_nameLen)))
        {
            if (nameLen)
            {
                *nameLen = _nameLen;
    ...
    /* [out] */ GUID* vid)
{
    ...
    EX_TRY
    {
        ...
        else
        {
            ...
            m_module->GetMDImport()->GetScopeProps(NULL, &mdVid);
            *vid = mdVid;
            status = S_OK;
        }
    }
    ...
    /* [out] */ CLRDATA_ENUM* handle)
{
    ...
    EX_TRY
    {
        if (!m_setExtents)
        {
            ...
            if (file->GetLoadedImageContents())
            {
                extent->base =
                    TO_CDADDR(file->GetLoadedImageContents(&extent->length));
            ...
            {
                extent->base = TO_CDADDR(file->GetLoadedNative()->GetBase());
...
HRESULT
ClrDataModule::GetMdInterface(REFIID interfaceId,
                              IUnknown** cacheIface,
                              PVOID* retIface)
{
    HRESULT status;
    ...
    EX_TRY
    {
        ...
        _ASSERTE(*cacheIface != NULL);
    
        (*cacheIface)->AddRef();
        *retIface = (PVOID)*cacheIface;
        status = S_OK;
...
ClrDataMethodDefinition::GetIlMethod(void)
{
    ...
    else
    {
        ...
        {
            return DacGetIlMethod((TADDR)m_module->GetIL((RVA)ilRva));
        }
    }
}
    ...
    EX_TRY
    {
        ...
        for (ULONG32 i = 0; i < numMap; i++)
        {
            if (mapLen && maps)
            {
                ...
                maps->endAddress =
                    TO_CDADDR(codeStart + map[i].nativeEndOffset);
                // XXX drewb - Define types as mapping of
    ...
    /* [size_is][in] */ BYTE cxRecord[  ])
{
    ...
    EX_TRY
    {
        ...
        // XXX drewb - This should also check that the
        ...
        // I want to see when this gets called since I'm not sure about
        // the EXCEPTION_RECORD64 in rotor_palrt.h, and I don't currently
        // understand who calls this function and when.
        _ASSERTE (0);
...
ClrDataExceptionState::GetCurrentExceptionRecord()
{
    ...
    pExRecord = PTR_HOST_MEMBER_TADDR(ExInfo, m_exInfo, m_ExceptionRecord);
...
#else  // !_X86_ && !_WIN64
    _ASSERTE(!"ClrDataExceptionState() - GetCurrentContextRecord()");
    return NULL;
                            ...
                            IXCLRDataMethodDefinition **method)
{
 ...
 NextMethod:
    ...
    if (m_useAddrFilter)
    {
        ...
        COR_ILMETHOD* ilMeth =
            DacGetIlMethod((TADDR)m_module->GetIL((RVA)ilRva));
        COR_ILMETHOD_DECODER ilDec(ilMeth);
