...
//-----------------------------------------------------------------------------
void CordbMDA::Neuter(NeuterTicket ticket) 
{ 
    // Release buffers. Once we're neutered, these can no longer be accessed anyways, 
    // so may as well free them now.
    // This is being done under the Stop-Go lock, and our accessors are also done
...
//-----------------------------------------------------------------------------
HRESULT CordbMDA::GetName(ULONG32 cchName, ULONG32 * pcchName, WCHAR szName[])
{
    PUBLIC_API_ENTRY(this);
    FAIL_IF_NEUTERED(this);
    ATT_REQUIRE_STOPPED_MAY_FAIL(GetProcess());
    
    return CopyOutString(m_szName, cchName, pcchName, szName);
}
...
// See CopyOutString for parameter details.
//-----------------------------------------------------------------------------
HRESULT CordbMDA::GetDescription(ULONG32 cchName, ULONG32 * pcchName, WCHAR szName[])
{
    PUBLIC_API_ENTRY(this);
    FAIL_IF_NEUTERED(this);
    ATT_REQUIRE_STOPPED_MAY_FAIL(GetProcess());
    
    return CopyOutString(m_szDescription, cchName, pcchName, szName);
}
