...
bool HelperCanary::AreLocksAvailable()
{
    ...
    if (!g_pRCThread->GetDCB(IPC_TARGET_OUTOFPROC)->m_rightSideIsWin32Debugger)
    {
    ...
    if (!m_fCachedAnswer)
    {
        ...
        if (shouldBreak == -1)
        {
            shouldBreak = UnsafeGetConfigDWORD(L"DbgBreakIfLocksUnavailable", 0);
        }
        if (shouldBreak)
        {
...
bool HelperCanary::AreLocksAvailableWorker()
{
    ...
    if (fShortcut == -1)
    {
        fShortcut = UnsafeGetConfigDWORD(L"DbgShortcutCanary", 0);
    }
    if (fShortcut == 1)
    {
        return false;
    ...
    if (m_CanaryThreadId == 0)
    {
        ...
        if (m_hPingEvent == NULL)
        {
            ...
            // If we can't setup, then assume it's unsafe.
            return false;
        }
        ...
        {   
            // If we can't startup, then assume it's unsafe.
            return false;
        }
        ...
        // debug events.
        DebuggerIPCControlBlock* dcb = g_pRCThread->GetDCB(IPC_TARGET_OUTOFPROC);
        dcb->m_CanaryThreadId = m_CanaryThreadId;
