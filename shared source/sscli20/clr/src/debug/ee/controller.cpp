...
#ifdef USE_BP_FOR_PATCH_SKIP
#define IsSingleStep(exception) (exception == EXCEPTION_BREAKPOINT)
#else
...
void DebuggerController::EnsureUniqueThreadStarter(DebuggerThreadStarter * pNew)
{
    ...
    ControllerLockHolder lockController(&g_criticalSection);
...
HRESULT DebuggerController::Initialize()
{
    ...
    {        
        ...
        g_criticalSection.Init("DebuggerConroller", NULL, 
...
DebuggerController::DebuggerController(Thread *thread, AppDomain *pAppDomain)
  : m_pAppDomain(pAppDomain), m_thread(thread), m_singleStep(false),
    m_exceptionHook(false), m_traceCall(0), m_traceCallFP(ROOT_MOST_FRAME),
    m_unwindFP(LEAF_MOST_FRAME), m_eventQueuedCount(0), m_deleted(false),
...
void DebuggerController::DisableAll()
{
    ...
    ControllerLockHolder ch(&g_criticalSection);
    {
        //
        ...
        // thus leaving the patchtable in an inconsistent state such that we may blow up trying to walk it.
...
bool DebuggerController::ApplyPatch(DebuggerControllerPatch *patch)
{
    ...
    if (patch->IsNativePatch())
    {
        ...
        IA64_ONLY(baseAddress = ALIGN_DOWN(baseAddress, IA64_BUNDLE_SIZE));
    ...
    EX_TRY
    {
        ...
        while(!it.IsAtEnd())
        {
            ...
            if (dji->m_encVersion == encVersion)
            {
                ...
                res = AddBindAndActivateILSlavePatch(master, dji) && res;
            }
            it.Next();
        }
...
// void DebuggerController::BindFunctionPatches()  Make sure
// that all the patches for the given function are active.  This
// is invoked when a method is moved, so that if any breakpoints
// were placed in the  method prior to it's being JITted, those
// breakpoints will now be properly placed in the actual, executable code.
...
// ********* NOTE ************
//
// MethodDesc *fd:  MethodDesc of the newly JITted method
// CORDB_ADDRESS_TYPE *code:  Address of the first instruction of the code
void DebuggerController::BindFunctionPatches(MethodDesc *fd,
                                             CORDB_ADDRESS_TYPE *code)
{
    if (g_patches == NULL)
        return;
    ...
    DebuggerControllerPatch *p = g_patches->GetPatch(g_pEEInterface->MethodDescGetModule(fd),fd->GetMemberDef());
    ...
    while (p != NULL)
    {
        ...
        if (DebuggerController::BindPatch(p, fd, code))
            DebuggerController::ActivatePatch(p);
...
// void DebuggerController::UnbindFunctionPatches()  Make sure
                                                ...
                                                bool fSaveOpcodes)
{
    ...
    while (p != NULL)
    {
        ...
        if ( p->IsBound() )
        {
            ...
            {
                MethodDesc *pFD = g_pEEInterface->FindLoadedMethodRefOrDef(
                                ...
                                pFD->m_pszDebugMethodName)==0)
                {
                    LOG((LF_CORDB,LL_INFO10000,
                        ...
                        pFD->m_pszDebugMethodName, p->offset));
                }
            }
#endif //LOGGING
            ...
            if (p->key.module == module && p->key.md == md)
            {
                ...
                _ASSERTE( !p->IsActivated() );
                _ASSERTE( !p->IsBound() );
                // These should be unnecessary - the above assert validates this
                InitializePRD(&(p->opcode)); //zero out opcode for right side
                p->address = NULL; //so we don't barf when we do a matching RebindFunctionPatches for an UnbindFunctionPatches
...
void DebuggerController::RemovePatchesFromModule(Module *pModule, AppDomain *pAppDomain )
{
    ...
    for (DebuggerControllerPatch *patch = g_patches->GetFirstPatch(&f);
         ...
         patch = g_patches->GetNextPatch(&f))
    {
        // Is the patch in the specified module, and optionally the specified domain?
        if (patch->HasDJI() &&
            (patch->GetDJI()->m_fd->GetModule() == pModule) &&
            ( (pAppDomain == NULL) || (patch->pAppDomain == pAppDomain) )  )
        {
...
//
static bool _AddrIsJITHelper(CORDB_ADDRESS_TYPE *addr)
{
    // ROTORTODO: Figure out what we want to do here
                                    ...
                                    bool fStopInUnmanaged)
{
...
#ifdef _X86_
        if (RunningOnWin95() && WIN9X_NONWRITABLE_ADDRESS(trace->GetAddress()))
            return false;
#endif
        if (fStopInUnmanaged && !_AddrIsJITHelper((CORDB_ADDRESS_TYPE *)trace->GetAddress()))
        {
    ...
    case TRACE_MANAGED:
        ...
        MethodDesc *fd;
        fd = g_pEEInterface->GetNativeCodeMethodDesc((const BYTE *)trace->GetAddress());
        _ASSERTE(fd);
                                    ...
                                    DebuggerControllerPatch *patch)
{
    ...
    if (patch->address != (CORDB_ADDRESS_TYPE *)GetIP(context))
    {
        return false;
    }

    //                               we'll make sure that intermediate patches have NULL
...
{
#ifdef _DEBUG
    BOOL shouldBreak = REGUTIL::GetConfigDWORD(L"ActivatePatchSkip", 0);
    if (shouldBreak > 0) {
                                         ...
                                         TP_RESULT *pTpr)
{
           ...
           !fDone)
    {
        ...
        if (MatchPatch(thread, context, patch))
        {
            ...
            {
                patch->controller->TriggerTraceCall(thread, (const BYTE*)::GetIP(context));
                tpr = TPR_IGNORE;
            }
            else
            {
                ...
                tpr == TPR_TRIGGER_ONLY_THIS_AND_LOOP)
            {
                ...
                pDcq->dcqEnqueue(patch->controller, TRUE);
            }
        ...
        tpr != TPR_TRIGGER_ONLY_THIS)
    {
        ...
        while (p != NULL)
        {
            ...
            {
#if !defined(STEPPING_SUPPORTED)
                PORTABILITY_ASSERT("ScanForTriggers (Controller.cpp) is not implemented on this platform.");
#endif
...
// bool DebuggerController::DispatchPatchOrSingleStep()  Ask any patches that are active at a given address if they
...
// of any DC's that are into an array, and if there are any, do the Hideous Dance of Death                                   
DPOSS_ACTION DebuggerController::DispatchPatchOrSingleStep(Thread *thread, CONTEXT *context, CORDB_ADDRESS_TYPE *address, SCAN_TRIGGER which)
{
    ...
    {        
        ...
        BOOL reabort;
        {
            ...
            // Whe could get blocked here if another thread sends a debug event and syncs the shell. Thus the whole world could
            // be changed underneath us. Practically, this would just be SetIp. Once we unlock, we may need to recheck some things.
            ...
            bool fIpChanged = (originalAddress != (TADDR) GetIP(context));

            // Send the events outside of the controller lock
            bool anyEventsSent = false;

            cEvents = dcq.dcqGetCount();
            iEvent = 0;
        ...
        if (reabort)
        {
            if (GetEEFuncEntryPoint(::FuncEvalHijack) != (LPVOID)GetIP(context))
            {
                HRESULT hr;
    ...
    ActivatePatchSkip(thread, (BYTE*)GetIP(pCtx), FALSE);
...
#ifdef _DEBUG
    // Some controllers don't need to set the SS to do their job, and if they are setting it, it's likely a bug.
...
void DebuggerController::ApplyTraceFlag(Thread *thread)
{
    ...
    // We should only be applying the Trace-Flag in managed code.
    // So we should always either have:
    // - a filter ctx - which means we hit some ss/patch (very likely in managed code)
    //   this isn't 100% guaranteed b/c a few patches are in native.
    // - a RedirectedThread (was suspended in managed code).


    // We shouldn't be toggling the trace flag if we're in native code.
    _ASSERTE(!thread->GetInteropDebuggingHijacked());
...
#ifdef USE_HARDWARE_TO_STEP
    SetSSFlag(context);
    _ASSERTE(IsSSFlagEnabled(context));

#elif defined(USE_OS_TO_STEP)
    _ASSERTE(thread->HasValidThreadHandle());
    LOG((LF_CORDB,LL_INFO1000, "DC::ApplyTraceFlag Creating watchpoint at 0x%08x %u\n", CORDbgGetIP(context)));
    DWORD dwError = PAL_CreateExecWatchpoint(thread->GetThreadHandle(), CORDbgGetIP(context));
    if (ERROR_SUCCESS != dwError)
    {
        LOG((LF_CORDB,LL_INFO1000, "DC::ApplyTraceFlag PAL_CreateExecWatchpoint returned %u\n", dwError));
        _ASSERTE(!"PAL_CreateExecWatchpoint failed.");
    }

#else
    PORTABILITY_ASSERT("ApplyTraceFlag (Controller.cpp) is not supported on this platform.");
#endif // USE_HARDWARE_TO_STEP
...
void DebuggerController::UnapplyTraceFlag(Thread *thread)
{
    ...
    //
    // Nothing to do there -- the PAL will automatically delete exec
    // watchpoints when they are hit.
    //

#else
    PORTABILITY_ASSERT("UnApplyTraceFlag (Controller.cpp) is not supported on this platform.");
#endif // USE_HARDWARE_TO_STEP
...
void DebuggerController::EnableTraceCall(FramePointer maxFrame)
{
    ...
    ControllerLockHolder lockController(&g_criticalSection);
    {
        ...
        EnableVirtualTraceCall(maxFrame);
    }    
}

//
// This function gets a list of "virtual" trace call targets from the stub managers and 
// puts a patch at each returned target.  Conceptually we are doing what EnableTraceCall() does,
// trying to trap the thread when it calls into managed code.  Currently this is only used for 
// reverse pinvoke on WIN64.
//
// Note that the patches for the virtual trace call targets will be cleaned up when DisableAll()
// is called.
//

void DebuggerController::EnableVirtualTraceCall(FramePointer maxFrame)
{
    CONTRACTL
    {
        SO_NOT_MAINLINE;
    ...
    TADDR* pVirtualTraceCallTargets = NULL;

    // Get the virtual trace call targets.  Don't free the memory.  The StubManager class owns it.
    g_pEEInterface->GetVirtualTraceCallTargets(&pVirtualTraceCallTargets, &dwNumTargets);
    ...
    if (dwNumTargets > 0)
    {
        EX_TRY
        {
            ...
            // Put a patch at each target.
            TraceDestination trace;
            for (DWORD i = 0; i < dwNumTargets; i++)
            {
                trace.InitForUnmanagedStub(pVirtualTraceCallTargets[i]);
                PatchTrace(&trace, maxFrame, true);
            }
        }
        EX_CATCH
        {
        ...
        while (p != NULL)
        {
            ...
            if (p->m_thread == thread && p->m_traceCall)
            {
                ...
                else
                {
                    ...
                    // in place, so what frame pointer is fpToCheck actually for? </STRIP>
...
bool DebuggerController::DispatchPossibleTraceCall(Thread *thread, UMEntryThunk *pUMEntryThunk, Frame *pFrame)
{
    CONTRACTL
    {
        ...
        PRECONDITION(CheckPointer(pUMEntryThunk, NULL_OK));
        PRECONDITION(CheckPointer(pFrame, NULL_OK));
    }
    CONTRACTL_END;
    ...
    if (thread != NULL)
    {
        ...
        if (pUMEntryThunk != NULL)
        {
            ...
            if (pMD != NULL)
            {
                CORDB_ADDRESS_TYPE *mt = (CORDB_ADDRESS_TYPE *)pMD->GetAddrofCode();

                // Do a DispatchTraceCall if we've got a valid target.
                if (mt != NULL)
                {
                    return DispatchTraceCall(thread, (const BYTE *)mt);
                }
            }
        }
        ...
        if ((pFrame != NULL) && (pFrame != FRAME_TOP))
        {
            ...
            if (pFrame->GetFrameType() == Frame::TYPE_ENTRY)
            {
                ...
                // Grab the managed target address.
                const BYTE *mt = utmf->GetManagedTarget(); // GC_TRIGGERS
...
void DebuggerController::EnableMethodEnter()
{
    ...
    ControllerLockHolder chController(&g_criticalSection);
...
// Adjust Ip to ensure it points  past the BP opcode for all platforms.
void AdjustIpHelper(DWORD dwCode, CONTEXT *pContext)
{
#if defined(_X86_) || defined(PLATFORM_UNIX)
#if defined(_X86_) && !defined(PLATFORM_UNIX)
        if (dwCode == EXCEPTION_BREAKPOINT && DebuggerController::g_runningOnWin95)
#else // PLATFORM_UNIX
        //
        // Note: on FreeBSD, EIP points past the breakpoint
        // opcode, while on Windows NT EIP points at the
        // breakpoint opcode. This will adjust EIP to the NT way,
        // so the debugger sees a consistent value.
        //
...
#endif // _X86_
        {
            ...
            LOG((LF_CORDB, LL_INFO1000000,  "DC::DNE: Adjusted IP: 0x%x\n", ::GetIP(pContext)));
        }
                                                 ...
                                                 Thread *pCurThread)
{
...

    {
        ...
        // Ensure the IP always points after the Breakpoint (if applicable).
        AdjustIpHelper(dwCode, pContext);
        ...
        _ASSERTE(pOldContext == NULL); // shouldn't nest.
    ...
    if (fDispatch)
    {
        ...
        ip = (CORDB_ADDRESS_TYPE *)((pContext == NULL) ? pException->ExceptionAddress : GetIP(pContext));

        switch (dwCode)
        {
        case EXCEPTION_BREAKPOINT:
#if !defined(USE_BP_FOR_PATCH_SKIP)
    ...
    m_address(patch->address)
{
    ...
    cbToProtect = MAX_INSTRUCTION_LENGTH * 2;
#else
    cbToProtect = MAX_INSTRUCTION_LENGTH;
#endif // !USE_BP_FOR_PATCH_SKIP

    _ASSERTE(DbgIsExecutable((BYTE*)m_patchBypass, cbToProtect));
#endif // _DEBUG
    ...
    _ASSERTE( patch->IsBound() );
    CopyInstructionBlock(&m_patchBypass[0], (const BYTE *)patch->address);

    // On non-IA64 platforms m_offset is always going to be 0.
    m_offset = 0;
    ...
    // set here.
    _ASSERTE( patch->IsActivated() );
    CORDbgSetInstruction((CORDB_ADDRESS_TYPE *)&m_patchBypass[m_offset], patch->opcode);

    LOG((LF_CORDB, LL_EVERYTHING, "SetInstruction was called\n"));

#ifdef USE_BP_FOR_PATCH_SKIP
    // Set the next instruction to trap
    _ASSERTE(m_offset == 0);
    CORDbgSetInstruction((CORDB_ADDRESS_TYPE *)&m_patchBypass[sizeof(CORDB_ADDRESS_TYPE)], CORDbg_BREAK_INSTRUCTION);
    LOG((LF_CORDB, LL_INFO10000,
         "DPS::DPS: Bypass BP at 0x%x = 0x%x\n", &m_patchBypass[sizeof(CORDB_ADDRESS_TYPE)], m_patchBypass[sizeof(CORDB_ADDRESS_TYPE)]));
#endif
    ...
    NativeWalker::DecodeInstructionForPatchSkip(&(m_patchBypass[m_offset]), &(m_isCall), &(m_isAbsoluteBranch)
                                                AMD64_ARG(&dwDispOffset)
                                                AMD64_ARG(&cbInstrSize));
    ...
    SetIP(context, (LPVOID)&(m_patchBypass[m_offset]));
...
#if defined(USE_OS_TO_STEP)

    //
    // The address that we just patched may have had a watchpoint created
    // on it. Since we're about to skip over that address (if it did
    // exist) delete any watchpoint that may exist for it.
    //

    _ASSERTE(thread->HasValidThreadHandle());
    LOG((LF_CORDB,LL_INFO10000, "DPS: Proactively removing watchpoint at 0x%08x\n", patch->address));
    PAL_DeleteExecWatchpoint(thread->GetThreadHandle(), (void*) patch->address);
...
#if !defined(STEPPING_SUPPORTED) && !defined(USE_BP_FOR_PATCH_SKIP)
    PORTABILITY_ASSERT("DebuggerPatchSkip (Controller.cpp) is not implemented on this platform.");
#endif
...
void DebuggerPatchSkip::CopyInstructionBlock(BYTE *to, const BYTE* from)
{
    ...
    PAL_TRY
    {
        memcpy(to, from, MAX_INSTRUCTION_LENGTH);
    }
    PAL_EXCEPT_FILTER(FilterAccessViolation2, NULL)
    {
                              ...
                              TRIGGER_WHY tyWhy)
{
    ...
    _ASSERTE((LPVOID)GetIP(context) == (LPVOID)GetBypassAddress());

    //We should be the _only_ patch here
    MethodDesc *md2 = (MethodDesc *)GetIP(context);
    ...
    EnableExceptionHook();
#if !defined(STEPPING_SUPPORTED)
    PORTABILITY_ASSERT("TriggerPatch (Controller.cpp) is not implemented on this platform.");
#endif
                                                  ...
                                                  EXCEPTION_RECORD *exception)
{
    ...
    if (m_isCall && IsSingleStep(exception->ExceptionCode))
    {
        // Fixup return address on stack
#if defined(_X86_) || defined (_PPC_) || defined(_AMD64_)
        SIZE_T *sp = (SIZE_T *) GetSP(context);

#if defined(_PPC_)
        // On ppc return address is 8 bytes (2 DWORDs) after the stack pointer
        sp += 2;
#endif // _PPC_
        ...
        *sp -= m_patchBypass - (BYTE*)m_address;
    ...
    if (!m_isAbsoluteBranch || !IsSingleStep(exception->ExceptionCode))
    {
        // Fixup IP
        ...
        if (IsSingleStep(exception->ExceptionCode))
        {
            ...
            // adjusting.
            if (m_isCall ||
                ((size_t)GetIP(context) >  (size_t)m_patchBypass &&
                 (size_t)GetIP(context) <= (size_t)(m_patchBypass + MAX_INSTRUCTION_LENGTH + 1)))
            {
                LOG((LF_CORDB, LL_INFO10000, "Bypass instruction redirected because still in skip area.\n"));
                LOG((LF_CORDB, LL_INFO10000, "m_isCall = %d, m_patchBypass = 0x%x, m_address = 0x%x\n",
                    m_isCall, m_patchBypass, m_address));
                SetIP(context, (LPVOID)((BYTE *)GetIP(context) - (&m_patchBypass[m_offset] - (BYTE *)m_address)));
            }
            else
            {
                // Otherwise, need to see if the IP is managed code - if not, we ignore the exception
                BYTE *newIP = (BYTE *)(GetIP(context));
                newIP -= &m_patchBypass[m_offset] - (BYTE *)m_address;

                if (g_pEEInterface->IsManagedNativeCode((const BYTE *)newIP))
                {
                    LOG((LF_CORDB, LL_INFO10000, "Bypass instruction redirected because we landed in managed code\n"));
                    SetIP(context, (LPVOID)newIP);
                }
...
{
    // JMC steppers should not need the JMC backstop.
    ...
    const BYTE *ipNext)
{
    ...
    if (TrapStepInto(info, ipCallTarget, &td))
    {
        ...
        if (td.GetTraceType() == TRACE_MANAGED )
        {
            ...
            md = g_pEEInterface->GetNativeCodeMethodDesc((const BYTE *)td.GetAddress());

            if ( g_pEEInterface->GetFunctionAddress(md) == (const BYTE *)td.GetAddress())
            {

                LOG((LF_CORDB,LL_INFO1000,"\tDS::TS 0x%x m_reason = STEP_CALL"
                     "@ip0x%x\n", this, (BYTE*)GetControlPC(&(info->m_activeFrame.registers))));
                  m_reason = STEP_CALL;
            }
            else
            {
...
bool DebuggerStepper::TrapStep(ControllerStackInfo *info, bool in)
{
        ...
        {            
            ...
            case WALK_CALL:
                ...
                if ( ( walker.GetNextIP() != NULL ) &&
                     ( in
                       WIN64_ONLY( || IsAddrWithinMethodIncludingFunclet(ji, info->m_activeFrame.md, walker.GetNextIP()) )
                     )
                   )
                {
                    if (TrapStepInHelper(info, walker.GetNextIP(), walker.GetSkipIP()))
                    {
                        return true;
                    }
...
#if !defined(STEPPING_SUPPORTED)
                PORTABILITY_ASSERT("TrapStep (Controller.cpp) is not implemented on this platform.");
#endif
                EnableSingleStep();
                return true;
    ...
    while (TRUE)
    {
        ...
        case WALK_CALL:
            ...
            if (in  WIN64_ONLY(|| IsAddrWithinMethodIncludingFunclet(ji, info->m_activeFrame.md, walker.GetNextIP())))
            {
...
void DebuggerStepper::TrapStepOut(ControllerStackInfo *info, bool fForceTraditional)
{
    ...
    while (info->HasReturnFrame())
    {
        ...
        // tracing up the towards the root of the stack. Likely a bug in the Left-Side's
        // stackwalker.
...
#define GROW_RANGES_IF_NECESSARY()                            \
    ...
    {                                                         \
        ...
        if (!ClrSafeInt<ULONG>::multiply(sizeof(COR_DEBUG_STEP_RANGE), (realRangeCount*2), NewSize) || \
            !ClrSafeInt<ULONG>::multiply(sizeof(COR_DEBUG_STEP_RANGE), realRangeCount, OldSize) || \
            NewSize < OldSize)                                \
        {                                                     \
            DeleteInteropSafe(m_range);                       \
            m_range = NULL;                                   \
    ...
    else
    {
        ...
        for(/**/; r < rEnd; r++, rTo++)
        {
            ...
            else
            {
                ...
                rTo->startOffset = rTo->endOffset = functionSize;
            }
        }
    }
        ...
        if (managed)
        {
                                    ...
                                    (true):(false)))
            {
                ...
                else
                {
                    ...
                    MethodDesc *mdNative = (dji == NULL) ? g_pEEInterface->GetNativeCodeMethodDesc(traceManagerRetAddr) : dji->m_fd;
                    _ASSERTE(mdNative != NULL);

                    // Find the method that the return is to.
                    _ASSERTE(g_pEEInterface->GetFunctionAddress(mdNative) != NULL);
                    SIZE_T offsetRet = traceManagerRetAddr -
                                            ...
                                            FramePointer fp)
{
    ...
    int fSkip = REGUTIL::GetConfigDWORD(L"DbgSkipMEOnStep", 0);
    if (fSkip)
    {
        return;
    }
                                    ...
                                    CorDebugStepReason unwindReason)
{
    ...
    //
    // An exception is blowing our stepping away.  Set a patch on
    // the filter/handler.
    //
...
{
    MethodDesc * pDesc = info->m_activeFrame.md;
                                            ...
                                            FramePointer fp)
{
    ...
    // If we've hit a Method-Enter, then we must have exited our original method,
    // so our ranges are no longer valid.
    ResetRange();
}
                                         ...
                                         TRIGGER_WHY tyWhy)
{
    ...
    if (module == NULL && managed)
    {
        ...
        else if ((patch->trace.GetTraceType() == TRACE_FRAME_PUSH) && (thread->GetFrame()->IsTransitionToNativeFrame()))
        {
            ...
            // do too much work to determine the destination of a ComPlusMethodFrame.                                  
            //
...
void DebuggerUserBreakpoint::HandleDebugBreak(Thread * pThread)
{
    ...
    else
    {
        ...
        g_pDebugger->AttachDebuggerForBreakpoint(pThread, NULL);
    }
}
...
bool DebuggerUserBreakpoint::SendEvent(Thread *thread, bool fIpChanged)
{
    ...
    // This is technically a bug, but it's too benign to fix.
    _ASSERTE(!fIpChanged);
                                            ...
                                            TRIGGER_WHY tyWhy)
{
    ...
    CONTEXT *pCtx = GetManagedLiveCtx(thread);
    CORDbgCopyThreadContext(pCtx, &(m_pDE->m_context));
