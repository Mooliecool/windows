...
inline int UTF78Support()
{
    // g_bUTF78Support:
    //     0 : Not tested.
    //     1 : Yes, it supports UTF7/8
    //    -1 : No, it does not support UTF7/8.

    static int g_bUTF78Support; // = 0

    if (g_bUTF78Support == 0)
    {
        // detect if utf-7/8 is supported
        char    testmb[] = "A";
        WCHAR   testwide[2];
        g_bUTF78Support = MultiByteToWideChar(CP_UTF8,MB_ERR_INVALID_CHARS,testmb,-1,testwide,2) ? 1 : -1;
    }

    return (g_bUTF78Support == 1);
}
