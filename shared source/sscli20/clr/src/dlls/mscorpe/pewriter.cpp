        ...
        if(base)
        {
            ...
            if(offset)
            {
                *offset = VAL16(IMAGE_REL_BASED_ABSOLUTE << 12);
                ...
                // padding
                if((offset = (unsigned short *) section->getBlock(2)) != NULL)
                    *offset = 0;
            }
        }
    }
...
static inline HRESULT SignedFitsIn31Bits(SSIZE_T immediate)
{
    return S_OK;
}

static inline HRESULT UnsignedFitsIn32Bits(size_t immediate)
{
    return S_OK;
}
...
static inline HRESULT AddOvf_S_U32(SSIZE_T& a, unsigned int b)
{
    SSIZE_T r = a + b;
    if (r < a)  // Check for overflow
        return E_FAIL;
...
static inline HRESULT AddOvf_S_S32(SSIZE_T& a, int b)
{
    SSIZE_T r = a + b;
    if ( ((r >= a) && (b >= 0)) ||
         ((r <  a) && (b <  0))    )
...
static inline HRESULT AddOvf_S_U(SSIZE_T& a, size_t b)
{
    SSIZE_T r = a + b;
    if (r < a)  // Check for overflow
        return E_FAIL;
...
static inline HRESULT AddOvf_U_U32(size_t& a, unsigned int b)
{
    size_t r = a + b;
    if (r < a)  // Check for overflow
        return E_FAIL;
...
static inline HRESULT AddOvf_U_U(size_t& a, size_t b)
{
    size_t r = a + b;
    if (r < a)  // Check for overflow
        return E_FAIL;
...
static inline HRESULT SubOvf_S_U32(SSIZE_T& a, unsigned int b)
{
    SSIZE_T r = a - b;
    if (r > a)  // Check for overflow
        return E_FAIL;
...
static inline HRESULT SubOvf_S_U(SSIZE_T& a, size_t b)
{
    SSIZE_T r = a - b;
    if (r > a)  // Check for overflow
        return E_FAIL;
...
static inline HRESULT SubOvf_U_U32(size_t& a, unsigned int b)
{
    size_t r = a - b;
    if (r > a)  // Check for overflow
        return E_FAIL;
...
static inline HRESULT SubOvf_U_U(SSIZE_T& r, size_t a, size_t b)
{
    r = a - b;
    if ( ((a >= b) && (r >= 0))  ||
         ((a <  b) && (r <  0)))
                                     ...
                                     DWORD                codeRvaBase)
{
    ...
    C_ASSERT(NumItems(RelocName) == srRelocSentinel);
#ifdef _DEBUG
    ...
    size_t imageBase = isPE32 ? (size_t) VAL32(((IMAGE_NT_HEADERS32*) pNtHeaders)->OptionalHeader.ImageBase)
                              : (size_t) VAL64(((IMAGE_NT_HEADERS64*) pNtHeaders)->OptionalHeader.ImageBase);
    ...
    for(PESectionReloc* cur = m_relocStart; cur < m_relocCur; cur++)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                {
                    _ASSERTE(slotNum == 1);
                    oldStarPos = (SSIZE_T) GetIA64Rel64((UINT64 *) pos);
                }
                else if (curType == srRelocIA64Imm64)
                {
                    oldStarPos = (SSIZE_T) GetIA64Imm64((UINT64 *)pos);
                }
                else
                {
        ...
        bool    fNeedBrl     = false;
        SSIZE_T newStarPos   = 0; // oldStarPos gets updated to newStarPos
        ...
        {
            if (externalAddress) {
                size_t targetAddr = GET_UNALIGNED_VAL32(pos);
        ...
        else if (curType == srRelocIA64PcRel64)
        {
            ...
            newStarPos = (SSIZE_T) GetIA64Rel64((UINT64 *) pos);
        ...
        else if (curType == srRelocHighLow)
        {
            ...
            size_t value = GET_UNALIGNED_VAL32(pos);

            if (!externalAddress)
            {
                IfFailRet(AddOvf_U_U32(value, (unsigned int) cur->section->m_baseRVA));
        ...
        else if (curType == srRelocDir64)
        {
            ...
            size_t UNALIGNED * p_value = (size_t *) pos;
        ...
        else if (curType == srRelocIA64Imm64)
        {
            ...
            targetOffset = (size_t) GetIA64Imm64((UINT64 *)pos);
...
HRESULT PEWriter::Init(PESectionMan *pFrom, DWORD createFlags, LPCWSTR seedFileName)
{
    ...
    m_ntHeaders->OptionalHeader.MajorLinkerVersion  = 8;
    ...
    if (isPE32())
    {
        ...
        p_ntHeaders32->OptionalHeader.ImageBase             = VAL32(0x400000);
    ...
    else
    {
        ...
        p_ntHeaders64->OptionalHeader.ImageBase             = VAL64(0x400000);
...
HRESULT PEWriter::linkSortHeaders(entry * entries, unsigned iEntries, unsigned iUniqueSections)
{
    ...
    headers = new (nothrow) IMAGE_SECTION_HEADER [iUniqueSections + 1]; // 1 extra for .reloc
    TESTANDRETURNMEMORY(headers);

    memset(headers, 0, sizeof(*headers) * (iUniqueSections + 1));
...
HRESULT PEWriter::linkPlaceSections(entry * entries, unsigned iEntries)
{
    ...
    for (IMAGE_SECTION_HEADER * h = headers; h < headersEnd; h++)
    {
        ...
        while (++e < entriesEnd)
        {
           ...
           bool isText6 = (strcmp(s->m_name, ".text6") == 0);
           ...
           {
               sPrev->m_filePad = padLen(dataSize, TEXT6_ALIGN);
               dataSize = roundUp(dataSize, TEXT6_ALIGN);
           }
...
void PEWriter::setSectionIndex(IMAGE_SECTION_HEADER * h, unsigned sectionIndex) {
    ...
    static char * const SpecialNames[] = { ".text", ".cormeta", NULL };
    enum { SPECIAL_NAMES_COUNT = NumItems(SpecialNames) };

    for (char * const * s = SpecialNames; /**/; s++)
    {
        if (*s == 0)
        {
...
HRESULT PEWriter::link() {
    ...
    if( VAL32(0) == m_ntHeaders->OptionalHeader.FileAlignment )
    {
        ...
        if (ExeOrDll)
        {
            const int smallFileAlignment   = 0x0200;
            const int optimalFileAlignment = 0x1000;

            int size = 0, waste = 0;
            IMAGE_SECTION_HEADER *curheader = headers;
            while (curheader < headersEnd)
            {
                size += roundUp(VAL32(curheader->Misc.VirtualSize), optimalFileAlignment);
                waste += padLen(VAL32(curheader->Misc.VirtualSize), optimalFileAlignment);
                curheader++;
            }
...
HRESULT PEWriter::write(const LPWSTR fileName) {
    ...
    if (!ExeOrDll)
    {
        ...
        long lena = padLen(VAL32(reloc->m_header->Misc.VirtualSize)+strtab->dataLen(), RoundUpVal);
        if (lena > 0)
