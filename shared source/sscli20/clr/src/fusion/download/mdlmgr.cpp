...
HRESULT CModDownloadMgr::Init(LPCWSTR pwzCodebase, LPCWSTR pwzModuleName)
{
    ...
    _ASSERTE(!_pwzModuleName && pwzModuleName && !_pwzCodebase && pwzCodebase);
    ...
    hr = StringCchCopy(_pwzModuleName, iLen, pwzModuleName);
    if (FAILED(hr)) {
        goto Exit;
    }
    ...
    hr = StringCchCopy(_pwzCodebase, iLen, pwzCodebase);
    if (FAILED(hr)) {
        goto Exit;
    }
...
HRESULT CModDownloadMgr::DoSetup(LPCWSTR wzSourceUrl, 
                                 LPCWSTR wzFilePath,
...
HRESULT CModDownloadMgr::PreDownloadCheck(void **ppv, void **ppvNI)
{
    *ppv = NULL;
    return E_NOTIMPL;
}
...
HRESULT CModDownloadMgr::GetCodebase(DWORD dwIndex, DWORD *pdwFlags, __out_ecount_opt(*pcbCodebase) LPWSTR wzCodebase,
                                     DWORD *pcbCodebase)
{
    HRESULT                              hr = S_OK;
    DWORD                                dwLen;
    ...
    *pdwFlags = 0;
    ...
    hr = StringCchCopy(wzCodebase, *pcbCodebase, _pwzCodebase);
    if (FAILED(hr)) {
        goto Exit;
    }
...
HRESULT CModDownloadMgr::GetBindInfo(FusionBindInfo *pBindInfo)
{
    return S_OK;
}

HRESULT CModDownloadMgr::CacheBindingResult(HRESULT hrResult)
{
    return S_OK;
}
...
{    
    ...
    hr = CAssemblyCacheItem::Create(pAppCtx, pName, (LPTSTR)pszURL, pftLastModTime,
                                    ASM_CACHE_DOWNLOAD,
                                    pManImport, NULL, NULL, (IAssemblyCacheItem **)&pAsmItem);
    if (FAILED(hr)) {
