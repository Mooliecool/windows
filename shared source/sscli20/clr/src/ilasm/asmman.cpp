...
AsmManComType*          AsmMan::GetComTypeByName(__in __nullterminated char* szComTypeName)
{
...
mdToken             AsmMan::GetComTypeTokByName(__in __nullterminated char* szComTypeName)
{
...
void    AsmMan::SetAssemblyLocale(BinStr* pLocale, BOOL bConvertToUnicode)
{
    if(m_pCurAsmRef)
    {
        if((m_pCurAsmRef->pLocale = bConvertToUnicode ? ::BinStrToUnicode(pLocale) : pLocale))
        {
            WCHAR* wz;
            for(wz = (WCHAR*)(m_pCurAsmRef->pLocale->ptr()); *wz; wz++)
            {
                if((*wz == '/')||(*wz == '\\')||(*wz == ':'))
                {
                    report->error("Invalid culture specified: %S\n", (WCHAR*)(m_pCurAsmRef->pLocale->ptr()));
                    delete  m_pCurAsmRef->pLocale;
                    m_pCurAsmRef->pLocale = NULL;
                    return;
                }
            }
        }
