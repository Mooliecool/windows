...
%type <int32> iidParamIndex genArity
%type <float64> float64
%type <int64> int64
%type <binstr> sigArgs0 sigArgs1 sigArg type bound bounds1 bytes hexbytes nativeType initOpt compQstring caValue
...
decl                    : classHead '{' classDecls '}'                          { PASM->EndClass(); }
                        ...
                        | secDecl
                        | customAttrDecl
                        | _SUBSYSTEM int32                                      { PASM->m_dwSubsystem = $2; }
...
customBlobArgs          : /* EMPTY */                                           { $$ = new BinStr(); $$->appendInt16(VAL16(0x0001)); }
                        | customBlobArgs serInit                                { $$ = $1; $$->appendFrom($2,1); }
...
customAttrDecl          : customDescr                                           { if($1->tkOwner) 
                                                                                    PASM->DefineCV($1);
                                                             ...
                                                             { __int8 i = (__int8) $3; FAIL_UNLESS(i == $3, ("Value %d too big\n", $3));
                                                               ...
                                                               if(p != NULL) {
                                                                 ...
                                                                 PASM->EmitData(p, sizeof(__int8)*$5); delete [] p;
                                                                     ...
                                                                     sizeof(__int8)*$5); }
                        | FLOAT32_ ddItemCount               { float* p = new (nothrow) float[$2];
                                                               if(p != NULL) {
                                                                 PASM->EmitData(p, sizeof(float)*$2); delete [] p;
                                                               } else PASM->report->error("Out of memory emitting data block %d bytes\n",
                                                                     sizeof(float)*$2); }
                        | FLOAT64_ ddItemCount               { double* p = new (nothrow) double[$2];
                                                               if(p != NULL) {
                                                                 PASM->EmitData(p, sizeof(double)*$2); delete [] p;
                                                               } else PASM->report->error("Out of memory emitting data block %d bytes\n",
                                                                     sizeof(double)*$2); }
                        | INT64_ ddItemCount                 { __int64* p = new (nothrow) __int64[$2];
                                                               if(p != NULL) {
                                                                 PASM->EmitData(p, sizeof(__int64)*$2); delete [] p;
                                                               } else PASM->report->error("Out of memory emitting data block %d bytes\n",
                                                                     sizeof(__int64)*$2); }
                        | INT32_ ddItemCount                 { __int32* p = new (nothrow) __int32[$2];
                                                               if(p != NULL) {
                                                                 PASM->EmitData(p, sizeof(__int32)*$2); delete [] p;
                                                               } else PASM->report->error("Out of memory emitting data block %d bytes\n",
                                                                     sizeof(__int32)*$2); }
                        | INT16_ ddItemCount                 { __int16* p = new (nothrow) __int16[$2];
                                                               if(p != NULL) {
                                                                 PASM->EmitData(p, sizeof(__int16)*$2); delete [] p;
                                                               } else PASM->report->error("Out of memory emitting data block %d bytes\n",
                                                                     sizeof(__int16)*$2); }
                        | INT8_ ddItemCount                  { __int8* p = new (nothrow) __int8[$2];
                                                               if(p != NULL) {
                                                                 PASM->EmitData(p, sizeof(__int8)*$2); delete [] p;
                                                               } else PASM->report->error("Out of memory emitting data block %d bytes\n",
                                                                     sizeof(__int8)*$2); }
                        ;
...
tyArgs2                 : tyArgs1 ',' type                   { $$ = $1; $$->append($3); delete $3; } 
                        | tyArgs1 ','                        { $$ = $1; }
                        ;
...
esHead                  : _LINE                               { nCurrPC = PASM->m_CurPC; PENV->bExternSource = TRUE; PENV->bExternSourceAutoincrement = FALSE; }
                        | P_LINE                              { nCurrPC = PASM->m_CurPC; PENV->bExternSource = TRUE; PENV->bExternSourceAutoincrement = TRUE; }
                        ;
...
extSourceSpec           : esHead int32 SQSTRING               { PENV->nExtLine = PENV->nExtLineEnd = $2;
                        ...
                        | esHead int32 ':' int32 SQSTRING     { PENV->nExtLine = PENV->nExtLineEnd = $2; 
                                                                PENV->nExtCol=$4; PENV->nExtColEnd = PENV->nExtCol;
                        ...
                        | esHead int32 ',' int32 ':' int32 SQSTRING     
                                                              { PENV->nExtLine = $2; PENV->nExtLineEnd = $4; 
                                                                PENV->nExtCol=$6; PENV->nExtColEnd = PENV->nExtCol;
                                                                PASM->SetSourceFileName($7);}
                        | esHead int32 ',' int32 ':' int32     
                                                              { PENV->nExtLine = $2; PENV->nExtLineEnd = $4; 
                                                                PENV->nExtCol=$6; PENV->nExtColEnd = PENV->nExtCol; }
...
exptypeDecl             : _FILE dottedName                    { PASMM->SetComTypeFile($2); }
                        | _CLASS EXTERN_ dottedName           { PASMM->SetComTypeComType($3); }
