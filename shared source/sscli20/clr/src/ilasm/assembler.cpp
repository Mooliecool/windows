...
mdToken Assembler::ResolveClassRef(mdToken tkResScope, __in __nullterminated char *pszFullClassName, Class** ppClass)
{
    ...
    if(ptkSpecial) // special token
    {
        if(*ptkSpecial) // already resolved
        {
            ...
            if(!m_fIsMscorlib) tkResScope = GetAsmRef("mscorlib");
    }
    if(tkResScope == 1)
    {
...
/**************************************************************************/
mdToken Assembler::MakeTypeRef(mdToken tkResScope, __in __nullterminated char* pszFullClassName)
{
    mdToken tkRet = mdTokenNil;
...
/**************************************************************************/
GlobalLabel *Assembler::FindGlobalLabel(__in __nullterminated char *pszName)
{
