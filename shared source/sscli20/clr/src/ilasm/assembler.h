...
struct CustomDescr
{
    ...
    CustomDescr(CustomDescr* pOrig) { tkType = pOrig->tkType; pBlob = new BinStr(); pBlob->append(pOrig->pBlob); tkOwner = pOrig->tkOwner;};
...
public:
    ...
    GlobalLabel(__in __nullterminated char *pszName, DWORD GlobalOffset, HCEESECTION section)
    {
        m_GlobalOffset  = GlobalOffset;
        m_Section       = section;
        m_szName = pszName;
        m_Hash = hash((BYTE*)pszName, (unsigned)strlen(pszName),10);
    }
    ~GlobalLabel(){ delete [] m_szName; }
    int ComparedTo(GlobalLabel* L) { return (m_Hash == L->m_Hash) ? 0 : ((m_Hash > L->m_Hash) ? 1 : -1); };
...
public:
    ...
    GlobalLabel *FindGlobalLabel(__in __nullterminated char *pszName);
    ...
    mdToken MakeTypeRef(mdToken tkResScope, __in __nullterminated char* szFullName);
