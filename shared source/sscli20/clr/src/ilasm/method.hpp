...
//public:
    ...
    Label(char *pszName, DWORD PC):m_szName(pszName),m_PC(PC){};
...
//    LabelList       m_lstLabel;
    ...
    int findArgNum(ARG_NAME_LIST *pFirst, __in __nullterminated char *szArgName, DWORD dwArgName)
    {
        int ret=-1;
        if(dwArgName)
        {
    ...
    int findLocSlot(ARG_NAME_LIST *pFirst, __in __nullterminated char *szArgName, DWORD dwArgName)
    {
        int ret=-1;
        if(dwArgName)
        {
    ...
    Label *FindLabel(__in __nullterminated char *pszName);
