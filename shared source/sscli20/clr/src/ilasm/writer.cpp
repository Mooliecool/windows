...
HRESULT Assembler::CreatePEFile(__in __nullterminated WCHAR *pwzOutputFilename)
{
    ...
    {
        wcscpy_s(wzScopeName,MAX_SCOPE_LENGTH,m_wzOutputFileName);
        WCHAR* pwc;
        if((pwc = wcsrchr(wzScopeName,'\\'))) wzScopeName = pwc+1;
        else if((pwc = wcsrchr(wzScopeName,':'))) wzScopeName = pwc+1;
    }
