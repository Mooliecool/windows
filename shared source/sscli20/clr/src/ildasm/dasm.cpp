...
WCHAR* RstrW(unsigned id)
{
    ...
    if (g_hSatelliteInstance == NULL)
    {
        ...
        if (sizeof(MESSAGE_DLL) + (pEnd - path) * sizeof(WCHAR) > (int) sizeof(path) - 1)
            return 0;
        wcscpy_s(pEnd, sizeof(MESSAGE_DLL)/sizeof(WCHAR)+1,MESSAGE_DLL);
    ...
    // Get the string
    PAL_LoadSatelliteStringW(g_hSatelliteInstance, id, buff, cchBuff);
    if(id == IDS_E_NORVA)
...
{
    {
        if(SUCCEEDED(CoInitializeCor(COINITCOR_DEFAULT)))
        {
            if(SUCCEEDED(CoInitializeEE(COINITEE_DEFAULT)))
            {
...
void Cleanup()
{
    ...
    if(rAsmRefName)
    {
        ...
        {
            if(rAsmRefName[i]) VDELETE(rAsmRefName[i]);
        }
        VDELETE(rAsmRefName);
        ulNumAsmRefs = 0;
    }
...
BOOL EnumClasses()
{
    ...
    //--------------------------------------------------------------
    g_pImport->EnumAllInit(mdtTypeRef,&hEnum);
    ...
    //--------------------------------------------------------------
    g_pImport->EnumAllInit(mdtAssemblyRef,&hEnum);
    ...
    for(i = 0; i <= g_NumClasses; i++)
    {
        ...
        {
            const char* pszName = g_pImport->GetNameOfMethodDef(pMemberList[j]);
            ULONG cSig;
            PCCOR_SIGNATURE pSig = g_pImport->GetSigOfMethodDef(pMemberList[j], &cSig);
            for(k = 0; k < j; k++)
            {
                if(0==strcmp(pszName, g_pImport->GetNameOfMethodDef(pMemberList[k])))
                {
                    ULONG cSig1;
                    PCCOR_SIGNATURE pSig1 = g_pImport->GetSigOfMethodDef(pMemberList[k], &cSig1);
        ...
        for(j = 1; j < NumMembers; j++)
        {
            const char* pszName = g_pImport->GetNameOfFieldDef(pMemberList[j]);
            ULONG cSig;
            PCCOR_SIGNATURE pSig = g_pImport->GetSigOfFieldDef(pMemberList[j], &cSig);
            for(k = 0; k < j; k++)
            {
                if(0==strcmp(pszName, g_pImport->GetNameOfFieldDef(pMemberList[k])))
                {
                    ULONG cSig1;
                    PCCOR_SIGNATURE pSig1 = g_pImport->GetSigOfFieldDef(pMemberList[k], &cSig1);
                    if((cSig == cSig1)&&(0==memcmp(pSig,pSig1,cSig)))
                    {
                        (*g_dups)[g_NumDups++] = pMemberList[j];
                        break;
...
BOOL EnumTypedefs()
{
    ...
    g_NumTypedefs = 0;
    if(SUCCEEDED(g_pImport->EnumAllInit(mdtTypeSpec, &hEnum)))
    {
        for (i = 0; g_pImport->EnumNext(&hEnum, &tk); i++)
        {
            ULONG cSig;
            PCCOR_SIGNATURE sig = g_pImport->GetSigFromToken(tk, &cSig);
            if(*sig == ELEMENT_TYPE_TYPEDEF)
            {
                ...
                {
                    pTDD->psig = g_pImport->GetSigFromToken(pTDD->tkTypeSpec,&(pTDD->cb));
                }
...
void DumpByteArray(__inout __nullterminated char* szString, BYTE* pBlob, ULONG ulLen, void* GUICookie)
{
    ...
    for(j = 0, k=0, m=0; j < ulLen; j++,k++,m++)
    {
        ...
        if(isprint(byt))
        {
            ...
            {
                if(byt == '<') { sz[m] = 0; strcat_s(sz,256,LTN()); m+=3; }
                else if(byt == '>') { sz[m] = 0; strcat_s(sz,256,GTN()); m+=3; }
                else sz[m] = byt;
            }
            else sz[m] = byt;
            printsz = TRUE;
                                         ...
                                         mdToken tkEncloser)
{
    ...
    if(SUCCEEDED(pIMDI->FindTypeDef((LPCSTR)szNamespace,(LPCSTR)szName,tkEncloser,&tk)))
        return tk;
    else
        return 0;
}
    ...
    if(str)
    {
        ...
        if(szAssembly != NULL)
        {
            ...
            if(mAsmRefs)
            {
                ...
                for (;tk <= tkmax; tk++) 
                {
                    pIMDI->GetAssemblyRefProps(tk,&pPKT,&ulPKT,&szAsmRefName,&MD,&pHash,&ulHash,&dwFlags);
...
unsigned UnderlyingTypeOfEnumTypeDef(mdToken tk, IMDInternalImport *pIMDI)
{
    ...
    mdToken tkParent;
    DWORD dwAttr;
    pIMDI->GetTypeDefProps(tk,&dwAttr,&tkParent);
    if(RidFromToken(tkParent)==0) return 0;
    LPCSTR szName, szNamespace;
    switch(TypeFromToken(tkParent))
    {
        case mdtTypeDef:
            pIMDI->GetNameOfTypeDef(tkParent,&szName,&szNamespace);
            break;
    
        case mdtTypeRef:
            pIMDI->GetNameOfTypeRef(tkParent,&szName,&szNamespace);
            break;
    
        default:
            return 0;
    }
    if(strcmp(szName,"Enum") || strcmp(szNamespace,"System")) return 0;
    
    // OK, it's an enum; find its instance field and get its type
    HENUMInternal hEnum;
    mdToken tkField;
    pIMDI->EnumInit(mdtFieldDef,tk,&hEnum);
    while(pIMDI->EnumNext(&hEnum,&tkField))
    {
        dwAttr = pIMDI->GetFieldDefProps(tkField);
        if(IsFdStatic(dwAttr)) continue;
        PCCOR_SIGNATURE psig = pIMDI->GetSigOfFieldDef(tkField,(ULONG*)&dwAttr);
...
mdToken TypeRefToTypeDef(mdToken tk, IMDInternalImport *pIMDI, IMDInternalImport **ppIMDInew)
{
    ...
    mdToken tkAlienTD = mdTypeDefNil;
    *ppIMDInew = NULL;

    // get the resolution scope of TypeRef
    mdToken tkRS = pIMDI->GetResolutionScopeOfTypeRef(tk);
    ...
    else if(TypeFromToken(tkRS) == mdtAssemblyRef)
    {
        *ppIMDInew = g_asmref_import[RidFromToken(tkRS)];
        if(*ppIMDInew == NULL)
        {
            ...
            md.szLocale = wzLocale;
            md.cbLocale = 1024;
            pAssemblyImport->GetAssemblyRefProps(tkRS,&pPKT,&cPKT,wzName,2048,&cName,&md,&pHash,&cHash,&dwFlags);

            IUnknown* pIAMDI[64];

            PAL_TRY {
                if(FAILED(pAssemblyImport->FindAssembliesByName(NULL,NULL,(LPCWSTR)wzName,pIAMDI,64,&cPKT)))
                    cPKT=0;
    ...
    {
        LPCSTR szName, szNamespace;
        pIMDI->GetNameOfTypeRef(tk,&szName,&szNamespace);


        if(FAILED((*ppIMDInew)->FindTypeDef(szName,szNamespace,tkEncloser,&tkAlienTD))) 
            tkAlienTD = mdTypeDefNil;
    }
  AssignAndReturn:
    return tkAlienTD;
}
unsigned UnderlyingTypeOfEnum(mdToken tk, IMDInternalImport *pIMDI)
{
    ...
    else if(TypeFromToken(tk)==mdtTypeRef)
    {
        if(g_enum_tr_type[ix] == 0xFF)
        {
            ...
            mdToken tkAlienTD = TypeRefToTypeDef(tk,pIMDI, &pIMDInew);
            if((RidFromToken(tkAlienTD)!=0)&&(pIMDInew != NULL))
            {
                uRet = UnderlyingTypeOfEnumTypeDef(tkAlienTD,pIMDInew);
            }
...
BOOL PrettyPrintCustomAttributeBlob(mdToken tkType, BYTE* pBlob, ULONG ulLen, void* GUICookie, __inout __nullterminated char* szString)
{
    ...
    if(TypeFromToken(tkType) == mdtMemberRef)
        pIMDI->GetNameAndSigOfMemberRef(tkType,&typePtr,&typeLen);
    else if(TypeFromToken(tkType) == mdtMethodDef)
        typePtr = pIMDI->GetSigOfMethodDef(tkType, &typeLen);
    else
        return FALSE;
                                                ...
                                                (const void**)&pvPermission, &cbPermission)))
            {
                ...
                if(pvPermission && cbPermission)
                {
                    ...
                    if(!DumpPermissionSetBlob(GUICookie,szString,(BYTE*)pvPermission,cbPermission))
                        DumpUnicodeString(GUICookie,szString,(WCHAR *)pvPermission,cbPermission>>1, true);
                    printLine(GUICookie,szString);
                }
...
void DumpGenericParsCA(mdToken tok, void* GUICookie/*=NULL*/)
{
    ...
    if(g_fShowCA)
    {
                    ...
                    &&(NumTyPars > 0); i++)
        {
            ...
            g_pImport->EnumInit(mdtCustomAttribute, tkTyPar,&hEnum);
            ulCAs = g_pImport->EnumGetCount(&hEnum);
            if(ulCAs)
            {
                char    *szptr = &szString[0];
...
void PrettyPrintOverrideDecl(ULONG i, __inout __nullterminated char* szString, void* GUICookie, mdToken tkOverrider)
{
    ...
    if(g_pImport->IsValidToken(tkDecl))
    {
        ...
        if(TypeFromToken(tkDecl) == mdtMethodSpec)
        {
            ...
            ULONG       cSig=0;
            g_pImport->GetMethodSpecProps(tkDecl, &meth, &pSig, &cSig);
        ...
        if(TypeFromToken(tkDecl) == mdtMethodDef)
            pszMemberName = g_pImport->GetNameOfMethodDef(tkDecl);
        else if(TypeFromToken(tkDecl) == mdtMemberRef)
        {
            PCCOR_SIGNATURE pComSig;
            ULONG       cComSig;

            pszMemberName = g_pImport->GetNameAndSigOfMemberRef(tkDecl,
                &pComSig,
                &cComSig
            );
        }
...
lDone: ;
    ...
    for(ULONG i = 0; i < g_NumMI; i++)
    {
        ...
        {
            PrettyPrintOverrideDecl(i,szString,GUICookie,FuncToken);
            printLine(GUICookie,szString);
        }
    }
...
//            +4-members
{
    ...
    if(WhatToDump & 4) // 4 - dump members
    {
        for(i = 0; i < g_NumMI; i++)
        {
            ...
            {
                PrettyPrintOverrideDecl(i,szString,GUICookie,cl);
...
void DumpHeader(IMAGE_COR20_HEADER *CORHeader, void* GUICookie)
{
    ...
    if (g_pPELoader->IsPE32())
    {
        ...
        printLine(GUICookie,szStr);
        sprintf_s(szString,SZSTRING_SIZE,"// Subsystem:                      0x%08x", VAL16(pOptHeader->Subsystem));
        printLine(GUICookie,szStr);
        sprintf_s(szString,SZSTRING_SIZE,"// DLL characteristics:            0x%08x", VAL16(pOptHeader->DllCharacteristics));
        printLine(GUICookie,szStr);
...
#else
        if((g_PerfDataFilePtr = CreateFile(L"/tmp/perfdata.dat", GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ, NULL, OPEN_ALWAYS, 0, NULL) ) == INVALID_HANDLE_VALUE)
#endif
        {
...
void DumpStatistics(IMAGE_COR20_HEADER *CORHeader, void* GUICookie)
{
   ...
   else
   {
    ...
    if (VAL32(CORHeader->MetaData.Size) - metaSize > 0)
    {
        ...
        sprintf_s(szString,SZSTRING_SIZE,"//   Uncategorized - %5d bytes", VAL32(CORHeader->MetaData.Size) - metaSize);
        printLine(GUICookie,szStr);
    }
    ...
    *szptr = 0;
    for(i = 0,k = 0,curPtr=fromPtr; curPtr < toPtr; i++,k++,curPtr++,pb++)
    {
        ...
        if(isprint(*pb))
        {
            ...
            {
                if(*pb == '<') { sz[k] = 0; strcat_s(sz,256,LTN()); k+=3; }
                else if(*pb == '>') { sz[k] = 0; strcat_s(sz,256,GTN()); k+=3; }
            }
            else sz[k] = *pb;
            printsz = TRUE;
        }
...
void DumpVtable(void* GUICookie)
{
    ...
    else
    {
        ...
        sprintf_s(szString,SZSTRING_SIZE,"%s%s 0x%08x", g_szAsmCodeIndent,KEYWORD(".imagebase"),VAL32(pOptHeader64->ImageBase));
        printLine(GUICookie,szString);
        ...
        printLine(GUICookie,szString);
        sprintf_s(szString,SZSTRING_SIZE,"%s%s 0x%016x", g_szAsmCodeIndent,KEYWORD(".stackreserve"),VAL64(pOptHeader64->SizeOfStackReserve));
        printLine(GUICookie,szString);
    }
    szptr = &szString[0];
    ...
    if(i != 0)
    {
        ...
        if(i & COMIMAGE_FLAGS_32BITREQUIRED) szp += sprintf_s(szp,256-(szp-sz)," 32BITREQUIRED");
...
// MetaInfo integration:
void DumpMI(__in __nullterminated char *str)
{
    ...
    HRESULT     hrRpt)                  // HRESULT for the message
{
    ...
    if(szMsg)
    {
        ...
        if((wzMsg = (WCHAR*)malloc(sizeof(WCHAR)*L))) //new WCHAR[lstrlenW(szMsg)+256])
        {
...
void DumpMetaInfo(__in __nullterminated char* pszFileName, __in_opt __nullterminated char* pszObjFileName, void* GUICookie)
{
    ...
    {   // This works only when all the rest does not
        // Init and run.
        {
            if(SUCCEEDED(CoInitializeCor(0)))
            {
...
void DumpPreamble()
{
    ...
    sprintf_s(szString,SZSTRING_SIZE,"//  Microsoft (R) Shared Source CLI IL Disassembler.  Version " VER_FILEVERSION_STR);
    printLine(g_pFile,COMMENT(szString));
    sprintf_s(szString,SZSTRING_SIZE,"//  %s", VER_LEGALCOPYRIGHT_LOGO_STR);
    printLine(g_pFile,COMMENT(szString));
    if(g_fDumpHTML)
    {
...
mdToken ClassOf(mdToken tok)
{
    ...
    switch(TypeFromToken(tok))
    {
        ...
        case mdtMethodDef:
        case mdtMemberRef:
            g_pImport->GetParentToken(tok,&retval);
...
FILE* OpenOutput(__in __nullterminated WCHAR* wzFileName)
{
    FILE*   pfile = NULL;
    if(g_fOnUnicode)
    {
    ...
    {
        if(g_uCodePage == 0xFFFFFFFF) fopen_s(&pfile,UnicodeToAnsi(wzFileName),"wb");
        else fopen_s(&pfile,UnicodeToAnsi(wzFileName),"wt");
    }
    if(pfile)
    {
...
BOOL DumpFile(__in __nullterminated char *pszFilename)
{
    ...
    }
    {
        ...
        if(pch && (!_stricmp(pch+1,"lib") || !_stricmp(pch+1,"obj")))
        {
                DumpMetaInfo(g_szInputFile,g_pszObjFileName,g_pFile);
            return FALSE;
        }
    }
    ...
    GetMetaDataPublicInterfaceFromInternal(g_pImport, IID_IMetaDataImport2, (void**)&g_pPubImport);
    // Get a symbol binder.
