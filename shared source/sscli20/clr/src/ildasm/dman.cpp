...
void DumpModuleRefs(void *GUICookie)
{
    ...
    for(ULONG i = 0; i < N; i++)
    {
        if(RidFromToken(tk[i]))
        {
            ...
            if(*szName) // ignore the no-name ModuleRef: it's an IJW artifact
            {
                szptr = &szString[0];
                        ...
                        void* GUICookie)
{
    ...
    if(RidFromToken(tkImplementation))
    {
        ...
        else if(TypeFromToken(tkImplementation) == mdtExportedType)
        {
            ...
            {
                {
                    UINT32 L = (UINT32)wcslen((*g_pLocalComType)[i]->wzName)*3+3;
                    char* sz = new char[L];
                    memset(sz,0,L);
                    WszWideCharToMultiByte(CP_UTF8,0,(*g_pLocalComType)[i]->wzName,-1,sz,L,NULL,NULL);
                    sprintf_s(szString,SZSTRING_SIZE,"%s%s %s",g_szAsmCodeIndent,KEYWORD(".class extern"),
                            JUMPPT(ProperName(sz),tkImplementation));
                    VDELETE(sz);
                }
                pc=&szString[strlen(szString)];
        ...
        if(nManRes)
        {
            ...
            for(ULONG ix = 0; ix < nManRes; ix++)
            {
                                                                ...
                                                                &dwFlags)))         // [OUT] Flags.
                {
                    ...
                    // check the name for ':' ',' ';' and '\\', form the alias if needed
                    bool fAlias = false;
                    for(pc = sz; (pc = strchr(pc,':'))!=NULL;)
                    {
                        fAlias = true;
                        *pc = '!';
                    }
                    ...
                    if(fAlias)
                    {
                        ...
                        // correct wzFileName accordingly
                        WszMultiByteToWideChar(CP_UTF8,0,sz,-1,wzName,1024);
                    }
                    printLine(GUICookie,szString);
                    sprintf_s(szString,SZSTRING_SIZE,"%s%s",g_szAsmCodeIndent,SCOPE());
                    printLine(GUICookie,szString);
                    ...
                    if(tkImplementation == mdFileNil) // embedded resource -- dump as .resources file
                    {
                        ...
                        if(pRes)
                        {
                            ...
                            {
                                DWORD L;
                                memcpy(&L,pRes+dwOffset,sizeof(DWORD));
                                sprintf_s(szString,SZSTRING_SIZE,COMMENT("%s// Offset: 0x%8.8X Length: 0x%8.8X"), g_szAsmCodeIndent,dwOffset,L);
                                printLine(GUICookie,szString);
                                if(g_pFile!=NULL) // embedded resource -- dump as .resources file
                                {
                                    FILE  *pF = NULL;
                                    if(g_fOnUnicode) _wfopen_s(&pF,wzFileName,L"wb");
                                    else
                                    {
                                        WszWideCharToMultiByte(g_uConsoleCP,0,wzFileName,-1,sz,sizeof(sz),NULL,NULL);
                                        fopen_s(&pF,sz,"wb");
                                    }
                                    if(pF)
                                    {
                                        PAL_TRY {
                                            ...
                                            sprintf_s(szString,SZSTRING_SIZE,RstrUTF(IDS_W_CREATEDMRES),g_szAsmCodeIndent,ProperName(sz));
                                            printLine(GUICookie,COMMENT(szString));
                                        }
                                        PAL_EXCEPT(EXCEPTION_EXECUTE_HANDLER)
                                        {
                                            sprintf_s(szString,SZSTRING_SIZE,RstrUTF(IDS_E_READINGMRES),g_szAsmCodeIndent,ProperName(sz),dwOffset);
                                            printError(GUICookie,szString);
                                        }
                                        PAL_ENDTRY
                                        fclose(pF);
...
{
    char* pVersionStr = (char*)g_pMetaData + 16;
