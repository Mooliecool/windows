...
struct HENUMInternal
{
    ...
    HENUMInternal() : m_EnumType(MDSimpleEnum) {LEAF_CONTRACT; }
...
EXTERN_GUID(IID_IMDInternalDispenser, 0xce0f34ee, 0xbbc6, 0x11d2, 0x94, 0x1e, 0x0, 0x0, 0xf8, 0x8, 0x34, 0x60);
...
class HENUMInternalHolderNoRef // doesn't do AddRef. Users of this holder should guarantee it's valid
{
    struct HENUMInternal _pEnum;
    IMDInternalImport* _pMDImport;
public:
    HENUMInternalHolderNoRef(IMDInternalImport* pMDImport)
    {
        LEAF_CONTRACT;
    ...
    ~HENUMInternalHolderNoRef()
    {
        WRAPPER_CONTRACT;

        if (_pMDImport)
            _pMDImport->EnumClose(&_pEnum);
    };
    HENUMInternal* operator &()
    {
        LEAF_CONTRACT;
