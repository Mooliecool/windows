...
#if !defined(GOLDEN) && !defined(_WIN64) && !defined(DACCESS_COMPILE)
...
extern wchar_t *wszUnitOfMeasureDescr[MAX_UNITS_OF_MEASURE];
...
// an improvement.
extern wchar_t *wszIDirection[MAX_UNITS_OF_MEASURE];
...
public:
    ...
    static int PerfLoggingEnabled () { LEAF_CONTRACT; return m_fLogPerfData; }
    
    // Perf automation format is desired.
    static bool PerfAutomationFormat () { LEAF_CONTRACT; return m_perfAutomationFormat; }

    // CSV format is desired.
    static bool CommaSeparatedFormat () { LEAF_CONTRACT; return m_commaSeparatedFormat; }
    ...
    static void Log(__in_z wchar_t *wszName, UINT val, UnitOfMeasure unit, __in_opt wchar_t *wszDescr = 0);
    static void Log(__in_z wchar_t *wszName, UINT64 val, UnitOfMeasure unit, __in_opt wchar_t *wszDescr = 0);
    static void Log(__in_z wchar_t *wszName, double val, UnitOfMeasure unit, __in_opt wchar_t *wszDescr = 0);
...
private:
    ...
    static void OutToPerfFile(__in_z wchar_t *wszName, UnitOfMeasure unit, __in_opt wchar_t *wszDescr = 0);
    
    // Helper routine to hide some details of output to stdout
    static void OutToStdout(__in_z wchar_t *wszName, UnitOfMeasure unit, __in_opt wchar_t *wszDescr = 0);
