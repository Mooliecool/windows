...
inline void SBuffer::DebugMoveBuffer(BYTE *to, BYTE *from, COUNT_T size)
{
    CONTRACT_VOID
    {
        INSTANCE_CHECK;
...
inline void SBuffer::DebugCopyConstructBuffer(BYTE *to, const BYTE *from, COUNT_T size)
{
    CONTRACT_VOID
    {
        INSTANCE_CHECK;
...
inline CHECK SBuffer::CheckUnusedBuffer(const BYTE *buffer, COUNT_T size) const
{
        ...
        || buffer < m_buffer || buffer > m_buffer + m_allocation) // Allocating a new buffer
    {
        while (size-- > 0)
        {
            CHECK_MSG(*buffer == GARBAGE_FILL_CHARACTER, "Overwrite of unused buffer region found");
            buffer++;
        }
    }
#endif
