...
private:
  ...
  public:
    ...
    ULONG HashCaseInsensitive(LCID locale) const;
    ...
    int CompareCaseInsensitive(const SString &s, LCID locale) const;
    ...
    BOOL EqualsCaseInsensitive(const SString &s, LCID locale) const;
    ...
    BOOL MatchCaseInsensitive(const CIterator &i, const SString &s, LCID locale) const;

    BOOL Match(const CIterator &i, WCHAR c) const;
    BOOL MatchCaseInsensitive(const CIterator &i, WCHAR c) const; // invariant locale
    BOOL MatchCaseInsensitive(const CIterator &i, WCHAR c, LCID locale) const;
 ...
 public:
    ...
    BOOL LoadResource(int resourceID, BOOL quiet = FALSE);
    BOOL LoadEnglishResource(int resourceID, BOOL quiet = FALSE);
    HRESULT LoadResourceAndReturnHR(int resourceID, BOOL quiet);
    HRESULT LoadEnglishResourceAndReturnHR(int resourceID, BOOL quiet);
    ...
    operator const WCHAR * () const { WRAPPER_CONTRACT; return GetUnicode(); }

    WCHAR operator[](int index) { WRAPPER_CONTRACT; return Begin()[index]; }
    WCHAR operator[](int index) const { WRAPPER_CONTRACT; return Begin()[index]; }

    SString &operator= (const SString &s) { WRAPPER_CONTRACT; Set(s); return *this; }
    SString &operator+= (const SString &s) { WRAPPER_CONTRACT; Append(s); return *this; }
 ...
 private:
    ...
    static int CaseCompareHelperA(const CHAR *buffer1, const CHAR *buffer2, COUNT_T count, LCID lcid, BOOL stopOnNull, BOOL stopOnCount);
    static int CaseCompareHelper(const WCHAR *buffer1, const WCHAR *buffer2, COUNT_T count, LCID lcid, BOOL stopOnNull, BOOL stopOnCount);
    
    // Internal helpers:
    static BYTE s_EmptyBuffer[2];

    static UINT s_ACP;
    static BOOL s_IsANSIMultibyte;

    static LCID s_defaultLCID;
...
public:
    ...
    FORCEINLINE InlineSString(const SString &s1, SString &s2)
      : SString(m_inline, MEMSIZE)
    {
        WRAPPER_CONTRACT;
        Set(s1, s2);
    ...
    FORCEINLINE InlineSString(const SString &s1, SString &s2, SString &s3)
      : SString(m_inline, MEMSIZE)
    {
        WRAPPER_CONTRACT;
        Set(s1, s2, s3);
    ...
    FORCEINLINE InlineSString(const SString &s1, SString &s2, SString &s3, SString &s4)
      : SString(m_inline, MEMSIZE)
    {
        WRAPPER_CONTRACT;
        Set(s1, s2, s3, s4);
