    ...
    sealed internal class SoapHandler
    {
        ...
        private void ProcessAttributes(ParseRecord pr, ParseRecord objectPr)
        {
            ...
            for (int i = 0; i<attributeValues.Count(); i++)
            {
                ...
                if (keyLength == 2 && String.Compare(key, "id", StringComparison.OrdinalIgnoreCase) == 0)
                    pr.PRobjectId = objectReader.GetId(value);
                else if (keyLength == 8 && String.Compare(key, "position", StringComparison.OrdinalIgnoreCase) == 0)
                    keyPosition = value;
                else if (keyLength == 6 && String.Compare(key, "offset", StringComparison.OrdinalIgnoreCase) == 0)
                    keyOffset = value;
                else if (keyLength == 14 && String.Compare(key, "MustUnderstand", StringComparison.OrdinalIgnoreCase) == 0)
                    keyMustUnderstand = value;
                else if (keyLength == 4 && String.Compare(key, "null", StringComparison.OrdinalIgnoreCase) == 0)
                {
                    pr.PRmemberValueEnum = InternalMemberValueE.Null;
                    pr.PRvalue = null;
                }
                else if (keyLength == 4 && String.Compare(key, "root", StringComparison.OrdinalIgnoreCase) == 0)
                {
                    if (value.Equals("1"))
                        pr.PRisHeaderRoot = true;
                }
                else if (keyLength == 4 && String.Compare(key, "href", StringComparison.OrdinalIgnoreCase) == 0)
                    pr.PRidRef = objectReader.GetId(value);
                else if (keyLength == 4 && String.Compare(key, "type", StringComparison.OrdinalIgnoreCase) == 0)
                {
