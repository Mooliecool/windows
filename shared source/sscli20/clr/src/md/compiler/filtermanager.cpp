...
HRESULT FilterManager::MarkStandAloneSig(mdSignature sig)
{
    ...
    pbSig = m_pMiniMd->getSignatureOfStandAloneSig(pRec, &cbSize);
...
HRESULT FilterManager::MarkTypeSpec(mdTypeSpec ts)
{
    ...
    pbSig = m_pMiniMd->getSignatureOfTypeSpec(pRec, &cbSize);
...
HRESULT FilterManager::MarkMemberRef(mdMemberRef mr)
{
    ...
    // Walk the signature and mark all of the embedded types
    pbSig = m_pMiniMd->getSignatureOfMemberRef(pRec, &cbSize);
    IfFailGo( MarkSignature(pbSig, cbSize, &cbUsed) );
...
HRESULT FilterManager::MarkMethodSpec(mdMethodSpec ms)
{
    ...
    pRec = m_pMiniMd->getMethodSpec(RidFromToken(ms));

    IfFailGo( Mark(m_pMiniMd->getMethodOfMethodSpec(pRec)) );

    pbSig = m_pMiniMd->getInstantiationOfMethodSpec(pRec, &cbSize);
    IfFailGo( MarkSignature(pbSig, cbSize, &cbUsed) );
...
HRESULT FilterManager::MarkMethod(mdMethodDef md)
{
    ...
    pbSig = m_pMiniMd->getSignatureOfMethod(pRec, &cbSize);
    IfFailGo( MarkSignature(pbSig, cbSize, &cbUsed) );
...
HRESULT FilterManager::MarkField(mdFieldDef fd)
{
    ...
    pbSig = m_pMiniMd->getSignatureOfField(pRec, &cbSize);
    IfFailGo( MarkSignature(pbSig, cbSize, &cbUsed) );
...
HRESULT FilterManager::MarkProperty(mdProperty pr)
{
    ...
    // Walk the signature and mark all of the embedded types
    pbSig = m_pMiniMd->getTypeOfProperty(pRec, &cbSize);
    IfFailGo( MarkSignature(pbSig, cbSize, &cbUsed) );
    ...
    mdToken     tk)
{
    ...
    for (; ulStart < ulEnd; ++ulStart)
    {
        ...
        pGenericParamRec = m_pMiniMd->getGenericParam(index);
        ...
        while (HENUMInternal::EnumNext(&hEnum, (mdToken *) &ridConstraint))
        {
            ...
            pRec = m_pMiniMd->getGenericParamConstraint(RidFromToken(ridConstraint));
    ...
    mdTypeDef   td)
{
    ...
    if (IsTdNested(dwFlags))
    {
        ...
        pNestClassRec = m_pMiniMd->getNestedClass(iNester);
    ...
    ULONG       *pcbUsed)               // [OUT] count of bytes consumed.
{
    ...
    switch (ulElementType)
    {
        case ELEMENT_TYPE_VALUEARRAY:
            // syntax: SDARRAY <BaseType> <an integer for size>
        ...
        default:
            ...
            if (ulElementType >= ELEMENT_TYPE_MAX || ulElementType == ELEMENT_TYPE_PTR || ulElementType == ELEMENT_TYPE_BYREF)
                IfFailGo(META_E_BAD_SIGNATURE);
