    ...
    RID         rid /* = 0*/)               // [IN] Optional rid to be ignored.
{
    ...
    for (i=1; i <= cMethodSpecs; ++i)
    {
        ...
        pRecord = pMiniMd->getMethodSpec(i);
        ...
        pInstantiationTmp = pMiniMd->getInstantiationOfMethodSpec(pRecord,&cbInstantiationTmp);
    ...
    RID         rid /* = 0*/)               // [IN] Optional rid to be ignored.
{
    ...
    for (i=1; i <= cGenericParamConstraints; ++i)
    {
        ...
        pRecord = pMiniMd->getGenericParamConstraint(i);
    ...
    RID         rid /* = 0*/)               // [IN] Optional rid to be ignored.
{
    ...
    for (i=1; i <= cGenericParams; ++i)
    {
        ...
        pRecord = pMiniMd->getGenericParam(i);
        ...
        if (szUTF8Name)
        {
            szCurName = pMiniMd->getNameOfGenericParam(pRecord);
    ...
    RID         rid /* = 0*/)               // [IN] Optional rid to be ignored.
{
    ...
    // 428735: Prefix complained correctly.
    if (!(szName && pmr))
    {
        _ASSERTE(szName &&  pmr);
        hr = CLDB_E_RECORD_NOTFOUND;
    ...
    PREFIX_ASSUME(szName != NULL && pmr != NULL);
    ...
    RID         rid /* = 0 */)              // [IN] Optional rid to be ignored.
{
    ...
    //_ASSERTE(! ns::FindSep(szName)); // no need for this assert
    ...
    ULONG       *piRecord)              // [OUT] Record for item if found.
{
    ...
    *piRecord = 0;
    ...
    for (ULONG i = 1; i <= cRecs; i++)
    {
        ...
        if (pRec->GetMappingFlags() != usMappingFlags)
            continue;
    ...
    mdAssemblyRef *pmar)                // [OUT] returned AssemblyRef token.
{
    ...
    *pmar = 0;
    ...
    for (ULONG i = 1; i <= cRecs; i++)
    {
        ...
        if (SString::_stricmp(pMiniMd->getNameOfAssemblyRef(pRec), "mscorlib") && 
            SString::_stricmp(pMiniMd->getNameOfAssemblyRef(pRec), "microsoft.visualc"))
        {
    ...
    mdToken     *ptkType)               // [OUT] Output token for the imported type in the emit scope.
{
    ...
    if (pCommonAssemImport)
        pCommonAssemImport->CommonGetScopeProps(0, &MvidAssemImport);
    pCommonImport->CommonGetScopeProps(&szModuleImport, &MvidImport);
    if (pMiniMdAssemEmit)
        static_cast<IMetaModelCommon*>(pMiniMdAssemEmit)->CommonGetScopeProps(0, &MvidAssemEmit);
    static_cast<IMetaModelCommon*>(pMiniMdEmit)->CommonGetScopeProps(0, &MvidEmit);
    ...
    mdToken     *ptkType)               // [OUT] Output token for the imported type in the emit scope.
{
    ...
    if (pCommonAssemImport)
        pCommonAssemImport->CommonGetScopeProps(0, &MvidAssemImport);
    pCommonImport->CommonGetScopeProps(0, &MvidImport);
    if (pMiniMdAssemEmit)
    {
