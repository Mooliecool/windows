...
HRESULT RegMeta::ValidateTypeDef(RID rid)
{
    ...
    // Check if flag value is valid
    {
        ...
        if (IsTdExplicitLayout(dwFlags))
        {
            ...
            if (SUCCEEDED(hr))
            {
                HENUMInternal::GetCount(&hEnumTyPars,&ulTypeDefArity);
    ...
    else
    {
        ...
        if (TypeFromToken(tkExtends) == mdtTypeSpec)
        {
            ...
            TypeSpecRec *pRec = pMiniMd->getTypeSpec( RidFromToken(tkExtends) );
            PCCOR_SIGNATURE pSig;           
            ULONG       cSig;   

            pSig = pMiniMd->getSignatureOfTypeSpec( pRec, &cSig );
        ...
        if (TypeFromToken(tkExtends) == mdtTypeRef)
        {
            ...
            if(TypeFromToken(tkResScope)==mdtAssemblyRef)
            {
                ...
                if((ridResScope >0)&&(ridResScope <= pMiniMd->vGetCountRecs(TBL_AssemblyRef)))
                {
                    if(pMiniMd->vGetCountRecs(TBL_Assembly) > 0)
                    {
                        AssemblyRefRec *pAsmRefRec = pMiniMd->getAssemblyRef(ridResScope);
                        AssemblyRec *pAsmRec = pMiniMd->getAssembly(1);
                        fLookForDef = FALSE;
                        if((pAsmRec != NULL)&&(pAsmRefRec != NULL))
                        {
                            LPCUTF8 szAsmName = pMiniMd->getNameOfAssembly(pAsmRec);
                            LPCUTF8 szAsmRefName = pMiniMd->getNameOfAssemblyRef(pAsmRefRec);
...
HRESULT RegMeta::ValidateMethod(RID rid)
{
    ...
    if(g_tkEntryPoint == veCtxt.Token)
    {
        ...
        for(tkTypeDefCur = tkTypeDef; tkTypeDefCur != mdTokenNil;)
        {
            ...
            if(ulTypeDefArity == 0)
            {
                ...
                ULONG iRecord = pMiniMd->FindNestedClassHelper(tkTypeDefCur);        

                tkTypeDefCur = InvalidRid(iRecord) ? mdTokenNil
                             : pMiniMd->getEnclosingClassOfNestedClass(pMiniMd->getNestedClass(iRecord));
            }
            else
        ...
        if(IsMdPinvokeImpl(dwFlags))
        {
            ...
            else
            {
                if(pRecord->GetRVA()!=0)
                {
                    ...
                    mdToken     tkModuleRef;
                    pIMRecord = pMiniMd->getImplMap(iRecord);
                    ...
                    if((TypeFromToken(tkModuleRef) == mdtModuleRef) && (!IsNilToken(tkModuleRef)))
                    {
                        ...
                        pMRRecord = pMiniMd->getModuleRef(RidFromToken(tkModuleRef));
                        // Check ModuleRef name is "".
                        szMRName = pMiniMd->getNameOfModuleRef(pMRRecord);
                        if (*szMRName)
                        {
    ...
    // Validate params
    {
        ...
        ParamRec* pRec;
        ULONG cbSigT;
        PCCOR_SIGNATURE typePtr = pMiniMd->getSignatureOfMethod(pRecord,&cbSigT);
                                ...
                                mdToken tkClass)
{
    ...
    if(TypeFromToken(tkMethod) == mdtMethodDef)
    {
        ...
        {
            TypeSpecRec* pTS = pMiniMd->getTypeSpec(RidFromToken(tkClass));
            if(pTS)
            {
                PCCOR_SIGNATURE pbSig;              // Signature.
                ULONG       cSig;                  // Size in bytes of the signature.
                pbSig = pMiniMd->getSignatureOfTypeSpec( pTS, &cSig );
                if(*pbSig == ELEMENT_TYPE_GENERICINST)
                {
                    pbSig++;
                    if((*pbSig == ELEMENT_TYPE_VALUETYPE)||(*pbSig == ELEMENT_TYPE_CLASS))
                    {
                        pbSig++;
                        CorSigUncompressToken(pbSig, &tkClass);
                    }
                }
            }
        }
        if(TypeFromToken(tkClass) == mdtTypeDef)
        {
            ...
            if(SUCCEEDED(hr))
            {
                MethodRec* pMethod = pMiniMd->getMethod(RidFromToken(tkFoundMethod));
                if(pMethod)
                {
        ...
        else if(TypeFromToken(tkClass) == mdtTypeRef)
        {
            ...
            // Get the TypeRef record.
            pRecord = pMiniMd->getTypeRef(RidFromToken(tkClass));
        
            // Check name is not NULL.
            szTRNamespace = pMiniMd->getNamespaceOfTypeRef(pRecord);
            szTRName = pMiniMd->getNameOfTypeRef(pRecord);
            if((strcmp(szTRNamespace,BASE_NAMESPACE)==0)&&
                ((strcmp(szTRName,BASE_OBJECT_CLASSNAME)==0)||
                 (strcmp(szTRName,BASE_VTYPE_CLASSNAME)==0)||
                 (strcmp(szTRName,BASE_ENUM_CLASSNAME)==0))) numImpl = 0; //these classes don't implement any methods
            else
                numImpl = -1;
        }
    }
    ...
    if((TypeFromToken(tkInterface) == mdtTypeDef) && fCheckTheMethods && (tkInterface != tkClass))
    {
             ...
             ||IsTdInterface(pClass->GetFlags())))
        {
            ...
            for(RID idxInt = ridIntStart; idxInt < ridIntEnd; idxInt++)
            {
                ...
                {
                LPCUTF8 szMethodName = pMiniMd->getNameOfMethod(pIntMethod);
                ULONG       cbSig;
                PCCOR_SIGNATURE pSig = pMiniMd->getSignatureOfMethod(pIntMethod, &cbSig);
                if(cbSig)
                {
                        int num = IsMethodImplementedByClass(pMiniMd,TokenFromRid(ridInt,mdtMethodDef),szMethodName,pSig,cbSig,tkClass);
                        if(num == 0) 
...
HRESULT RegMeta::ValidateGenericParam(RID rid)
{
    ...
    veCtxt.uOffset = 0;
    pRecord = pMiniMd->getGenericParam(rid);
            ...
            switch (dwVariance)
            {
                ...
                case gpContravariant:
                    if (TypeFromToken(tkOwner)==mdtTypeDef)
                    {
                        if (IsNilToken(tkOwner))
                            break;
                        TypeDefRec  *pTypeDefRec = pMiniMd->getTypeDef(RidFromToken(tkOwner));
                        ...
                        if(!IsTdInterface(pTypeDefRec->GetFlags()))
                        {
                            ...
                            if (TypeFromToken(tkExtends) != mdtTypeSpec) 
                            {
                                if (TypeFromToken(tkExtends) == mdtTypeRef)
                                {
                                    TypeRefRec  *pExtTypeRefRec = pMiniMd->getTypeRef(RidFromToken(tkExtends));
                                    ...
                                    if(RidFromToken(tkResScope) && (TypeFromToken(tkResScope)==mdtAssemblyRef))
                                    {
                                        ...
                                        if(0 == SString::_stricmp("mscorlib",pMiniMd->getNameOfAssemblyRef(pARRec)))
                                        // otherwise don't even bother extracting the name
                                        {
                                            szExtName = pMiniMd->getNameOfTypeRef(pExtTypeRefRec);
                                            szExtNameSpace = pMiniMd->getNamespaceOfTypeRef(pExtTypeRefRec);
                                        }
                                    }
                                }
                                else if (TypeFromToken(tkExtends) == mdtTypeDef) 
                                {
                                    ...
                                    {
                                        TypeDefRec  *pExtTypeRefRec = pMiniMd->getTypeDef(RidFromToken(tkExtends));
                                        szExtName = pMiniMd->getNameOfTypeDef(pExtTypeRefRec);
                                        szExtNameSpace = pMiniMd->getNamespaceOfTypeDef(pExtTypeRefRec);
                                    }
                                }
    ...
    //    be ordered by increasing Number value; there can be no gaps in the Number sequence.
    {
        ...
        {  if(rid==1)
           ...
           {  
               GenericParamRec *pPredRecord = pMiniMd->getGenericParam(rid-1);
    ...
    szName = pMiniMd->getNameOfGenericParam(pRecord);
    if (!*szName)
    {
        // name is NULL.
    ...
    if (hr == S_OK)
    {
        ...
        if (TypeFromToken(tkClass) == mdtMethodDef)
        {
            ...
            else if (RidFromToken(tkClass))
            {
                ...
                pMethodRecord = pMiniMd->getMethod(RidFromToken(tkClass));
                szMethodName = pMiniMd->getNameOfMethod(pMethodRecord);
                pbMethodSig = pMiniMd->getSignatureOfMethod(pMethodRecord, &cbMethodSig);
...
HRESULT RegMeta::ValidateEvent(RID rid)
{
    ...
    if((TypeFromToken(tkClass) != mdtTypeDef) || !IsValidToken(tkClass))
    {
    ...
    if(!IsNilToken(tkEventType))
    {
             ...
             (TypeFromToken(tkEventType)==mdtTypeSpec)))
        {
            ...
            {
                TypeSpecRec    *pRec = pMiniMd->getTypeSpec( RidFromToken(tkEventType) );
                PCCOR_SIGNATURE pSig;           
                ULONG           cSig;   
...
HRESULT RegMeta::ValidateProperty(RID rid)
{
    ...
    if((TypeFromToken(tkClass) != mdtTypeDef) || !IsValidToken(tkClass) || IsNilToken(tkClass))
    {
...
HRESULT RegMeta::ValidateTypeSpec(RID rid)
{
    ...
    veCtxt.uOffset = 0;
    pRecord = pMiniMd->getTypeSpec(rid);
    pbSig = pMiniMd->getSignatureOfTypeSpec( pRecord, &cbSig );
...
HRESULT RegMeta::ValidateMethodSpec(RID rid)
{
    ...
    veCtxt.uOffset = 0;
    pRecord = pMiniMd->getMethodSpec(rid);
    ...
    { 
        pInstantiation = pMiniMd->getInstantiationOfMethodSpec(pRecord,&cbInstantiation);
    ...
    // check the method is generic and that the arity of the instantiation is correct
    {
        ...
        if(TypeFromToken(tkMethod) == mdtMethodDef)        
        {
           MethodRec *pMethodRec = m_pStgdb->m_MiniMd.getMethod(RidFromToken(tkMethod)); 
           pbGenericMethodSig = pMiniMd->getSignatureOfMethod(pMethodRec, &cbGenericMethodSig);
        }
        else
        {
            _ASSERTE(TypeFromToken(tkMethod) == mdtMemberRef);
            MemberRefRec* pMethodRefRec = pMiniMd->getMemberRef(RidFromToken(tkMethod));
            pbGenericMethodSig = pMiniMd->getSignatureOfMemberRef(pMethodRefRec, &cbGenericMethodSig);
        }
...
HRESULT RegMeta::ValidateGenericParamConstraint(RID rid)
{
    ...
    veCtxt.uOffset = 0;
    pRecord = pMiniMd->getGenericParamConstraint(rid);
    ...
    {   
        ...
        { 
            curRid--;
            pCurRecord = pMiniMd->getGenericParamConstraint(curRid);
...
HRESULT RegMeta::ValidateAssembly(RID rid)
{
    ...
    // Validate the flags 
    invalidAssemblyFlags = dwFlags & (~(afPublicKey |afRetargetable |afPA_FullMask |afEnableJITcompileTracking |afDisableJITcompileOptimizer));
    ...
    if (m_pStgdb->m_MiniMd.m_Schema.m_major >= METAMODEL_MAJOR_VER_V2_0
        /* && m_pStgdb->m_MiniMd.m_Schema.m_minor >= METAMODEL_MINOR_VER_V2_0*/)
    {
        if ((dwFlags & afPA_Mask) > afPA_AMD64)
                        ...
                        case ELEMENT_TYPE_CLASS: 
                            { 
                                ...
                                if (TypeFromToken(tkGenericType) == mdtTypeDef)
                                {
                                    ...
                                    if (SUCCEEDED(hr))
                                    {
                                        HENUMInternal::GetCount(&hEnumTyPars,&ulGenericArity);
                                        HENUMInternal::ClearEnum(&hEnumTyPars);
                                        fCheckArity = TRUE;
                                    }
                                    ; 
    ...
    DWORD       dwFlags)                // [IN] Method flags.
{
    ...
    if (ulCallConv & IMAGE_CEE_CS_CALLCONV_GENERIC)
    {
      ...
      if (TypeFromToken(tk) == mdtMethodDef)
      {
          ...
          if (SUCCEEDED(hr))
          {
              HENUMInternal::GetCount(&hEnumTyPars,&ulGenericParamCount);
              HENUMInternal::ClearEnum(&hEnumTyPars);
...
MDSigComparer::_CompareExactlyOne()
{
    ...
    if (!CorIsPrimitiveType((CorElementType)typ))
    {
        switch (typ)
        {
            ...
            {
                IfFailRet(_CompareData(NULL));                  // Compare index
                IfFailRet(_CompareExactlyOne());                // Compare type
                break;
            }
            ...
            case ELEMENT_TYPE_VALUEARRAY: 
            {
                ...
                IfFailRet(_CompareData(NULL));                  // Compare array size
                break;
            }
#endif
...
static BOOL _IsValidLocale(LPCUTF8 szLocale)
{
    ...
    if(szLocale && *szLocale)
    {
        for(i = 0; i < N; i++)
        {
            if(!SString::_stricmp(szLocale,g_szValidLocale[i])) return TRUE;
        }
        return FALSE;
    }
    return TRUE;
