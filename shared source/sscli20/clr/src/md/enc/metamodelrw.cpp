...
static const ULONG g_PoolSizeInfo[2][4][2] =
...
static const ULONG g_HashSize[2] =
{
    257, 709
};
static const ULONG g_TblSizeInfo[2][TBL_COUNT] =
{
...
TblIndex g_TblIndex[TBL_COUNT] =
{
    ...
    {(ULONG) -1,        (ULONG) -1,     (ULONG) -1},        // ImplMap                                                                     
    {(ULONG) -1,        (ULONG) -1,     (ULONG) -1},        // FieldRVA                                                                     
    {(ULONG) -1,        (ULONG) -1,     (ULONG) -1},        // ENCLog
    {(ULONG) -1,        (ULONG) -1,     (ULONG) -1},        // ENCMap
    {(ULONG) -1,        (ULONG) -1,     mdtAssembly},       // Assembly                                                    
    {(ULONG) -1,        (ULONG) -1,     (ULONG) -1},        // AssemblyProcessor                                                    
    {(ULONG) -1,        (ULONG) -1,     (ULONG) -1},        // AssemblyOS                                                    
    {(ULONG) -1,        (ULONG) -1,     mdtAssemblyRef},    // AssemblyRef                                                    
    {(ULONG) -1,        (ULONG) -1,     (ULONG) -1},        // AssemblyRefProcessor                                                    
    {(ULONG) -1,        (ULONG) -1,     (ULONG) -1},        // AssemblyRefOS                                                    
    {(ULONG) -1,        (ULONG) -1,     mdtFile},           // File                                                    
    ...
    void DoMapTokens()
    {
        if (m_bMapToken)
        {
            ...
            for (int i=1; i<=m_iCount; ++i)
                m_MiniMd.MapToken(*(m_pRidMap->Get(i)), i, typ);
        }
    }
    ...
    int         bReadOnly)              // Is the memory read-only?
{
    ...
    case MDPoolUSBlobs:
        if (pbData == 0)
            hr = m_USBlobs.InitNew(0,0, !IsMinimalDelta());
        else
    ...
    int         bReadOnly)              // Is the memory read-only?
{
    ...
    // Convvert our Open flag to the enum so we can open the record
    fReadOnly = bReadOnly ? READ_ONLY : READ_WRITE;
    ...
    for (i=0; i<(int)m_TblCount; ++i)
    {
        if (m_Schema.m_cRecs[i])
        {
            ...
            IfFailGo(m_Table[i].InitOnMem(m_TableDefs[i].m_cbRec, pBuf, cbTable, fReadOnly));
            pBuf += cbTable;
        }
        else
            IfFailGo(m_Table[i].InitNew(m_TableDefs[i].m_cbRec, 0));
    }
    ...
    int         bReadOnly)                      // Will updates be allowed?
{
    ...
    for (i=0; i<m_TblCount; ++i)
    {
        ...
        {
            IfFailGo(m_Table[i].InitOnMem(m_TableDefs[i].m_cbRec, pMd->m_pTable[i]+m_TableDefs[i].m_cbRec, m_Schema.m_cRecs[i]*m_TableDefs[i].m_cbRec, fReadOnly));
...
HRESULT CMiniMdRW::GetFullSaveSize(         // S_OK or error.
    ...
    CorProfileData *profileData)        // [IN] Optional IBC profile data for working set optimization
{
    HRESULT     hr=S_OK;                // A result.
    ...
    *pbSaveCompressed = true;
    ...
    if (profileData != NULL)
    {
        ULONG hotPoolsSize = 0;
        
        IfFailGo( GetHotPoolsSaveSize( & hotPoolsSize, profileData ) );
        cbTotal += hotPoolsSize;
        
        if (cbTotal <= 4)
            cbTotal = 0;
        else
            cbTotal += sizeof(ULONG) + m_TblCount*sizeof(ULONG)
                       + 2 * sizeof( ULONG );  // plus the size of hot metadata header
    }
    else
        m_cbSaveSize = cbTotal;

    LOG((LOGMD, "CMiniMdRW::GetFullSaveSize: Total %ssize = %d\n", profileData ? "hot " : "", cbTotal));
...
//  
HRESULT CMiniMdRW::GetHotPoolsSaveSize( ULONG * size, CorProfileData * profileData )
{
    HRESULT hr = S_OK;
    ULONG savedDirSize = 0;
    ULONG savedPoolsSize = 0;
    
    NullStream temp_strm;
    IfFailGo( SaveHotPoolsToStream( & temp_strm, profileData, & savedDirSize, & savedPoolsSize ) );
    *size = savedDirSize + savedPoolsSize;
    ...
    CorProfileData *profileData)        // [IN] Optional IBC profile data for working set optimization
{
    ...
    {
    case MDUpdateFull:
        hr = GetFullSaveSize(fSave, pulSaveSize, pbSaveCompressed, profileData);
        break;
    case MDUpdateIncremental:
    case MDUpdateENC:        
        hr = GetFullSaveSize(fSave, pulSaveSize, pbSaveCompressed, profileData);
    ...
    ULONG       *pulSaveSize)           // [OUT] Put the size here.
{
    ...
    {
    case MDPoolStrings:
        hr = m_Strings.GetEditSaveSize(pulSaveSize);
        break;
    case MDPoolGuids:
        // We never save a 'delta' GUID pool
        hr = m_Guids.GetSaveSize(pulSaveSize);
        break;
    case MDPoolBlobs:
        hr = m_Blobs.GetEditSaveSize(pulSaveSize);
        break;
    case MDPoolUSBlobs:
        hr = m_USBlobs.GetEditSaveSize(pulSaveSize);
        break;
    default:
        hr = E_INVALIDARG;
    }
...
{   
    ...
    for (ix=0; ix<g_CodedTokens[CDTKN_HasCustomAttribute].m_cTokens; ++ix)
        if (g_CodedTokens[CDTKN_HasCustomAttribute].m_pTokens[ix] == tk)
            return true;
    return false;
} // bool CMiniMdRW::CanHaveCustomAttribute()
#endif // debug
...
HRESULT CMiniMdRW::PreSaveFull()
{
            ...
            for (ridPtr=1; ridPtr<=m_Schema.m_cRecs[TBL_Method]; ++ridPtr)
            {
                ...
                for (; ixStart<ixEnd; ++ixStart)
                {
                    ...
                    if (HasIndirectTable(TBL_Param))
                    {
                        ...
                        ridParam = ixStart;
                    void *pOldRecord = m_Table[TBL_Param].GetRecord(ridParam);
                    RID ridNew;
                    void *pNewRecord = NewParams.AddRecord(&ridNew);
                    IfNullGo(pNewRecord);
    ...
    if (SupportsGenerics())
    {
        ...
        if ((m_OptionValue.m_UpdateMode & MDUpdateMask)  == MDUpdateFull)
        {
            ...
            sortGenericParamConstraint.Sort();
        }        
    }
    ...
    {   // Keep track of ENC recs we've seen.
        ...
        for (rid=1; rid<=m_Schema.m_cRecs[TBL_ENCLog]; ++rid)
        {
            ...
            {   // No func code.  Skip if we've seen this token before.
                ...
                RIDBinarySearch searcher(m_rENCRecs[ixTbl].Ptr(), m_rENCRecs[ixTbl].Count());
        ...
        if (((m_OptionValue.m_UpdateMode & MDUpdateDelta)) == MDUpdateDelta)
        {
            ...
            for (ixTbl=0; ixTbl<m_TblCount; ++ixTbl)
            {
                ...
                {
                    pNew = AddENCMapRecord(&nNew); // pre-allocated for all rows.
                    _ASSERTE(nNew == ++cRecs);
    ...
    CorProfileData *profileData)
{
    ...
    for (ixTbl=0; ixTbl<m_TblCount; ++ixTbl)
    {
        ...
        ULONG itemCount = vGetCountRecs(ixTbl);
        if (itemCount)
        {
    ...
    cbTotal += cbAlign;
    _ASSERTE(m_cbSaveSize == 0 || m_cbSaveSize == cbTotal || profileData != NULL);

    if (profileData != NULL)
    {
        // write hot table directory (HotTableDirectory in MetaModelRO.h)
        ...
        IfFailGo( SaveHotPoolsToStream( pIStream, profileData, & poolDirSize, & savedPoolsSize ) );

        // write hot metadata (including pools) header
        IfFailGo( WriteToStream( pIStream, savedPoolsSize + poolDirSize ) );
        IfFailGo( WriteToStream( pIStream, poolDirSize ) );
    }
...
//
HRESULT CMiniMdRW::SaveHotPoolsToStream( IStream * strm,
                                         CorProfileData * profileData,
                                         ULONG * poolDirSize,
                                         ULONG * savedPoolsSize )
{
    HRESULT hr = S_OK;
    ULONG savedPools[ MDPoolCount ] = { 0 };
    
    // save pools in the order they are described in MDPools enum 
    // 
    IfFailRet( SaveHotPoolToStream( strm, profileData, m_Strings, MDPoolStrings, savedPools + MDPoolStrings ) );

    STATIC_ASSERT( MDPoolStrings + 1 == MDPoolGuids );
    IfFailRet( SaveHotPoolToStream( strm, profileData, m_Guids, MDPoolGuids, savedPools + MDPoolGuids ) );

    STATIC_ASSERT( MDPoolGuids + 1 == MDPoolBlobs );
    IfFailRet( SaveHotPoolToStream( strm, profileData, m_Blobs, MDPoolBlobs, savedPools + MDPoolBlobs ) );

    STATIC_ASSERT( MDPoolBlobs + 1 == MDPoolUSBlobs );
    IfFailRet( SaveHotPoolToStream( strm, profileData, m_USBlobs, MDPoolUSBlobs, savedPools + MDPoolUSBlobs ) );

    // fix pool offsets, they need to point to the header of each saved pool
    *savedPoolsSize = 0;
    ULONG curHeaderOffset = sizeof( HotPoolDataHeader );

    STATIC_ASSERT( MDPoolCount > 1 );
    for ( int i = MDPoolCount - 1; i >= 0; --i )
    {
        if ( savedPools[ i ] != 0 )
        {
            ULONG temp = savedPools[ i ];
            *savedPoolsSize += temp;
            savedPools[ i ] = curHeaderOffset;
            curHeaderOffset += temp;
        }
    }
    ...
    *poolDirSize = 0;
    for ( int i = 0; i < MDPoolCount; ++i )
    {
        if ( savedPools[ i ] != 0 )
        {
            IfFailRet( WriteToStream( strm, i, poolDirSize ) );
            IfFailRet( WriteToStream( strm, savedPools[ i ], poolDirSize ) );
        }
    }
...
//
HRESULT CMiniMdRW::SaveHotPoolToStream( IStream * strm, CorProfileData * profileData, StgPool & pool, MDPools poolId, ULONG * savedSize )
{
    ...
    CorProfileData *profileData)
{
    ...
    case MDUpdateIncremental:
    case MDUpdateENC:        
        hr = SaveFullTablesToStream(pIStream, profileData);
        break;
    case MDUpdateDelta:
    ...
    IStream     *pIStream)              // The stream.
{
    ...
    {
    case MDPoolStrings:
        if (m_Strings.HaveEdits())
            hr = m_Strings.PersistPartialToStream(pIStream, m_Strings.GetOffsetOfEdit());
        break;
    case MDPoolGuids:
        // We never save a delta GUID pool.
        hr = m_Guids.PersistToStream(pIStream);
        break;
    case MDPoolBlobs:
        if (m_Blobs.HaveEdits())
            hr = m_Blobs.PersistPartialToStream(pIStream, m_Blobs.GetOffsetOfEdit());
        break;
    case MDPoolUSBlobs:
        if (m_USBlobs.HaveEdits())
        {
            ...
            _ASSERTE(*(m_USBlobs.GetData(0)) == 0);
            ULONG ulOffsetOfEdit = m_USBlobs.GetOffsetOfEdit();
            
            if (ulOffsetOfEdit == 0)
                ulOffsetOfEdit = 1;    
            
            hr = m_USBlobs.PersistPartialToStream(pIStream, ulOffsetOfEdit);
...
HRESULT CMiniMdRW::InitWithLargeTables()
{
    CMiniMdSchema   Schema;             // Temp schema by which to build tables.
    HRESULT hr = S_OK;
...
//*****************************************************************************
HRESULT CMiniMdRW::Impl_GetStringW(ULONG ix, __out_ecount (cchBuffer) LPWSTR szOut, ULONG cchBuffer, ULONG *pcchBuffer)
{
...
HRESULT CMiniMdRW::ChangeMvid(            // S_OK or E_UNEXPECTED.
    REFGUID newMvid)
{
    HRESULT hr = S_OK;
    
    ModuleRec * pMod = getModule(1);
    ULONG iOffset = GetCol(TBL_Module, ModuleRec::COL_Mvid, pMod);

    if (int(iOffset) > m_Guids.Count())
        IfFailRet(CLDB_E_FILE_CORRUPT);
        
    GUID *pMvid = m_Guids.GetGuid(iOffset);
...
//*****************************************************************************
HRESULT CMiniMdRW::FindGenericParamHelper(// return HRESULT
    ...
    HENUMInternal *phEnum)              // fill in the enum
{
    ...
    if (IsSorted(TBL_GenericParam))
    {
        ...
        ridStart = SearchTableForMultipleRows(TBL_GenericParam, 
    ...
    else if (pHashTable)
    {
        ...
        for (p = pHashTable->FindFirst(iHash, pos);
             ...
             p = pHashTable->FindNext(pos))
        {
            pGenericParam = getGenericParam(p->tok);
    ...
    else
    {
        ...
        {
            pGenericParam = getGenericParam(index);
            if (getOwnerOfGenericParam(pGenericParam) == tkOwner)
            {
...
ErrExit:
    return hr;
} // HRESULT CMiniMdRW::FindGenericParamHelper()
...
//*****************************************************************************
HRESULT CMiniMdRW::FindGenericParamConstraintHelper(// return HRESULT
    ...
    HENUMInternal *phEnum)              // fill in the enum
{
    ...
    {
        ridStart = getGenericParamConstraintsForGenericParam(ridParam, &ridEnd);
    ...
    else if (pHashTable)
    {
        ...
        for (p = pHashTable->FindFirst(iHash, pos);
             ...
             p = pHashTable->FindNext(pos))
        {
            pConstraint = getGenericParamConstraint(p->tok);
    ...
    else
    {
        ...
        {
            pConstraint = getGenericParamConstraint(index);
            if (getOwnerOfGenericParamConstraint(pConstraint) == tkParam)
            {
...
ErrExit:
    return hr;
} // HRESULT CMiniMdRW::FindGenericParamConstraintHelper()
    ...
    ULONG       ixCol)                  // col that we hash.
{
    ...
    if (!pHashTable)
    {
        ...
        {
            // Create a new hash.
            pHashTable = new (nothrow) CLookUpHash;
            IfNullGo( pHashTable );
    ...
    ULONG       ulTarget)               // Target for search.
{
    ...
    {
        for (int i = 1; i <= (int)vGetCountRecs(ixTbl); i ++)
        {
            if (getIX(vGetRow(ixTbl, i), sColumn) == ulTarget)
                return i;
        }
    }
    ...
    mdMemberRef mr)                     // Token of new guy.
{
    ...
    if (!m_pMemberRefHash)
    {
        ...
        {
            // Create a new hash.
            m_pMemberRefHash = new (nothrow) CMemberRefHash;
...
HRESULT CMiniMdRW::CreateMemberDefHash()  // Return code.
{
    ...
    if (!m_pMemberDefHash)
    {
        ...
        {
            // Create a new hash.
            m_pMemberDefHash = new (nothrow) CMemberDefHash;
    ...
    mdTypeDef   *ptd)                   // [OUT] the parent token
{
    ...
    if ( HasIndirectTable(TBL_Method) )
    {
        if (m_pMethodMap == NULL)
        {
            ...
            m_pMethodMap = new (nothrow) TOKENMAP;
            ...
            for (indexTd = 1; indexTd<= m_Schema.m_cRecs[TBL_TypeDef]; indexTd++)
            {
                ...
                for (indexMd = ridStart; indexMd < ridEnd; indexMd++)
                {
                    ...
                    PREFIX_ASSUME(m_pMethodMap->Get(getMethodOfMethodPtr(pMethodPtrRec)) != NULL);
    ...
    mdTypeDef   *ptd)                   // [OUT] parent token
{
    ...
    if ( HasIndirectTable(TBL_Field) )
    {
        if (m_pFieldMap == NULL)
        {
            ...
            m_pFieldMap = new (nothrow) TOKENMAP;
            ...
            for (indexTd = 1; indexTd<= m_Schema.m_cRecs[TBL_TypeDef]; indexTd++)
            {
                ...
                for (indexFd = ridStart; indexFd < ridEnd; indexFd++)
                {
                    ...
                    PREFIX_ASSUME(m_pFieldMap->Get(getFieldOfFieldPtr(pFieldPtrRec)) != NULL);
    ...
    mdTypeDef   *ptd)
{
    ...
    if ( HasIndirectTable(TBL_Property) )
    {
        if (m_pPropertyMap == NULL)
        {
            ...
            m_pPropertyMap = new (nothrow) TOKENMAP;
            ...
            for (indexMap = 1; indexMap<= m_Schema.m_cRecs[TBL_PropertyMap]; indexMap++)
            {
                ...
                for (indexPr = ridStart; indexPr < ridEnd; indexPr++)
                {
                    ...
                    mdToken* tok =  m_pPropertyMap->Get(getPropertyOfPropertyPtr(pPropertyPtrRec));
    ...
    mdTypeDef   *ptd)
{
    ...
    if ( HasIndirectTable(TBL_Event) )
    {
        if (m_pEventMap == NULL)
        {
            ...
            m_pEventMap = new (nothrow) TOKENMAP;
            ...
            for (indexMap = 1; indexMap<= m_Schema.m_cRecs[TBL_EventMap]; indexMap++)
            {
                ...
                for (indexEv = ridStart; indexEv < ridEnd; indexEv++)
                {
                    ...
                    mdToken* tok = m_pEventMap->Get(getEventOfEventPtr(pEventPtrRec));
                    PREFIX_ASSUME(tok != NULL);
    ...
    mdMethodDef *pmd)
{
    ...
    if ( HasIndirectTable(TBL_Param) )
    {
        if (m_pParamMap == NULL)
        {
            ...
            m_pParamMap = new (nothrow) TOKENMAP;
            ...
            for (indexMd = 1; indexMd<= m_Schema.m_cRecs[TBL_Method]; indexMd++)
            {
                ...
                for (indexPd = ridStart; indexPd < ridEnd; indexPd++)
                {
                    ...
                    PREFIX_ASSUME(m_pParamMap->Get(getParamOfParamPtr(pParamPtrRec)) != NULL);
...
HRESULT CMiniMdRW::ResetENCLog()           // S_OK or error.
{
    HRESULT hr = S_OK;
    ModuleRec *pMod;
    GUID    encid;
    ...
    if (REGUTIL::GetConfigDWORD(L"MD_UseMinimalDeltas", 1))
    {
        // Copy EncId as BaseId.
    ...
    m_Strings.ResetOffsetOfEdit();
    m_Blobs.ResetOffsetOfEdit();
    m_USBlobs.ResetOffsetOfEdit();
    
    // Clear the ENCLog
    m_Table[TBL_ENCLog].Uninit();
