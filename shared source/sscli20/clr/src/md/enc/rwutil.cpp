...
//
//*********************************************************************************************************
HRESULT CMDSemReadWrite::ObtainLock()
{
    HRESULT hr = S_OK;
    ...
    if (m_fReadLock)
    {
        LOG((LF_METADATA, LL_EVERYTHING, "LockRead called from CSemReadWrite::ObtainLock \n"));
        IfFailRet(m_pSem->LockRead());
        m_fLockedForRead = true;
    }
    else
    {
        LOG((LF_METADATA, LL_EVERYTHING, "LockWrite called from CSemReadWrite::ObtainLock \n"));
        IfFailRet(m_pSem->LockWrite());
        m_fLockedForWrite = true;
    }

    return hr;
}// CMDSemReadWrite::ObtainLock
