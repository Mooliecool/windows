    ...
    LPSECURITY_ATTRIBUTES pAttributes)  // Security token.
{
    ...
    else if ((fFlags & DBPROP_TMODEF_WRITE) == 0)
    {
        ...
        DWORD dwFileSharingFlags = 0;
        
        if (!RunningOnWin95())
            dwFileSharingFlags |= FILE_SHARE_DELETE;
...
ErrExit:
    ...
    m_fFlags = fFlags;
    if (szName && *szName)
        wcscpy_s(m_rcFile, _MAX_PATH, szName);
...
//*****************************************************************************
HRESULT StgIO::LoadFileToMemory()
{
    HRESULT     hr;
    ...
    ULONG       cbSize)                 // How big is the range.
{
    ...
    _ASSERTE(m_iType != STGIO_SHAREDMEM || ((LONG_PTR) pbStart >= (LONG_PTR) m_pBaseData));
    _ASSERTE(m_iType != STGIO_SHAREDMEM || ((LONG_PTR) pbStart + cbSize <= (LONG_PTR) m_pBaseData + m_cbData));
    ...
    void        *&ptr)                  // Return pointer to memory here.
{
    ...
    // Memory version or memory mapped file work the same way.
            ...
            m_iType == STGIO_SHAREDMEM || m_iType == STGIO_HFILEMEM || m_iType == STGIO_SHAREDMEM)
    {   
        if (!(cbStart <= m_cbData))
            return (PostError(E_INVALIDARG));
