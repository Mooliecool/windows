...
HRESULT TiggerStorage::GetHeaderPointer(
...
//*****************************************************************************
HRESULT TiggerStorage::GetDefaultVersion(LPCSTR* ppVersion)
{
    ...
    if (g_pDefaultVersion == NULL)
    {
        ...
        DWORD dwVersion = 0;
        IfFailRet(GetCORRequiredVersion(wszVersion, _MAX_PATH, &dwVersion));
        ...
        CHAR* pVersion = new (nothrow) CHAR[dwSize];
        IfNullRet(pVersion);

        memcpy(pVersion, szVersion, dwSize);

        if (InterlockedCompareExchangePointer((void**)&g_pDefaultVersion, pVersion, NULL) != NULL)           
            delete [] pVersion;
#else
        DacNotImpl();
...
HRESULT TiggerStorage::SizeOfStorageSignature(LPCSTR pVersion, ULONG *pcbSignatureSize)
{
    HRESULT     hr;
    ...
    BYTE        *&pbData)               // Return a pointer to extra data.
{
    ...
    if (m_pbExtra)
    {
        if (!(m_StgHdr.GetFlags() & STGHDR_EXTRADATA))
...
HRESULT TiggerStorage::FindStream(LPCSTR szName, __out PSTORAGESTREAM *stream)
{
    *stream = NULL;
    ...
    if (m_pStreamList)
    {
        ...
        for (int i=0;  i<m_StgHdr.GetiStreams();  i++)
        {
            ...
            if (!SString::_stricmp(p->GetName(), szName))
                {
                    *stream = p;
                    return S_OK;
                }
    ...
    else
    {
        ...
        {
            if (!SString::_stricmp(m_Streams[j].GetName(), szName))
            {
                *stream = &m_Streams[j];
                return S_OK;
            }
...
//*****************************************************************************
HRESULT TiggerStorage::WriteSignature(LPCSTR pVersion)
{
