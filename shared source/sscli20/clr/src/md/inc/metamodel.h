...
public:
    ...
    virtual HRESULT CompareCustomAttribute( 
    ...
    virtual HRESULT GetTypeDefRefTokenInTypeSpec(mdTypeSpec  tkTypeSpec,     // [IN] TypeSpec token to look 
                                                          mdToken    *tkEnclosedToken) = 0; // [OUT] The enclosed type token
...
{   _VALIDP(tbl); getGuid(getI4(pRec, _COLDEF(tbl,fld)) & m_iGuidsMask, pGuid); }
...
public:
    ...
    BOOL CMiniMdBase::FindSharedColDefs(// TRUE if we found a match in the global cache and updated pTable, FALSE otherwise
    ...
    inline static ULONG getIX(const void *pRec, CMiniColDef &def)
    {
        ...
        if (def.m_cbColumn == 2)
        {
            METADATATRACKER_ONLY(pVal = MetaDataTracker::NoteAccess(pVal, 2));
            ULONG ix = GET_UNALIGNED_VAL16(pVal);
            return ix;
        }
        _ASSERTE(def.m_cbColumn == 4);
        METADATATRACKER_ONLY(pVal = MetaDataTracker::NoteAccess(pVal, 4));
    ...
    FORCEINLINE static ULONG getI4(const void *pRec, CMiniColDef &def)
    {
        ...
        return GET_UNALIGNED_VAL32((BYTE*)pRec + def.m_oColumn);
    }
...
protected:
    ...
    static const BYTE* s_TableColumnDescriptors[TBL_COUNT];
...
public:
    ...
    FORCEINLINE HRESULT getStringW(ULONG ix, __inout_ecount (cchBuffer) LPWSTR szOut, ULONG cchBuffer, ULONG *pcchBuffer)
    { return static_cast<Impl*>(this)->Impl_GetStringW(ix, szOut, cchBuffer, pcchBuffer); }
    FORCEINLINE void getGuid(ULONG ix, GUID *pGuid)
    { static_cast<Impl*>(this)->Impl_GetGuid(ix, pGuid);}
    ...
    // IMetaModelCommon interface beginning
   ...
   void CommonGetAssemblyRefProps(
        ...
        case mdtTypeSpec:
            {
                ...
                // This doesn't have a coded typedef or typeref. We'll say this isn't a match
                goto ErrExit;
            }
        case mdtMethodDef:
            {
...

    {
        ...
        TypeSpecRec *pTS = getTypeSpec(RidFromToken(tkTypeSpec));
        ULONG cbSig = 0;
        PCCOR_SIGNATURE pSig = getSignatureOfTypeSpec(pTS, &cbSig);
        PCCOR_SIGNATURE pEnd = pSig + cbSig;
        ULONG data = 0;
...
//  friend class CLiteWeightStgdb;
    ...
    virtual HRESULT vGetStringW(ULONG ix, __inout_ecount (cchBuffer) LPWSTR szOut, ULONG cchBuffer, ULONG *pcchBuffer)
...
#define MiniMdTable(tbl) ULONG getCount##tbl##s() { return _TBLCNT(tbl); }
    ...
    void _GETGUID(Module,Mvid);
    void _GETGUID(Module,EncId);
    void _GETGUID(Module,EncBaseId);
    ...
    RID getGenericParamsForTypeDef(RID rid, RID *pEnd=0)
    { 
    ...
    RID getGenericParamsForMethodDef(RID rid, RID *pEnd=0)
    { 
        return SearchTableForMultipleRows(TBL_GenericParam, 
                            _COLDEF(GenericParam,Owner),
    ...
    RID getMethodSpecsForMethodDef(RID rid, RID *pEnd=0)
    { 
    ...
    RID getMethodSpecsForMemberRef(RID rid, RID *pEnd=0)
    { 
        return SearchTableForMultipleRows(TBL_MethodSpec, 
                            _COLDEF(MethodSpec,Method),
...
//  RID getInterfaceImplsForTypeDef(TypeDefRec *pRec, RID *pEnd=0) {return getInterfaceImplsForTypeDef(getRidForRow(pRec, TBL_TypeDef), pEnd);}
    ...
    LPCUTF8 _GETSTR(GenericParam,Name);
    RID getGenericParamConstraintsForGenericParam(RID rid, RID *pEnd=0)
    { 
    ...
    //*****************************************************************************
    BOOL _IsValidTokenBase( // true if tk is valid token
