...
public:
    ...
    FORCEINLINE HRESULT MarkMethodSpec(mdToken tk) { return MarkToken(tk, MethodSpecMarkedBit); }
...
public:
    ...
    HRESULT GetSaveSize(CorSaveSize fSave, ULONG *pulSize, DWORD *pbCompressed, CorProfileData *profileData = NULL);
    int IsPoolEmpty(int iPool);
    HRESULT GetPoolSaveSize(int iPool, ULONG *pulSize);

    HRESULT SaveTablesToStream(IStream *pIStream, CorProfileData *profileData);
...
protected:
    ...
    HRESULT GetFullSaveSize(CorSaveSize fSave, ULONG *pulSize, DWORD *pbCompressed, CorProfileData *profileData = NULL);
    HRESULT GetENCSaveSize(ULONG *pulSize);
    HRESULT GetHotPoolsSaveSize( ULONG * size, CorProfileData * profileData );

    HRESULT SaveFullTablesToStream(IStream *pIStream, CorProfileData *profileData = NULL);
    HRESULT SaveENCTablesToStream(IStream *pIStream);
    HRESULT SaveHotPoolsToStream( IStream * strm, CorProfileData * profileData, ULONG * poolDirSize, ULONG * savedPoolsSize );
    HRESULT SaveHotPoolToStream( IStream * strm, CorProfileData * profileData, StgPool & pool, MDPools poolId, ULONG * savedSize );
...
protected:
    ...
    HRESULT ApplyHeapDeltasWithFullDeltaToPool(StgPool* deltaPool, StgPool* targetPool);
