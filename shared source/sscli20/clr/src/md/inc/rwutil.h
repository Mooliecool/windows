...
#define     UTF8STR(pwsz, psz) \
    psz = ((pwsz) ? (char *) _alloca((wcslen(pwsz) * 3) + 1) : NULL); \
    if ((pwsz) != NULL) Unicode2UTF((pwsz), (psz));
...
#if MD_THREADSAFE
#define LOCKREADIFFAILRET()         CMDSemReadWrite cSem(true, m_pSemReadWrite);\
                                    IfFailRet(cSem.ObtainLock());
#define LOCKWRITEIFFAILRET()        CMDSemReadWrite cSem(false, m_pSemReadWrite);\
                                    IfFailRet(cSem.ObtainLock());

#define LOCKREADNORET()             CMDSemReadWrite cSem(true, m_pSemReadWrite);\
                                    hr = cSem.ObtainLock();
#define LOCKWRITENORET()            CMDSemReadWrite cSem(false, m_pSemReadWrite);\
                                    hr = cSem.ObtainLock();

#define LOCKREAD()                  CMDSemReadWrite cSem(true, m_pSemReadWrite);\
                                    IfFailGo(cSem.ObtainLock());
...
#define LOCKREADIFFAILRET()
#define LOCKWRITEIFFAILRET()
#define LOCKREADNORET()
#define LOCKWRITENORET()
#define LOCKREAD()    
#define LOCKWRITE()   
#define UNLOCKWRITE() 
