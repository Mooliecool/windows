    ...
    void        **ppIUnk)               // [out] Return interface on success.
{
    ...
    MEMORY_REPORT_CONTEXT_SCOPE("Metadata");
...
struct ModIntPair
{
    ...
    HCORMODULE  m_hMod;
    ModIntPair* m_pNext;
    ModIntPair() { m_pIUnk = NULL; m_hMod = NULL; m_pNext = NULL; };
    ModIntPair(IUnknown* pi, HCORMODULE hm) { m_pIUnk = pi; m_hMod = hm; m_pNext = NULL; };
};

class ModIntPairList
{
private:
    ModIntPair m_Head;
    ModIntPair* m_pTail; 
    CRITSEC_COOKIE m_cSect;
    void Init() 
    { 
        if(m_cSect ==NULL)
            m_cSect = ClrCreateCriticalSection("ModIntPairList",CrstModIntPairList,CrstFlags(CRST_DEFAULT/*CRST_UNSAFE_ANYMODE | CRST_DEBUGGER_THREAD*/));
    };
...
public:
    ...
    ~ModIntPairList()
    {
        if (m_cSect != NULL)
        {
            ModIntPair* pMIP = NULL;
            ModIntPair* ptmp = NULL;
            ClrEnterCriticalSection(m_cSect);
            for(pMIP = m_Head.m_pNext; pMIP != NULL; pMIP = ptmp)
            {
                ptmp = pMIP->m_pNext;
                delete pMIP;
            }
            ClrLeaveCriticalSection(m_cSect);
            ClrDeleteCriticalSection(m_cSect);
        }
    };
    void Append(ModIntPair* pMIP)
    {
        Init();
        if(pMIP)
        {
            CRITSEC_Holder csh(m_cSect);
            m_pTail->m_pNext = pMIP;
            m_pTail = pMIP;
        }
    };
    ModIntPair* FindAndExtract(IUnknown* pi)
    {
        if (m_cSect != NULL) // otherwise, there's nothing in the list
        {
            ModIntPair* pMIP = NULL;
            ModIntPair* ptmp = NULL;
            CRITSEC_Holder csh(m_cSect);
            for(pMIP = &m_Head; pMIP->m_pNext != NULL; pMIP = pMIP->m_pNext)
            {
                if((pMIP->m_pNext)->m_pIUnk == pi)
                {
                    ptmp = pMIP->m_pNext;
                    pMIP->m_pNext = ptmp->m_pNext;
                    if(ptmp == m_pTail)
                        m_pTail = pMIP;
                    return ptmp;
                }
    ...
    ModIntPair* FindAndExtract(HCORMODULE hMod)
    {
        if (m_cSect != NULL) // otherwise, there's nothing in the list
        {
            ModIntPair* pMIP = NULL;
    ...
    IUnknown    **ppIUnk)               // [out] Return interface on success.
{
    ...
    IfFailGoto(RuntimeOpenImageInternal(szFileName, &hModule, &dwFileLength, flags), ErrAsmExpected);

    IfFailGo(GetAssemblyMDInternalImportHelper(hModule, dwFileLength, riid, flags, ppIUnk));

    MIPList.Append(new (nothrow) ModIntPair(*ppIUnk,hModule));
...
{
    ModIntPair* pMIP = MIPList.FindAndExtract(pIUnk);
    HRESULT hr = S_FALSE;
    if(pMIP != NULL)
    {
        hr = RuntimeReleaseHandle(pMIP->m_hMod);
        delete pMIP;
    }
    return hr;
}
                                          ...
                                          IUnknown   **ppIUnk)
{
    ...
    hModule = NULL; // ownership transfered
...
AssemblyMDInternalImport::~AssemblyMDInternalImport () 
{
    ...
    else if(m_pHandle) 
    {
        ...
        ModIntPair* pMIP = MIPList.FindAndExtract(m_pHandle);
        if(pMIP != NULL) delete pMIP;
    }
...
{
    DWORD dwKind = m_dwPEKind;
    HRESULT hr = S_OK;
    if(pdwPEKind) *pdwPEKind = dwKind;
