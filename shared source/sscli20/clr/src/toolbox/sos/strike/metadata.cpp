...
IMetaDataImport* MDImportForModule (DWORD_PTR pModule)
{
    ...
    if(moduleData.Request(g_clrData,(CLRDATA_ADDRESS) pModule)==S_OK)
                  ...
                  bool bClassName)
{
    DacpModuleData ModuleData;
    mdName[0] = L'\0';
    if(ModuleData.Request(g_clrData,(CLRDATA_ADDRESS) ModuleAddr)==S_OK)
...
BOOL IsValidToken(DWORD_PTR ModuleAddr, mdTypeDef mb)
{
    ...
    if(ModuleData.Request(g_clrData,(CLRDATA_ADDRESS) ModuleAddr)==S_OK)
    {
        IMetaDataImport *pImport = MDImportForModule(&ModuleData);
        if (pImport)
        {
                  ...
                  bool bClassName)
{
    ...
    {
        WCHAR moduleName[mdNameLen+19]; // for the "Dynamic Module In " below
        FileNameForModule(pModule,moduleName);
        if (moduleName[0] == L'\0') {
            DacpAssemblyData assembly;
            assembly.Request(g_clrData,pModule->Assembly);
            if (assembly.isDynamic) {
                wcscpy_s(moduleName, _countof(moduleName), L"Dynamic ");
            }
            wcscat_s (moduleName, _countof(moduleName), L"Module in ");            
            if(DacpAssemblyData::GetName(g_clrData,pModule->Assembly,mdNameLen,g_mdName)==S_OK)
            {
                wcscat_s(moduleName, _countof(moduleName),g_mdName);
            }
        }
...
void MDInfo::GetMethodName(mdTypeDef token, CQuickBytes *fullName)
{
    ...
    wcscat_s (m_szName, _countof(m_szName), m_szTempBuf);
...
HRESULT MDInfo::GetOneElementType(PCCOR_SIGNATURE pbSigBlob, ULONG ulSigBlob, ULONG *pcb)
{
    ...
    if (ulData == ELEMENT_TYPE_INTERNAL)
    {
        ...
        IfFailGo(DacpMethodTableData::GetMethodTableName(g_clrData, (CLRDATA_ADDRESS)pvMethodTable, mdNameLen, g_mdName));
