...
public:
    ...
    UINT32 grab( UINT32 n ) {
        if( !grow ( m_instanceCount + n ) ) 
            return NULL;
        _ASSERTE( m_instanceCount < m_spaceSize );
        m_instanceCount += n;
        return m_instanceCount - n;
    }
    ...
    UINT32      m_spaceSize;     // Total size of array
...
public:
    ...
    COM_METHOD SetMethodSourceRange(ISymUnmanagedDocumentWriter *startDoc,
                                    ...
                                    ULONG32 endLine,
                                    ULONG32 endColumn);
    COM_METHOD GetDebugCVInfo(ULONG32 cData,
                           ULONG32 *pcData,
                           BYTE data[]);
    ...
    HRESULT DocumentReleased(SymDocumentWriter *sdw);
    HRESULT DefineSrc(UINT32 DocumentEntry,
                      GUID AlgorithmId,
                      DWORD CheckSumSize,
                      BYTE* pCheckSum,
                      DWORD SourceSize,
...
private:
    ...
    struct DocWriter {
        SymDocumentWriter*  pdoc;
        DocWriter*        pnext;
    };
    DocWriter*         m_pDocsHead;
...
public:
    ...
    GUID m_AlgorithmId;     // Algorithm ID for the CheckSum
    DWORD m_CheckSumSize;   // CheckSum size
    DWORD m_SourceSize;     // SourceFile
    BYTE* m_pcheckSum;      // CheckSym
    BYTE* m_source;         // Source
    bool m_written;         // Have we been written yet?
};
