...
MDInfo::MDInfo(IMetaDataDispenserEx *pDispenser, LPCWSTR szScope, strPassBackFn inPBFn, ULONG DumpFilter)
{
    ...
    hr = pDispenser->OpenScope(szScope, 0, IID_IMetaDataImport2, (IUnknown**)&m_pImport);
...
int MDInfo::VWriteMarker(__in_z __in char *str, va_list marker)
{
    ...
    if (FAILED(hr = m_output.ReSizeNoThrow(STRING_BUFFER_LEN * 32)))
        Error("ReSize failed.", hr);
    else
    {
        count = vsprintf_s((char *)m_output.Ptr(), STRING_BUFFER_LEN * 32, str, marker);
...
void MDInfo::DisplayFieldMarshal(mdToken inToken)
{
    ...
    if (hr != CLDB_E_RECORD_NOTFOUND)
    {
        ...
        while (cbCur < cbNativeType)
        {
            ...
            ulStrLoc += sprintf_s(szNTDesc + ulStrLoc, STRING_BUFFER_LEN-ulStrLoc, "%s ", (ulData < sizeof(g_szNativeType)/sizeof(*g_szNativeType)) ? g_szNativeType[ulData] : "*OUTOFRANGE*");
            switch (ulData)
            {
...
void MDInfo::DisplayAssemblyInfo()
{
    ...
    ISFLAG(Af, Library);
    ISFLAG(Af, AppDomainPlatform);
    ISFLAG(Af, ProcessPlatform);
    ISFLAG(Af, SystemPlatform);
    */
