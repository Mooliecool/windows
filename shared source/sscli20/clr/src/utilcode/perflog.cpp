...
void PerfLog::OutToStdout(__in_z wchar_t *wszName, UnitOfMeasure unit, __in_opt wchar_t *wszDescr)
{
    LEAF_CONTRACT;

    if (wszDescr)
        _snwprintf_s(m_wszOutStr_2, PRINT_STR_LEN, PRINT_STR_LEN - 1, L" (%s)\n", wszDescr);
    else
        _snwprintf_s(m_wszOutStr_2, PRINT_STR_LEN, PRINT_STR_LEN - 1, L"\n");
    
    printf("%S", m_wszOutStr_1);
    printf("%S", m_wszOutStr_2);
}

void PerfLog::OutToPerfFile(__in_z wchar_t *wszName, UnitOfMeasure unit, __in_opt wchar_t *wszDescr)
{
    LEAF_CONTRACT;
    ...
    else
    {
        if (wcscmp(wszName, L"ExecTime") == 0)
            _snwprintf_s(m_wszOutStr_2, PRINT_STR_LEN, PRINT_STR_LEN - 1, L"ExecUnitDescr=%s\nExecIDirection=%s\n", wszDescr, wszIDirection[unit]);
        else
        {
            if (wszDescr)
                _snwprintf_s(m_wszOutStr_2, PRINT_STR_LEN, PRINT_STR_LEN - 1, L"%s Descr=%s\n%s Unit Descr=None\n%s IDirection=%s\n", wszName, wszDescr, wszName, wszName, wszIDirection[unit]);
            else
                _snwprintf_s(m_wszOutStr_2, PRINT_STR_LEN, PRINT_STR_LEN - 1, L"%s Descr=None\n%s Unit Descr=None\n%s IDirection=%s\n", wszName, wszName, wszName, wszIDirection[unit]);
        }
...
void PerfLog::Log(__in_z wchar_t *wszName, UINT64 val, UnitOfMeasure unit, __in_opt wchar_t *wszDescr)
{
    LEAF_CONTRACT;
    
    // Format the output into two pieces: The first piece is formatted here, rest in OutToStdout.
    _snwprintf_s(m_wszOutStr_1, PRINT_STR_LEN, PRINT_STR_LEN - 1, L"%-30s%12.3I64u %s", wszName, val, wszUnitOfMeasureDescr[unit]);
    ...
    if (CommaSeparatedFormat())
    {
        _snwprintf_s(m_wszOutStr_1, PRINT_STR_LEN, PRINT_STR_LEN - 1, L"%s;%0.3I64u;", wszName, val);
    ...
    {
        if (wcscmp(wszName, L"ExecTime") == 0)
            _snwprintf_s(m_wszOutStr_1, PRINT_STR_LEN, PRINT_STR_LEN - 1, L"%s=%0.3I64u\nExecUnit=%s\n", wszName, val, wszUnitOfMeasureDescr[unit]);
        else
...
// Output stats in pretty print to stdout and perf automation format to file 
// handle m_hPerfLogFileHandle
void PerfLog::Log(__in_z wchar_t *wszName, double val, UnitOfMeasure unit, __in_opt wchar_t *wszDescr)
{
    LEAF_CONTRACT;
    
    // Format the output into two pieces: The first piece is formatted here, rest in OutToStdout.
    _snwprintf_s(m_wszOutStr_1, PRINT_STR_LEN, PRINT_STR_LEN - 1, L"%-30s%12.3g %s", wszName, val, wszUnitOfMeasureDescr[unit]);
    OutToStdout (wszName, unit, wszDescr);

    // Format the output into two pieces: The first piece is formatted here, rest in OutToPerfFile
    if (CommaSeparatedFormat())
    {
        _snwprintf_s(m_wszOutStr_1, PRINT_STR_LEN, PRINT_STR_LEN - 1, L"%s;%0.3g;", wszName, val);
        OutToPerfFile (wszName, unit, wszDescr);
    }
    ...
    {
        if (wcscmp(wszName, L"ExecTime") == 0)
            _snwprintf_s(m_wszOutStr_1, PRINT_STR_LEN, PRINT_STR_LEN - 1,  L"%s=%0.3g\nExecUnit=%s\n", wszName, val, wszUnitOfMeasureDescr[unit]);
        else
            _snwprintf_s(m_wszOutStr_1, PRINT_STR_LEN, PRINT_STR_LEN - 1, L"%s=%0.3g\n%s Unit=%s\n", wszName, val, wszName, wszUnitOfMeasureDescr[unit]);
        OutToPerfFile (wszName, unit, wszDescr);
    }
...
// Output stats in pretty print to stdout and perf automation format to file 
// handle m_hPerfLogFileHandle
void PerfLog::Log(__in_z wchar_t *wszName, UINT32 val, UnitOfMeasure unit, __in_opt wchar_t *wszDescr)
{
    LEAF_CONTRACT;
    
    // Format the output into two pieces: The first piece is formatted here, rest in OutToStdout.

    _snwprintf_s(m_wszOutStr_1, PRINT_STR_LEN, PRINT_STR_LEN - 1, L"%-30s%12d %s", wszName, val, wszUnitOfMeasureDescr[unit]);
    OutToStdout (wszName, unit, wszDescr);

    // Format the output into two pieces: The first piece is formatted here, rest in OutToPerfFile
    if (CommaSeparatedFormat())
    {
        _snwprintf_s(m_wszOutStr_1, PRINT_STR_LEN, PRINT_STR_LEN - 1, L"%s;%d;", wszName, val);
        OutToPerfFile (wszName, unit, wszDescr);
    }
    ...
    {
        if (wcscmp(wszName, L"ExecTime") == 0)
            _snwprintf_s(m_wszOutStr_1, PRINT_STR_LEN, PRINT_STR_LEN - 1, L"%s=%0.3d\nExecUnit=%s\n", wszName, val, wszUnitOfMeasureDescr[unit]);
        else
            _snwprintf_s(m_wszOutStr_1, PRINT_STR_LEN, PRINT_STR_LEN - 1, L"%s=%0.3d\n%s Unit=%s\n", wszName, val, wszName, wszUnitOfMeasureDescr[unit]);
        OutToPerfFile (wszName, unit, wszDescr);
    }
