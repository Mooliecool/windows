...
const unsigned base[MAX_HEADER+2] = {BASE_0, BASE_1, BASE_2, BASE_3, BASE_4, BASE_5, BASE_6};
...
const unsigned bitlength[MAX_HEADER+2] =
        {
            BIT_LENGTH_0,
            BIT_LENGTH_1,
            BIT_LENGTH_2,
...
const Decoder::Decode empty = {decoded_end, DECODING_HEADER(0)};
...
// --------------------------------------------------------
void Decoder::Nibbles::SetContents( BYTE* bytes)
{
...
tryagain:
    ...
    unsigned bitsNeeded = bitlength[index]-NUMBERGOTTEN(state.next);
    result = (PARTIALBITS(state.next)<<bitsNeeded)+data.Bits(bitsNeeded)+base[index];
    state = empty;
    unsigned skip = bitsNeeded % 4; // this works since we are always 4-bit aligned
    if (skip > 0)
    {
        // state.next is always 0, because we did "state = empty;" above
        state = transition[state.next][data.Next()];
        state.decoded += skip;
    }
    return result; 
...
void Encoder::Add(unsigned value, unsigned length)
{
    ...
    {
        length -= unusedBits;
        encoding = (encoding<<unusedBits)+(value>>(length));
    ...
    encoding = (encoding<<length)+value;
    unusedBits -= length;       
}
