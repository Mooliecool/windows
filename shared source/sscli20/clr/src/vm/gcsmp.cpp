...
heap_segment* empty_segment = NULL;
...
public:
    ...
    void SetFree(size_t size)
    {
        ...
        DWORD* numComponentsPtr = (DWORD*) &((BYTE*) pNewFreeObject)[ArrayBase::GetOffsetOfNumComponents()];
        *numComponentsPtr = (DWORD)(size - base_size);
#ifdef _DEBUG
...
void* findRegion (void* start_address, size_t size)
{
    ...
    while ((size_t)start_address <= (size_t)TOP_MEMORY - size)
    {
...
size_t size_brick_of (BYTE* from, BYTE* end)
{
    ...
    return (((size_t)(end - from)) / brick_size) * sizeof (short);
}
...
size_t size_card_of (BYTE* from, BYTE* end)
{
    ...
    return (((size_t)(end - from)) /card_size);
}
...
BYTE* translate_card_table (BYTE* c_table)
{
    return (c_table - size_card_of (0, card_table_lowest_address( c_table )));
}
...
int gc_heap::grow_brick_card_tables (BYTE* start, BYTE* end)
{
    ...
    if ((la != g_lowest_address ) || (ha != g_highest_address))
    {
        ...
        card_table_next (ct) = &((BYTE *)g_card_table)[gcard_of(la)];
        ...
        {
            for (size_t i = 0;i < ((size_t)(g_highest_address - g_lowest_address)) / brick_size; i++)
                bt[i] = -32768;
        }
        ...
        g_card_table = (DWORD *)translate_card_table (ct);
                                     ...
                                     BYTE* start, BYTE* end, BOOL heap_expand)
{
    ...
    if (old_brick_table)
    {
        ...
        memcpy (brick_start, &old_brick_table[brick_offset],
                (((size_t)(end - start))/brick_size)*sizeof (short));
    ...
    BYTE * ct = card_table_next (&card_table[card_of(lowest_address)]);
    assert (ct);
    ...
    while (ct)
    {
            ...
            (card_table_lowest_address (ct) <= start))
        {
            ...
            BYTE* src = &((translate_card_table (ct)) [card_of (start)]);
...
void gc_heap::copy_brick_card_table(BOOL heap_expand)
{
    ...
    assert (la == card_table_lowest_address (&old_card_table[card_of (la)]));
    assert (ha == card_table_highest_address (&old_card_table[card_of (la)]));

    BYTE* ct = &((BYTE *)g_card_table)[card_of(g_lowest_address)];
    ...
    BYTE* cd = card_table_next (&((BYTE *)g_card_table)[card_of(g_lowest_address)]);
    while (cd)
    {
    ...
    card_table_next (&((BYTE *)g_card_table)[card_of(g_lowest_address)]) = 0;
...
void gc_heap::copy_brick_card_table_l_heap ()
{
    ...
    if (lheap_card_table != ((BYTE *)g_card_table))
    {
        ...
        BYTE* old_card_table = &lheap_card_table[card_of(lowest_address)];
...
heap_segment* gc_heap::strip_empty_segment()
{
    heap_segment* seg =
        generation_start_segment (generation_of (max_generation));
    _ASSERTE( seg != empty_segment );

    while (heap_segment_next (seg) != empty_segment)
       seg = heap_segment_next (seg);

    return seg;
}

heap_segment* gc_heap::find_heap_segment( size_t size )
{
    heap_segment* seg =
        generation_start_segment (generation_of (max_generation));
    heap_segment* prev_seg = 0;
    ...
    while (seg && (seg != ephemeral_heap_segment))
    {
        ...
        if (a_size_fit_p(size, heap_segment_allocated (seg), heap_segment_reserved (seg)))
        {
        if ( prev_seg )
               heap_segment_next (prev_seg) = next_seg;
            else if ( next_seg )
        {
           generation_start_segment (generation_of (max_generation)) = next_seg;
           generation_allocation_start (generation_of (max_generation)) = heap_segment_mem (next_seg);
            }
            // Reset the next pointer on the segment that is taken out of the list
            heap_segment_next (seg) = NULL;
            return seg;
        }
                                ...
                                size_t heap_size)
{
    ...
    // Create an empty segment that is used for OOM
    empty_segment = new heap_segment();
    if (!empty_segment)
        return E_OUTOFMEMORY;
    heap_segment_mem (empty_segment) = 0;
    heap_segment_reserved (empty_segment) = 0;
    heap_segment_committed (empty_segment) = 0;
    heap_segment_next (empty_segment) = 0;
    heap_segment_plan_allocated (empty_segment) = 0;
    heap_segment_allocated (empty_segment) = 0;
    heap_segment_used (empty_segment) = 0;
...
again:
        ...
        if (old_size < (dsize))
        {
            ...
            else
            {
                if (!grow_heap_segment (seg,
                                        ...
                                        heap_segment_committed(seg)))
                {
            // We can't get here on Windows but it is possible on operating systems that
            // don't maintain backing space for all reservations
                    heap_segment_next (ephemeral_heap_segment) = empty_segment;
                    ephemeral_heap_segment = empty_segment;

                    generation_allocation_segment (youngest_generation) = ephemeral_heap_segment;
    ...
    {
        // Try to reuse one of the already allocated segments
        seg = find_heap_segment( 2*dsize );

        if ( seg )
    {
        // Remove empty segment from the end of the linked list
        if ( ephemeral_heap_segment == empty_segment )
                 heap_segment_next(strip_empty_segment()) = seg;
            else
             heap_segment_next (ephemeral_heap_segment) = seg;
            ephemeral_heap_segment = seg;
            start = heap_segment_allocated( seg );
    }
    else
    {
            ...
            if ( start == 0 && ephemeral_heap_segment != empty_segment )
        {
           heap_segment_next (ephemeral_heap_segment) = empty_segment;
               ephemeral_heap_segment = empty_segment;
            }
...
void gc_heap::gc1()
{
    ...
    clear_card_for_addresses (
        ...
        align_on_card (generation_allocation_start (generation_of (0))));
                             ...
                             )
{
...

    {
        ...
        CNameSpace::GcFixAllocContexts ((void*)TRUE, NULL);
    ...
    if ( ephemeral_heap_segment != empty_segment )
    {
        ...
        condemned_generation_num = max_generation;
...
{
    if (!gen0_bricks_cleared && ephemeral_heap_segment != empty_segment )
    {
...

{
    ...
    // scan for deleted short weak pointers
    CNameSpace::GcShortWeakPtrScan(GCHeap::Promote, condemned_gen_number, max_generation,
                                    &sc);
    //Handle finalization.
...
void gc_heap::sweep_phase (int condemned_gen_number)
{
    ...
    while (1)
    {
        if (x >= end)
        {
            ...
            heap_segment_used (seg) = plug_end;
...
void gc_heap::scavenge_pinned_objects (BOOL free_list_p)
{
    ...
    while (1)
    {
        if (x >= end)
        {
            ...
            {
            //clear the cards
            clear_card_for_addresses( plug_end, heap_segment_allocated (seg) );
                //adjust the end of the segment.
                heap_segment_allocated (seg) = plug_end;
        ...
        if (pinned (x))
        {
            ...
            if (free_list_p)
            {
                //set bricks for pinned objects
                size_t br = brick_of (plug_start);
                size_t size = plug_end - plug_start;
                ptrdiff_t begoffset = brick_table [brick_of (plug_start)];
                if (begoffset < (plug_start + size) - brick_address (br))
                    set_brick (br, plug_start - brick_address (br));
                br++;
                short offset = 0;
                while (br <= brick_of (plug_start+size-1))
                {
                    set_brick (br, --offset);
                    br++;
                }
                                  ...
                                  size_t size)
{
    ...
    if (!new_seg || new_seg == empty_segment )
        return 0;
    ...
    if ( ephemeral_heap_segment == empty_segment )
         heap_segment_next(strip_empty_segment()) = new_seg;
    else
...
volatile unsigned            GCHeap::GcCount                 = 0;
#ifdef TRACE_GC
...
CLREvent            *GCHeap::hEventFinalizer           = NULL;
CLREvent            *GCHeap::hEventFinalizerDone       = NULL;
CLREvent            *GCHeap::hEventFinalizerToShutDown = NULL;
CLREvent            *GCHeap::hEventShutDownToFinalizer = NULL;
...
gc_heap::verify_heap()
{
    ...
    while (1)
    {
            ...
            (curr_free_item < heap_segment_allocated (seg)))
        {
            ...
            {
                printf ("Current free item %p is invalid (inside %p)", curr_free_item,
                        prev_object);
                FATAL_GC_ERROR();
            }
...
void GCHeap::EnableFinalization()
{
    hEventFinalizer->Set();
}
...
// Encourage a thread to cooperate with the GC by hiding its quantum into
// alloc_save_limit.
void GCHeap::HideAllocContext(alloc_context* context)
{
}

void GCHeap::RevealAllocContext(alloc_context* context)
{
}
...
bool GCHeap::IsThreadUsingAllocationContextHeap(alloc_context ac, int thread_number)
{
    return true;
}

GCSpinLock gc_heap::gc_lock = SPIN_LOCK_INITIALIZER;
...
Object * GCHeap::NextObj (Object * object)
{
    ...
    while (lh)
    {
        if ((o < (BYTE*)lh->heap + lh->size) && (o >= lh->heap))
        {
            BYTE * nextobj = o + AlignQword(size(o));
            if (nextobj < (BYTE*)lh->heap + lh->size)
            {                        
    ...
    heap_segment * hs = gc_heap::find_segment (o);
    if (!hs)
    {
        return NULL;        
    }

    BYTE * nextobj = o + Align (size(o));
    if (nextobj < heap_segment_mem (hs) || nextobj >= heap_segment_allocated (hs))
    {
        return NULL;
    }
...
size_t StressHeapPostIP = (size_t) -1;
    ...
    // return TRUE if GC is performed, FALSE otherwise
...
Object *
GCHeap::Alloc( size_t size, DWORD flags)
{
    CONTRACTL {
        THROWS;
        GC_TRIGGERS;
...
Object *
GCHeap::AllocLHeap( size_t size, DWORD flags)
{
    CONTRACTL {
        THROWS;
        GC_TRIGGERS;
...
Object*
GCHeap::Alloc(alloc_context* acontext, size_t size, DWORD flags )
{
    CONTRACTL {
        THROWS;
        GC_TRIGGERS;
...
retry:
    ...
    return dd_collection_count (pGenGCHeap->dynamic_data_of (gen));
}
...
int GCHeap::CollectionCount (int generation)
{
...
GCHeap::SetCardsAfterBulkCopy( Object **StartPoint, size_t len )
{
    ...
    if (GCHeap::GetGCHeap()->WhichGeneration( (Object*) StartPoint ) == 0)
        return;
        ...
        for (Object** i = m_FillPointers [Seg]-1; i >= endIndex ;i--)
        {
            ...
            if (obj->GetMethodTable()->IsAgileAndFinalizable())
            {
                ...
                if (! pThread || ! pThread->IsUnstarted())
                {
                    ...
                    obj->GetHeader()->ResetAppDomainIndex(SystemDomain::System()->DefaultDomain()->GetIndex());
                    continue;
                }
            }
                                ...
                                gc_heap* hp)
{
    ...
    if (passNumber == 1)
    {
        ...
        for (unsigned int Seg = startSeg; Seg < NUMBERGENERATIONS; Seg++)
        {
            ...
            {
                Object* obj = *i;
                if (!GCHeap::GetGCHeap()->IsPromoted (obj, &sc))
                {
                    if ((obj->GetHeader()->GetBits()) & BIT_SBLK_FINALIZER_RUN)
                    {
                        //remove the object because we don't want to
...
void CFinalize::CheckFinalizerObjects()
{
    for (size_t i = 0; i < NUMBERGENERATIONS; i++)
    {
        ...
        for (Object **po = startIndex; po > stopIndex; po++)
        {
            if (GCHeap::GetGCHeap()->WhichGeneration (*po) < (NUMBERGENERATIONS - i -1))
                FATAL_GC_ERROR ();
            (*po)->Validate();
