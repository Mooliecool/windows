...
//
//
//    Copyright (c) 2006 Microsoft Corporation and/or its suppliers.  All rights reserved.
//
...
DWORD mem_reserve = (MEM_RESERVE);

#define AlignPage(add) ((((DWORD_PTR)(add)) + (malloc_getpagesize-1)) &~(malloc_getpagesize-1))

/* reserve 64MB to insure large contiguous space */
...
void gmallocHeap::gcleanup ()
{
    ...
    {
        rval = ClrVirtualFree (gAddressBase,
    ...
    while (head)
    {
        ...
        rval = ClrVirtualFree (head->base, 0, MEM_RELEASE);
        ASSERT (rval);
        delete head;
        head = next;
    }
...
void* gmallocHeap::wsbrk (long size)
{
#ifndef DACCESS_COMPILE
    void* tmp;
    if (size > 0)
    {
        ...
        if (gAddressBase == 0)
        {
            ...
            gAllocatedSize = max ((long)gInitialReserve, (long)AlignPage (size));
            gNextAddress = gAddressBase = (BYTE*)ClrVirtualAlloc (NULL, gAllocatedSize,
                                            MEM_RESERVE, PAGE_NOACCESS);
        } else if (AlignPage (gNextAddress + size) > (DWORD_PTR)(gAddressBase + gAllocatedSize))
        {

            // can't add another segment if client requested fixed heap

            if (gmFlags & GM_FIXED_HEAP)
                return (void*)-1;

            long new_size = max ((long)gAllocatedSize, (long)AlignPage (size));
            void* new_address = gAddressBase + gAllocatedSize;
            ptrdiff_t delta = 0;
            ...
            for(;;)
            {
                ...
                if (gVirtualAllocedHook != 0)
                {
                    ...
                    gAddressBase = gNextAddress = (BYTE*)prgmem;
                }
                else
                ...
                {
                    gAddressBase = 0;
                    void* prgmem = ClrVirtualAlloc (new_address, new_size,
                                                    MEM_RESERVE, PAGE_NOACCESS);
                    if (!prgmem)
                        continue;

                    void* gcmem = ClrVirtualAlloc (NULL, new_size,
                                                   MEM_RESERVE, PAGE_NOACCESS);
                    if (!gcmem)
                        return (void*)-1;
...
#endif //MAP_VIEW
                {
                    gAddressBase = gNextAddress =
                        (BYTE*)ClrVirtualAlloc (new_address, new_size,
                                                    mem_reserve, PAGE_NOACCESS);
                ...
                new_address = (BYTE*)new_address + OS_PAGE_SIZE;
            }
            ...
            if (gVirtualAllocedHook != 0)
                ...
                {
                    //restore old values
                    gAddressBase = (BYTE*)prev_base;
                    gNextAddress = (BYTE*)prev_next;
                    gAllocatedSize = (UINT32)prev_size;
                    return (void*)-1;
                }
        ...
        if ((DWORD_PTR)(size + gNextAddress) > AlignPage (gNextAddress))
        {
            void* res;
            res = ClrVirtualAlloc ((void*)AlignPage (gNextAddress),
                                   (SIZE_T)(size + gNextAddress -
        ...
        tmp = gNextAddress;
        gNextAddress = (BYTE*)tmp + size;
        return tmp;
    }
    else if (size < 0)
    {
        BYTE* alignedGoal = (BYTE*)AlignPage (gNextAddress + size);
        /* Trim by releasing the virtual memory */
        if (alignedGoal >= gAddressBase)
        {
            ClrVirtualFree (alignedGoal, gNextAddress - alignedGoal,
                            MEM_DECOMMIT);
        ...
        {
            ClrVirtualFree (gAddressBase, gNextAddress - gAddressBase,
                            MEM_DECOMMIT);
            gNextAddress = gAddressBase;
            return (void*)-1;
        }
...
void gmallocHeap::malloc_extend_top(INTERNAL_SIZE_T nb)
{
  ...
  brk = (char*)(MORECORE ((long)sbrk_size));
  ...
  else
  {
    ...
    /* Allocate correction */
    new_brk = (char*)(MORECORE ((long)correction));
...
#endif
{
  ...
  size_t    block;               /* block traverser bit */
...
#ifdef _DEBUG
    if (g_pConfig->ShouldInjectFault(INJECTFAULT_GMHEAP))
    {
...
#endif
{
  ...
  if (next == top)                            /* merge with top */
  {
    ...
    if (!(hd & PREV_INUSE))                    /* consolidate backward */
    {
      prevsz = p->prev_size;
#ifdef _MSC_VER
#pragma warning(disable:4146)  // turn off "unary minus applied to unsigned" warning
#endif
...
#endif
{
  ...
  else /* misaligned */
  {
    ...
    */
    ...
    if ((unsigned long)(brk - (char*)(p)) < MINSIZE) brk = brk + alignment;
...
HRESULT gmallocHeap::Init (__in_z char *name, DWORD InitialReserve, DWORD flags)
{
    // Initialize bins
    av_[0] = 0;
    av_[1] = 0;
...
HRESULT gmallocHeap::Init (__in_z char *name, DWORD* start, DWORD Size, gmhook_fn fn,
                           gmprehook_fn pre_fn, DWORD flags)
{
    ...
    {
        gAddressBase = gNextAddress = (BYTE*)start;
        gAllocatedSize = Size;
...
inline HANDLE gmallocHeapCreate (const char * name,
                                 DWORD flOptions,
                                 DWORD dwInitialSize,
                                 DWORD dwMaximumSize)
{
