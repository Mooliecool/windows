...
public:
    ...
    HRESULT Init (__in_z char *name, DWORD InitialReserve = 2*1024*1024, DWORD flags = 0);
    HRESULT Init (__in_z char *name, DWORD* address, DWORD Size, gmhook_fn fn=0,
    ...
    LPVOID AlignedAlloc (DWORD Alignment, DWORD Size)
    {
        WRAPPER_CONTRACT;

        return gmemalign(Alignment, Size);
    }

    LPVOID ReAlloc (LPVOID pMem, DWORD NewSize )
    {
        WRAPPER_CONTRACT;

        return grealloc(pMem, NewSize);
    }
