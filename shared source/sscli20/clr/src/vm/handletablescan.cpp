...
void BlockResetAgeMapForBlocksWorker(ULONG32 *pdwGen, ULONG32 dwClumpMask, ScanCallbackInfo *pInfo)
{
    ...
    {        
        ...
        if (dwClumpMask & GEN_CLUMP_0_MASK)
        {
            ...
            BYTE minAge = GEN_MAX_AGE;
            for ( ; pValue < pLast; pValue++)
            {
                if (!HndIsNullOrDestroyedHandle(*pValue))
                {
                    BYTE thisAge = GCHeap::GetGCHeap()->WhichGeneration(*pValue);
                    if (minAge > thisAge)
                        minAge = thisAge;
                    ...
                    if ((*pValue)->GetGCSafeMethodTable() == g_pOverlappedDataClass)
                    {
                        ...
                        if (pOverlapped->m_userObject != NULL)
                        {
                            ...
                            if (pOverlapped->m_isArray)
                            {
                                ...
                                DWORD num = pUserArrayObject->GetNumComponents();
                                DWORD i;
                                for (i = 0; i < num; i ++)
                                {
            ...
            ((BYTE *)pSegment->rgGeneration)[uClump] = minAge;
        }
        // skip to the next clump
        dwClumpMask = NEXT_CLUMP_IN_MASK(dwClumpMask);
        pValue = pLast;
...
static void VerifyObject(_UNCHECKED_OBJECTREF *pValue, _UNCHECKED_OBJECTREF from, _UNCHECKED_OBJECTREF obj, BYTE minAge)
{
    ...
    BYTE thisAge = GCHeap::GetGCHeap()->WhichGeneration(obj);
    ...
    if (minAge >= GEN_MAX_AGE || (minAge > thisAge && thisAge < GCHeap::GetGCHeap()->GetMaxGeneration()))
    {
 ...
 *
 */
void xxxTableScanQueuedBlocksAsync(HandleTable *pTable, TableSegment *pSegment, CrstHolder *pCrstHolder)
{
    WRAPPER_CONTRACT;
 ...
 *
 */
TableSegment * CALLBACK QuickSegmentIterator(HandleTable *pTable, TableSegment *pPrevSegment, CrstHolder *pCrstHolder)
{
    LEAF_CONTRACT;
 ...
 *
 */
TableSegment * CALLBACK StandardSegmentIterator(HandleTable *pTable, TableSegment *pPrevSegment, CrstHolder *pCrstHolder)
{
    CONTRACTL
    {
        WRAPPER(THROWS);
 ...
 *
 */
TableSegment * CALLBACK FullSegmentIterator(HandleTable *pTable, TableSegment *pPrevSegment, CrstHolder *pCrstHolder)
{
    CONTRACTL
    {
        WRAPPER(THROWS);
 ...
 *
 */
TableSegment * CALLBACK xxxAsyncSegmentIterator(HandleTable *pTable, TableSegment *pPrevSegment, CrstHolder *pCrstHolder)
{
    WRAPPER_CONTRACT;
