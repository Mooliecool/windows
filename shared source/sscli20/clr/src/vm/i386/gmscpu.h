...
struct MachState {
    ...
    MachState(PREGDISPLAY pRD, void ** RetAddr) { 
                   WRAPPER_CONTRACT;
                   Init( (TADDR*)pRD->pEdi, (TADDR*)pRD->pEsi, 
                         (TADDR*)pRD->pEbx, (TADDR*)pRD->pEbp, (TADDR)pRD->Esp, (TADDR*)RetAddr); }
    MachState(void** aPEdi, void** aPEsi, void** aPEbx, void** aPEbp, void* aEsp, void** aPRetAddr) 
                  { WRAPPER_CONTRACT; Init( (TADDR*)aPEdi, (TADDR*)aPEsi, (TADDR*)aPEbx, (TADDR*)aPEbp, (TADDR)aEsp, (TADDR*)aPRetAddr ); }
 
    void Init(TADDR* aPEdi, TADDR* aPEsi, TADDR* aPEbx, TADDR* aPEbp, TADDR aEsp, TADDR* aPRetAdd);
    ...
    bool   isValid()    { LEAF_CONTRACT; _ASSERTE(PTR_TO_TADDR(_pRetAddr) != INVALID_POINTER_CC); return(_pRetAddr != 0); }
    TADDR* pEdi()       { LEAF_CONTRACT; _ASSERTE(PTR_TO_TADDR(_pEdi) != INVALID_POINTER_CC); return(_pEdi); }
    TADDR* pEsi()       { LEAF_CONTRACT; _ASSERTE(PTR_TO_TADDR(_pEsi) != INVALID_POINTER_CC); return(_pEsi); }
    TADDR* pEbx()       { LEAF_CONTRACT; _ASSERTE(PTR_TO_TADDR(_pEbx) != INVALID_POINTER_CC); return(_pEbx); }
    TADDR* pEbp()       { LEAF_CONTRACT; _ASSERTE(PTR_TO_TADDR(_pEbp) != INVALID_POINTER_CC); return(_pEbp); }
    TADDR  esp()        { LEAF_CONTRACT; _ASSERTE(isValid()); return(_esp); }
