...
public:
    ...
    static void LockHolderEnter(ListLockEntry *pThis)
    {
        WRAPPER_CONTRACT;
        pThis->Enter();
    }

    static void LockHolderLeave(ListLockEntry *pThis)
    {
        WRAPPER_CONTRACT;
        pThis->Leave();
    }

    void FinishDeadlockAwareEnter()
    {
        DeadlockAwareLock::BlockingLockHolder dlLock;
        m_Crst.Enter();
        m_deadlock.EndEnterLock();
...
template void DoNothing(ListLock*);
template BOOL CompareDefault(ListLock*,ListLock*);
typedef Wrapper<ListLock*, DoNothing, DoNothing> HACKListLockHolder;
#endif
...
class ListLock
{
 ...
 public:
    ...
    void Init(LPCSTR szTag, CrstLevel crstlevel, CrstFlags flags, BOOL fHostBreakable = FALSE)
    {
	 WRAPPER_CONTRACT;
        PreInit();
        m_Crst.Init(szTag, crstlevel, flags);
        m_fInited = TRUE;
    ...
    // Must own the lock before calling this!
    void Unlink(ListLockEntry *pItem)
    {
		LEAF_CONTRACT;
        _ASSERTE(m_Crst.OwnedByCurrentThread());
    ...
    static void HolderEnter(ListLock *pThis)
    {
        WRAPPER_CONTRACT;
        pThis->Enter();
    }

    static void HolderLeave(ListLock *pThis)
    {
        WRAPPER_CONTRACT;
        pThis->Leave();
    }
...
#if defined (_MSC_VER) && _MSC_VER <= 1300
template void DoNothing(ListLockEntry*);
template void DoTheRelease(ListLockEntry*);
#endif
