...
IsSerializerRelatedInterface(MethodTable *pItfMT)
{
    ...
    if (g_Mscorlib.IsClass(pItfMT, CLASS__ISERIALIZABLE))
        return TRUE;
    if (g_Mscorlib.IsClass(pItfMT, CLASS__IOBJECTREFERENCE))
        return TRUE;
    if (g_Mscorlib.IsClass(pItfMT, CLASS__IDESERIALIZATIONCB))
        return TRUE;
...
TypeHandle LoadExactFieldType(FieldDesc *pFD, OBJECTREF orefParent, AppDomain *pDomain)
{
    ...
    if (pEnclosingMT->GetDomain() == GetAppDomain() ||
        (pEnclosingMT->IsDomainNeutral() && pEnclosingMT->GetAssembly()->FindDomainAssembly(GetAppDomain())))
...
#endif // !_X86_
MethodDesc   *ObjectClone::s_pMDPrepareData = NULL;
MethodDesc   *ObjectClone::s_pMDGetObjData = NULL;
BYTE    *ObjectClone::s_pAddrOfGetObjData = NULL;
BYTE    *ObjectClone::s_pAddrOfPrepareData = NULL;
MethodTable *ObjectClone::s_pMTObjRef = NULL;

extern TypeHandle GetTypeByName(LPUTF8 szFullClassName,
                                BOOL bThrowOnError, 
                                BOOL bIgnoreCase, 
...
#define CUSTOM_GCPROTECT_BEGIN(context)           do {              \
                FrameWithCookie<CustomGCFrame> __gcframe(GCScanRootsInCollection,    \
                context);                                           \
...
int GCSafeObjectHashTable::FindElement(OBJECTREF refObj, BOOL &seenBefore)
{
    ...
    int hashcode = 0;
    GCPROTECT_BEGIN(refObj);
    hashcode = ObjectNative::GetHashCodeEx(OBJECTREFToObject(refObj));
    GCPROTECT_END();
    ...
    do
    {
        int index = (hashcode % currentNumBuckets) * NUM_SLOTS_PER_BUCKET;
        _ASSERTE(index >= 0 && index < (int)m_currArraySize);
...
OBJECTREF ObjectClone::Clone(OBJECTREF refObj, TypeHandle expectedType, AppDomain* fromDomain, AppDomain* toDomain, OBJECTREF refExecutionContext)
{
    ...
    LOG((LF_REMOTING, LL_INFO100, "Clone. Cloning instance of type %s.\n", 
        GetFullyQualifiedNameForClassNestedAware(m_currObject->GetMethodTable()->GetClass())));
#endif
    ...
    while ((m_currObject = QOM.Dequeue(&dummy1, &dummy2, &currObjFixupInfo)) != NULL)
    {
        ...
        if (currID != -1)
        {
            ...
            LOG((LF_REMOTING, LL_INFO1000, "Clone. Object of type %s with id %d seen before.\n",
                GetFullyQualifiedNameForClassNestedAware(m_currObject->GetMethodTable()->GetClass()), currID));
#endif
        }
        else
        {
...
PTRARRAYREF ObjectClone::MakeObjectLookLikeISerializable(int objectId)
{
    ...
    while (pCurrMT)
    {
        ...
        for (DWORD i = 0; i < numInstanceFields; i++)
        {
            ...
            STRINGREF refName = COMString::NewString(szFieldName);
            ...
            switch (typ)
            {
                ...
                {
                    MethodTable *pFldMT = g_Mscorlib.FetchElementType(typ);
                    void *pData = m_currObject->GetData() + offset;
                    OBJECTREF refBoxed = pFldMT->Box(pData, FALSE);
...
PTRARRAYREF ObjectClone::AllocateISerializable(int objectId, BOOL bIsRemotingObject)
{
    ...
    const BYTE *pCode = s_pMDGetObjData->GetCallTarget(NULL);
    _ASSERTE(pCode);

#if !defined(_X86_) && !defined(_WIN64)
    PORTABILITY_ASSERT("ObjectClone::AllocateISerializable() - NYI on this platform");
#endif
    ...
    args[0] = (LPVOID)&gc.typeName;                     // arg 1
    args[1] = (LPVOID)OBJECTREFToObject(m_currObject);  // arg 0
    args[2] = (LPVOID)&gc.fieldValues;                  // arg 4 
    args[3] = (LPVOID)&gc.fieldNames;                   // arg 3
    args[4] = (LPVOID)&gc.assemblyName;                 // arg 2

    pRegArgs   = (SIZE_T *)(&args[0]);
    pStackArgs = (SIZE_T *)(&args[2]);
    numStackSlots = 3;
#else  // !CALLDESCR_ARGREGS
    args[0] = (LPVOID)OBJECTREFToObject(m_currObject);  // arg 0
    args[1] = (LPVOID)&gc.typeName;                     // arg 1
    args[2] = (LPVOID)&gc.assemblyName;                 // arg 2
    args[3] = (LPVOID)&gc.fieldNames;                   // arg 3
    args[4] = (LPVOID)&gc.fieldValues;                  // arg 4 

    pStackArgs = (SIZE_T *)(&args[0]);
    numStackSlots = 5;
#endif // !CALLDESCR_ARGREGS

    ARG_SLOT retVal = DispatchCall(
            pStackArgs,
            numStackSlots,
            pRegArgs,
            0,                   // uRegTypeMap
    ...
    if (gc.refException != NULL)
    {
        COMPlusThrow(gc.refException);
    }
    gc.refObjRef = ArgSlotToObj(retVal);
    ...
    else
    {
        ...
        if (!th.IsUnsharedMT() || th.ContainsGenericVariables())
        {
        ...
        {
            DefineFullyQualifiedNameForClass();
            LPCUTF8 __szTypeName = GetFullyQualifiedNameForClassNestedAware(pSrvMT->GetClass());
...
void ObjectClone::AllocateArray()
{
    ...
    if (CorTypeInfo::IsPrimitiveType(elemType) || elemType == ELEMENT_TYPE_PTR)
    {
        ...
        // Copy contents. 
        DWORD numBytesToCopy = refArray->GetComponentSize() * numComponents;
        ...
        m_memmove(pDest, pSrc, numBytesToCopy);
        m_skipFieldScan = TRUE;
    }
...
void ObjectClone::AllocateObject()
{
    ...
    MethodTable *pCurrMT = m_currObject->GetMethodTable();
    _ASSERTE(!pCurrMT->IsArray());
    _ASSERTE(!pCurrMT->IsMarshaledByRef() && !pCurrMT->IsTransparentProxyType());
...
TypeHandle ObjectClone::GetCorrespondingTypeForTargetDomain(TypeHandle thCli)
{
    ...
    else
    {
        ...
        _ASSERTE(thBaseType.IsUnsharedMT());
...
MethodTable * ObjectClone::GetCorrespondingTypeForTargetDomain(MethodTable *pCliMT)
{
    ...
    if (pCliMT->IsArray())
        pCliMT->GetClass()->_GetFullyQualifiedNameForClass(__ssTypeName);
    else
        pCliMT->GetClass()->_GetFullyQualifiedNameForClassNestedAware(__ssTypeName);
#endif
    ...
    if (pCliMT->IsArray())
    {
        pCliMT->GetClass()->_GetFullyQualifiedNameForClass(ssCliTypeName);
    }
    ...
    {
        pCliMT->GetClass()->_GetFullyQualifiedNameForClassNestedAware(ssCliTypeName);
    }
...
TypeHandle ObjectClone::GetType(const SString &ssTypeName, const SString &ssAssemName)
{
    ...
    EX_CATCH
    {
        ...
        DomainAssembly *pDomainAssembly = LoadAssemblyFromPartialNameHack((SString*)&ssAssemName, TRUE);
...
void ObjectClone::ScanMemberFields(DWORD IObjRefTSOIndex, DWORD BoxedValTSOIndex)
{
    ...
    _ASSERTE(!pMT->IsMarshaledByRef() && !pMT->IsTransparentProxyType());
...
DWORD ObjectClone::CloneField(FieldDesc *pSrcField, FieldDesc *pDstField)
{
    ...
    if (srcType != dstType)
    {
        ...
        if (CorTypeInfo::IsPrimitiveType(srcType))
            pSrcFieldMT = g_Mscorlib.FetchElementType(srcType);
        else
    ...
    case ELEMENT_TYPE_VALUETYPE:
        {
            ...
            else
            {
                ...
                BYTE *pByte = m_currObject->GetData() + srcOffset;
                BYTE *pDest = m_newObject->GetData() + dstOffset; 
                m_memmove(pDest, pByte, numBytesToCopy);
...
FieldDesc **CrossDomainFieldMap::LookupOrCreateFieldMapping(MethodTable *pDstMT, MethodTable *pSrcMT)
{
    ...
    {
        void *pLockSpace = SystemDomain::System()->GetLowFrequencyHeap()->AllocMem(sizeof(SimpleRWLock));
        SimpleRWLock *pLock = new (pLockSpace) SimpleRWLock(COOPERATIVE_OR_PREEMPTIVE, LOCK_TYPE_DEFAULT);
        
        if (FastInterlockCompareExchangePointer((void **)&s_pFieldMapLock, pLock, NULL) != NULL)
            // We lost the race, give up our copy.
            SystemDomain::System()->GetLowFrequencyHeap()->BackoutMem(pLockSpace, sizeof(SimpleRWLock));
    }
    ...
    if (s_pFieldMap == NULL)
    {
        ...
        if (s_pFieldMap == NULL)
        {
            PtrHashMap *pMap = new (SystemDomain::System()->GetLowFrequencyHeap()) PtrHashMap();
...
Object *ObjectClone::GetObjectFromArray(BASEARRAYREF* arrObj, DWORD dwOffset)
{
    CONTRACTL {
        THROWS;
        if ((*arrObj)->GetArrayElementTypeHandle().GetMethodTable()->IsValueClass()) GC_TRIGGERS; else GC_NOTRIGGER;
    } CONTRACTL_END;
...
void ObjectClone::CompleteValueTypeFields(OBJECTREF newObj, OBJECTREF refParent, QueuedObjectInfo *objInfo)
{
...
#ifdef _DEBUG
    {
        ...
        newObj->GetMethodTable()->GetClass()->_GetFullyQualifiedNameForClassNestedAware(ssTypeName);
        refParent->GetMethodTable()->GetClass()->_GetFullyQualifiedNameForClassNestedAware(ssParentTypeName);
...
void ObjectClone::EnsureMDInitialized()
{
    CONTRACTL
    {
        GC_TRIGGERS;
    ...
    if (s_pMDPrepareData == NULL)
    {
        s_pMDPrepareData = g_Mscorlib.GetMethod(METHOD__OBJECTCLONEHELPER__PREPARE_DATA);
        _ASSERTE(s_pMDPrepareData);
    }


    if (s_pMDGetObjData == NULL)
    {
        s_pMDGetObjData = g_Mscorlib.GetMethod(METHOD__OBJECTCLONEHELPER__GET_OBJECT_DATA);
        _ASSERTE(s_pMDGetObjData);
    }
}
...
void ObjectClone::CompleteSpecialObjects()
{
    ...
    {
        // we only need this for cross appdomain cloning, not for freezing
        EnsureMDInitialized();
    }
...
void ObjectClone::CompleteISerializableObject(OBJECTREF IserObj, OBJECTREF refNames, OBJECTREF refValues, ISerializableInstanceInfo *iserInfo)
{
    ...
    GCPROTECT_BEGIN(gc);
    GCPROTECT_BEGIN(IserObj);

#ifdef _DEBUG
    {
        DefineFullyQualifiedNameForClass();
        LOG((LF_REMOTING, LL_INFO1000, "CompleteISerializableObject. Completing ISerializable object of type %s.\n",
            GetFullyQualifiedNameForClassNestedAware(IserObj->GetMethodTable()->GetClass())));
    }
#endif

    BOOL    bIsBoxed = IserObj->GetMethodTable()->IsValueClass();

    // StreamingContext is passed by value in one arg slot on x86 and by reference on WIN64
#if !defined(_X86_) && !defined(_WIN64)
    PORTABILITY_ASSERT("ObjectClone::CompleteISerializableObject() and StreamingContext - NYI on this platform");
#endif
    ...
    const BYTE *pCode = s_pMDPrepareData->GetCallTarget(NULL);
    ...
    DWORD   numStackSlots = 0;
#ifdef CALLDESCR_ARGREGS
    args[0] = (LPVOID)OBJECTREFToObject(refNames);             // arg 1
    args[1] = (LPVOID)OBJECTREFToObject(IserObj);              // arg 0
    args[2] = (LPVOID)(&context);                              // arg 3
    args[3] = (LPVOID)OBJECTREFToObject(refValues);            // arg 2

    pRegArgs   = (SIZE_T*)(&args[0]);
    pStackArgs = (SIZE_T*)(&args[2]);
    numStackSlots = 3;
#else  // !CALLDESCR_ARGREGS
    args[0] = (LPVOID)OBJECTREFToObject(IserObj);              // arg 0
    args[1] = (LPVOID)OBJECTREFToObject(refNames);             // arg 1
    args[2] = (LPVOID)OBJECTREFToObject(refValues);            // arg 2
    args[3] = (LPVOID)(&context);                              // arg 3

    pStackArgs = (SIZE_T*)(&args[0]);
    numStackSlots = 5;
#endif // !CALLDESCR_ARGREGS

    ARG_SLOT ret = DispatchCall(
            pStackArgs,
            numStackSlots,
            pRegArgs,
            0,                    // uRegTypeMap
    ...
    gc.refSerInfo = ArgSlotToObj(ret);
    ...
    {
        _ASSERTE(gc.refSerInfo != NULL);
        MethodTable *pMT = IserObj->GetMethodTable();
        _ASSERTE(pMT);

        MethodDesc * pCtor;

            pCtor = pMT->GetClass()->FindConstructor(&gsig_IM_SerInfo_StrContext_RetVoid);
        
        if (pCtor == NULL)
        {
            DefineFullyQualifiedNameForClassW();
            COMPlusThrow(kSerializationException, IDS_SERIALIZATION_CTOR_NOT_FOUND,
                         GetFullyQualifiedNameForClassNestedAwareW(pMT->GetClass()));
        }
        ...
        _ASSERTE(!Nullable::IsNullableType(IserObj->GetMethodTable()));
        argSlots[0] = (bIsBoxed ? (ARG_SLOT)(SIZE_T)(IserObj->UnBox()) : ObjToArgSlot(IserObj));
        argSlots[1] = ObjToArgSlot(gc.refSerInfo);
#if defined(_X86_)
        C_ASSERT(sizeof(context) == sizeof(ARG_SLOT));
        argSlots[2] = *(ARG_SLOT*)(&context);           // StreamingContext is passed by value on x86
#else  // !_X86_ && !_WIN64
...
BOOL ObjectClone::CompleteIObjRefObject(OBJECTREF IObjRef, DWORD tsoIndex, IObjRefInstanceInfo *iorInfo)
{
    ...
    if (iorInfo->IsRepeatObject())
    {
        ...
        OBJECTREF dummy;
        dummy = TSO.GetAt(tsoIndex, &refFromObj, &refParent, (QueuedObjectInfo **)&iorInfo);
        PREFIX_ASSUME(refFromObj != NULL);

        // Look in the Table of Seen objects whether this IObjRef has been resolved
        OBJECTREF resolvedObject = NULL;
        int currId;
        currId = TOS.HasID(refFromObj, &resolvedObject);
        _ASSERTE(currId != -1);

        MethodTable *pResolvedMT = resolvedObject->GetMethodTable();
        if (!pResolvedMT->IsTransparentProxyType() && 
...
#ifdef _DEBUG
        {
            ...
            LOG((LF_REMOTING, LL_INFO1000, "CompleteIObjRefObject. Found IObjectReference object of type %s already resolved.\n",
                GetFullyQualifiedNameForClassNestedAware(IObjRef->GetMethodTable()->GetClass())));
        }
#endif
        ...
        {
            TSO.SetAt(tsoIndex, resolvedObject, refFromObj, refParent, iorInfo);
            iorInfo->SetHasBeenProcessed();
            return TRUE;
        }

        if (refParent == NULL)
            m_topObject = resolvedObject;
        else
        {
            m_newObject = resolvedObject;
            if (pFixupInfo->NeedsUnboxing())
                CompleteValueTypeFields(resolvedObject, refParent, pFixupInfo);
            else
                Fixup(resolvedObject, refParent, pFixupInfo);
        }
        iorInfo->SetHasBeenProcessed();
        return TRUE;
    }
    ...
    BOOL bResult = FALSE;
    MethodTable *pMT = IObjRef->GetMethodTable();
    _ASSERTE(pMT);

    GCPROTECT_BEGIN(IObjRef);

    MethodTable *pItf = MethodTable::GetIObjectReferenceMT();
    MethodDesc *pMeth = GetInterfaceMethodImpl(pMT, pItf, 0);
    MethodDescCallSite method(pMeth, &IObjRef);
    
    // Ensure Streamingcontext type is loaded. Do not delete this line
    MethodTable *pMTStreamingContext;
    pMTStreamingContext = g_Mscorlib.GetClass(CLASS__STREAMING_CONTEXT);
    _ASSERTE(pMTStreamingContext);

    ARG_SLOT arg[2];
    arg[0] = ObjToArgSlot(IObjRef);

    RuntimeMethodHandle::StreamingContextData context = { NULL, GetStreamingContextState() };
    C_ASSERT(sizeof(context) <= sizeof(ARG_SLOT));
    arg[1] = *(ARG_SLOT*)(&context);

    OBJECTREF newObj = method.CallWithValueTypes_RetOBJECTREF(&arg[0]);

    INDEBUG(DefineFullyQualifiedNameForClass();)

    _ASSERTE(newObj != NULL);
    MethodTable *pNewMT = newObj->GetMethodTable();
    if (!pNewMT->IsTransparentProxyType() && 
        newObj != IObjRef &&
        m_cbInterface->IsIObjectReferenceType(pNewMT))
    {
        ...
        OBJECTREF dummy;
        dummy = TSO.GetAt(tsoIndex, &refFromObj, &refParent, (QueuedObjectInfo **)&iorInfo);
        TSO.SetAt(tsoIndex, newObj, refFromObj, refParent, iorInfo);
        bResult = FALSE;
    }
    else
    {
...
void MakeIDeserializationCallback(OBJECTREF refTarget)
{
    ...
    MethodTable *pItf = MethodTable::GetIDeserializationCBMT();
    MethodDesc *pMeth = GetInterfaceMethodImpl(pMT, pItf, 0);
    SLOT pCode = (SLOT) pMeth->GetSafeAddrofCode();
    ...
    arg[0] = NULL;                                      // arg 1
    arg[1] = (LPVOID)OBJECTREFToObject(gc.refTarget);   // arg 0

    pRegArgs   = (SIZE_T*)(&arg[0]);
    pStackArgs = NULL;
#else  // !CALLDESCR_ARGREGS
    arg[0] = (LPVOID)OBJECTREFToObject(gc.refTarget);   // arg 0
    arg[1] = NULL;                                      // arg 1

    pStackArgs = (SIZE_T*)(&arg[0]);
    numStackSlots = 2;
#endif // !CALLDESCR_ARGREGS
...
OBJECTREF ObjectClone::BoxValueTypeInWrongDomain(OBJECTREF refParent, DWORD offset, MethodTable *pValueTypeMT)
{
    CONTRACTL
    {
        ...
        GC_TRIGGERS;
        MODE_COOPERATIVE;
        PRECONDITION(pValueTypeMT->IsValueClass());
    }
    CONTRACTL_END;
    ...
    // so we want to avoid it if possible.  IsScopedByModule(mscorlib) && CanBeBlittedByObjectCloner is a
    // conservative first approximation of agile types.
    ENTER_DOMAIN_PTR_PREDICATED(m_fromDomain, ADV_RUNNINGIN,
                            pValueTypeMT->GetClass()->CannotBeBlittedByObjectCloner()
                            || !TypeHandle(pValueTypeMT).IsScopedByModule(g_Mscorlib.GetModule()));
...
BOOL ObjectClone::HasVtsCallbacks(MethodTable *pMT, RemotingVtsInfo::VtsCallbackType eCallbackType)
{
    ...
    while (pMT)
    {
        if (pMT->HasRemotingVtsInfo())
        {
            ...
            if (pVtsInfo->m_pCallbacks[eCallbackType] != NULL)
                return TRUE;
        }
...
void ObjectClone::InvokeVtsCallbacks(OBJECTREF refTarget, RemotingVtsInfo::VtsCallbackType eCallbackType, AppDomain* pDomain)
{
    ...
    while (pMT)
    {
        if (pMT->HasRemotingVtsInfo())
        {
            ...
            if (pVtsInfo->m_pCallbacks[eCallbackType] != NULL)
            {
                ...
                pLastCallback = pVtsInfo->m_pCallbacks[eCallbackType];
            }
        }
        pMT = pMT->GetParentMethodTable();
    }
    ...
    if (cMethods > 1)
    {
        ...
        while (pMT)
        {
            if (pMT->HasRemotingVtsInfo())
            {
                ...
                if (pVtsInfo->m_pCallbacks[eCallbackType] != NULL)
                    pCallbacks[--dwSlotIndex] = pVtsInfo->m_pCallbacks[eCallbackType];
            }
            pMT = pMT->GetParentMethodTable();
        }
    ...
    BOOL bIsBoxed = refTarget->GetMethodTable()->IsValueClass();
...
#if defined(_X86_)
        C_ASSERT(sizeof(sContext) == sizeof(ARG_SLOT));
        argSlots[1] = *(ARG_SLOT*)(&sContext);           // StreamingContext is passed by value on x86
#else  // !_X86_ && !_WIN64
