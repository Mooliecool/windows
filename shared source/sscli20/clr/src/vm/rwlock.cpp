...
void CRWLock::ProcessInit()
{
    ...
    s_RWLockCrst.Init("RWLock", CrstDummy, CRST_UNSAFE_ANYMODE);
}
...
FCIMPL2(void, CRWLock::StaticAcquireReaderLockPublic, CRWLock *pRWLockUNSAFE, DWORD dwDesiredTimeout)
{
    CONTRACTL
    {
        THROWS;               // Can throw NullReferenceException
        DISABLED(GC_NOTRIGGER);
        SO_TOLERANT;
    }
    CONTRACTL_END;
    ...
    DWORD dwDesiredTimeout)
{
    ...
    else if(pLockEntry->wReaderLevel != 0)
    {
        ...
        if (pLockEntry->wReaderLevel == RWLOCK_MAX_ACQUIRE_COUNT) {
            COMPlusThrow(kOverflowException, L"Overflow_UInt16");        
        }
    ...
    LockCookie *pLockCookie)
{
    ...
    if(pLockCookie->dwThreadID == dwThreadID)
    {
        ...
        {
            COMPlusThrow(kSynchronizationLockException, L"Arg_RWLockRestoreException");
        }
...
FCIMPL1(void, CRWLock::StaticPrivateDestruct, CRWLock *pRWLock)
{
    CONTRACTL
    {
        DISABLED(NOTHROW);   // This function does not throw, but HELPER_METHOD macro calls some function that may throw. 
        DISABLED(GC_NOTRIGGER);
    }
    CONTRACTL_END;

	ThreadPreventAbortHolder prevent(TRUE);
    HELPER_METHOD_FRAME_BEGIN_1(pRWLock);
