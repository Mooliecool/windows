...
/*
    Thread safety in GlobalStringLiteralMap / AppDomainStringLiteralMap

    A single lock protects the N AppDomainStringLiteralMap objects and single
    ...
    At one time each AppDomainStringLiteralMap had it's own lock to protect
    ...
    Any insert/deletes from the AppDomainStringLiteralMap or GlobalStringLiteralMap
    need to be done under the lock.

    The only thing you can do without the lock is look up an existing StringLiteralEntry
    in an AppDomainStringLiteralMap hash table. This is true because these lookup calls
...
, m_pDomain(pDomain)
{
    ...
    MEMORY_REPORT_CONTEXT_SCOPE("StringLiteral");

    // Allocate the memory pool and set the initial count to quarter as grow count
    NewHolder<MemoryPool> pMemoryPool(new MemoryPool (SIZEOF_EEHASH_ENTRY, EEHASH_MEMORY_POOL_GROW_COUNT, EEHASH_MEMORY_POOL_GROW_COUNT/4));

    m_StringToEntryHashTable =  new EEUnicodeStringLiteralHashTable ();
    pMemoryPool.SuppressRelease();
    m_MemoryPool=pMemoryPool;

}

void AppDomainStringLiteralMap::Init()
{
    CONTRACTL
    {
        THROWS;
...
AppDomainStringLiteralMap::~AppDomainStringLiteralMap()
{
    ...
    CrstPreempHolder gch(&(SystemDomain::GetGlobalStringLiteralMapNoCreate()->m_HashTableCrstGlobal));
...
: m_StringToEntryHashTable(NULL)
, m_MemoryPool(NULL)
, m_HashTableCrstGlobal("GlobalStringLiteralMap", CrstGlobalStrLiteralMap)
, m_LargeHeapHandleTable(SystemDomain::System(), GLOBAL_STRING_TABLE_BUCKET_SIZE)
{
    ...
    // Allocate the memory pool and set the initial count to quarter as grow count
    NewHolder<MemoryPool> MemoryPool( new MemoryPool (SIZEOF_EEHASH_ENTRY, EEHASH_MEMORY_POOL_GROW_COUNT, EEHASH_MEMORY_POOL_GROW_COUNT/4));
    m_StringToEntryHashTable =  new EEUnicodeStringLiteralHashTable ();
    MemoryPool.SuppressRelease();
...
#ifdef LOGGING
static void LogStringLiteral(__in char* action, EEStringData *pStringData)
{
...
STRINGREF AllocateStringObject(EEStringData *pStringData)
{
    ...
    // Prefix needs a hint to be sure that cCount won't overflow
    PREFIX_ASSUME(cCount < cCount + 1);
    
    STRINGREF strObj = AllocateString(cCount + 1);
