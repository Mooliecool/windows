...
quit:
        ...
        //    bytesRead == -1 when  we got a connection close plus when errorStatus == sucess we got a graceful close.
        ...
        private void DebugDumpArrayListEntries(ArrayList list,string listType) {
            for (int i = 0; i < list.Count; i++) {
                GlobalLog.Print(listType+"[" + i.ToString() + "] Req: #" + ValidationHelper.HashString(list[i] as HttpWebRequest));
            }
        }
