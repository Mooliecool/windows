...
namespace System.Net {
    ...
    public class ServicePoint {
        ...
        internal Socket GetConnection(PooledStream PooledStream, object owner, bool async, out IPAddress address, ref Socket abortSocket, ref Socket abortSocket6, int timeout)
        {
            ...
            ws = ConnectSocket(socket,socket6,ref finalSocket, ref address, state, timeout, out innerException);
        ...
        */
        ...
        internal bool ReleaseConnectionGroup(string connName) {
            ...
            lock(this) {
                ...
                ConnectionGroup connectionGroup = FindConnectionGroup(connName, true);
                //
        ...
        private static void ConnectSocketCallback(IAsyncResult asyncResult) {
            ...
            try {
                ws = state.servicePoint.ConnectSocketInternal(state.connectFailure, state.s4, state.s6, ref socket, ref address, state, asyncResult, Timeout.Infinite, out innerException);
            }
        ...
        private WebExceptionStatus ConnectSocketInternal(bool connectFailure, Socket s4, Socket s6, ref Socket socket, ref IPAddress address, ConnectSocketState state, IAsyncResult asyncResult, int timeout, out Exception exception) {
            ...
            for (int unsuccessfulAttempts = 0; unsuccessfulAttempts < 2; unsuccessfulAttempts++) {
                ...
                if (asyncResult == null)
                {
                    ...
                    GlobalLog.Print("ServicePoint#" + ValidationHelper.HashString(this) + "::ConnectSocketInternal() calling GetIPAddressInfoList() timeout:" + timeout.ToString());

                    addresses = GetIPAddressInfoList(out currentIndex, addresses, timeout, out timedOut);
            ...
            return connectFailure ? WebExceptionStatus.ConnectFailure :
                timedOut ? WebExceptionStatus.Timeout :
        ...
        /*
        //                           
        private WebExceptionStatus ConnectSocket(Socket s4, Socket s6, ref Socket socket, int timeout, out Exception exception) {
            IPAddress address = null;
            return ConnectSocket(s4, s6, ref socket, ref address, null, timeout, out exception);
        }
        */
        ...
        private WebExceptionStatus ConnectSocket(Socket s4, Socket s6, ref Socket socket, ref IPAddress address, ConnectSocketState state, int timeout, out Exception exception) {
            ...
            //
            return ConnectSocketInternal(false, s4, s6, ref socket, ref address, state, null, timeout, out exception);
        }
        ...
        private IPAddress[] GetIPAddressInfoList(out int currentIndex, IPAddress[] addresses, int timeout, out bool timedOut)
        {
            ...
            if (needDnsResolution) {
                try {
                    GlobalLog.Print("IPHostInformation#" + ValidationHelper.HashString(this) + "::GetIPAddressInfoList() calling Dns.InternalResolveFast() for:" + m_Host + " timeout:" + (uint) timeout);
                    ipHostEntry = Dns.InternalResolveFast(m_Host, timeout, out timedOut);
                    GlobalLog.Print("IPHostInformation#" + ValidationHelper.HashString(this) + "::GetIPAddressInfoList() Dns.InternalResolveFast() returns:" + ValidationHelper.ToString(ipHostEntry) + " timedOut:" + timedOut);
                    if (timedOut)
                    {
