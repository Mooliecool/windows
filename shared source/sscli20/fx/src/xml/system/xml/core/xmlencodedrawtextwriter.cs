...
//
        ...
        public XmlEncodedRawTextWriter( TextWriter writer, XmlWriterSettings settings ) : this( settings, settings.CloseOutput ) {
            ...
            this.bufChars = new char[BUFSIZE + OVERFLOW];
        ...
        public XmlEncodedRawTextWriter( Stream stream, Encoding encoding, XmlWriterSettings settings, bool closeOutput ) : this( settings, closeOutput ) {
            ...
            // the buffer is allocated will OVERFLOW in order to reduce checks when writing out constant size markup
            bufChars = new char[ BUFSIZE + OVERFLOW ];
                ...
                for (;;) {
                    ...
                    switch ( ch ) {
                        ...
                        case (char)0xA:
                            ...
                            break;
                        default:
                           if ( InRange( ch, SurHighStart, SurLowEnd ) ) { pDst = EncodeSurrogate( pSrc, pSrcEnd, pDst ); pSrc += 2; } else if ( ch <= 0x7F || ch >= 0xFFFE ) { pDst = InvalidXmlChar( ch, pDst, true ); pSrc++; } else { *pDst = (char)ch; pDst++; pSrc++; };
                            continue;
                    }
                    pSrc++;
                }
        ...
        protected unsafe void RawText( char * pSrcBegin, char * pSrcEnd ) {
            fixed ( char * pDstBegin = bufChars ) {
                ...
                for (;;) {
                    ...
                    while ( pDst < pDstEnd && ( ( ch = *pSrc ) < SurHighStart ) ) {
                    ...
                    if ( InRange( ch, SurHighStart, SurLowEnd ) ) { pDst = EncodeSurrogate( pSrc, pSrcEnd, pDst ); pSrc += 2; } else if ( ch <= 0x7F || ch >= 0xFFFE ) { pDst = InvalidXmlChar( ch, pDst, false ); pSrc++; } else { *pDst = (char)ch; pDst++; pSrc++; };
                }
