...
//Constructor
    ...
    // Attribute Accessors
       ...
       public override string GetAttribute( string name, string namespaceURI ) {
        ...
        internal ValidatingReaderNodeData RecordTextNode(string textValue, int depth, int lineNo, int linePos) {
            ValidatingReaderNodeData textNode = AddContent(XmlNodeType.Text);
            textNode.SetItemData(textValue);
            textNode.SetLineInfo(lineNo, linePos);
            textNode.Depth = depth;
            return textNode;
        }

        internal void SwitchTextNodeAndEndElement(string textValue) {
            ...
            ValidatingReaderNodeData textNode = RecordTextNode(textValue, coreReader.Depth + 1, 0, 0);
