    ...
    internal class Datatype_List : Datatype_anySimpleType {
        ...
        internal override Exception TryParseValue(string s, XmlNameTable nameTable, IXmlNamespaceResolver nsmgr, out object typedValue) {
            ...
            if (itemType.Variety == XmlSchemaDatatypeVariety.Union) {
                ...
                foreach (string s1 in XmlConvert.SplitString(s)) {
                    //Parse items in list according to the itemType
                    exception = itemType.TryParseValue(s1, nameTable, nsmgr, out unionTypedValue);
                    if (exception != null) goto Error;
                ...
                foreach (string s1 in XmlConvert.SplitString(s)) {
                    exception = itemType.TryParseValue(s1, nameTable, nsmgr, out typedValue);
                    if (exception != null) goto Error;
    ...
    internal class Datatype_union : Datatype_anySimpleType {
        ...
        internal bool HasAtomicMembers() {
            foreach(XmlSchemaSimpleType st in types) {
                if (st.Datatype.Variety == XmlSchemaDatatypeVariety.List) {
                    return false;
                }
            }
            return true;
        ...
        internal override Exception TryParseValue(string s, XmlNameTable nameTable, IXmlNamespaceResolver nsmgr, out object typedValue) {
            ...
            foreach(XmlSchemaSimpleType st in types) {
                exception = st.Datatype.TryParseValue(s, nameTable, nsmgr, out typedValue);
                if (exception == null) {
                    memberType = st;
                    break;
                }
        ...
        internal override Exception TryParseValue(object value, XmlNameTable nameTable, IXmlNamespaceResolver nsmgr, out object typedValue) {
            ...
            XmlSchemaSimpleType memberType = null;
            foreach(XmlSchemaSimpleType st in types) {
                if (st.Datatype.TryParseValue(value, nameTable, nsmgr, out valueToCheck) == null) { //no error
                    memberType = st;
                    break;
                }
            }
        ...
        internal override int Compare(object value1, object value2) {
         ...
         internal override Exception TryParseValue(string s, XmlNameTable nameTable, IXmlNamespaceResolver nsmgr, out object typedValue) {
            ...
            if (!XsdDateTime.TryParse(s, dateTimeFlags, out dateTime)) {
                exception = new FormatException(Res.GetString(Res.XmlConvert_BadFormat, s, "XsdDateTime"));
                goto Error;
            }
