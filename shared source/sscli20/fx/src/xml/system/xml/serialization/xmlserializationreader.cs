...
namespace System.Xml.Serialization {
    ...
    internal class XmlSerializationReaderCodeGen : XmlSerializationCodeGen {
            ...
            for (int i = 0; i < members.Length; i++) {
                ...
                if (member.MultiRef) {
                    ...
                    if (td.IsCollection || td.IsEnumerable) {
                        ...
                        if (member.Mapping.CheckSpecified && member.CheckSpecifiedSource != null && member.CheckSpecifiedSource.Length > 0) { 
            ...
            for (int i = 0; i < members.Length; i++) {
                ...
                for (int j = 0; j < elements.Length; j++) {
                    ...
                    if (checkType) {
                        ...
                        if (isSequence && e.Any && e.Namespace == null) {
                            Writer.Write("true");
                        }
                        else {
            ...
            if (element.Mapping is ArrayMapping) {
                ...
                case TypeKind.Serializable:
                    ...
                    if (sm.DerivedMappings != null) {
                        Writer.Indent--;
                        Writer.WriteLine("}");
                        WriteDerivedSerializable(sm, sm, source);
        ...
        void WriteDerivedSerializable(SerializableMapping head, SerializableMapping mapping, string source) {
            if (mapping == null)
                return;
            for (SerializableMapping derived = mapping.DerivedMappings; derived != null; derived = derived.NextDerivedMapping) {
                ...
                WriteDerivedSerializable(head, derived, source);
            }
        }
