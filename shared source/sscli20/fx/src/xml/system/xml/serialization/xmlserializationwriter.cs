...
namespace System.Xml.Serialization {
        ...
        void WriteElement(string source, ElementAccessor element, string arrayName, bool writeAccessor) {
            ...
            if (element.Mapping is NullableMapping) {
                ...
                if (element.Mapping is SerializableMapping) {
                    WriteElementCall("WriteSerializable", typeof(IXmlSerializable), source, name, ns, element.IsNullable, !IsAnyElement(mapping));
                }
                else {
                    // XmlNode, XmlElement
                    Writer.Write("if ((");
    ...
    internal class DynamicAssemblies {
        ...
        static Hashtable nameToAssemblyMap;
        static Hashtable assemblyToNameMap;
        static Hashtable tableIsTypeDynamic = new Hashtable();
        ...
        internal static bool IsTypeDynamic(Type type) {
            ...
            if (oIsTypeDynamic == null) {
                ...
                bool isTypeDynamic = (module is ModuleBuilder || assembly.Location == null || assembly.Location.Length == 0);
                if (!isTypeDynamic)
                {
                    if (type.IsArray)
                    {
