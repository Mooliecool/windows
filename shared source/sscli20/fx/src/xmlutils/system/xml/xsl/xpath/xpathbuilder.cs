        ...
        */
        ...
        QilNode NameOf(QilNode arg) {
            ...
            if (arg is QilIterator) {
                // We use arg in several places here and can do this only if arg is QilIterator.
                return f.Conditional(f.Eq(f.StrLength(f.PrefixOf(arg)), f.Int32(0)),
                    f.LocalNameOf(arg),
                    f.StrConcat(f.PrefixOf(arg), f.String(":"), f.LocalNameOf(arg))
                );
            } else {
        ...
        static XmlNodeKindFlags[] XPathAxisMask = {
            ...
            /*Root            */ XmlNodeKindFlags.Element | XmlNodeKindFlags.Document,
        };
