          ...
          const wchar_16 *ext)
{
    ...
    _makepath( Path, NULL, Dir, FileName, Ext );
          ...
          size_t maxLength)
{
    ...
    ENTRY("_fullpath (absPath=%p, relPath=%p (%s), maxLength = %d)\n",
          absPath, relPath ? relPath:"NULL", relPath ? relPath:"NULL", maxLength);
    
    strcpy(path_copy, relPath ? relPath : ".");
    FILEDosToUnixPathA(path_copy);

    if(NULL == realpath(path_copy, realpath_buf))
    {
        TRACE("realpath() failed; problem path is '%s'. errno is %d (%s)\n",
    ...
    else 
    {
        ...
        min_length = strlen(realpath_buf)+1;

        if(NULL == absPath)
        {
            ASSERT("Invalid absPath parameter value (NULL)\n");
        }
        else
        {
            if(min_length > maxLength)
            {
                TRACE("maxLength is %d, we need at least %d\n",
                      maxLength, min_length);
            }
            else
            {
                strcpy(absPath, realpath_buf);
                retval = absPath;
            }
        }
    }
