...
Using Debug channels at Run Time
    ...
    set PAL_DBG_CHANNELS="+PAL.TRACE:-FILE.ERROR"
    set PAL_DBG_CHANNELS="+all.ENTRY"
    set PAL_DBG_CHANNELS="-all.all"
...
#define W16_NULLSTRING ((LPCWSTR)"N\0U\0L\0L\0\0")


/* extern variables */

extern DWORD dbg_channel_flags[DCI_LAST];
extern BOOL dbg_asserts_enabled;
...
#define ASSERT(args...) \
    {if (output_file && dbg_master_switch) \
        DBG_printf_gcc(defdbgchan,DLI_ASSERT,TRUE,__FUNCTION__,__FILE__,\
                   __LINE__,args); \
    if(dbg_asserts_enabled){\
        DebugBreak();\
    }}
