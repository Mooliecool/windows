...
processes. The SHMPTR_TO_PTR macro can be used to convert a SHMPTR value inot
...
extern LPVOID shm_segment_bases[MAX_SEGMENTS];

#define SHMPTR_TO_PTR(shmptr)\
    ((LPVOID)((shmptr)?((((shmptr)>>24)<shm_numsegments)?\
    (shm_segment_bases[(shmptr)>>24]+((shmptr)&0x00FFFFFF)):\
    SHMPtrToPtr(shmptr)):NULL))
