            ...
            IN DWORD flProtection )     /* Protections flags on the memory. */
{
    ...
    pNewEntry->pProtectionState = (BYTE*)malloc( memSize / VIRTUAL_PAGE_SIZE );
#if MMAP_DOESNOT_ALLOW_REMAP
    pNewEntry->pDirtyPages  = (BYTE*)malloc( nBufferSize );
#endif // 
         ...
         IN UINT_PTR ucb)
{
    ...
    if( (UINT_PTR)lp + (UINT_PTR)ucb < (UINT_PTR)lp )
    {
    ...
    ptr = (LPSTR)((DWORD_PTR)lp & (~VIRTUAL_PAGE_MASK));
    ...
    PAL_TRY
    {
        ...
        while( ptr<=end )
        {
            ...
            /* Avoid wrap-around */
            if(((LPSTR)-1)-VIRTUAL_PAGE_SIZE < ptr)
            {
                break;
            }

            ptr += VIRTUAL_PAGE_SIZE;
        }
    }
