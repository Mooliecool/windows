                   ...
                   LPCSTR function, LPCSTR file, INT line, LPCSTR format, ...)
{
    ...
    /* We are calling SYNC(Enter/Leave)CriticalSectionCore 
	to avoid potential infinite recursion due to debug messages
	from the old SYNC(Enter/Leave)CriticalSection. 
	SYNC(Enter/Leave)CriticalSectionCore, with the third parameter
	set to TRUE, stops debug messages from being printed, thus
	avoiding the potential recursion. */
    SYNCEnterCriticalSectionCore(&fprintf_crit_section, TRUE, TRUE);
    fprintf( output_file,"%s%s",indent, buffer );
    SYNCLeaveCriticalSectionCore(&fprintf_crit_section, TRUE, TRUE);
