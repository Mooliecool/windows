...
so the locking sequence becomes SYNCEnterCriticalSection - take semaphore - try to
    lock - open FIFO - release semaphore - SYNCLeaveCriticalSection - read
...
typedef struct
{
    ...
    SHMPTR first_pool_blocks[SPS_LAST];
    SHMPTR last_pool_blocks[SPS_LAST];
} SHM_SEGMENT_HEADER;
...
typedef struct
{
    ...
    SHMPTR shm_info[SIID_LAST]; /* basic blocks of shared information.*/
}SHM_FIRST_HEADER;
...
LPVOID shm_segment_bases[MAX_SEGMENTS];
...
BOOL SHMInitialize(void)
{
    ...
    if (0 != SYNCInitializeCriticalSection(&shm_critsec))
    {
        ERROR("Couldn't initialize SHM critical section\n");
        return FALSE;
    }
...
#ifdef O_EXLOCK
    fd_lock = open(lockfile_name,O_RDWR|O_CREAT|O_EXLOCK, 0600);
#else   // O_EXLOCK
    fd_lock = open(lockfile_name,O_RDWR|O_CREAT, 0600);
#endif  // O_EXLOCK
    if(fd_lock == -1)
    {
        int retry_count = 0;
        ...
        if(-1 == fd_lock)
        {
            ERROR("open() failed; error is %d (%s)\n", errno, strerror(errno));
            return FALSE;
        }
    }
#ifndef O_EXLOCK
    ...
    if(n_pal_processes == 0)
    {
        ...
        fd_map = open(first_segment_name,O_RDWR|O_CREAT,0600);
        if(fd_map == -1)
        {
            ERROR("open() failed; error is %d (%s)\n", errno, strerror(errno));
            free(pal_processes);
        ...
        header = (SHM_FIRST_HEADER *)shm_segment_bases[0];
    ...
    else
    {
        ...
        fd_map = open(first_segment_name, O_RDWR);
        if(fd_map == -1)
        {
            ERROR("open() failed; error is %d (%s)\n", errno, strerror(errno));
#ifdef O_EXLOCK
...
void SHMCleanup(void)
{
    ...
    if( my_pid==InterlockedCompareExchange((LONG *) &header->spinlock, 0, my_pid))
    {
        WARN("SHMCleanup called while we still had the lock!\n");
    }
...
#ifdef O_EXLOCK
    fd_lock = open(lockfile_name,O_RDWR | O_EXLOCK);
#else   // O_EXLOCK
    fd_lock = open(lockfile_name,O_RDWR);
#endif  // O_EXLOCK
    if(fd_lock == -1)
    {
        ERROR("open() failed; error is %d (%s)\n", errno, strerror(errno));
    ...
    else /* Clean up everything. */
    {
        ...
        while ( strlen(suffix) > 0 )
        {
            ...
            fd_segment = open( FileName, O_RDONLY );
            if( fd_segment == -1 )
            {
                WARN("Unable to open shared memory segment file: %s "
                     "errno is %d (%s)\n", 
                     FileName, errno, strerror( errno ) );                    
                break;
            }
           
            if ( read(fd_segment, &header, sizeof(SHM_SEGMENT_HEADER)) !=
...
SHMPTR SHMalloc(size_t size)
{
    ...
    shmptr_ptr = SHMPTR_TO_PTR(first_free);
...
void SHMfree(SHMPTR shmptr)
{
    ...
    shmptr_ptr = SHMPTR_TO_PTR(shmptr);
    ...
    header = (SHM_SEGMENT_HEADER *)shm_segment_bases[segment];
    ...
    first_header = (SHM_FIRST_HEADER *)shm_segment_bases[0];
...
int SHMLock(void)
{
    ...
    SYNCEnterCriticalSection(&shm_critsec, TRUE);
...
int SHMRelease(void)
{
    ...
    if(lock_count==0)
    {
        ...
        SYNCLeaveCriticalSection(&shm_critsec, TRUE);
        return 0;
    }
    ...
    /* This matches the SYNCEnterCriticalSection from SHMRelease */
    SYNCLeaveCriticalSection(&shm_critsec, TRUE);

    /* This matches the SYNCEnterCriticalSection from SHMLock */
    SYNCLeaveCriticalSection(&shm_critsec, TRUE);
...
LPVOID SHMPtrToPtr(SHMPTR shmptr)
{
    ...
    if(segment == 0)
    {
        if(offset<roundup(sizeof(SHM_FIRST_HEADER), sizeof(INT64)))
        {
    ...
    {
        if(offset<sizeof(SHM_SEGMENT_HEADER))
        {
            ASSERT("Offset %d is in segment header! returning NULL\n", offset);
            return NULL;
        }
...
BOOL SHMSetInfo(SHM_INFO_ID element, SHMPTR value)
{
    ...
    TRACE("Setting SHM info element %d to %08x; used to be %08x\n",
          element, value, header->shm_info[element] );
...
static SHMPTR SHMLinkPool(SHMPTR first, int block_size, int num_blocks)
{
    ...
    item_ptr = shm_segment_bases[SHMPTR_SEGMENT(first)]+(SHMPTR_OFFSET(first));
...
static LPVOID SHMMapSegment(char *segment_name)
{
    ...
    sprintf(segment_path+strlen(segment_path), "/%s_%s",
    ...
    segment = mmap(NULL, sb.st_size,PROT_READ|PROT_WRITE,
                            MAPFLAGS, segment_fd, 0);
    close(segment_fd);
...
static BOOL SHMMapUnknownSegments(void)
{
    ...
    header = (SHM_SEGMENT_HEADER *) shm_segment_bases[shm_numsegments-1];
...
static BOOL SHMAddSegment(void)
{
    ...
    for(sps=0; sps<SPS_LAST; sps++)
    {
        ...
        shmptr_ptr = shm_segment_bases[SHMPTR_SEGMENT(header->last_pool_blocks[sps])] +
...
static int SHMGetProcessList(int fd, pid_t **process_list, BOOL strip_me)
{
        ...
        && n_pal_processes > 0)
    {
        ...
        list = (pid_t *)malloc(n_pal_processes*sizeof(pid_t));
        if(!list)
        {
...
SHMPTR SHMStrDup( LPCSTR string )
{
    ...
    if ( string )
    {
        ...
        if ( retVal != 0 ) 
        {
            memcpy( SHMPTR_TO_PTR( retVal ), string, length );
        }
    }
    return retVal;
}
