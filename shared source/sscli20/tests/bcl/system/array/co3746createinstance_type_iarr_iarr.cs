...
public class Co3746CreateInstance_type_iArr_iArr
{
 public static String s_strActiveBugNums = "";
 public static String s_strDtTmVer       = "";
 public static String s_strClassMethod   = "Array.CreateInstance(Type, int[], int[])";
 public static String s_strTFName        = "Co3746CreateInstance_type_iArr_iArr";
 public static String s_strTFAbbrev      = "Co3746";
 ...
 public bool runTest()
   ...
   int[] iLowerBoundArr = {-100, -50, 0, 50};
   Array objArr;
   try
     {
     do
       ...
       iArr = (Int32[,,,])Array.CreateInstance(tpValue, iIndexArr, iLowerBoundArr);
       ++iCountTestcases;
       if ( iArr.Rank != 4 )
	 {
	 ++iCountErrors;
	 ...
	 ++iCountTestcases;
	 if ( iArr.GetLowerBound(ii) != iLowerBoundArr[ii] )
	   {
	   ++iCountErrors;
	   Console.WriteLine( s_strTFAbbrev + "Err_763sf!  iArr.GetLowerBound(ii)=="+ iArr.GetLowerBound(ii) );
	   }
	 ++iCountTestcases;
	 if ( iArr.GetUpperBound(ii) != iLowerBoundArr[ii] + iIndexArr[ii]-1 )
	   {
	   ++iCountErrors;
	   Console.WriteLine( s_strTFAbbrev + "Err_268aq!  iArr.GetUpperBound(ii)=="+ iArr.GetUpperBound(ii) );
	   }
	 }
       for(int i = iArr.GetLowerBound(0); i <= iArr.GetUpperBound(0); i++)
	 for(int j=iArr.GetLowerBound(1); j <= iArr.GetUpperBound(1); j++)
	   for(int k=iArr.GetLowerBound(2); k<= iArr.GetUpperBound(2); k++)
	     for(int l=iArr.GetLowerBound(3); l<= iArr.GetUpperBound(3); l++)
	       {
	       iIndexArr[0]=i;
	       iIndexArr[1]=j;
	       iIndexArr[2]=k;
	 ...
	 for(int j=iArr.GetLowerBound(1); j <= iArr.GetUpperBound(1); j++)
       ...
       iIndexArr[2]=7;
       iIndexArr[3]=8;
       iLowerBoundArr[0]=-100;
       iLowerBoundArr[1]=-50;
       iLowerBoundArr[2]=0;
       iLowerBoundArr[3]=100;
       for(int ii=0; ii < ClassTypes.Length; ii++)
	 {
	 try
	   {
	   tpValue = ClassTypes[ii];
	   objArr  = Array.CreateInstance(tpValue, iIndexArr, iLowerBoundArr);
	   ++iCountTestcases;
	   if ( objArr.Rank != 4 )
	     {
	     ++iCountErrors;
	   ...
	   for(int jj = 0; jj < iArr.Rank; jj++)
	     ...
	     ++iCountTestcases;
	     if ( objArr.GetLowerBound(jj) != iLowerBoundArr[jj] )
	       {
	       ++iCountErrors;
	       Console.WriteLine( s_strTFAbbrev + ", Err_763sr_" + ii + ",  objArr.GetLowerBound(jj)=="+ objArr.GetLowerBound(jj) );
	       }
	     ++iCountTestcases;
	     if ( objArr.GetUpperBound(jj) != iLowerBoundArr[jj] + iIndexArr[jj]-1 )
	       {
	       ++iCountErrors;
	 ...
	 Console.WriteLine( s_strTFAbbrev + "Err_926sp!  wrong exception thrown=="+ ex);
       ...
       try
	 {
	 Array myarray = Array.CreateInstance(typeof(Int32), new Int32[] { 3 }, new Int32[] { 0 });
	 int[] imyintarr = (int[]) myarray;
	 }
       catch (Exception ex)
	 {
	 ++iCountErrors;
	 Console.WriteLine( s_strTFAbbrev + "Err_927dp!  wrong exception thrown=="+ ex);
	 }
       } while ( false );
     }
 ...
 public static void Main( String[] args )
   {
   bool bResult = false;	
   Co3746CreateInstance_type_iArr_iArr oCbTest = new Co3746CreateInstance_type_iArr_iArr();
   try
     {
