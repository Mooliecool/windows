...
public class IsDigit_str_i
{
 ...
 public virtual bool runTest()
   ...
   string[] testStrs;   
   IList charData;      
   Console.WriteLine( "IsDigit_str_i runTest started." );
   ...
   foreach (string strCurTest in testStrs)
     ...
     for (int iCurChar = 0; iCurChar < strCurTest.Length; iCurChar++)
       ...
       if ( curCharInfo.eCategory == UnicodeCategory.DecimalDigitNumber )
	 {
	 if ( Char.IsDigit(strCurTest,iCurChar) )
	   {                    
	   iCountTestcases++ ;
	   ...
	   Console.WriteLine("Failed :: " +
			     ...
			     " NLS+ Category : " + 
			     Char.IsDigit(strCurTest,iCurChar) );
	   }                
	 }                                
       }
     }
   if ( iCountErrors == 0 ){
   Console.Error.Write( "Char_IsDigit_str_i: paSs. iCountTestcases== " + iCountTestcases.ToString("d3"));
   return true;
   }
   else{
   Console.Error.Write( "Char_IsDigit_str_i: Fail. iCountErrors== " + iCountErrors.ToString("d3"));
   return false;
   }
   }
 ...
 public static void Main(String[] strArgList)
   ...
   try
     {
     IsDigit_str_i ObjIsDigit_str_i = new IsDigit_str_i();
     bDataOK = ObjIsDigit_str_i.ReadUnicodeData();
     if(! bDataOK )
       {
       Console.WriteLine("Not able to read the UniCode data from the file");
       return ;
       }
     bResult = ObjIsDigit_str_i.runTest();
     }
   catch ( Exception exc )
     {
     bResult = false;
     Console.Error.WriteLine( "IsDigit_str_i main caught an Exception!" + exc.ToString());
