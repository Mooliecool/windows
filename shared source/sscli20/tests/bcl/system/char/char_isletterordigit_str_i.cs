...
public class IsLetterOrDigit_str_i
{
 ...
 public virtual bool runTest()
   ...
   string[] testStrs;   
   IList charData;      
   Console.WriteLine( "IsLetterOrDigit_str_i runTest started." );
   ...
   foreach (string strCurTest in testStrs)
     ...
     for (int iCurChar = 0; iCurChar < strCurTest.Length; iCurChar++)
       ...
       if ( (curCharInfo.eCategory == UnicodeCategory.DecimalDigitNumber) || (curCharInfo.eCategory == UnicodeCategory.LowercaseLetter) || (curCharInfo.eCategory == UnicodeCategory.UppercaseLetter) || (curCharInfo.eCategory == UnicodeCategory.TitlecaseLetter)) 
	 {                        
	 iCountTestcases++ ;
	 if ( Char.IsLetterOrDigit(strCurTest,iCurChar) )
	   {                    
	   ...
	   Console.WriteLine("Failed :: " +
			     ...
			     " NLS+ Category : " + 
			     Char.IsLetterOrDigit(strCurTest,iCurChar) );
	   }
	 }                                
       }
     }
   if ( iCountErrors == 0 ){
   Console.Error.Write( "Char_IsLetterOrDigit_str_i: paSs. iCountTestcases== " + iCountTestcases.ToString("d3"));
   return true;
   }
   else{
   Console.Error.Write( "Char_IsLetterOrDigit_str_i: Fail. iCountErrors== " + iCountErrors.ToString("d3"));
   return false;
   }
   }
 ...
 public static void Main(String[] strArgList)
   ...
   try
     {
     IsLetterOrDigit_str_i ObjIsLetterOrDigit_str_i = new IsLetterOrDigit_str_i();
     bDataOK = ObjIsLetterOrDigit_str_i.ReadUnicodeData();
     if(! bDataOK )
       {
       Console.WriteLine("Not able to read the UniCode data from the file");
       return ;
       }
     bResult = ObjIsLetterOrDigit_str_i.runTest();
     }
   catch ( Exception exc )
     {
     bResult = false;
     Console.Error.WriteLine( "IsLetterOrDigit_str_i main caught an Exception!" + exc.ToString());
