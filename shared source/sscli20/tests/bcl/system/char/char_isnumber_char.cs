...
public class IsNumber_char
{
 ...
 public virtual bool runTest()
   ...
   for (int iCurChar = 0; iCurChar < m_CharData.Count; iCurChar++)
     ...
     if ( (curCharInfo.eCategory == UnicodeCategory.LetterNumber) ||
	  (curCharInfo.eCategory == UnicodeCategory.DecimalDigitNumber) )
       {
       iCountTestcases++ ;
       if ( Char.IsNumber(curCharInfo.chChar) )
	 {                    
	 ...
	 Console.WriteLine("Failed :: " +
			   ...
			   " NLS+ Category : " + 
			   Char.IsNumber(curCharInfo.chChar) );
	 }
       }         
     }
   if ( iCountErrors == 0 ){
   Console.Error.Write( "Char_IsNumber_char: paSs. iCountTestcases== " + iCountTestcases.ToString("d3"));
   return true;
   }
   else{
   Console.Error.Write( "Char_IsNumber_char: Fail. iCountErrors== " + iCountErrors.ToString("d3"));
   return false;
   }
   }
 ...
 public static void Main(String[] strArgList)
   ...
   try
     {
     IsNumber_char ObjIsNumber_char = new IsNumber_char();
     bDataOK = ObjIsNumber_char.ReadUnicodeData();
     if(! bDataOK )
       {
       Console.WriteLine("Not able to read the UniCode data from the file");
       return ;
       }
     bResult = ObjIsNumber_char.runTest();
     }
   catch ( Exception exc )
     {
     bResult = false;
     Console.Error.WriteLine( "IsNumber_char main caught an Exception!" + exc.ToString());
