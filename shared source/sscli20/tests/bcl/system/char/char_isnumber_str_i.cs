...
public class IsNumber_str_i
{
 ...
 public virtual bool runTest()
   ...
   string[] testStrs;   
   IList charData;      
   Console.WriteLine( "IsNumber_str_i runTest started." );
   ...
   foreach (string strCurTest in testStrs)
     ...
     for (int iCurChar = 0; iCurChar < strCurTest.Length; iCurChar++)
       ...
       if ( curCharInfo.eCategory == UnicodeCategory.LetterNumber || curCharInfo.eCategory == UnicodeCategory.DecimalDigitNumber || curCharInfo.eCategory == UnicodeCategory.OtherNumber)
	 {
	 iCountTestcases++ ;
	 if ( Char.IsNumber(strCurTest,iCurChar) )
	   {                    
	   ...
	   Console.WriteLine("Failed :: " +
			     ...
			     " NLS+ Category : " + 
			     Char.IsNumber(strCurTest,iCurChar) );
	   }
	 }                                
       }
     }
   if ( iCountErrors == 0 ){
   Console.Error.Write( "Char_IsNumber_str_i: paSs. iCountTestcases== " + iCountTestcases.ToString("d3"));
   return true;
   }
   else{
   Console.Error.Write( "Char_IsNumber_str_i: Fail. iCountErrors== " + iCountErrors.ToString("d3"));
   return false;
   }
   }
 ...
 public static void Main(String[] strArgList)
   ...
   try
     {
     IsNumber_str_i ObjIsNumber_str_i = new IsNumber_str_i();
     bDataOK = ObjIsNumber_str_i.ReadUnicodeData();
     if(! bDataOK )
       {
       Console.WriteLine("Not able to read the UniCode data from the file");
       return ;
       }
     bResult = ObjIsNumber_str_i.runTest();
     }
   catch ( Exception exc )
     {
     bResult = false;
     Console.Error.WriteLine( "IsNumber_str_i main caught an Exception!" + exc.ToString());
