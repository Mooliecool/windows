...
public class IsPunctuation_str_i
{
 ...
 public virtual bool runTest()
   ...
   string[] testStrs;   
   IList charData;      
   Console.WriteLine( "IsPunctuation_str_i runTest started." );
   ...
   foreach (string strCurTest in testStrs)
     ...
     for (int iCurChar = 0; iCurChar < strCurTest.Length; iCurChar++)
       ...
       if ( (curCharInfo.eCategory == UnicodeCategory.ConnectorPunctuation) || 
	    (curCharInfo.eCategory == UnicodeCategory.DashPunctuation) ||
	    (curCharInfo.eCategory == UnicodeCategory.OpenPunctuation) ||
	    (curCharInfo.eCategory == UnicodeCategory.ClosePunctuation) ||
	    (curCharInfo.eCategory == UnicodeCategory.InitialQuotePunctuation) ||
	    (curCharInfo.eCategory == UnicodeCategory.FinalQuotePunctuation) ||
	    (curCharInfo.eCategory == UnicodeCategory.OtherPunctuation) )
	 {
	 iCountTestcases++ ;
	 if ( Char.IsPunctuation(strCurTest,iCurChar) )
	   {                    
 ...
 public static void Main(String[] strArgList)
   ...
   try
     {
     IsPunctuation_str_i ObjIsPunctuation_str_i = new IsPunctuation_str_i();
     bDataOK = ObjIsPunctuation_str_i.ReadUnicodeData();
     if(! bDataOK )
       {
       Console.WriteLine("Not able to read the UniCode data from the file");
       return ;
       }
     bResult = ObjIsPunctuation_str_i.runTest();
     }
   catch ( Exception exc )
     {
     bResult = false;
     Console.Error.WriteLine( "IsPunctuation_str_i main caught an Exception!" + exc.ToString());
