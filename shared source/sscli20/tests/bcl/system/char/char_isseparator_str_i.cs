...
public class IsSeparator_str_i
{
 ...
 public virtual bool runTest()
   ...
   string[] testStrs;   
   IList charData;      
   Console.WriteLine( "IsSeparator_str_i runTest started." );
   ...
   foreach (string strCurTest in testStrs)
	 ...
	 {
	 iCountTestcases++ ;
	 if ( Char.IsSeparator(strCurTest,iCurChar) )
	   {                    
	   ...
	   Console.WriteLine("Failed :: " +
			     ...
			     " NLS+ Category : " + 
			     Char.IsSeparator(strCurTest,iCurChar) );
	   }
	 }                                
       }
     }        
   ...
   if ( iCountErrors == 0 ){
   Console.Error.Write( "Char_IsSeparator_str_i: paSs. iCountTestcases== " + iCountTestcases.ToString("d3"));
   return true;
   }
   else{
   Console.Error.Write( "Char_IsSeparator_str_i: Fail. iCountErrors== " + iCountErrors.ToString("d3"));
   return false;
   }
   }
 ...
 public static void Main(String[] strArgList)
   ...
   try
     {
     IsSeparator_str_i ObjIsSeparator_str_i = new IsSeparator_str_i();
     bDataOK = ObjIsSeparator_str_i.ReadUnicodeData();
     if(! bDataOK )
       {
       Console.WriteLine("Not able to read the UniCode data from the file");
       return ;
       }
     bResult = ObjIsSeparator_str_i.runTest();
     }
   catch ( Exception exc )
     {
     bResult = false;
     Console.Error.WriteLine( "IsSeparator_str_i main caught an Exception!" + exc.ToString());
