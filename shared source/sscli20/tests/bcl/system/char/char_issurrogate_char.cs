 ...
 public virtual bool runTest()
   ...
   for (int iCurChar = 0; iCurChar < m_CharData.Count; iCurChar++)
     ...
     if ( (curCharInfo.eCategory == UnicodeCategory.Surrogate) )
       {
       iCountTestcases++ ;
       if ( Char.IsSurrogate(curCharInfo.chChar) )
	 {                    
	 ...
	 Console.WriteLine("Failed :: " +
			   ...
			   " NLS+ Category : " + 
			   Char.IsSurrogate(curCharInfo.chChar) );
	 }
       }         
     }
   if ( iCountErrors == 0 ){
   Console.Error.Write( "Char_IsSurrogate_char: paSs. iCountTestcases== " + iCountTestcases.ToString("d3"));
   return true;
   }
   else{
   Console.Error.Write( "Char_IsSurrogate_char: Fail. iCountErrors== " + iCountErrors.ToString("d3"));
   return false;
   }
   }
