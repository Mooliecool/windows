...
public class IsSurrogate_str_i
{
 ...
 public virtual bool runTest()
   ...
   string[] testStrs;   
   IList charData;      
   Console.WriteLine( "IsSurrogate_str_i runTest started." );
   ...
   foreach (string strCurTest in testStrs)
     ...
     for (int iCurChar = 0; iCurChar < strCurTest.Length; iCurChar++)
       ...
       if ( curCharInfo.eCategory == UnicodeCategory.Surrogate )
	 {
	 iCountTestcases++ ;
	 if ( Char.IsSurrogate(strCurTest,iCurChar) )
	   {                    
	   ...
	   Console.WriteLine("Failed :: " +
			     ...
			     " NLS+ Category : " + 
			     Char.IsSurrogate(strCurTest,iCurChar) );
	   }
	 }                                
       }
     }
   ...
   if ( iCountErrors == 0 ){
   Console.Error.Write( "Char_IsSurrogate_str_i: paSs. iCountTestcases== " + iCountTestcases.ToString("d3"));
   return true;
   }
   else{
   Console.Error.Write( "Char_IsSurrogate_str_i: Fail. iCountErrors== " + iCountErrors.ToString("d3"));
   return false;
   }
   }
 ...
 public static void Main(String[] strArgList)
   ...
   try
     {
     IsSurrogate_str_i ObjIsSurrogate_str_i = new IsSurrogate_str_i();
     bDataOK = ObjIsSurrogate_str_i.ReadUnicodeData();
     if(! bDataOK )
       {
       Console.WriteLine("Not able to read the UniCode data from the file");
       return ;
       }
     bResult = ObjIsSurrogate_str_i.runTest();
     }
   catch ( Exception exc )
     {
     bResult = false;
     Console.Error.WriteLine( "IsSurrogate_str_i main caught an Exception!" + exc.ToString());
