...
public class IsSymbol_str_i
{
 ...
 public virtual bool runTest()
   ...
   string[] testStrs;   
   IList charData;      
   Console.Error.WriteLine( "IsSymbol_str_i runTest started." );
   ...
   foreach (string strCurTest in testStrs)
	 ...
	 {
	 iCountTestcases++ ;
	 if ( Char.IsSymbol(strCurTest,iCurChar) )
	   {                    
	   ...
	   Console.WriteLine("Failed :: " +
			     ...
			     " NLS+ Category : " + 
			     Char.IsSymbol(strCurTest,iCurChar) );
	   }                
	 }                                
       }
     }
   if ( iCountErrors == 0 ){
   Console.Error.Write( "Char_IsSymbol_str_i: paSs. iCountTestcases== " + iCountTestcases.ToString("d3"));
   return true;
   }
   else{
   Console.Error.Write( "Char_IsSymbol_str_i: Fail. iCountErrors== " + iCountErrors.ToString("d3"));
   return false;
   }
   }
 ...
 public static void Main(String[] strArgList)
   ...
   try
     {
     IsSymbol_str_i ObjIsSymbol_str_i = new IsSymbol_str_i();
     bDataOK = ObjIsSymbol_str_i.ReadUnicodeData();
     if(! bDataOK )
       {
       Console.WriteLine("Not able to read the UniCode data from the file");
       return ;
       }
     bResult = ObjIsSymbol_str_i.runTest();
     }
   catch ( Exception exc )
     {
     bResult = false;
     Console.Error.WriteLine( "IsSymbol_str_i main caught an Exception!" + exc.ToString());
