...
public class IsUpper_str_i
{
 ...
 public virtual bool runTest()
   ...
   string[] testStrs;   
   IList charData;      
   Console.WriteLine( "IsUpper_str_i runTest started." );
   ...
   foreach (string strCurTest in testStrs)
     ...
     for (int iCurChar = 0; iCurChar < strCurTest.Length; iCurChar++)
       ...
       if ( curCharInfo.eCategory == UnicodeCategory.UppercaseLetter )
	 {
	 iCountTestcases++ ;
	 if ( Char.IsUpper(strCurTest,iCurChar) )
	   {                    
	   ...
	   Console.WriteLine("Failed :: " +
			     ...
			     " NLS+ Category : " + 
			     Char.IsUpper(strCurTest,iCurChar) );
	   }
	 }                                
       }
     }
   if ( iCountErrors == 0 ){
   Console.Error.Write( "Char_IsUpper_str_i: paSs. iCountTestcases== " + iCountTestcases.ToString("d3"));
   return true;
   }
   else{
   Console.Error.Write( "Char_IsUpper_str_i: Fail. iCountErrors== " + iCountErrors.ToString("d3"));
   return false;
   }
   }
 ...
 public static void Main(String[] strArgList)
   ...
   try
     {
     IsUpper_str_i ObjIsUpper_str_i = new IsUpper_str_i();
     bDataOK = ObjIsUpper_str_i.ReadUnicodeData();
     if(! bDataOK )
       {
       Console.WriteLine("Not able to read the UniCode data from the file");
       return ;
       }
     bResult = ObjIsUpper_str_i.runTest();
     }
   catch ( Exception exc )
     {
     bResult = false;
     Console.Error.WriteLine( "IsUpper_str_i main caught an Exception!" + exc.ToString());
