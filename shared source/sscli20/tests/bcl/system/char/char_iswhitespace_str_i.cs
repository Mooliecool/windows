...
public class IsWhiteSpace_str_i
{
 ...
 public virtual bool runTest()
   ...
   string[] testStrs;   
   IList charData;      
   Console.WriteLine( "IsWhiteSpace_str_i runTest started." );
   ...
   foreach (string strCurTest in testStrs)
	 ...
	 {
	 iCountTestcases++ ;
	 if ( Char.IsWhiteSpace(strCurTest,iCurChar) )
	   {                    
	   ...
	   Console.WriteLine("Failed :: " +
			     ...
			     " NLS+ Category : " + 
			     Char.IsWhiteSpace(strCurTest,iCurChar) );
	   }
	 }                                
       }
     }
   if ( iCountErrors == 0 ){
   Console.Error.Write( "Char_IsWhiteSpace_str_i: paSs. iCountTestcases== " + iCountTestcases.ToString("d3"));
   return true;
   }
   else{
   Console.Error.Write( "Char_IsWhiteSpace_str_i: Fail. iCountErrors== " + iCountErrors.ToString("d3"));
   return false;
   }
   }
 ...
 public static void Main(String[] strArgList)
   ...
   try
     {
     IsWhiteSpace_str_i ObjIsWhiteSpace_str_i = new IsWhiteSpace_str_i();
     bDataOK = ObjIsWhiteSpace_str_i.ReadUnicodeData();
     if(! bDataOK )
       {
       Console.WriteLine("Not able to read the UniCode data from the file");
       return ;
       }
     bResult = ObjIsWhiteSpace_str_i.runTest();
     }
   catch ( Exception exc )
     {
     bResult = false;
     Console.Error.WriteLine( "IsWhiteSpace_str_i main caught an Exception!" + exc.ToString());
