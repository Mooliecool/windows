...
namespace NLSPlus.GetNumericValue_char
{
...
public class GetNumericValue_char
{
 ...
 public static void Main(String[] strArgList)
   {
   GetNumericValue_char GetNumericValue_char = new GetNumericValue_char();
   bool bDataOK;            
   try
     {
     bDataOK = GetNumericValue_char.ReadUnicodeData();
     if( bDataOK )
       GetNumericValue_char.TestCharMethods();
     }
   catch(Exception exc)
     {
     Console.WriteLine
       ("An exception occurred while running GetNumericValue_char." +
