...
namespace NLSPlus.GetNumericValue_str_i
{
...
public class GetNumericValue_str_i
{
 ...
 public bool TestStringMethods(uint uNumStrings, int iMaxLength)
   {
   string[] testStrs;   
   Random rdmStrGen,    
     rdmCharGen;   
   ...
   if (uNumStrings == 0)
     ...
     throw new ArgumentOutOfRangeException
       ("uNumStrings","Value must be positive");
     }
   if (iMaxLength <= 0)
     {
     throw new ArgumentOutOfRangeException
       ("iMaxLength","Value must be positive");
     }
   charData = m_CharData.GetValueList();
   testStrs = new String[uNumStrings];
   rdmStrGen = new Random(unchecked((int)(DateTime.Now.Ticks)));
   for (int iCurStr = 0; iCurStr < testStrs.Length; iCurStr++)
     {
     int iRdmStrLen = 0;           
     StringBuilder strbRdmString;  
     iRdmStrLen = rdmStrGen.Next(1,iMaxLength + 1);
     ...
     for (int iCurChar = 0; iCurChar < iRdmStrLen; iCurChar++)
       ...
       iRdmCharIdx = rdmCharGen.Next(0,(int)m_CharData.Count);
   ...
   foreach (string strCurTest in testStrs)
     ...
     for (int iCurChar = 0; iCurChar < strCurTest.Length; iCurChar++)
       ...
       iCountTestcases++ ;
       if ( Math.Abs(curCharInfo.dNumericValue - 
		     Char.GetNumericValue(strCurTest,iCurChar))
	    <= DBL_TOLERANCE
	    ){
       }                 
       else{
       ...
       Console.WriteLine("Failed :: " +
			 ...
			 " NLS+ Category : " + 
			 Char.GetNumericValue(curCharInfo.chChar) );
       }
       }            
     }     
   if ( iCountErrors == 0 ){
 ...
 public static void Main(String[] strArgList)
   {
   GetNumericValue_str_i GetNumericValue_str_i = new GetNumericValue_str_i();
   bool bDataOK;            
   try
     {
     bDataOK = GetNumericValue_str_i.ReadUnicodeData();
     if( bDataOK )
       {
       GetNumericValue_str_i.TestStringMethods
	 (NUM_RANDOM_STRINGS,MAX_RANDOM_STR_LENGTH);
       }
     }
   catch(Exception exc)
     {
     Console.WriteLine
       ("An exception occurred while running GetNumericValue_str_i." +
