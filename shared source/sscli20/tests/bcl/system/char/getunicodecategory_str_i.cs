...
namespace NLSPlus.GetUnicodeCategory_str_i
{
...
public class GetUnicodeCategory_str_i
{
 ...
 public GetUnicodeCategory_str_i()
   {
   ...
   foreach (string strCurTest in testStrs)
     ...
     for (int iCurChar = 0; iCurChar < strCurTest.Length; iCurChar++)
       ...
       if ( curCharInfo.eCategory == 
	    Char.GetUnicodeCategory(strCurTest,iCurChar)
	    ){
       }                 
       else{
       iCountErrors++ ;
 ...
 public static void Main(String[] strArgList)
   {
   GetUnicodeCategory_str_i GetUnicodeCategory_str_i = new GetUnicodeCategory_str_i();
   bool bDataOK;            
   try
     {
     bDataOK = GetUnicodeCategory_str_i.ReadUnicodeData();
     if( bDataOK )
       {
       GetUnicodeCategory_str_i.TestStringMethods
     ...
     {
     Console.WriteLine
       ("An exception occurred while running GetUnicodeCategory_str_i." +
