...
public class Co8700ctor_bool
{
    public static String s_strActiveBugNums = "";
    public static String s_strDtTmVer       = "";
    public static String s_strClassMethod   = "HybridDictionary.ctor(bool)";
    public static String s_strTFName        = "Co8700ctor_bool.cs";
    ...
    public virtual bool runTest()
    {
        ...
        try
        {
            ...
            {  
                iCountErrors++;
                Console.WriteLine("Err_0009c: Count returned {0} instead of 2", hd.Count);
            }
            if (String.Compare(hd["NaMe"].ToString(), "Value", false) != 0)  
            {  
                iCountErrors++;
                Console.WriteLine("Err_0009d: Item() returned unexpected value");
            }
            if (hd["NAME"] != null)  
            {  
                iCountErrors++;
                Console.WriteLine("Err_0009e: Item() returned non-null value for uppercase key");
            }
            ...
            try 
            {
                hd.Add("NaMe", "vl");
                iCountErrors++;
                Console.WriteLine("Err_0009Noex: no exception");
            }
            ...
            {
                iCountErrors++;
                Console.WriteLine("Err_0009ex: unexpected exception: {0}", e.ToString());
            }
            if (hd.Count != 1) 
            {  
                iCountErrors++;
                Console.WriteLine("Err_00013a: Count returned {0} instead of 1", hd.Count);
            }
            if (String.Compare(hd["NAME"].ToString(), "Value", false) != 0)  
            {  
                iCountErrors++;
                Console.WriteLine("Err_0009b: Item() returned unexpected value for uppercase key");
            }
            Console.WriteLine("10. Clear() short dictionary");
            iCountTestcases++;
            hd.Clear();
            ...
            for (int i = 0; i < len; i++) 
            {
                ...
                {  
                    iCountErrors++;
                    Console.WriteLine("Err_0011c_{0}: Item() returned unexpected value for uppercase key", i);
                }
            }
            Console.WriteLine("12. Clear() long dictionary");
            iCountTestcases++;
    ...
    {
        bool bResult = false;
        Co8700ctor_bool cbA = new Co8700ctor_bool();
        try 
        {
            bResult = cbA.runTest();
        } 
