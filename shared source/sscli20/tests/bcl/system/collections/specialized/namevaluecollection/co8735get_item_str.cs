...
using GenStrings;
public class Co8735get_Item_str
{
    public static String s_strActiveBugNums = "";
    public static String s_strDtTmVer       = "";
    public static String s_strClassMethod   = "NameValueCollection.Item(string)";
    public static String s_strTFName        = "Co8735get_Item_str.cs";
    ...
    public virtual bool runTest()
    {
        ...
        try
        {
            ...
            Console.WriteLine("1. Item(string) on empty collection");
            iCountTestcases++;
            ...
            Console.WriteLine("2. add simple strings access them via Item(string)");
            strLoc = "Loc_002oo"; 
            iCountTestcases++;
            cnt = nvc.Count;
            ...
            for (int i = 0; i < len; i++) 
            {
                ...
                {
                    iCountErrors++;
                    Console.WriteLine("Err_0002_{0}b, returned: \"{1}\", expected \"{2}\"", i, nvc[keys[i]], values[i]);
                }  
            }
            ...
            for (int i = 0; i < len; i++) 
            {
                ...
                {
                    iCountErrors++;
                    Console.WriteLine("Err_0003_{0}b, returned \"{1}\" instead of \"{2}\"", i, nvc[intlValues[i+len]], intlValues[i]);
                }  
            }
            ...
            for (int i = 0; i < len; i++) 
            {
                ...
                {
                    iCountErrors++;
                    Console.WriteLine("Err_0004_{0}a, returned \"{1}\" instead of \"{2}\"", i, nvc[intlValuesLower[i+len]], intlValues[i]);
                }
                iCountTestcases++;
            ...
            Console.WriteLine("5. multiple items with same key and Item(string)");
            strLoc = "Loc_005oo"; 
            iCountTestcases++;
            nvc.Clear();
            len = values.Length;
            ...
            {
                iCountErrors++;
                Console.WriteLine("Err_0005c, returned \"{0}\" instead of \"{1}\"", nvc[k1], exp1);
            }  
            Console.WriteLine("6. Item(null) when there is item with null key");
            strLoc = "Loc_006oo"; 
            iCountTestcases++;
            cnt = nvc.Count;
            ...
            {
                iCountErrors++;
                Console.WriteLine("Err_0006: returned \"{}\" instead of \"{}\"", nvc[null], "nullValue");
            }
            Console.WriteLine("7. Item(null) when no such key");
            strLoc = "Loc_07oo"; 
            iCountTestcases++;
            nvc.Clear();
            for (int i = 0; i < len; i++) 
    ...
    {
        bool bResult = false;
        Co8735get_Item_str cbA = new Co8735get_Item_str();
        try 
        {
            bResult = cbA.runTest();
        } 
