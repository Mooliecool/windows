...
using System.Net;
public class Co5612get_Position
{
    public static String s_strActiveBugNums = "";
    public static String s_strDtTmVer       = "";
    public static String s_strClassMethod   = "BufferedStream.Flush()";
    public static String s_strTFName        = "Co5612get_Position.cs";
    public static String s_strTFAbbrev      = "Co5612";
    ...
    public bool runTest()
    {
        ...
        try
        {
            ...
            FileStream fs2;
            StreamWriter sw2;
            if(File.Exists("Co5612Test.tmp"))
                File.Delete("Co5612Test.tmp");
            strLoc = "Loc_9289x";
            ...
            strLoc = "Loc_27yxc";
            fs2 = new FileStream("Co5612Test.tmp", FileMode.Create);
            ...
            {
                iCountErrors++;
                printerr( "Error_4324324! Incorrect exception thrown, exc=="+exc.ToString());
            } 
            catch (Exception exc) 
            {
                iCountErrors++;
            ...
            try 
            {
                bs2.Position = 0;
                iCountErrors++;
                printerr( "Error_298gb! Expected exception not thrown, bs2.Position=="+bs2.Position);
            } 
            catch (ObjectDisposedException iexc) 
            {
                printinfo("Info_8798b! Caught expected exception, iexc=="+iexc.Message);
            } 
            catch (Exception exc) 
            {
                iCountErrors++;
                printerr( "Error_9100x! Incorrect exception thrown, exc=="+exc.ToString());
            }
            iCountTestcases++;
            m_PortSetEvent.Reset();
            Thread tcpListenerThread = new Thread(new ThreadStart(Co5612get_Position.StartListeningTcp));
    ...
    {
        bool bResult = false;
        Co5612get_Position cbA = new Co5612get_Position();
        try 
        {
            bResult = cbA.runTest();
        } 
