...
public class Co5587Copy_str_str
{
	public static String s_strActiveBugNums = "";
	public static String s_strDtTmVer       = "";
	public static String s_strClassMethod   = "File.CopyTo(String, Boolean)";
	public static String s_strTFName        = "Co5587Copy_str_str.cs";
	...
	public bool runTest()
	{
		...
		try
		{
			...
			iCountTestcases++;
			try {
				File.Copy(fil2.Name, String.Empty);
				iCountErrors++;
			...
			iCountTestcases++;
try {
			File.Copy(fil2.Name, "TestFile.tmp1");
...
try {
			...
			sw2.Close();
			fil2 = new FileInfo(filName);
			File.Copy(fil2.Name, "TestTest.tmp");
	...
	printerr( "Error_28vc8! Unexpected exception, exc=="+exc.ToString());
			...
			iCountTestcases++;
try {
			File.Copy(filName, "TestFile.tmp1", true);
			fil1 = new FileInfo("TestFile.tmp1");
			fil2 = new FileInfo(filName);   
			if(!fil1.Exists) {
				iCountErrors++;
	...
	printerr( "Error_2987v! Unexpected exception, exc=="+exc.ToString());
			...
			try {
				new FileInfo(filName).Delete();
				new FileInfo("TestFile.tmp1").Delete();
				new FileInfo("TestTest.tmp").Delete();
			} catch (Exception ) {}
	...
	{
		bool bResult = false;
		Co5587Copy_str_str cbA = new Co5587Copy_str_str();
		try {
			bResult = cbA.runTest();
