...
class Co1805
{
    ...
    public String s_strComponentBeingTested   = "ctor_Biib";
    public String s_strTFName                 = "Co1805ctor_Biib";
    public String s_strTFAbbrev               = "Co1805";
    ...
    public Boolean runTest()
    {
        ...
        {
            ++iCountTestcases;
            MemoryStream ms = new MemoryStream( new byte[] { Byte.MinValue, Byte.MaxValue }, 1, 1, false );
            if ( ms.CanWrite != false )
            {
                ++iCountErrors;
                Console.WriteLine( "Err_001b, CanWrite should be false here but it is true" );
            }
        }
        catch (Exception ex)
        {
        ...
        {
            ++iCountTestcases;
            MemoryStream ms = new MemoryStream( new byte[] { Byte.MinValue, Byte.MaxValue }, 1, 1, true );
            if ( ! ms.CanWrite )
            {
                ++iCountErrors;
                Console.WriteLine( "Err_002b, CanWrite should be true here but it is false" );
            }
        }
        catch (Exception ex)
        {
