...
class Co1810
{
    ...
    public String s_strComponentBeingTested   = "get_CanSeek";
    public String s_strTFName                 = "Co1810get_CanSeek";
    public String s_strTFAbbrev               = "Co1810";
    ...
    public Boolean runTest()
    {
        ...
        int iCountErrors    = 0;
        if ( verbose ) Console.WriteLine( "try to see if CanSeek is true for newly created memorystream" );
        try
        {
            ++iCountTestcases;
            ...
            {
                ++iCountErrors;
                Console.WriteLine( "Error_001a,  Should of been able to Seek MemoryStream." );
            }
        }
        catch (Exception ex)
        {
        ...
        if ( verbose ) Console.WriteLine( "try to see if CanSeek is false when stream is closed" );
        try
        {
            ++iCountTestcases;
            MemoryStream ms = new MemoryStream();
            ...
            {
                ++iCountErrors;
                Console.WriteLine( "Error_002a,  Should of been able to Seek MemoryStream." );
            }
        }
        catch (Exception ex)
        {
