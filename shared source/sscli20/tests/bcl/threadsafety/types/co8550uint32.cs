...
public class Co8550UInt32
{
	public static readonly String s_strClassMethod   = "UInt32 - threadsafety test";
	public static readonly String s_strTFName        = "Co8550UInt32.cs";
	public static readonly String s_strTFAbbrev      = s_strTFName.Substring(0, 6);
	public static readonly String s_strTFPath        = Environment.CurrentDirectory;
	private UInt32 instanceInt_1;
	private static UInt32 staticInt_1;
	private static Random randomNumGen;
        private static UInt32[] ValueArray = { 96, 0, 1, 303, 10, 2, 1024, 16, 7, 32 };
        const Int32 ValueArraySize = 10;
	public virtual bool runTest()
	{
		const UInt32 numberOfThreads = 200;
		...
		try {
			...
			thdPool = new Thread[numberOfThreads];
			for(int i=0; i<numberOfThreads; i++){
				tdst1 = new ThreadStart(Co8550UInt32.WorkOnStatic);
				thdPool[i] = new Thread(tdst1);
			}
			for(int i=0; i<numberOfThreads; i++)
				thdPool[i].Start();
	...
	private void WorkOnInstanceAndLocal()
	{
		...
		localInt_1 = UInt32.MaxValue + UInt32.MinValue;
		if(localInt_1 != UInt32.MaxValue)
		...
		localInt_1 = UInt32.Parse("32");
		if(localInt_1 != 32)
			throw new Exception("Loc_975esg! Major Error here, " + localInt_1);
		localInt_1 = UInt32.MaxValue;
		if(localInt_1.GetHashCode() != -1)
	...
	public static void Main(String[] args)
	{
		...
		bool bResult = false;
	   Co8550UInt32 cbA = new Co8550UInt32();
	   try {
	      bResult = cbA.runTest();
