...
function deuri002a ()
  {
    ...
    //----------------------------------------------------------------------
     apInitScenario( "Scenario 1: decodeURI with one or more %'s") ;

     Str1 = RunTest ("%") ;
     Verify ( Str2, Str1, "decodeURI.1" ) ;

     for ( iTmp = 0 ; iTmp < 100 ; iTmp++ )
       Str3 = Str3 + "%" ;
     Str1 = RunTest (Str3) ;
     Verify ( Str2, Str1, "decodeURI.1.2."+iTmp ) ;

    //----------------------------------------------------------------------
     apInitScenario( "Scenario 2: decodeURI with %<single char>") ;

     for ( iTmp = 48 ; iTmp < 58 ; iTmp++ )	    // [%0, %9]
       {
	  Str3 = "%" + String.fromCharCode(iTmp) ;
	  Str1 = RunTest (Str3) ;
	  Verify ( Str2, Str1, "decodeURI.2.1."+iTmp ) ;
       }
     ...
     for ( iTmp = 65 ; iTmp < 71 ; iTmp++ )	    // [%A, %F]
	  ...
	  Str3 = "%" + String.fromCharCode(iTmp) ;
	  Str1 = RunTest (Str3) ;
	  Verify ( Str2, Str1, "decodeURI.2.2."+iTmp ) ;
       }
     ...
     for ( iTmp = 97 ; iTmp < 103 ; iTmp++ )	    // [%a, %f]
	  ...
	  Str3 = "%" + String.fromCharCode(iTmp) ;
	  Str1 = RunTest (Str3) ;
	  Verify ( Str2, Str1, "decodeURI.2.3."+iTmp ) ;
       }
     ...
     for ( iTmp = 0 ; iTmp < 65535 ; iTmp += 1024 )  // all other char's
	  ...
	  Str3 = "%" + String.fromCharCode(iTmp) ;
	  Str1 = RunTest (Str3) ;
	  Verify ( Str2, Str1, "decodeURI.2.4."+iTmp ) ;
       }

    //----------------------------------------------------------------------
     apInitScenario( "Scenario 3: decodeURI with %<hex> - leading bit is 1") ;

     for ( iTmp = 0x80 ; iTmp <= 0xFF  ; iTmp++ )
       {
	  Str3 = "%" + hex(iTmp) ;
	  Str1 = RunTest (Str3) ;
	  Verify ( Str2, Str1, "decodeURI.3."+iTmp ) ;
       }

    //----------------------------------------------------------------------
     apInitScenario( "Scenario 4: decodeURI with %<hex>%<hex> - leading hex != [C2, DF]") ;

     for ( iTmp = 0x00 ; iTmp < 0xC2 ; iTmp += 4 )
	for ( jTmp = 0xBF ; jTmp > 0x7F ; jTmp -=9 )
	  {					    // According to Unicode Standard
	     if ( iTmp < 0x10 ) 		    // 2.0 (Appendix A):
	       Str3 = "%0" + hex(iTmp) + "%" + hex(jTmp) ;
	     else				    // when iTmp = 0xC0, (str3
	       Str3 = "%" + hex(iTmp) + "%" + hex(jTmp) ;
	     Str1 = RunTest (Str3) ;		    // = %C0%<nn>, string will
	     if ( iTmp != 0xC0 )		    // be decoded into %(jTmp-0x80)
	       Verify ( Str2, Str1, "decodeURI.4.1."+iTmp+"."+jTmp ) ;
	     else
	       {
		  kTmp = jTmp - 0x80 ;		    // see if kTmp represents
		  if ( isReserved (kTmp) )	    // a uriReserved character
		      Verify ( Str3, Str1, "decodeURI.4.1."+iTmp+"."+jTmp ) ;
		   else 			    // which are char's won't be
		      Verify ( String.fromCharCode(kTmp), Str1, "decodeURI.4.1."+iTmp+"."+jTmp ) ;
		}				    // encoded; i.e. won't be decoded
	   }
	...
	for ( jTmp = 0xBF ; jTmp > 0x7F ; jTmp -=9 )
	  {
	     Str3 = "%" + hex(iTmp) + "%" + hex(jTmp) ;
	     Str1 = RunTest (Str3) ;
	     Verify ( Str2, Str1, "decodeURI.4.2."+iTmp+"."+jTmp ) ;
	  }

    //----------------------------------------------------------------------
     apInitScenario( "Scenario 5: decodeURI with %<hex>%<hex> - second hex < 0x80") ;
     ...
     for ( iTmp = 0xC2 ; iTmp < 0xE0 ; iTmp++ )
	{
	   if ( jTmp < 0x10 )
	     ...
	     Str3 = "%" + hex(iTmp) + "%" + hex(jTmp) ;
	   Str1 = RunTest (Str3) ;
	   Verify ( Str2, Str1, "decodeURI.5."+iTmp ) ;

	   jTmp -= 4 ;
	}

    //----------------------------------------------------------------------
     apInitScenario( "Scenario 6: decodeURI with %<hex>%<hex> - second hex > 0xBF") ;
     ...
     for ( iTmp = 0xC2 ; iTmp < 0xE0 ; iTmp++ )
	{
	   Str3 = "%" + hex(iTmp) + "%" + hex(jTmp) ;
	   Str1 = RunTest (Str3) ;
	   Verify ( Str2, Str1, "decodeURI.6."+iTmp ) ;
    ...
    //----------------------------------------------------------------------
     apInitScenario( "Scenario 7: decodeURI with %<hex>%<hex>%<hex> - leading hex != 0xE<n>") ;
	 ...
	 for ( jTmp = 0x80 ; jTmp < 0xC0 ; jTmp += 9 )
	   {
		...
		Str3 = "%" + hex(iTmp) + "%" + hex(jTmp) + "%" + hex(kTmp) ;
	      Str1 = RunTest (Str3) ;
	      Verify ( Str2, Str1, "decodeURI.7.1."+iTmp+"."+jTmp ) ;
	 ...
	 for ( jTmp = 0x80 ; jTmp < 0xC0 ; jTmp += 9 )
	   {
	      Str3 = "%" + hex(iTmp) + "%" + hex(jTmp) + "%" + hex(kTmp) ;
	      Str1 = RunTest (Str3) ;
	      Verify ( Str2, Str1, "decodeURI.7.2."+iTmp+"."+jTmp ) ;
    ...
    //----------------------------------------------------------------------
     apInitScenario( "Scenario 8: decodeURI with %<hex>%<hex>%<hex> - second hex < 0x80") ;

     for ( iTmp = 0xE0 ; iTmp < 0xEE ; iTmp++ )
	{
	   kTmp = 0x80 ;
	   for ( jTmp = 0x7F ; jTmp >= 0 ; jTmp -= 9 )
	     {
	       if ( jTmp < 0x10 )
		 ...
		 Str3 = "%" + hex(iTmp) + "%" + hex(jTmp) + "%" + hex(kTmp) ;
	       Str1 = RunTest (Str3) ;
	       Verify ( Str2, Str1, "decodeURI.8.1."+iTmp+"."+jTmp ) ;
	     }
     ...
     kTmp = 0x80 ;
     for ( jTmp = 0x80 ; jTmp < 0xA0 ; jTmp++ )      // 1st <hex> = 0xE0
	{					     // and 2nd <hex> < 0xA0
	   Str3 = "%E0" + "%" + hex(jTmp) + "%" + hex(kTmp) ;
	   Str1 = RunTest (Str3) ;		     // this will decode to
	   iTmp = (jTmp - 0x80) * 64 + (kTmp - 0x80) ; // characters [0, 2047]
	   if ( isReserved (iTmp) )		     // see scenario 4.1
	     Verify ( Str3, Str1, "decodeURI.8.2."+jTmp ) ;
	   else
	     Verify ( String.fromCharCode(iTmp), Str1, "decodeURI.8.2."+jTmp ) ;

	   kTmp += 2 ;
	}

    //----------------------------------------------------------------------
     apInitScenario( "Scenario 9: decodeURI with %<hex>%<hex>%<hex> - second hex > 0xBF") ;
	   ...
	   for ( jTmp = 0xC0 ; jTmp <= 0xFF ; jTmp++ )
	     {
	       Str3 = "%" + hex(iTmp) + "%" + hex(jTmp) + "%" + hex(kTmp) ;
	       Str1 = RunTest (Str3) ;
	       Verify ( Str2, Str1, "decodeURI.9.1."+iTmp+"."+jTmp ) ;
	...
	{					     // and 2nd <hex> > 0x9F
	   ...
	   Str1 = RunTest (Str3) ;
	   Verify ( Str2, Str1, "decodeURI.9.2."+jTmp ) ;

	   kTmp += 2 ;
	}

    //----------------------------------------------------------------------
     apInitScenario( "Scenario 10: decodeURI with %<hex>%<hex>%<hex> - third hex < 0x80") ;
     ...
     for ( iTmp = 0xE0 ; iTmp < 0xEE ; iTmp++ )
	{
	   ...
	   for ( jTmp = 0x7F ; jTmp >= 0 ; jTmp -= 9 )
		 ...
		 Str3 = "%" + hex(iTmp) + "%" + hex(kTmp) + "%" + hex(jTmp) ;
	       Str1 = RunTest (Str3) ;
	       Verify ( Str2, Str1, "decodeURI.10."+iTmp+"."+jTmp ) ;
	     }
