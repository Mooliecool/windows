...
/* -------------------------------------------------------------------------
 ...
 ---------------------------------------------------------------------------
  ...
  Scenarios:	1.  decodeURI with one or more %'s

		2.  decodeURI with %<single char>

		3.  decodeURI with %<hex> - leading bit is 1

		4.  decodeURI with %<hex>%<hex> - leading hex != 0xC<n> or 0xD<n>

		5.  decodeURI with %<hex>%<hex> - second hex < 0x80

		6.  decodeURI with %<hex>%<hex> - second hex > 0xBF

		7.  decodeURI with %<hex>%<hex>%<hex> - leading hex != 0xE<n>

		8.  decodeURI with %<hex>%<hex>%<hex> - second hex < 0x80

		9.  decodeURI with %<hex>%<hex>%<hex> - second hex > 0xBF

		10. decodeURI with %<hex>%<hex>%<hex> - third hex < 0x80

		11. decodeURI with %<hex>%<hex>%<hex> - third hex > 0xBF

		12. decodeURI with %<hex>%<hex>%<hex>%<hex> - leading hex != [0xF0, 0xF4]

		13. decodeURI with %<hex>%<hex>%<hex>%<hex> - 2nd hex < 0x80

		14. decodeURI with %<hex>%<hex>%<hex>%<hex> - 3rd hex < 0x80

		15. decodeURI with %<hex>%<hex>%<hex>%<hex> - 3rd hex < 0xBF

		16. decodeURI with %<hex>%<hex>%<hex>%<hex> - 4th hex < 0x80

		17. decodeURI with %<hex>%<hex>%<hex>%<hex> - 4th hex < 0xBF

  Abstract:	Set up error handler then call decodeURI with escape sequences
		that are not recognized by the encoding routines as valid
		encoding representation.  Check error code to ensure that
 ...
 ---------------------------------------------------------------------------
	...
	[00]	03-Nov-1999	    Angelach: Created Test
 -------------------------------------------------------------------------*/
...
function RunTest (Param)
  {
      ...
      try
	{
	  sTmp = decodeURI(Param) ;	// call decodeURI
	}
...
function deuri002b ()
  {
    ...
    //----------------------------------------------------------------------
     apInitScenario( "Scenario 11: decodeURI with %<hex>%<hex>%<hex> - third hex > 0xBF") ;

     for ( iTmp = 0xE0 ; iTmp < 0xEE ; iTmp++ )
	{
	   kTmp = 0x80 ;
	   for ( jTmp = 0xC0 ; jTmp <= 0xFF ; jTmp++ )
	     {
	       Str3 = "%" + hex(iTmp) + "%" + hex(kTmp) + "%" + hex(jTmp) ;
	       Str1 = RunTest (Str3) ;
	       Verify ( Str2, Str1, "decodeURI.11."+iTmp+"."+jTmp ) ;
    ...
    //----------------------------------------------------------------------
     apInitScenario( "Scenario 12: decodeURI with %<hex>%<hex>%<hex>%<hex> - leading hex != [0xF0, 0xF4]") ;

     jTmp = 0x80 ;
     kTmp = 0xBF ;
     for ( iTmp = 0x00 ; iTmp < 0xF0 ; iTmp += 4 )
	{
	   Str3 = "%" + hex(iTmp) + "%" + hex(jTmp) + "%" + hex(kTmp) + "%" + hex(kTmp-2) ;
	   Str1 = RunTest (Str3) ;
	   Verify ( Str2, Str1, "decodeURI.12.1."+iTmp ) ;
     ...
     jTmp = 0x80 ;
     kTmp = 0xBF ;
     for ( iTmp = 0xF5 ; iTmp <= 0xFF ; iTmp++ )
	{
	   Str3 = "%" + hex(iTmp) + "%" + hex(jTmp) + "%" + hex(kTmp) + "%" + hex(kTmp-3) ;
	   Str1 = RunTest (Str3) ;
	   Verify ( Str2, Str1, "decodeURI.12.2."+iTmp ) ;
    ...
    //----------------------------------------------------------------------
     apInitScenario( "Scenario 13: decodeURI with %<hex>%<hex>%<hex>%<hex> - 2nd hex < 0x80") ;

     jTmp = 0xBF ;				     // 1st hex == [0xF0, 0xF3]
     for ( iTmp = 0x00 ; iTmp < 0x80 ; iTmp +=5 )    // 2nd hex < 0x80
	{
	   kTmp = 0xF0 + (iTmp % 4) ;
	   if ( iTmp < 0x10 )
	     Str3 = "%" + hex(kTmp) + "%0" + hex(iTmp) + "%" + hex(jTmp) + "%" + hex(jTmp-2) ;
	   else
	     Str3 = "%" + hex(kTmp) + "%" + hex(iTmp) + "%" + hex(jTmp) + "%" + hex(jTmp-2) ;
	   Str1 = RunTest (Str3) ;
	   Verify ( Str2, Str1, "decodeURI.13.1."+iTmp ) ;

	   jTmp -= 2 ;
	}

     jTmp = 0x80 ;				     // 1st hex == [0xF0, 0xF3]
     for ( iTmp = 0xC0 ; iTmp <= 0xFF ; iTmp +=2 )   // 2nd hex > 0xBF
	{
	   if ( iTmp % 6 )
	     kTmp = 0xF0 + ((jTmp + 1) % 4) ;
	   else
	     kTmp = 0xF0 + (jTmp % 4) ;
	   Str3 = "%" + hex(kTmp) + "%" + hex(iTmp) + "%" + hex(jTmp) + "%" + hex(0x13F-jTmp) ;
	   Str1 = RunTest (Str3) ;
	   Verify ( Str2, Str1, "decodeURI.13.2."+iTmp ) ;

	   jTmp += 2 ;
	}

     jTmp = 0xBF ;				     // 1st hex == 0xF4
     for ( iTmp = 0x00 ; iTmp < 0x80 ; iTmp +=5 )    // 2nd hex != [0x80, 0x8F]
	{
	   if ( iTmp < 0x10 )
	     Str3 = "%F4" + "%0" + hex(iTmp) + "%" + hex(jTmp) + "%" + hex(jTmp-3) ;
	   else
	     Str3 = "%F4" + "%" + hex(iTmp) + "%" + hex(jTmp) + "%" + hex(jTmp-3) ;
	   Str1 = RunTest (Str3) ;
	   Verify ( Str2, Str1, "decodeURI.13.3."+iTmp ) ;

	   jTmp -= 3 ;
	}

     jTmp = 0xBF ;
     for ( iTmp = 0x90 ; iTmp <= 0xFF ; iTmp +=5 )
	{
	   Str3 = "%F4" + "%" + hex(iTmp) + "%" + hex(jTmp) + "%" + hex(jTmp-3) ;
	   Str1 = RunTest (Str3) ;
	   Verify ( Str2, Str1, "decodeURI.13.4."+iTmp ) ;

	   jTmp -= 3 ;
	}

    //----------------------------------------------------------------------
     apInitScenario( "Scenario 14: decodeURI with %<hex>%<hex>%<hex>%<hex> - 3rd hex < 0x80") ;

     jTmp = 0xBF ;
     for ( iTmp = 0x00 ; iTmp < 0x80 ; iTmp +=5 )
	{
	   Str3 = "%" + hex(0xF0 + (iTmp % 4)) + "%" + hex(jTmp) ;
	   if ( iTmp < 0x10 )
	     Str3 = Str3 + "%0" + hex(iTmp) + "%" + hex(jTmp-4) ;
	   else
	     Str3 = Str3 + "%" + hex(iTmp) + "%" + hex(jTmp-4) ;
	   Str1 = RunTest (Str3) ;
	   Verify ( Str2, Str1, "decodeURI.14.1."+iTmp ) ;
     ...
     for ( iTmp = 0x00 ; iTmp < 0x80 ; iTmp +=5 )
	{
	   Str3 = "%F4" + "%" + hex(0x80 + (iTmp % 0x10)) ;
	   if ( iTmp < 0x10 )
	     Str3 = Str3 + "%0" + hex(iTmp) + "%" + hex(jTmp) ;
	   else
	     Str3 = Str3 + "%" + hex(iTmp) + "%" + hex(jTmp) ;
	   Str1 = RunTest (Str3) ;
	   Verify ( Str2, Str1, "decodeURI.14.2."+iTmp ) ;

	   jTmp -= 2 ;
	}

    //----------------------------------------------------------------------
     apInitScenario( "Scenario 15: decodeURI with %<hex>%<hex>%<hex>%<hex> - 3rd hex > 0xBF") ;

     jTmp = 0x80 ;
     kTmp = 0xBF ;
     for ( iTmp = 0xC0 ; iTmp <= 0xFF ; iTmp +=3 )
	{
	   Str3 = "%" + hex(0xF0 + (iTmp % 4)) + "%" + hex(jTmp) ;
	   Str3 = Str3 + "%" + hex(iTmp) + "%" + hex(kTmp) ;
	   Str1 = RunTest (Str3) ;
	   Verify ( Str2, Str1, "decodeURI.15.1."+iTmp ) ;
     ...
     for ( iTmp = 0xC0 ; iTmp <= 0xFF ; iTmp +=3 )
	{
	   ...
	   Str3 = Str3 + "%" + hex(iTmp) + "%" + hex(jTmp) ;
	   Str1 = RunTest (Str3) ;
	   Verify ( Str2, Str1, "decodeURI.15.2."+iTmp ) ;

	   jTmp += 3 ;
	}

    //----------------------------------------------------------------------
     apInitScenario( "Scenario 16: decodeURI with %<hex>%<hex>%<hex>%<hex> - 4th hex < 0x80") ;
     ...
     for ( iTmp = 0x00 ; iTmp < 0x80 ; iTmp +=5 )
	{
	   Str3 = "%" + hex(0xF0 + (iTmp % 4)) + "%" + hex(jTmp) ;
	   if ( iTmp < 0x10 )
	     Str3 = Str3 + "%" + hex(jTmp-4) + "%0" + hex(iTmp) ;
	   else
	     Str3 = Str3 + "%" + hex(jTmp-4) + "%" + hex(iTmp) ;
	   Str1 = RunTest (Str3) ;
	   Verify ( Str2, Str1, "decodeURI.16.1."+iTmp ) ;
     ...
     for ( iTmp = 0x00 ; iTmp < 0x80 ; iTmp +=5 )
	{
	   Str3 = "%F4" + "%" + hex(0x80 + (iTmp % 0x10)) ;
	   if ( iTmp < 0x10 )
	     Str3 = Str3 + "%" + hex(jTmp) + "%0" + hex(iTmp) ;
	   else
	     Str3 = Str3 + "%" + hex(jTmp) + "%" + hex(iTmp) ;
	   Str1 = RunTest (Str3) ;
	   Verify ( Str2, Str1, "decodeURI.16.2."+iTmp ) ;

	   jTmp -= 2 ;
	}

    //----------------------------------------------------------------------
     apInitScenario( "Scenario 17: decodeURI with %<hex>%<hex>%<hex>%<hex> - 4th hex > 0xBF") ;
	...
	{
	   Str3 = "%" + hex(0xF0 + (iTmp % 4)) + "%" + hex(jTmp) ;
	   Str3 = Str3 + "%" + hex(kTmp) + "%" + hex(iTmp) ;
	   Str1 = RunTest (Str3) ;
	   Verify ( Str2, Str1, "decodeURI.17.1."+iTmp ) ;
     ...
     for ( iTmp = 0xC0 ; iTmp <= 0xFF ; iTmp +=3 )
	{
	   ...
	   Str3 = Str3 + "%" + hex(jTmp) + "%" + hex(iTmp) ;
	   Str1 = RunTest (Str3) ;
	   Verify ( Str2, Str1, "decodeURI.17.2."+iTmp ) ;
