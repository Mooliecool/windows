...
function duric002b ()
  {
    ...
    //----------------------------------------------------------------------
     apInitScenario( "Scenario 11: decodeURIComponent with %<hex>%<hex>%<hex> - third hex > 0xBF") ;
    ...
    //----------------------------------------------------------------------
     apInitScenario( "Scenario 12: decodeURIComponent with %<hex>%<hex>%<hex>%<hex> - leading hex != [0xF0, 0xF4]") ;

     jTmp = 0x80 ;
     kTmp = 0x80 ;
     for ( iTmp = 0x80 ; iTmp < 0xF0 ; iTmp += 7 )
	{
	   Str3 = "%" + hex(iTmp) + "%" + hex(jTmp) + "%" + hex(kTmp) + "%" + hex(kTmp+3) ;
	   Str1 = RunTest (Str3) ;
	   Verify ( Str2, Str1, "decodeURIComponent.12.1."+iTmp ) ;
     ...
     for ( iTmp = 0xF5 ; iTmp <= 0xFF ; iTmp++ )
	{
	   Str3 = "%" + hex(iTmp) + "%" + hex(jTmp) + "%" + hex(kTmp) + "%" + hex(kTmp+3) ;
	   Str1 = RunTest (Str3) ;
	   Verify ( Str2, Str1, "decodeURIComponent.12.2."+iTmp ) ;
    ...
    //----------------------------------------------------------------------
     apInitScenario( "Scenario 13: decodeURIComponent with %<hex>%<hex>%<hex>%<hex> - 2nd hex < 0x80") ;

     jTmp = 0x80 ;				     // 1st hex == [F0, F3]
     for ( iTmp = 0x00 ; iTmp < 0x80 ; iTmp +=5 )    // 2nd hex < 0x80
	{
	   ...
	   Str1 = RunTest (Str3) ;
	   Verify ( Str2, Str1, "decodeURIComponent.13.1."+iTmp ) ;

	   jTmp += 2 ;
	}

     jTmp = 0xFF ;				     // 1st hex == [F0, F3]
     kTmp = 0x80 ;				     // 2nd hex > 0xBF
     for ( iTmp = 0xC0 ; iTmp <= 0xFF ; iTmp +=2 )
	{
	   if ( iTmp % 6 )
	     Str3 = "%" + hex(0xF0 + ((jTmp + 1) % 4)) ;
	   else
	     Str3 = "%" + hex(0xF0 + (jTmp % 4)) ;
	   Str3 = Str3 + "%" + hex(iTmp) + "%" + hex(jTmp-0x40) + "%" + hex(kTmp) ;
	   Str1 = RunTest (Str3) ;
	   Verify ( Str2, Str1, "decodeURIComponent.13.2."+iTmp ) ;

	   jTmp -= 2 ;
	   kTmp += 2 ;
	}

     jTmp = 0x80 ;				     // 1st hex == F4
     for ( iTmp = 0x00 ; iTmp < 0x80 ; iTmp +=5 )    // 2nd hex != [0x80, 0x8F]
	{
	   ...
	   Str1 = RunTest (Str3) ;
	   Verify ( Str2, Str1, "decodeURIComponent.13.3."+iTmp ) ;
     ...
     for ( iTmp = 0x90 ; iTmp <= 0xFF ; iTmp +=5 )
	{
	   Str3 = "%F4" + "%" + hex(iTmp) + "%" + hex(jTmp) + "%" + hex(jTmp+3) ;
	   Str1 = RunTest (Str3) ;
	   Verify ( Str2, Str1, "decodeURIComponent.13.4."+iTmp ) ;

	   jTmp += 3 ;
	}

    //----------------------------------------------------------------------
     apInitScenario( "Scenario 14: decodeURIComponent with %<hex>%<hex>%<hex>%<hex> - 3rd hex < 0x80") ;
     ...
     for ( iTmp = 0x00 ; iTmp < 0x80 ; iTmp +=5 )
	{
	   ...
	   Str1 = RunTest (Str3) ;
	   Verify ( Str2, Str1, "decodeURIComponent.11.1."+iTmp ) ;
     ...
     for ( iTmp = 0x00 ; iTmp < 0x80 ; iTmp +=5 )
	{
	   ...
	   Str1 = RunTest (Str3) ;
	   Verify ( Str2, Str1, "decodeURIComponent.11.2."+iTmp ) ;

	   jTmp += 2 ;
	}

    //----------------------------------------------------------------------
     apInitScenario( "Scenario 15: decodeURIComponent with %<hex>%<hex>%<hex>%<hex> - 3rd hex > 0xBF") ;

     jTmp = 0x80 ;
     kTmp = 0xBF ;
     for ( iTmp = 0xFF ; iTmp > 0xBF ; iTmp -=3 )
	{
	   Str3 = "%" + hex(0xF0 + (iTmp % 4)) + "%" + hex(jTmp) ;
	   Str3 = Str3 + "%" + hex(iTmp) + "%" + hex(kTmp) ;
	   Str1 = RunTest (Str3) ;
	   Verify ( Str2, Str1, "decodeURIComponent.15.1."+iTmp ) ;
     ...
     for ( iTmp = 0xFF ; iTmp > 0xBF ; iTmp -=3 )
	{
	   ...
	   Str3 = Str3 + "%" + hex(iTmp) + "%" + hex(jTmp) ;
	   Str1 = RunTest (Str3) ;
	   Verify ( Str2, Str1, "decodeURIComponent.15.2."+iTmp ) ;

	   jTmp -= 3 ;
	}

    //----------------------------------------------------------------------
     apInitScenario( "Scenario 16: decodeURIComponent with %<hex>%<hex>%<hex>%<hex> - 4th hex < 0x80") ;
     ...
     for ( iTmp = 0x00 ; iTmp < 0x80 ; iTmp +=5 )
	{
	   ...
	   Str1 = RunTest (Str3) ;
	   Verify ( Str2, Str1, "decodeURIComponent.16.1."+iTmp ) ;
     ...
     for ( iTmp = 0x00 ; iTmp < 0x80 ; iTmp +=5 )
	{
	   ...
	   Str1 = RunTest (Str3) ;
	   Verify ( Str2, Str1, "decodeURIComponent.16.2."+iTmp ) ;

	   jTmp += 2 ;
	}

    //----------------------------------------------------------------------
     apInitScenario( "Scenario 17: decodeURIComponent with %<hex>%<hex>%<hex>%<hex> - 4th hex > 0xBF") ;
     ...
     for ( iTmp = 0xFF ; iTmp > 0xBF ; iTmp -=3 )
	{
	   ...
	   Str1 = RunTest (Str3) ;
	   Verify ( Str2, Str1, "decodeURIComponent.17.1."+iTmp ) ;
     ...
     for ( iTmp = 0xFF ; iTmp > 0xBF ; iTmp -=3 )
	{
	   ...
	   Str3 = Str3 + "%" + hex(jTmp) + "%" + hex(iTmp) ;
	   Str1 = RunTest (Str3) ;
	   Verify ( Str2, Str1, "decodeURIComponent.17.2."+iTmp ) ;
