...
function escap011() {
    ...
    var exp = "%00";
    var act = escape(unescape("%00"));
    if ( exp != act )
        apLogFailInfo( "char 0x00 failed, null mask", exp, act, "");
    ...
    for (i=0; i<5; i++) {
        ...
        for (j=0; j<10; j++) { sExp+=s; sActTmp+=sEsc.charAt(iPos); };
  ...
  } // end if langhost is 1033, US English
  else{
  /**/apInitScenario("skipping scenario")
  /**/apInitScenario("skipping scenario")
  /**/apInitScenario("skipping scenario")
  /**/apInitScenario("skipping scenario")
  }
    ...
    sExp=sAct="";
    for (i=0x01; i<=0xff; i++) {
        if (i > 0x7F && apGlobalObj.LangHost() != 1033)
            break;
