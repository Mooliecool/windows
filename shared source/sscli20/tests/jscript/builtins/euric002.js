...
/* -------------------------------------------------------------------------
 ...
 ---------------------------------------------------------------------------
  ...
  Scenarios:	1.  encodeURIComponent with character in [0xD800, 0xDBFF] - single char

		2.  encodeURIComponent with character in [0xDC00, 0xDFFF] - single char

		3.  encodeURIComponent with char in [0xD800, 0xDBFF] not followed by [DC00, 0xDFFFF]

		4.  encodeURIComponent with char in [0xD800, 0xDBFF] or [0xDC00, DFFFF] among other characters

  Abstract:	Set up error handler then call encodeURIComponent with char's
		that are not recognized by the encoding routines as valid
		characters. Check error code to ensure that proper error was
		generated. Also check result after the call to ensure that
		no encoding was taken place.
 ---------------------------------------------------------------------------
...
function RunTest (Param)
  {
      ...
      try
	{
	  sTmp = encodeURIComponent(Param) ;	 // call encodeURIComponent
	}
...
function euric002 ()
  {
    ...
    //----------------------------------------------------------------------
     apInitScenario( "Scenario 1: encodeURIComponent with character in [0xD800, 0xDBFF] - single character") ;

     for ( iTmp = 0xD800 ; iTmp <= 0xDBFF ; iTmp++ )
       {
	  Str1 = RunTest (String.fromCharCode(iTmp)) ;
	  Verify ( Str1, Str2, "encodeURIComponent.1."+iTmp ) ;
       }

    //----------------------------------------------------------------------
     apInitScenario( "Scenario 2: encodeURIComponent with character in [0xDC00, 0xDFFF] - single character") ;

     for ( iTmp = 0xDC00 ; iTmp <= 0xDFFF ; iTmp++ )
       {
	  Str1 = RunTest (String.fromCharCode(iTmp)) ;
	  Verify ( Str1, Str2, "encodeURIComponent.2."+iTmp ) ;
       }

    //----------------------------------------------------------------------
     apInitScenario( "Scenario 3: encodeURIComponent with char in [0xD800, 0xDBFF] not followed by [DC00, 0xDFFFF]") ;

     for ( iTmp = 0 ; iTmp < 1024 ; iTmp++ )
       {
	  Str1 = RunTest (String.fromCharCode(iTmp+0xD800, iTmp)) ;
	  Verify ( Str1, Str2, "encodeURIComponent.3.1."+iTmp ) ;

	  Str1 = RunTest (String.fromCharCode(iTmp+0xD800, iTmp+1023)) ;
	  Verify ( Str1, Str2, "encodeURIComponent.3.2."+iTmp ) ;

	  Str1 = RunTest (String.fromCharCode(iTmp+0xD800, 2048+iTmp*52)) ;
	  Verify ( Str1, Str2, "encodeURIComponent.3.3."+iTmp ) ;

	  Str1 = RunTest (String.fromCharCode(iTmp+0xD800, iTmp+0xD800)) ;
	  Verify ( Str1, Str2, "encodeURIComponent.3.4."+iTmp ) ;

	  Str1 = RunTest (String.fromCharCode(iTmp+0xD800, 57344+iTmp*8)) ;
	  Verify ( Str1, Str2, "encodeURIComponent.3.5."+iTmp ) ;
       }

    //----------------------------------------------------------------------
     apInitScenario( "Scenario 4: encodeURIComponent with char in [0xD800, 0xDBFF] or [0xDC00, DFFFF] among other characters") ;

     for ( iTmp = 0 ; iTmp < 1024 ; iTmp++ )
       {
	  Str3 = Str3 + String.fromCharCode(1024+iTmp, 55295-iTmp*52, 0xD800+iTmp, 0xDC00+iTmp, 65535-iTmp*8, 0xDC00+iTmp, 0xD800+iTmp, iTmp) ;
	  Str1 = RunTest (Str3) ;
	  Verify ( Str1, Str2, "encodeURIComponent.4.1."+iTmp ) ;

	  Str4 = Str4 + String.fromCharCode(0xDC00+iTmp, iTmp, 1024+iTmp, 65535-iTmp*8, 0xD800+iTmp, 0xD800+iTmp, 0xDC00+iTmp, 2048+iTmp*52) ;
	  Str1 = RunTest (Str4) ;
	  Verify ( Str1, Str2, "encodeURIComponent.4.2."+iTmp ) ;
       }
