...
function tostrs03 () {
   ...
   apInitTest("toStrS03 ");
    ...
    var stExp = S1;
    var stAct = S1.toString(MAX-2,MAX);
    if ( stExp != stAct ) {
	apLogFailInfo("toString() didn't work properly", stExp, stAct, "");
    }

    apInitScenario("2. toString()(Max, Max)");

    S1 = bldStr(MAX-1, "b")+"C"

    stExp = S1;
    stAct = S1.toString(MAX,MAX-1);
    if ( stExp != stAct ) {
	apLogFailInfo("toString() didn't work properly", stExp, stAct, "");
    }

    apInitScenario("3. toString()(Max, Max / 2, Max / 2)");

    S1 = bldStr(MAX / 2, "a") + bldStr(MAX /2, "Z")

    stExp = S1;
    stAct = S1.toString(MAX / 2, MAX )
    if ( stExp != stAct ) {
	apLogFailInfo("toString() didn't work properly", stExp.length, stAct.length, "");
    }

    apInitScenario("4. toString()(Todd, Max, 5)");

    stExp = "Todd"
    stAct = "Todd".toString(MAX,5)
    if ( stExp != stAct ) {
	apLogFailInfo("toString() didn't work properly", stExp, stAct, "");
    }

    apInitScenario("5. toString()(MAX, MAX-400)");

    S1 = bldStr(MAX, "U")

    stExp = S1;

    stAct = S1.toString( MAX-400,MAX);
    if ( stExp != stAct ) {
	apLogFailInfo("toString() didn't work properly", stExp.length, stAct.length, "");
    }

    apInitScenario("6. toString()(MAX, 2)");

    S1 = bldStr(MAX, "U")

    stExp = S1;

    stAct = S1.toString( MAX,1 );

    if ( S1.length != MAX || stAct.length != MAX ) {
	apLogFailInfo("toString() didn't work properly", stExp.length, stAct.length , "OOB 6569");
    }
    if ( stExp != stAct ) {
	apLogFailInfo("toString() didn't work properly", "", "", "OOB 6569");
    }
