...
function unesc016() {
    ...
    sAct = unescape( retVar("\b\b\b\b\b\b\b\b") );
    ...
    //---------------Chars 0x20-0x7f------------------------------------------
        ...
        sAct = unescape( retVar('QQQQQQQQQQQQQQQQQQQQQQQQ') );

        if (sAct != sExp)
            apLogFailInfo( "repeated char uc alpha failed", sExp, sAct, "");

        sExp = 'MMMMMMMMMMMMMMMMMMM';        
        sAct = unescape( retVar('MMMMMMMMMMMMMMMMMMM') );
        ...
        sAct = unescape( retVar('pppppppppppppppppppppppppppppppp') );

        if (sAct != sExp)
            apLogFailInfo( "repeated char lc alpha failed", sExp, sAct, "");

        sExp = 'jjjjjjjjjjjjjj';        
        sAct = unescape( retVar('jjjjjjjjjjjjjj') );
        ...
        sAct = unescape( retVar("11111111111111111111111111111111111") );

        if (sAct != sExp)
            apLogFailInfo( "repeated char num failed", sExp, sAct, "");

        sExp = "0000000000000000000000000000";        
        sAct = unescape( retVar("0000000000000000000000000000") );
        ...
        sAct = unescape( retVar(" !\"#$%&\'()*+,-./0123456789:;<=>?@ABCDEFGHIJKLMNOPQRSTUVWXYZ[\]^_`abcdefghijklmnopqrstuvwxyz{|}~") );
        ...
        sAct = unescape( retVar("0tjzL>E!(B#CJHg-;XwU^vI9nQ\'G`Wm&hDA i6k8c]=|YOKN)b3*:xd[,.~2?y/uS+MqlV\"pPR4TZ%7{o$f@e_F5<r1}as") );
    ...
    //---------------Chars 0x80-0xff-(Extended)-------------------------------
        ...
        sAct = unescape( retVar('çççççççççççççççççç') );

        if (sAct != sExp)
            apLogFailInfo( "repeated char extended failed", sExp, sAct, "");

        sExp = 'ýýýýýý';        
        sAct = unescape( retVar('ýýýýýý') );
        ...
        sAct = unescape( retVar("€‚ƒ„…†‡ˆ‰Š‹ŒŽ‘’“”•–—˜™š›œžŸ ¡¢£¤¥¦§¨©ª«¬­®¯°±²³´µ¶·¸¹º»¼½¾¿ÀÁÂÃÄÅÆÇÈÉÊËÌÍÎÏÐÑÒÓÔÕÖ×ØÙÚÛÜÝÞßàáâãäåæçèéêëìíîïðñòóôõö÷øùúûüýþÿ") );
        ...
        sAct = unescape( retVar("¾ÑÒž•ïçƒ®±Ùè´þù¨¢«˜ÇûöêßœŸ÷—›àÐæ”Üäü¥éÂ¼ÌŠì™È§²íâóŽã‡ÅºÀ½Øš–ýÊ’ÆÚË°îªòñµ¤‘Ä„“¦‰Ô‹ÁÓô†Õ€ÏúðÖ£ÃÞ¯ ˆÿ³¸Û»ëáÉ¡¶ø©ÍÎ…åŒ¿¹¬­õ×·‚Ý") );
