...
@if(!@_fast) 
    ...
    ob.a=ob.b=ob.c=ob.d=ob.e=null;

    for (i in Array) {
        if (n % 2 && n < 6) ob[++tMem] = null;
        Array[i]=n;
        n++;
    }
    n--;
    ...
    tActual = ob.a*ob.b*ob.c*ob.d*ob.e*ob[n-2]*ob[n-1]*ob[n];

    if (n != tMem || (tActual != tExpect)) apLogFailInfo( "built-in, exec, not instantiated--Array failed", tMem, n,"");
    ...
    if (n != tMem || (tActual != tExpect)) apLogFailInfo( "built-in, exec, not instantiated--Boolean failed", tMem, n,"");
    ...
    if (n != tMem || (tActual != tExpect)) apLogFailInfo( "built-in, exec, not instantiated--Date failed", tMem, n,"");
    ...
    if (n != tMem || (tActual != tExpect)) apLogFailInfo( "built-in, exec, not instantiated--Number failed", tMem, n,"");
    ...
    if (n != tMem || (tActual != tExpect)) apLogFailInfo( "built-in, exec, not instantiated--Object failed", tMem, n,"");
...
@if(!@_fast)
    ...
    if (n != tMem || (tActual != tExpect)) apLogFailInfo( "user-defined, not instantiated failed", tMem, n,"");
@end
