...
function verify(scen, ob, nob, cMem) {
    ...
    var obOrig = ob, obNew = new Object();

    ob.e=ob.d=ob.c=ob.b=ob.a=null;
    obNew.f=obNew.e=obNew.d=obNew.foo=obNew.b=obNew.a=null;

    for (var i in false ? n=100 : ob) {
        if (i=='b') ob=obNew;
        else if (i=='e') ob=obOrig;
        ob[i]=n++;
    }
    n--;
	...
	var tActual = ob.a*obNew.b*obNew.c*obNew.d*ob.e;

    if (n != tMem || (obNew.a|ob.b|ob.c|obNew.foo|ob.d|obNew.e|obNew.f) || tActual != tExpect)
        apLogFailInfo( scen+"--"+nob+" failed ","Check values tested in the preceding if","","");
