...
function verify(sCat, ob, bugNum)
{
    ...
    var vAct = Math.exp(Math.log(retVar(ob)));
    var vExp = (typeof ob == "boolean" || typeof ob == "object") ? ob+0 : parseFloat(ob);

    // checks below assume log(x) is defined for x<0 !?
    // let's take care of that...
    if (ob<0) {
      vAct = Math.log(retVar(ob));
      if (!isNaN(vAct))
...
@if(@_jscript_version >=7)
    verify("pos max", 1.7976931348000000e308, null);
@else
