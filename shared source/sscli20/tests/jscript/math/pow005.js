...
function verify(sCat, ob, bugNum){
 ...
 @end
    ...
    var sAct = Math.pow(retVar(ob),exp);
    for (var i=0; i<exp; i++) vExp*=ob;


//    if (verify.length==4) vExp = verify[3];
...
function pow005() {
    ...
    verify("pos min < n < pos hex excl-min", "01234567", null);
    verify("pos hex excl-min: 020000000000", "020000000000", null);
    verify("max pos: 037777777777", "037777777777", null, Math.pow(37777777777,3));

    verify("neg max > n > neg hex excl-max", "-076543210", null);
    verify("pos hex excl-min: -020000000001", "-020000000001", null);
    verify("min neg: -037777777777", "-037777777777", null, Math.pow(-37777777777,3));
