...
function verify(sCat, ob, sExp, bugNum) {
    ...
    if (fFailed)
        apLogFailInfo( m_scen+" - "+sCat+" failed", sExp, sAct,bugNum);
}
...
@end
    ...
    verify("max neg",-2.2250738585072012595e-308,-2.2250738585072012595e-308, 16479);
    ...
    verify("max pos","1.7976931348623158e+308",1.7976931348623158e+308, null);
    ...
    // float lengths for nums between the min neg and the max neg floats
        ...
        for (cbNum=1; cbNum<=15; cbNum++)
        {
            for (iDec=0; iDec<=cbNum+1; iDec++)
            {
                ...
                verify("min pos < n < max pos: ",sNum,eval(sNum), null);
            }
        }
