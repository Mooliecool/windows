...
function verify(sCat, ob, sExp, bugNum) {
    ...
    var sAct = parseFloat(eval(ob));

    var fFailed = false;
    if (isNaN(sExp))
    {
        if (!isNaN(sAct)) fFailed = true;
    }
    else
        if (sAct + "" != sExp + "") fFailed = true;
...
@end
    ...
    verify("min neg: -2147483647",-2147483647,-2147483647, null);
    ...
    // i.e., we display what the user types.  See psFlt001;2 for details on the discrepancies.
    //////////////////////////////////////////////////////////////////////////////
    ...
    // float lengths for nums between the min pos and the max pos floats
        ...
        for (cbNum=1; cbNum<=15; cbNum++)
        {
            ...
            for (iDec=0; iDec<=cbNum+1; iDec++) // decimal places
            {
                ...
                verify("min pos < n < max pos: "+sNum,num,num, null);
            }
        }


    verify("max pos",1.7976931348623158e308,1.7976931348623157E+308, 368);
    verify("max pos",1.7976931348623158e+308,1.7976931348623157E+308, 368);
    verify("> max pos float (1.#INF)",1.797693134862315807e309, Infinity, null);
    ...
    verify("min neg",-1.797693134862315807e308, -1.7976931348623157E+308, 368);
    verify("< min neg float (-1.#INF)",-1.797693134862315807e309, -Infinity, null);
    ...
    verify("max pos: 037777777777",037777777777, 4294967295, null);
    ...
    verify("min neg: -037777777777",-037777777777, -4294967295, null);
    ...
    verify("min pos: 2.2250738585072014e-308","'2.2250738585072014e-308'", 2.2250738585072014e-308, 16479);
    ...
    // float lengths for ns's between the min pos and the max pos floats
        ...
        for (cbNum=1; cbNum<=15; cbNum++)
        {
            for (iDec=0; iDec<=cbNum+1; iDec++) 
            {
                ...
                if (iDec != cbNum+1)
                    sNum = eval(sNum.substring(0,iDec)+"."+sNum.substring(iDec,sNum.length))+"";
                else if ( eval(sNum) < 2147483647) break;

                verify("min pos < n < max pos: "+sNum,sNum,eval(sNum), null);
            }
        }


    verify("max pos","'1.7976931348623158e308'",1.7976931348623158e308, 16479);
    verify("max pos","'1.7976931348623158e+308'",1.7976931348623158e+308, 16479);
    verify("> max pos float (1.#INF)","'1.797693134862315807e309'",1.797693134862315807e309, null);

    verify("max neg","-2.2250738585072012595e-308",-2.2250738585072014E-308, null);
    ...
    verify("min neg","'-1.797693134862315807e308'",-1.797693134862315807e308, 16479);
    ...
    for (i=0; i<10; i++)
    {
        ...
        num = Math.random()*4294967295;
        sRnd = "'"+num+genRndStr(100,0x01|0x02|0x10|0x20)+"'";
    ...
    {
        num = Math.random()*4294967295;
        sRnd = "'"+genRndStr(100,0x01|0x02|0x10|0x20)+num+"'";
