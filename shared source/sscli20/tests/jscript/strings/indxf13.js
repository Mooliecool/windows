...
function indxf13 () {
  ...
  @end
    ...
    apInitScenario("indxf( Null, Null) ");
    ...
    apInitScenario("indxf( Null, string) ");

    stExp = -1;
    stAct = "".indexOf(bldStr(129, " "));
    if (stAct != stExp) {
	apLogFailInfo("", stExp, stAct, stBug);
    }

    apInitScenario("indxf( string, Null) ");

    stExp = 1;
    stAct = ("" + bldStr(257, ".")).indexOf( "", 1 );
    if (stAct != stExp) {
	apLogFailInfo("", stExp, stAct, stBug);
    }

    apInitScenario("indxf( start, A, B) ");

    stExp = 6;
    stAct = ("xxxxxxxx" + "yyyyyyyyyyyyy").indexOf( "xxyyy" , 6);
    if (stAct != stExp) {
	apLogFailInfo("", stExp, stAct, stBug);
    }

    apInitScenario("indxf( start, A, B, override) ");

    stExp = 517;
    stAct = ("" + bldStr(1025, "Q")).indexOf("QQQQ",517);
    if (stAct != stExp) {
	apLogFailInfo("", stExp, stAct, stBug);
    }

    apInitScenario("indxf( max, A )  ");
