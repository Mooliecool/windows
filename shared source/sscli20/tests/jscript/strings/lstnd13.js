...
function lstnd13 () {
    ...
    apInitScenario("lstnd( Null, Null) ");
    ...
    apInitScenario("lstnd( Null, string) ");

    stExp = -1;
    stAct = "".lastIndexOf(bldStr(129, " "));
    if (stAct != stExp) {
	apLogFailInfo("", stExp, stAct, stBug);
    }

    apInitScenario("lstnd( string, Null) ");

    stExp = 1;
    stAct = ("" + bldStr(257, ".")).lastIndexOf( "", 1 );
    if (stAct != stExp) {
	apLogFailInfo("", stExp, stAct, "VBE 13693");
    }

    apInitScenario("lstnd( start, A, B) ");

    stExp = 6;
    stAct = ("xxxxxxxx" + "yyyyyyyyyyyyy").lastIndexOf( "xxyyy" ,11)
    if (stAct != stExp) {
	apLogFailInfo("", stExp, stAct, stBug);
    }

    apInitScenario("lstnd( start, A, B, override) ");

    stExp = 517;
    stAct = (bldStr(1025, "Q")).lastIndexOf("QQQQ",517);
    if (stAct != stExp) {
	apLogFailInfo("", stExp, stAct, stBug);
    }

    apInitScenario("lstnd( max, A )  ");

    stExp = 4;
    stAct = ("aaabaaa" + bldStr(31 * 1024 - 256, "\n")).lastIndexOf("aaa",9);
    if (stAct != stExp) {
	apLogFailInfo("", stExp, stAct, stBug);
    }
