...
function ucase04 () {
    ...
    var Inc = 16;
    var MaxString = 240000;

    apInitScenario( "Scenario 1 - Use ucase$ with string variable");

    Result = bldStr(MaxString, "a").toUpperCase();

    Flag = 0;
    I = 1;

    apWriteDebug("Please wait - this takes some time");
    while (I <= MaxString - 50 && Flag == 0) {
      if (Result.substring(I-1,I) != "A" ) {
       Flag = 1;
      }
      I = I + Inc;
    }
    ...
    apInitScenario( "Check last 50 chars");
    while (I <= MaxString && Flag == 0) {
      if (Result.substring(I-1,I) != "A" ) {
       Flag = 1;
      }
      I = I + 1;
    ...
    if ( Flag != 0 ) {
      apLogFailInfo( "(ucase with 64k var failed)", "", "", "");
    }


	    apInitScenario( "Scenario 1b - Use ucase$ with DBCS string variable");
    if(g_OnDBCS)
    {
	    Result = bldStr(MaxString, "‚¤").toUpperCase();
