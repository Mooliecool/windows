...
**          Call connect with an incompatible address family.
**
** 
...
int __cdecl main(int argc, char *argv[])
{
    ...
    mySockaddr.sin_family = AF_UNIX; 
    mySockaddr.sin_port = getRotorTestPort();
    mySockaddr.sin_addr.S_un.S_addr = inet_addr("127.0.0.1");
    memset( &(mySockaddr.sin_zero), 0, 8);

    /*connect to a stream server with incompatible address family*/
    ...
    if(WSAEAFNOSUPPORT != GetLastError() || SOCKET_ERROR != err)
    {
        Trace("\nFailed to call connect API for a negative test "
                "by passing incompatible address family!\n");
