...
**          Connect to a server which is inactive. 
**
** 
...
int __cdecl main(int argc, char *argv[])
{
    ...
    /*connect to a stream server which is inactive*/
    err = connect(socketID, (struct sockaddr *)&mySockaddr, 
                sizeof(struct sockaddr));
    if(WSAECONNREFUSED != GetLastError() || SOCKET_ERROR != err)
    {
        Trace("\nFailed to call connect API for a negative test, "
            "connect an inactive server!\n");
