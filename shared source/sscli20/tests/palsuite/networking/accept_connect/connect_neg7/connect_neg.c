...
**          Call connect by passing a listening socket.
**
** 
...
int __cdecl main(int argc, char *argv[])
{
    ...
    if(SOCKET_ERROR == err)
    {
        Trace("\nFailed to call listen API to setup the backlog number!\n");
        err = closesocket(socketID);
        if(SOCKET_ERROR == err)
        {    
            Trace("\nFailed to call closesocket API!\n");
    ...
    /*connect to a stream server by passing a listening socket*/
