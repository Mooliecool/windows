...
**          with a created stream socket by passing an invalid 
**          parameter (pass a smaller address space).
**
** 
...
int __cdecl main(int argc, char *argv[])
{
    ...
    /*pass zero as the size of struct to generate error*/
    err = bind(socketID,(struct sockaddr *)&mySockaddr,0);
    
    if(WSAEFAULT!= GetLastError() || SOCKET_ERROR != err)
    {
        Trace("\nFailed to call bind API for a negative test "
                "by passing a small size of struct!\n");
