...
**          with a created stream socket. Bind an invalid socket.
**
** 
...
int __cdecl main(int argc, char *argv[])
{
    ...
    //bind address with an invalid socket*/
    err = bind(InvalidSocketID,(struct sockaddr *)&mySockaddr,sizeof(struct sockaddr));

    if(WSAENOTSOCK != GetLastError() || SOCKET_ERROR != err)
    {
        Trace("\nFailed to call bind API for a negative test by passing "
