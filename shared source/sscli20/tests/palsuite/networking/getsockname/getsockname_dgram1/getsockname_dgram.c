...
**          the local name for a connected datagram socket
**
** 
...
int __cdecl main(int argc, char *argv[])
{
    ...
    if(SOCKET_ERROR == err)
    {
        Trace("\nFailed to call connect API to connect a "
                "datagram server!, error code=%d\n", GetLastError());
    ...
    if(SOCKET_ERROR == err)
    {
        Trace("\nFailed to call getsockname API! error code=%d\n",
                GetLastError());
        if(SOCKET_ERROR == err)
        {    
            Trace("\nFailed to call closesocket API! error code=%d\n",
