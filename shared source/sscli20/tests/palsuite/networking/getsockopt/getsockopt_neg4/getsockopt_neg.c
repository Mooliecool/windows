...
int __cdecl main(int argc, char *argv[])
{
    ...
    /*
    pass 100 as socket level which is not supported
    to generate error code 10022 for Winsock2
    */
    err = getsockopt(SocketID, 100, SO_SNDTIMEO, (char *)nValue, &size);
    if(WSAEINVAL != GetLastError() || SOCKET_ERROR != err)
    {  
      /*no error occured*/
        Trace("\nFailed to call getsockopt API for negative test "
                "by passing not supported socket level!\n");
