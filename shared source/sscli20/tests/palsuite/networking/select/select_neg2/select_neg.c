...
** Source: select_neg.c
**
** Purpose: Negatively test the select API for readable socket 
**          when waiting time is zero. If no readable socket is ready, 
**          the select call will return 0 immediately.
**          Monitor the socket set which contains the entry that is
**          not a socket.
**
** 
...
int __cdecl main(int argc, char *argv[])
{
    ...
    /*add an invlide socket descriptor to this socket set*/
    FD_SET(InvalidSocketID, &readfds);

    /*monitor the readable socket set which contains an invalid socket*/
    socketFds = select(0, &readfds, NULL, NULL, &tv);

    if(WSAENOTSOCK != GetLastError() || SOCKET_ERROR != socketFds) 
    {
        Trace("\nFailed to call select API for a negative test, "
                "call select to monitor readable socket set which "
                "contains an invalid socket!\n");
        err = closesocket(socketID);
        if(SOCKET_ERROR == err)
        {    
            Trace("\nFailed to call closesocket API!\n");
