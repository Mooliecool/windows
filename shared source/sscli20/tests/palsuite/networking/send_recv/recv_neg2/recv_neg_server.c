...
**            started before related client started.
**            This test case is to test recv with an invalid buf
**            parameter.
**
** 
...
int __cdecl main(int argc, char *argv[])
{
    ...
    /*retrive data with invalid buf parameter(NULL)*/
    err = recv(aSocket, NULL, BUFFERSIZE, 0);
    if(WSAEFAULT != GetLastError() || SOCKET_ERROR != err)
        {    
        Trace("\nFailed to call recv API for a negative test "
            "retrive data with invalid buffer parameter!\n");
