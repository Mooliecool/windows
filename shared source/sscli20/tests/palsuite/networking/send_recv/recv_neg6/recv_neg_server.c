...
int __cdecl main(int argc, char *argv[])
{
    ...
    /*set the socket non-blocking for new created child socket*/
    err = ioctlsocket(aSocket, FIONBIO, &argp);
    if(SOCKET_ERROR == err)
    {
        ...
        if(SOCKET_ERROR == err)
        {    
            Trace("\nFailed to call closesocket API to close child socket!\n");
            }
        err = closesocket(socketID);
        if(SOCKET_ERROR == err)
            {
            Trace("\nFailed to call closesocket API to close listening socket!\n");
        }
        err = WSACleanup();
        if(SOCKET_ERROR == err)
        {
    ...
    /*retrive data from a non-blocking socket*/
    err = recv(aSocket,data,BUFFERSIZE,0);
    
    if(WSAEWOULDBLOCK != GetLastError() || SOCKET_ERROR != err)
        {    
        Trace("\nFailed to call recv API for a negative test, returned %d "
              "(expected:%d) set LastError to %d (expected %d)\n", 
              err, SOCKET_ERROR, GetLastError(), WSAEWOULDBLOCK);

        err = closesocket(aSocket);/*close the child socket*/
            if(SOCKET_ERROR == err)
            {
            Trace("\nFailed to call closesocket API!\n");
        }
