...
**          This test case is to test the send API by sending
**          a non-zero length data through a non-connected
**          stream socket. 
**
** 
...
int __cdecl main(int argc, char *argv[])
{
    ...
    /*send data to server through a not-connected socket*/
    err = send(socketID, data, nBuffer, 0);
    if(WSAENOTCONN != GetLastError() || SOCKET_ERROR != err)
    {
        Trace("\nFailed to call send API for a negative test by passing "
            "not-connected socket, an error WSAENOTCONN is expected, "
