...
**          This test case is to test send and recv non-zero length
**          data with flag = 0.
**
** 
...
int __cdecl main(int argc, char *argv[])
{
    ...
    char *dataVerify = "None-zero length data test";
    ...
    /*retrive data from the new created socket*/
    ...
    if((strcmp(data, dataVerify)) != 0)
    {    
        Trace("\nData recieved \"%s\", is not equal to expected"
              " data \"%s\"\n");
        err = closesocket(aSocket);/*close the child socket*/
            if(SOCKET_ERROR == err)
            {
