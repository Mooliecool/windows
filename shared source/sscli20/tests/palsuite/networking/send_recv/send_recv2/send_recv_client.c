...
**            This test case is to test send and recv zero length
**            data with flag = 0.
**
** 
...
int __cdecl main(int argc, char *argv[])
{
    ...
    /*send a zero length data to server*/
    err = send(socketID,data,nBuffer,0);
    if(SOCKET_ERROR == err)
    {
        Trace("\nFailed to call send API to send a zero length "
                "data to a server!\n");
        err = closesocket(socketID);
        if(SOCKET_ERROR == err)
        {    
