...
int __cdecl main(int argc, char *argv[])
{
    ...
    /*send data to server*/
    err = send(socketID, data, nBuffer, MSG_OOB|MSG_DONTROUTE);
    if(SOCKET_ERROR == err)
    {
        Trace("\nFailed to call send API to send data to a server "
                "with flag MSG_OOB|MSG_DONTROUTE!\n");
        err = closesocket(socketID);
        if(SOCKET_ERROR == err)
        {    
            Trace("\nFailed to call closesocket API!\n");
    ...
    /* wait a few seconds to close socket so that server can read the OOB data */
    Sleep(3000);
    err = closesocket(socketID);
    if(SOCKET_ERROR == err)
    {
