...
**            Note: 
**              Test to send and recv the non-zero length data with 
**              flag = 0 through a connected datagram socket.
**
** 
...
int __cdecl main(int argc, char *argv[])
{
    ...
    /*create a datagram(UDP) socket in AF_INET domain*/
    ...
    /*connect to a datagram server*/
    err = connect(socketID, (struct sockaddr *)&mySockaddr,
                sizeof(struct sockaddr));
    if(SOCKET_ERROR == err)
    {
        Trace("\nFailed to call connect API to connect a datagram "
            "server!\n");
    ...
    /*send non-zero length data to server*/
    ...
    /*further check the total data-send number is correct*/
    
    if(strlen(data) != err)
    {
        Trace("\nFailed to call send API to send data to a server, "
            "the sending out data number is not correct\n");
        err = closesocket(socketID);
        if(SOCKET_ERROR == err)
        {
            Trace("\nFailed to call closesocket API!\n");
