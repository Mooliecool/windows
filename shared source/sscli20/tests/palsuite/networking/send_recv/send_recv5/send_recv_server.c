...
**          This test case is to test send and recv 
...
int __cdecl main(int argc, char *argv[])
{
    ...
    /*set the connected socket as non-blocking socket*/
    ...
    /*delay 2 sec and let client side send data*/
    Sleep(2000);

    /*retrieve data from the connected socket*/
    err = recv(aSocket, data, BUFFERSIZE - 1, 0);
    ...
    /*cast data back to the struct*/
    mydata = (struct stru *)data;

    /*further check the retrieved value is valid*/
    if(mydata->i != INT_MAX  ||
       mydata->ul != UINT_MAX)
    {
        Trace("\nthe retrieved data is not equal to the one that "
            "is sent from client\n");
