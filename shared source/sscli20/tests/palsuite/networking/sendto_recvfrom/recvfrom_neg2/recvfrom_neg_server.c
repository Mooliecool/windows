...
**          This test case is to test recvfrom by passing an
...
int __cdecl main(int argc, char *argv[])
{
    ...
    {
        Trace("\nFailed to call select API to "
                "monitor readable socket set!\n");
        err = closesocket(socketID);
        if(SOCKET_ERROR == err)
        {    
            Trace("\nFailed to call closesocket API!\n");
    ...
    err = recvfrom(socketID, NULL, BUFFERSIZE, 0,
    ...
    if(WSAEFAULT != GetLastError() || SOCKET_ERROR != err)
    {
        Trace("\nFailed to call recvfrom API for a negative test "
            "by passing invalid buffer parameter!\n");
        err = closesocket(socketID);
        if(SOCKET_ERROR == err)
        {    
            Trace("\nFailed to call closesocket API!\n");
