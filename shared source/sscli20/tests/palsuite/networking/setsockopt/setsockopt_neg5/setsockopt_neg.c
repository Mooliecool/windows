...
**          Call getsockopt with NULL optval.
**
** 
...
int __cdecl main(int argc, char *argv[])
{
    ...
    /*pass a NULL optval to generate an error*/
    err = setsockopt(SocketID, SOL_SOCKET, SO_SNDTIMEO, (char *)NULL, size);
    if(WSAEFAULT != GetLastError() || SOCKET_ERROR != err)
    {  
        Trace("\nFailed to call Setsockopt API for negative test, "
                "call setsockopt by passing a NULL optval, "
                "an error WSAEFAULT is expected, but no error or no "
