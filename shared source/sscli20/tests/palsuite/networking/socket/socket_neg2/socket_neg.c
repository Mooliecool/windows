...
**          by passing AF_UNIX which is not supported by the
**          protocol family.
**
** 
...
int __cdecl main(int argc, char *argv[])
{
    ...
    /*create a stream socket in Unix domain*/
    SocketID = socket(AF_UNIX, SOCK_STREAM, IPPROTO_TCP);
    if( (WSAEAFNOSUPPORT != GetLastError() &&
         WSAEPROTONOSUPPORT != GetLastError()) ||
        INVALID_SOCKET != SocketID)
    {
        /*or no error occured*/
        Trace("\nFailed to call socket API for a negative test "
            "by passing invalid domain!\n");
