/*=============================================================================
**
** Source: test6.c (WSASend, WSARecv)
**
** Purpose: This test tests WSAEventSelect with FD_CONNECT in order to handle 
**          a pending connect call. After WSAEventSelect is called, 
**          it calls connect. Connect may return immediately with the
...
INT __cdecl main(INT argc, CHAR **argv)
{
    ...
    */
    ...
    if( err != SOCKET_ERROR )
    {
        if (dwNbrOfByteRecv!=255 )
        {
            Trace("Server error: WSARecv() "
                  "didn't returned an error (%d)"
                  "but dwNbrOfByteRecv (%d) != 255 as expected\n",
                  err, dwNbrOfByteRecv);
...
{     
   ...
   /* Wait 10 seconds for ReadEvent to be signaled 
      ...
      accept can be done. If connect return immediately
      then hWriteEvent should be in a signaled state and
      WaitForSingleObject should return immediately too.
   */
