/*=============================================================================
**
** Source: wsasendto5_client.c (WSASendTo, WSARecvFrom)
**
** Purpose: Test to ensure that WSASento returns the WSAESHUTDOWN
**          if the socket is shutdown before sending.
** 
    ...
    else
    {
        ...
        */
        ...
        /* On some platforms (e.g. AIX) shutdown(s,...) fails with errno=ENOTCONN
           ...
           simulate the shutdown feature setting the socket send and/or receive buffer
           to 0, which causes any further send/receive to fail.
           Anyway the failure will be a WSAEMSGSIZE rather than a WSAESHUTDOWN one.
        */
        if ((dwLastError != WSAESHUTDOWN) && (dwLastError != WSAEMSGSIZE))
        {        
            Trace("ERROR: "
                  "WSASendTo(...) sets LastError to %d expected WSAESHUTDOWN\n",
                  dwLastError );
