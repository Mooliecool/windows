    ...
    BOOL fTruncateFileName)     // TRUE: drop last component of path
{
    ...
    if ((strlen(pszDir) + sizeof(PATH_SEPARATOR) + strlen(pszRelPath) + 1) > DB_MAX_PATH_LENGTH) {
        BuildError(
            "%s%s%s : Path too long - need to rebuild build.exe with larger DB_MAX_PATH_LENGTH\n",
            pszDir,
            PATH_SEPARATOR,
            pszRelPath);
        return NULL;
    ...
    if (pszDir[0] == '\0') {
        ...
        _snprintf(path, sizeof(path) - 1, "%s%s%s", pszDir, PATH_SEPARATOR, pszRelPath);
    }
    ...
    DIRREC **ppdr)
{
    ...
    if (FindFirstPathSeparator(pszRelPath) != NULL) {
        // There's a path component in this filename.  Let's see where it points to.
        if ( IsPathSeparator(pszRelPath[0])   /* Absolute from root or UNC Path */
#if !PLATFORM_UNIX
    ...
    UCHAR SrcFlags)
{
    ...
    AllocMem(sizeof(SOURCEREC), (VOID **) &psr, MT_SOURCEDB);
...
FreeIncludeDB(INCLUDEREC **ppir)
{
    if (*ppir != NULL) {
        ...
        pirNext = pir->Next;
        SigCheck(pir->Sig = 0);
        FreeMem((VOID **) ppir, MT_INCLUDEDB);
        *ppir = pirNext;
    }
}
...
FreeDirDB(DIRREC **ppdr)
{
    if (*ppdr != NULL) {
        ...
        AssertDir(pdr);
        FreeDirData(pdr);
        FreeConsumeDependencies(pdr);
        while (pdr->Files) {
            FreeFileDB(&pdr->Files);
        }
        pdrNext = pdr->Next;
        SigCheck(pdr->Sig = 0);
        FreeMem((VOID **) ppdr, MT_DIRDB);
        *ppdr = pdrNext;
    }
}
    ...
    )
{
    ...
    AllocMem(sizeof(DIRREC) + strlen(DirName), (VOID **) &DirDB, MT_DIRDB);
...
PrintSourceDBList(SOURCEREC *psr, int i)
{
    ...
    for ( ; psr != NULL; psr = psr->psrNext) {
        ...
        BuildMsgRaw(
            "    %s%s%s%s%s",
            (psr->SourceSubDirMask & TMIDIR_PARENT)? ".." PATH_SEPARATOR : "",
            (psr->SourceSubDirMask & ~TMIDIR_PARENT)?
...
{   { "/0dirs",       szMakefile,  FALSE, 0,    DIRDB_DIRS },
    ...
    { ".mcpp",        szClang,     TRUE,  FILEDB_SOURCE | FILEDB_C | FILEDB_MCPP,  0 },
    ...
    { ".jsl",         szClang,     TRUE,  FILEDB_SOURCE | FILEDB_JSHARP,  0 },
// MUST BE LAST
    { NULL,           "",          FALSE, 0,                            0 }
};
...
#if PLATFORM_UNIX
    if (pszExt == NULL && strcmp(pszFile, "build") && strcmp(pszFile, "nmake"))
        pszExt = ".h";
#else
    if (pszExt == NULL)
        pszExt = ".h";
    ...
    ULONG  FileFlags)
{
    ...
    AllocMem(sizeof(FILEREC) + strlen(FileName), (VOID **) &FileDB, MT_FILEDB);
...
LinkToCycleRoot(INCLUDEREC *pirOrg, FILEREC *pfrCycleRoot)
{
    ...
    if (DEBUG_1) {
        BuildMsgRaw(
            "%x CycleAlloc  %s" PATH_SEPARATOR "%s <- %s" PATH_SEPARATOR "%s\n",
            pir,
    ...
    while ((pirT = pirList) != NULL) {
        ...
        for (ppir = &pfr->IncludeFilesTree;
            ...
            if (pirT->pfrInclude == pir->pfrInclude) {
                ...
                if (DEBUG_1) {
                    BuildMsgRaw(
                        "%x CycleOrphan %s" PATH_SEPARATOR "%s <- %s" PATH_SEPARATOR "%s\n",
                        pirT,
        ...
        if (*ppir == NULL) {
            *ppir = pirT;
            ...
            if (DEBUG_1) {
                BuildMsgRaw(
                    "%x CycleRoot   %s" PATH_SEPARATOR "%s <- %s" PATH_SEPARATOR "%s\n",
                    pirT,
                    pirT->pfrInclude->Dir->Name,
                    pirT->pfrInclude->Name,
...
RemoveFromCycleRoot(INCLUDEREC *pir, FILEREC *pfrRoot)
{
    ...
    if (pfrRoot != NULL) {
        ...
        if (DEBUG_1) {
            BuildMsgRaw(
                "%x CycleUnroot %s" PATH_SEPARATOR "%s <- %s" PATH_SEPARATOR "%s\n",
                pir,
                pir->pfrInclude->Dir->Name,
                pir->pfrInclude->Name,
                pfrRoot->Dir->Name,
    ...
    BuildError(
        "%s" PATH_SEPARATOR "%s: %x %s: not on cycle root's list\n",
        pir->pfrCycleRoot->Dir->Name,
        pir->pfrCycleRoot->Name,
        pir,
        pir->Name);
    ...
    INCLUDEREC *IncludeDB)
{
    ...
    while (n--) {
        ...
        if (pfr != NULL) {
            if (DEBUG_1) {
                BuildMsgRaw(
                    "Found include file %s" PATH_SEPARATOR "%s\n",
                    pfr->Dir->Name,
                    pfr->Name);
            }
            return(pfr);
...
SaveMasterDB(VOID)
{
    ...
    DirDBNext = &AllDirs;
    fprintf(fh, "V %x" EOL, BUILD_VERSION);
    while ((DirDB = *DirDBNext)) {
        fprintf(fh, "D \"%s\" %x" EOL, DirDB->Name, DirDB->DirFlags);
        FileDBNext = &DirDB->Files;
        while ((FileDB = *FileDBNext)) {
    ...
    )
{
    ...
    *s++ = '\0';
    ...
    if (!AToX(&s, &FileFlags) ||
        !AToX(&s, &Attr) ||
        !AToX(&s, &DateTime.value) ||
        !AToD(&s, &SourceLines) ||
        !AToD(&s, &Version) ||
        FindFirstPathSeparator(FileName) != NULL) {
        return(NULL);
    }
    AllocMem(sizeof(FILEREC) + strlen(FileName), (VOID **) &FileDB, MT_FILEDB);
    memset(FileDB, 0, sizeof(*FileDB));
