    ...
    )
{
    ...
    //
    if ( 
       TestPrefixPath( &p, "rc ") || TestPrefixPath( &p, "rc.exe ")
       || TestPrefixPath( &p, "resourcecompiler ") || TestPrefixPath( &p, "resourcecompiler.exe ")
       ) 
    {
        if (*p == ':')
    ...
    // GCC will be one of these...
    if ( TestPrefixPath( &p, "gcc " ) || TestPrefixPath( &p, "cc " ) ) {
    ...
    if ( TestPrefixPath( &p, "vjc " ) || TestPrefixPath( &p, "vjc.exe " ) ) {        
        ThreadState->ChildState = STATE_LINKING;
        ThreadState->ChildFlags = 0;
        ThreadState->ChildTarget = "all platforms";
