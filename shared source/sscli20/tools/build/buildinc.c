...
CreateRelativePath(
    ...
    //
    //       1) Found common ancestor path ( *PathSeparatorIndex is a path separator )
    //       2) Don't have common ancestor ( *PathSeparatorIndex is not a path separator )
    //

    if ( !IsPathSeparator(Source[ PathSeparatorIndex ]) ) {
