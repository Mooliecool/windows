...
//+---------------------------------------------------------------------------
//
//  Function:   ComputePrecompiledHeaderPath
//
//  Synopsis:   Computes the path name of the precompiled header for a given
//              TargetMachine and CompileDir
//
//  Arguments:  [DirDB]              -- The current CompileDir
//              [TargetMachineIndex] -- Index of the TargetMachine
//              [PchPath]            -- Place to put constructed name
//
//----------------------------------------------------------------------------

static
void
ComputePrecompiledHeaderPath (
    PDIRREC DirDB,
    int TargetMachineIndex,
    char PchPath[DB_MAX_PATH_LENGTH]
)
{
    FormatLinkTarget(
        PchPath,
        TargetMachines[TargetMachineIndex]->ObjectDirectory,
        DirDB->TargetPath,
        DirDB->PchObj,
        "");
}
...
VOID
ProcessSourceDependencies(DIRREC *DirDB, DIRSUP *pds, DOS_TIME DateTimeSources)
{
    TARGET *Target;
    DOS_TIME DateTimePch;    // Actual timestamp of pch preserved here.
    UINT i;
    SOURCEREC *apsr[3];
    SOURCEREC **ppsr;
    ...
    if (fFirstScan && fClean && !fKeep) {
        ...
        for (i = 0; i < CountTargetMachines; i++) {
            ...
            if (DirDB->PchObj) {
                char path[DB_MAX_PATH_LENGTH];
                ComputePrecompiledHeaderPath (DirDB, i, path);
                // On OSX, precompiled headers are directories, and so won't
                // get deleted by that last DeleteMultipleFiles. This could
                // also occur on other unixes using gcc in the future.
                DeleteSingleFileOrDirectory(NULL, path, TRUE);
            }
#endif
            ...
            if (pds->PchTarget != NULL)
            {
                ...
                //
                if ((*pDateTimeFile)(TargetDir, pds->PchTarget).value <= BuildStartTime.value)
                {
                    DeleteSingleFileOrDirectory(TargetDir, pds->PchTarget, FALSE);
                    if (DirDB->PchObj != NULL) {
                        ExpandObjAsterisk(path,
                                          DirDB->PchObj,
                                          TargetMachines[i]->ObjectDirectory);
                        DeleteSingleFileOrDirectory(NULL, path, FALSE);
                    } else {
                        char *p;
                        ...
                        if (p != NULL && strcmp(p, ".pch") == 0) {
                            strcpy(p, ".obj");
                            DeleteSingleFileOrDirectory(TargetDir, path, FALSE);
                        }
                    }
                }
            }
                    ...
                    if (pfr->FileFlags & FILEDB_PASS0)
                    {
                        ...
                        for (j = 0;
                            ...
                            if ((psr->SrcFlags & SOURCEDB_FILE_MISSING) ||
                                (Target->DateTime.value == 0) ||
                                ((pfr->FileFlags & FILEDB_C) &&
                                Target->DateTime.value < DateTimePch.value))
                            {
                                fNeedCompile = TRUE;
                            }
        ...
        (fClean && !fKeep))
    {
        ...
        if (_strnicmp(DirDB->TargetPath, pszObjDirSlash, strlen(pszObjDirSlash) -1)) {
            for (i = 0; i < CountTargetMachines; i++) {
                FormatLinkTarget(
                    path,
                    TargetMachines[i]->ObjectDirectory,
    ...
    if (DirDB->TargetPath != NULL &&
        ...
        for (i = 0; i < CountTargetMachines; i++) {
            ...
            else if (DirDB->DirFlags & DIRDB_SYNC_PRODUCES) {
                // If this dir is the producer, we need to recompile it,
                // so that it can signal the event for the consumers to continue.
                DirDB->DirFlags |= DIRDB_COMPILENEEDED;
            }
            else
    ...
    for (i = 0; i < CountTargetMachines; i++) {
        ...
        if (!fPassZero && (pds->PchInclude != NULL || pds->PchTarget != NULL)) {
            ...
            if (!CanonicalizePathName(path, CANONICALIZE_DIR, path)) {
                DateTimePch.value = ULONG_MAX;        // always out of date
            ...
            DateTimePch.value = Target->DateTime.value;

            if (DateTimePch.value == 0) {             // Target doesn't exist
                DateTimePch.value = ULONG_MAX;        // Always out of date
            }
            ...
            if (fClean && !fKeep && fFirstScan) {
                ...
                //         "PRECOMPILED_INCLUDE.\n", szFullPath);
                Target->DateTime.value = 0;           // Don't delete pch target
            }
            else {
                FILEREC *pfrPch = NULL;
                ...
                if (pfrPch != NULL) {
                    ...
                    switch(0) {
                      default:
                        ...
                        fCase2 = (Target->DateTime.value == 0);
                        if ( fCase2 ) {
                            fNeedCompile = TRUE;
                            break;
                        }
            ...
            // rebuilt.
            //
            if (Target->DateTime.value != 0) {
                DeleteSingleFileOrDirectory(NULL, Target->Name, FALSE);
                if (DirDB->PchObj != NULL) {
                    ExpandObjAsterisk(
                                path,
                                DirDB->PchObj,
                            ...
                            j++)
                        {
                            ...
                            switch(0) {
                              default:
                                ...
                                fCase3 = (Target->DateTime.value == 0);
                                if ( fCase3 ) {
                                    fNeedCompile = TRUE;
                                    break;
                                }
                                fCase4 = ((pfr->FileFlags & FILEDB_C) && Target->DateTime.value < DateTimePch.value);
                                if ( fCase4 ) {
                                    fNeedCompile = TRUE;
                                    break;
                                }
                            ...
                            if ( fNeedCompile )
                            {
                                ...
                                if (Target->DateTime.value != 0 && !fKeep)
                                {
                                    DeleteSingleFile(NULL, Target->Name, FALSE);
                                }
...
ScanSourceDirectories(LPSTR DirName)
{
    ...
    BOOL DirsPresent = FALSE;
    DOS_TIME DateTimeSources;
    UINT i;

    DateTimeSources.value = 0;
    if (DEBUG_4) {
        BuildMsgRaw(
    ...
    if (DirDB->DirFlags & DIRDB_SOURCES) {
        ...
        if (fFirstScan)
        {
            AllocMem(sizeof(DIRSUP), (VOID **) &pds, MT_DIRSUP);
        ...
        else
        {
            ...
            DateTimeSources.value = pds->DateTimeSources.value;
    ...
    if (!fLinkOnly) {
        ...
        if ((DirDB->DirFlags & DIRDB_COMPILENEEDED) &&
            ...
            if (DirDB->CountOfFilesToCompile) {
                ...
                BuildMsgRaw(
                    "- %d source file%s (%s lines)\n",
                    DirDB->CountOfFilesToCompile,
                    (DirDB->CountOfFilesToCompile == 1 ? "" : "s"),
...
{
    PDIRREC DirDB;
    LPSTR SavedCurrentDirectory = NULL;
    UINT i;
    PCHAR s;
    ...
    for (i = 0; i < CountPassZeroDirs; i++) {
        ...
        if (DirDB->DirFlags & DIRDB_DIRS) {
            if (DirDB->DirFlags & DIRDB_MAKEFIL0) {
                ...
                strcat( MakeParametersTail, " BUILD_PHASE=");
                strcat( MakeParametersTail, pszBuildPhasePostString);                
        ...
        if (DirDB->DirFlags & DIRDB_DIRS) {
            ...
            if (fQuicky && DirDB->PchObj) {
                for (j = 0; j < CountTargetMachines; j++) {
                    ComputePrecompiledHeaderPath (DirDB, j, path);
            ...
            if (fQuery) {
                ...
                CurrentCompileDirDB = DirDB;
                RecurseLevel = DirDB->RecurseLevel;
                if (!(DirDB->DirFlags & DIRDB_SYNC_CONSUMES) || ((DirDB->DirFlags & DIRDB_SYNC_CONSUMES) && (DirDB->DirFlags & DIRDB_SYNCHRONIZE_BLOCK )))
                {   
                    ExecuteProgram(                                 
                                    MakeProgram,
                                    MakeParameters,
        ...
        while(checkAgain) { //to check the waiting consumer list from the start in case even one of the consumer Directory fire
            ...
            while (consumerListPtr != &Head ) {
                ...
                while (List != &waitingConsumer->DirDB->Consumes) {
                    ...
                    if (!Wait->Dependency->Done) {
                        if (finalCall){ 
                            ...
                            if (DEBUG_1) {
                                BuildMsgRaw("CheckAllConsumer: Waiting for dependency '%s'.\n", Wait->Dependency->Name);
                            }
                ...
                if (CanBeFired) {
                    ...
                    ExecuteProgram(MakeProgram,waitingConsumer->MakeParameters,MakeTargets,FALSE);
                    PopCurrentDirectory(SavedCurrentDirectory);             
    ...
    LPSTR    pszSourceDir)
{
    ...
    if (pfr != NULL) {
        ...
        if (p[0] == '.' && p[1] == '\0') {
            ...
            sprintf(path, "%s" PATH_SEPARATOR "%s", p, p1);
        }
    ...
    AllocMem(sizeof(TARGET) + strlen(p), (VOID **) &Target, MT_TARGET);
    strcpy(Target->Name, p);
    Target->pdrBuild = pdrBuild;
    Target->DateTime.value = (*pDateTimeFile)(NULL, p).value;
    ...
    LPSTR TargetExt)
{
    ...
    sprintf(path, "%s" PATH_SEPARATOR "%s" PATH_SEPARATOR "%s%s", TargetPath, p1, TargetName, TargetExt);
}
    ...
    for (i = 0; i < CountTargetMachines; i++) {
        ...
        if (DirDB->DirFlags & DIRDB_DIRS) {
            if (fDebug && (DirDB->DirFlags & DIRDB_MAKEFILE)) {
                BuildError(
                    "%s" PATH_SEPARATOR "makefile. unexpected in directory with DIRS file\n",
                    DirDB->Name);
            }
            if ((DirDB->DirFlags & DIRDB_SOURCES)) {
                BuildError(
                    "%s" PATH_SEPARATOR "sources. unexpected in directory with DIRS file\n",
                    DirDB->Name);
                BuildError("Ignoring %s" PATH_SEPARATOR "sources.\n", DirDB->Name);
...
IncludeError(TARGET *pt, FILEREC *pfr, INCLUDEREC *pir, LPSTR pszError)
{
    ...
    BuildError("%s" PATH_SEPARATOR "%s: ", pt->pfrCompiland->Dir->Name, pt->pfrCompiland->Name);
    if (pt->pfrCompiland != pfr) {
        if (pt->pfrCompiland->Dir != pfr->Dir) {
            BuildErrorRaw("%s" PATH_SEPARATOR, pfr->Dir->Name);
        }
    ...
    FILEREC **ppfrRoot)
{
    ...
    *ppfrRoot = NULL;
    ...
    if (CheckDate &&
        ...
        CheckVersion = FALSE;
        FileDB->DateTimeTree.value = 1;       // never out of date
    }

    if (FileDB->IncludeFiles == NULL && FileDB->DateTimeTree.value == 0) {
    ...
    if (CheckDate &&
        (Target->DateTime.value < FileDB->DateTime.value ||
         Target->DateTime.value < FileDB->DateTimeTree.value)) {
        if (Target->DateTime.value != 0) {
            if (DEBUG_1 || fShowOutOfDateFiles) {
                BuildMsg("%s is out of date with respect to %s" PATH_SEPARATOR "%s.\n",
                         Target->Name,
    ...
    if (FileDB->DateTimeTree.value == 0) {
        ...
        *ppirTree = NULL;       // truncate any links from previous sequence
        FileDB->DateTimeTree.value = FileDB->DateTime.value;
...
rescan:
        for (IncludeDBNext = &FileDB->IncludeFilesTree;
            ...
            if (IncludeDB->pfrInclude != NULL) {
                ...
                if (IncludeDB->Version != IncludeDB->pfrInclude->Version) {
                    if (CheckVersion) {
                        if (DEBUG_1 || fShowOutOfDateFiles) {
                            BuildError(
                                 "%s (v%d) is out of date with "
                                         "respect to %s" PATH_SEPARATOR "%s (v%d).\n",
                                 FileDB->Name,
                                 IncludeDB->Version,
                                 IncludeDB->pfrInclude->Dir->Name,
                                 IncludeDB->pfrInclude->Name,
                        ...
                        FileDB->DateTimeTree.value = ULONG_MAX; // always out of date
                        fOutOfDate = TRUE;
                    }
                    else
                ...
                // the cycle, then process the tree again from cycle root.
                    ...
                    *IncludeDBNext = IncludeDB->NextTree;
                    ...
                    if ((*ppfrRoot)->DateTimeTree.value < FileDB->DateTime.value) {
                        (*ppfrRoot)->DateTimeTree.value = FileDB->DateTime.value;
                ...
                if (FileDB->DateTimeTree.value < IncludeDB->pfrInclude->DateTimeTree.value)
                {
                    FileDB->DateTimeTree.value = IncludeDB->pfrInclude->DateTimeTree.value;
            ...
            else
            {
                ...
                //
                DOS_TIME Time = (*pDateTimeFile)(NULL, IncludeDB->Name);
                if (FileDB->DateTimeTree.value < Time.value)
                {
                    FileDB->DateTimeTree.value = Time.value;
                    //
    ...
    )
{
    ...
    if (DEBUG_4)
    {
        BuildError("WriteObjectsDefinition DirName: %s\n", DirName);
    }
    ...
    for (iPass = 1; iPass >= 0; iPass--)
    {
        ...
        while ((psr = PickFirst(&psrComCopy, &psrMachCopy)) != NULL) {
            ...
            if (DEBUG_4)
            {
                BuildError("WriteObjectsDefinition PSR found\n");
            }
            ...
            if (DEBUG_4)
            {
                BuildError("WriteObjectsDefinition psr->SrcFlags & SOURCEDB_SOURCES_LIST\n");
                BuildError("WriteObjectsDefinition FileName %s\n", psr->pfrSource->Name);
            }
            ...
            while ((pData = GetTargetData(pszextsrc, iPass, i, pds->IdlType)))
            {
                if (DEBUG_4)
                {
                    BuildError("WriteObjectsDefinition TargetData %d\n", i);
                }
                if (pData == &DefaultData)
                {
                    ...
                    else 
                    {
                        ...
                        case 'c':    // C file (.c, .cxx, .cpp or .cs)
                        case 'm':    // MCPP file (.mcpp)
                        case 'v':    // VB.NET
                        case 'j':    // J# file (.jsl)
                ...
                if (DEBUG_4)
                {
                    BuildError("WriteObjectsDefinition into pbuf FileName %s\n", psr->pfrSource->Name);
                }
                sprintf(
                    pbuf,
                    " \\" EOL "    %s" PATH_SEPARATOR "%.*s%s",
                    pszextdir,
    ...
    if (DEBUG_4)
    {
        BuildError("WriteObjectsDefinition Exit DirName: %s\n", DirName);
    }
}
    ...
    LPSECURITY_ATTRIBUTES   psa)
{
    ...
    if (pszPath && *pszPath)
    {
...
#if PLATFORM_UNIX
        // Skip the first character to avoid calling
        // CreateDirectory("", 0).
        pch++;
#endif  // PLATFORM_UNIX
        ...
        for (; *pch; pch++)
        {
            ...
            //
            if (IsPathSeparator(*pch))
            {
                BOOL fOk;

                *pch = 0;
                fOk = CreateDirectory (pszPath, psa);
                *pch = PATH_SEPARATOR_CHAR;
...
CreatedBuildFile(LPSTR DirName, LPSTR FileName)
{
    ...
    if (DirName == NULL || DirName[0] == '\0') {
        ...
        _snprintf( Name, sizeof(Name)-1, "%s" PATH_SEPARATOR "%s", DirName, FileName );
    }
...
GenerateObjectsDotMac(DIRREC *DirDB, DIRSUP *pds, DOS_TIME DateTimeSources)
{
    FILE *OutFileHandle;
    UINT i;
    DOS_TIME ObjectsDateTime;
    char szObjectsMac[_MAX_PATH];

    strcpy(szObjectsMac, pszObjDir);
    strcat(szObjectsMac, PATH_SEPARATOR "_objects.mac");
    ...
    if ((ObjectsDateTime = (*pDateTimeFile)(DirDB->Name, szObjectsMac)).value != 0) {

        if (DateTimeSources.value == 0) {
            BuildError("%s: no sources timestamp\n", DirDB->Name);
        }

        if (ObjectsDateTime.value >= DateTimeSources.value) {
            if (!fForce) {
                return;
            }
        }
    ...
    if (fFirstScan && fPassZero)
    {
        ...
        if (hf != INVALID_HANDLE_VALUE) {
            ...
            if (ObjectsDateTime.value != 0) {
                time = ObjectsDateTime;
            }
            else if (DateTimeSources.value != 0) {
                //
                ...
                if (time.time.wTime != 0)
                    time.value &= 0xFFFF0000;  // 00:00:00 on the same date
                else
                    time.value = 0x1421A000;       // 12:00:00 1/1/1990
            }
            else {
                time.value = 0x1421A000;       // 12:00:00 1/1/1990
            }

            DosDateTimeToFileTime(time.time.wDate, time.time.wTime, &ft);
