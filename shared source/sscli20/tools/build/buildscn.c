...
ScanSubDir(LPSTR pszDir, DIRREC *pdr)
{
    ...
    do {
        ...
        if ((pdr->DirFlags & DIRDB_NEW) == 0 &&
            ...
            //
            if (FileDB->DateTime.value != DateTime.value) {
    ...
    {
        BuildError("%s" PATH_SEPARATOR "sources. unexpected in directory with DIRS file\n",
                   pdr->Name);
...
#define BUILD_VER_COMMENT "/*++ BUILD Version: "              // comment
...
parsefilename:
            if (p != NULL) {
                ...
                if (*p != CloseQuote || (UCHAR) CloseQuote == 0xff) {
