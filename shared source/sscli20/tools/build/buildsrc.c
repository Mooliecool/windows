...
#define MACRO_CHAR(ch) __iscsym(ch)
...
#define SOURCES_CLR_TARGETTYPE          51
...
BOOL
FindDirectory(LPSTR TextLine, LPSTR Directory);

ULONG
FixDirInclude(PDIRREC DirDB, LPSTR dirbuf, LPSTR* pp);
...
SaveMacro(LPSTR pszName, LPSTR pszValue)
{
    ...
    if (ppm == &apMacro[cMacro]) {
        cMacro++;
        AllocMem(sizeof(MACRO) + strlen(pszName), (VOID **) ppm, MT_MACRO);
...

{
    ...
    while (SplitToken(path, ' ', &plist)) {
        ...
        // Convert all path separators to the native path separator.
        char *tmp = path;
...
#if PLATFORM_UNIX
        // On Unix, convert the paths to lowercase. We only support
        // lowercase file names, but sources macros contain mixed-case names.
        tmp = path;
        while(*tmp) {
            *tmp = tolower(*tmp);
            tmp++;
        ...
        pszsubdir = path;
        if (pszsubdir[0] == '.' && IsPathSeparator(pszsubdir[1])) {
            BuildError(
        ...
        pszpath = path;
        pszfile = FindFirstPathSeparator(pszsubdir);
        if (pszfile == NULL) {
            pszfile = pszsubdir;
            ...
            if ((SubDirMask & TMIDIR_PARENT) == 0) {
                pszSecondSlash = FindFirstPathSeparator(pszfile + 1);
            ...
            *pszfile = '\0';
            ...
            if (((_stricmp(pszsubdir, pszAssociateDir) != 0) &&
                ...
                *pszfile = PATH_SEPARATOR_CHAR;
                ...
                pszpath = FindLastPathSeparator(path);
                assert(pszpath != NULL);
                pszpath++;
                SubDirMask = 0;
...
NewDirectory:
        ...
        if ((pfr == NULL) && !fPassZero) {
            ...
            pfr = InsertFileDB(*ppdr, pszfile, zeroTime, 0, FILEDB_FILE_MISSING);
            if (pfr == NULL) {
                BuildError(
    ...
    )
{
    ...
    while (SplitToken(produces, ' ',&Value)) {
        ...
        while (Dependency) {
            if (strcmp(Dependency->Name, produces)==0) {
                break;
            }
        ...
        if (Dependency != NULL) {
                BuildMsgRaw(
                    "WARNING: Dependency '%s' is already produced by directory '%s'\n",
                    Value,
        ...
        AllocMem(sizeof(DEPENDENCY)+strlen(produces), (VOID **) &Dependency, MT_DEPENDENCY);
    ...
    )
{
    ...
    while (SplitToken(consumes, ' ',&Value)) {
        ...
        if (Dependency != NULL) {
            AllocMem(sizeof(DEPENDENCY_WAIT), (VOID **) &Wait, MT_DEPENDENCY_WAIT);
...
void
FreeDependency(PDEPENDENCY* ppDependency)
{
    if (*ppDependency != NULL) {
        PDEPENDENCY pDependency;
        PDEPENDENCY pDependencyNext;

        pDependency = *ppDependency;
        pDependencyNext = pDependency->Next;

        CloseHandle(pDependency->hEvent);
        FreeMem((VOID **)ppDependency, MT_DEPENDENCY);

        *ppDependency = pDependencyNext;
    }
} 
...
FreeAllDependencies()
{
    while (AllDependencies != NULL) {
        FreeDependency(&AllDependencies);
    }
}

void
FreeConsumeDependencies(DIRREC *pdr)
{
    PLIST_ENTRY pLink;
    PDEPENDENCY_WAIT pWait;
    
    while (!IsListEmpty(&pdr->Consumes)) {
        pLink = RemoveHeadList(&pdr->Consumes);
        pWait = CONTAINING_RECORD(pLink, DEPENDENCY_WAIT, ListEntry);
        FreeMem((VOID **)&pWait, MT_DEPENDENCY_WAIT);
    }
}
    ...
    if (MacroName != NULL) {    // if macro name found in list
        switch (iMacro) {
            ...
            case SOURCES_TARGETPATH:
                ...
                if (DirDB->TargetPath != NULL) {
                    ...
                    for (i = 0; i < CountTargetMachines; i++) {
                        ...
                        sprintf(path, "%s" PATH_SEPARATOR "%s", DirDB->TargetPath, p1);
            ...
            case SOURCES_TARGETPATHLIB:
                ...
                if (DirDB->TargetPathLib != NULL) {
                    ...
                    for (i = 0; i < CountTargetMachines; i++) {
                        ...
                        assert(strncmp(pszObjDirSlash, p1, strlen(pszObjDirSlash)) == 0);
                        p1 += strlen(pszObjDirSlash);
                        sprintf(path, "%s" PATH_SEPARATOR "%s", DirDB->TargetPathLib, p1);
                        CreateBuildDirectory(path);
                    }
                }
                break;
            ...
            case SOURCES_PRECOMPILED_TARGET:
                {
                    ...
                    while (p > *ppszFile && !IsPathSeparator(*--p))
                        ;
            ...
            case SOURCES_PROJECT_NAME:
                sprintf(path, "%s" PATH_SEPARATOR "%s", NtRoot, pValue);
            ...
            case SOURCES_SYNC_PRODUCES:
                ...
                FreeString(&pszTemp, MT_DIRSTRING);
                break;
    ...
    )
{
    ...
    strcat(path, PATH_SEPARATOR "project.mk");
    Found = (GetFileAttributesA(path) != 0xffffffff);
    while (!Found && strlen(pszProjectMkPath))
    {
        pBackSlash=FindLastPathSeparator(pszProjectMkPath);
        if (pBackSlash) {
            *pBackSlash = '\0';
        } else {
            return;
...
ReadSourcesFile(DIRREC *DirDB, DIRSUP *pds, DOS_TIME *pDateTimeSources)
{
    ...
    for (iTarget = -1; iTarget < 2*((int)MaxInitializedTargets); iTarget++) {
        path[0] = '\0';
        if (iTarget >= 0) {
            if (iTarget & 1) {
    ...
    if (pDateTimeSources->value < DateTime.value) {
...
PostProcessSources(DIRREC *pdr, DIRSUP *pds)
{
    ...
    for (i = 0; i < MaxInitializedTargets + 1; i++) {
...
//+---------------------------------------------------------------------------
//
//  Function:   OpenDirsFile
//
//  Synopsis:   Opens the proper DIRS file in a directory
//
//  Arguments:  [Directory] -- Directory to look in
//
...
BOOL
OpenDirsFile(LPSTR Directory, FILE** pInFileHandle)
{
    CHAR TargetName[16];
    ...
    for (ppCurrentDirsFileName = apszDirs;
        ...
        if (SetupReadFile(Directory, *ppCurrentDirsFileName, "#", pInFileHandle)) {
            break;
        }
    }

    if (*ppCurrentDirsFileName == NULL || !*pInFileHandle) {
        return( FALSE );
    }

    if (fFirstScan && (ppCurrentDirsFileName <= &apszDirs[1])) {
        BuildMsg("Using ." PATH_SEPARATOR "%s instead of DIRS...\n",
                 FormatPathName(Directory, *ppCurrentDirsFileName));
    }
...

{
    ...
    CHAR DirsName[MAX_DIRS_NAME_LENGTH], OptionalDirsName[MAX_OPTIONAL_DIRS_NAME_LENGTH];
 
    if (!OpenDirsFile(DirDB->Name, &InFileHandle)) {
        return( FALSE );
    }
    ...
    strcpy(DirsName, "DIRS");
    strcpy(OptionalDirsName, "OPTIONAL_DIRS");
    if ((MAX_DIRS_NAME_LENGTH < strlen(DirsName) + strlen(pszBuildPhasePostString) + 1) ||
        (MAX_OPTIONAL_DIRS_NAME_LENGTH < strlen(OptionalDirsName) + strlen(pszBuildPhasePostString) + 1)) {
      BuildError("Not enough space in dirs variables for holding phase variables.\r\n");
      exit(1);
    }
    strcat(DirsName, pszBuildPhasePostString);
    strcat(OptionalDirsName, pszBuildPhasePostString);

    if ((pValue = FindMacro(DirsName)) != NULL) {
        MarkDirNames(DirDB, pValue, TRUE);
    }
    if ((pValue = FindMacro(OptionalDirsName)) != NULL) {
    ...
    if ((FindMacro("IGNORE_MAKEFILE")) != NULL) {
...
OpenParentDirDirsFile(LPSTR Directory, FILE** pInFileHandle)
{
    ...
    char ParentDirFullPath[DB_MAX_PATH_LENGTH];
    LPSTR p;
    strcpy(ParentDirFullPath, Directory);
    p = strrchr(ParentDirFullPath, PATH_SEPARATOR_CHAR);
    if (p != NULL)
    {
        p[0] = '\0';

        return( OpenDirsFile(ParentDirFullPath, pInFileHandle));
    }
    return( FALSE );
}

// Returns TRUE if:
//     This is a SOURCES directory
//     There is a DIRS file in the parent
//     The DIRS[phase] macro doesn't contain this directory
...
NeedToSkipDirectory(PDIRREC DirDB)
{
    ...
    if (DirDB && (DirDB->DirFlags & DIRDB_SOURCES) && OpenParentDirDirsFile(DirDB->Name, &InFileHandle)) {
        assert(cMacro == 0);
        while ((TextLine = ReadLine(InFileHandle)) != NULL) {
            if ((pValue = SplitMacro(&TextLine)) != NULL) {
                SaveMacro(TextLine, pValue);
        ...
        bSkip = TRUE;
        if ((pValue = FindMacro(DirsName)) != NULL) {
            LPSTR p = DirDB->Name + strlen(DirDB->Name);
            while (p >= DirDB->Name && p[0] != PATH_SEPARATOR_CHAR) {
                --p;
            }
            bSkip = !FindDirectory(pValue, p+1);
        }

        // if ((pValue = FindMacro(OptionalDirsName)) != NULL) {
        //     BuildAllOptionalDirs);
        // }
...
PrintDirSupData(DIRSUP *pds)
{
    ...
    for (i = 0; i < (int)MaxInitializedTargets + 1; i++) {
        if (pds->SourcesVariables[i] != NULL) {
            BuildMsgRaw(
...
FreeDirSupData(DIRSUP *pds)
{
    ...
    if (pds->LocalIncludePath != NULL) {
        FreeMem((VOID **) &pds->LocalIncludePath, MT_DIRSTRING);
    }
    if (pds->UserIncludePath != NULL) {
        FreeMem((VOID **)&pds->UserIncludePath, MT_DIRSTRING);
    }
    if (pds->LastIncludePath != NULL) {
        FreeMem((VOID **)&pds->LastIncludePath, MT_DIRSTRING);
    }
    if (pds->TestType != NULL) {
        FreeMem((VOID **) &pds->TestType, MT_DIRSTRING);
    }
    if (pds->PchInclude != NULL) {
        FreeMem((VOID **) &pds->PchInclude, MT_DIRSTRING);
    }
    if (pds->PchIncludeDir != NULL) {
        FreeMem((VOID **) &pds->PchIncludeDir, MT_DIRSTRING);
    }
    if (pds->PchTargetDir != NULL) {
        FreeMem((VOID **) &pds->PchTargetDir, MT_DIRSTRING);
    }
    if (pds->PchTarget != NULL) {
        FreeMem((VOID **) &pds->PchTarget, MT_DIRSTRING);
    }
    if (pds->ConditionalIncludes != NULL) {
        FreeMem((VOID **) &pds->ConditionalIncludes, MT_DIRSTRING);
    }
    if (pds->PassZeroHdrDir != NULL) {
        FreeMem((VOID **) &pds->PassZeroHdrDir, MT_DIRSTRING);
    }
    if (pds->PassZeroSrcDir1 != NULL) {
        FreeMem((VOID **) &pds->PassZeroSrcDir1, MT_DIRSTRING);
    }
    if (pds->PassZeroSrcDir2 != NULL) {
        FreeMem((VOID **) &pds->PassZeroSrcDir2, MT_DIRSTRING);
    }
    for (i = 0; i < (int)MaxInitializedTargets + 1; i++) {
        if (pds->SourcesVariables[i] != NULL) {
            FreeMem((VOID **) &pds->SourcesVariables[i], MT_DIRSTRING);
        }
...
FreeDirData(DIRREC *pdr)
{
    if (pdr->TargetPath != NULL) {
        FreeMem((VOID **) &pdr->TargetPath, MT_DIRSTRING);
    }
    if (pdr->TargetPathLib != NULL) {
        FreeMem((VOID **) &pdr->TargetPathLib, MT_DIRSTRING);
    }
    if (pdr->TargetName != NULL) {
        FreeMem((VOID **) &pdr->TargetName, MT_DIRSTRING);
    }
    if (pdr->TargetExt != NULL) {
        FreeMem((VOID **) &pdr->TargetExt, MT_DIRSTRING);
    }
    if (pdr->KernelTest != NULL) {
        FreeMem((VOID **) &pdr->KernelTest, MT_DIRSTRING);
    }
    if (pdr->UserAppls != NULL) {
        FreeMem((VOID **) &pdr->UserAppls, MT_DIRSTRING);
    }
    if (pdr->UserTests != NULL) {
        FreeMem((VOID **) &pdr->UserTests, MT_DIRSTRING);
    }
    if (pdr->NTTargetFile0 != NULL) {
        FreeMem((VOID **) &pdr->NTTargetFile0, MT_DIRSTRING);
    }
    if (pdr->Pch != NULL) {
        FreeMem((VOID **) &pdr->Pch, MT_DIRSTRING);
    }
    if (pdr->PchObj != NULL) {
        FreeMem((VOID **) &pdr->PchObj, MT_DIRSTRING);
    }
    if (pdr->pds != NULL) {
        FreeDirSupData(pdr->pds);
        FreeMem((VOID **) &pdr->pds, MT_DIRSUP);
        pdr->pds = NULL;
    }
}
...
FindDirectory(LPSTR TextLine, LPSTR Directory)
{
    ...
    while (SplitToken(dirbuf, '*', &TextLine)) {
        DirInclude = FixDirInclude(NULL, dirbuf, &p);

        if (!(DirInclude & TargetMachines[0]->DirIncludeMask) || !p) {
            continue;
        }

        if (!strcmp(dirbuf, Directory)) {
            return( TRUE );
        }
        ...
        if ( dirbuf != p && *p == '{' ) {
            ...
            *p = '\0';
            ...
            while (token) {
                ...
                } else if (!_stricmp(token, DynamicProcessor)) {
                    DirInclude |= DIR_INCLUDE_DYNAMIC;
                }
                token = strtok(NULL, ",}");
            }
            break;
...
MarkDirNames(PDIRREC DirDB, LPSTR TextLine, BOOL Required)
{
    ...
    zeroTime.value = 0;
    while (SplitToken(dirbuf, '*', &TextLine)) {
        DirInclude = FixDirInclude(DirDB, dirbuf, &p);
