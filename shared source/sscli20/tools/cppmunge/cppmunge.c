...
#if defined(__ROTOR_PAL_H__)
#undef EINVAL
#define EINVAL          (-1)
#define EIO             (-1)
#define strerror(err)   "error"
#else
#include <errno.h>
#include <unistd.h>
#endif

#if !defined(PLATFORM_UNIX) || defined(__ROTOR_PAL_H__)
#define PATH_MAX                    MAX_PATH
#define remove(filename)            (DeleteFileA(filename)?0:-1)
#define rename(oldname,newname)     (MoveFileExA(oldname,newname,MOVEFILE_REPLACE_EXISTING)?0:-1)
#define realpath(filename,resolved) (GetFullPathNameA((filename),PATH_MAX,(resolved),NULL)?(resolved):NULL)
...
Usage:
    ...
    return ProcessFile(argc == 4 ? argv[3] : NULL);
}
...
static int ProcessFile(const char *filename) {
    ...
    if (filename) {
        ...
        err = StripPCH(stdin, stdout);
        if (err) {
            printf ("#error \"cppmunge failed when stripping precompiled header\"\n");
            goto done;
        }
...
#define matchforstrip(expected) \
    matched = 0; \
    for (;;) { \
        if ((ch = fgetc(original)) == EOF) \
            goto DoneReading; \
        if (ch == expected) { \
            extrachars[cCopiedChars] = ch; \
            cCopiedChars++; \
            matched = 1; \
            ch = EOF; \
        } \
        ...
        break; \
            ...
            DoneReading:
                // if we didn't actually match, copy what we did match back to the output
                if (matched == 0) {
                    if (fputc('_', destination) == EOF) {
                        return ferror(destination);
                    }
                    for (; iChar < cCopiedChars; iChar++) {
                        if (fputc(extrachars[iChar], destination) == EOF) {
                            return ferror(destination);
                        }
                    }
                    if (fputc(ch, destination) == EOF) {
