    ...
    )
{
    ...
    } else if ((pOldTop->type == STR) && (pTop->type == STR)) {
        int i = _tcscmp((char *) *left, (char *) *right);
...
getTok()
{
    ...
    if (c == '[') {
        ...
        if (_istdigit(c)) {
            ...
            errno = 0;                  // Accept decimal, octal or hex no
