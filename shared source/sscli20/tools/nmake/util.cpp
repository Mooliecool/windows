    ...
    )
{
    ...
    if (*string) {
        ...
        if (targetFlag) {
            ...
            //Look for .\string
            if (string[0] == '.' && IsPathSeparator(string[1])) {
                string += 2;
            } else {
                string = (char *)rallocate(2 + _tcslen(str) + 1);
                _tcscat(_tcscpy(string, "." PATH_SEPARATOR), str);
                fAllocStr = (BOOL)TRUE;
            }
    ...
    if (listMacro) {
        ...
        for (value = pVal; p; p = p->next) {
            ...
            *value = '\0';
            ...
            // Append a space if there are more elements in the list.                     
    ...
    )
{
    ...
    for (;value >= buf; value--) {
        if (IsPathSeparator(*value)) {   // scan upto first path separator
            lastSlash = value;
            break;
        } else
        ...
        if (*u && (t == u)) {               // if not at end & no path sep
            ...
            if (*u) {
                ...
                if (*u == '.'                   // if you find it, with . just
                    && !FindFirstPathSeparator(u+1)) // next to it & no path seps.,
                                                     // it's a rule
    ...
    )
{
    ...
    for (p = table[hash]; p; p = p->next) {
        ...
        for (s = string, t = p->text; *s && *t; s++, t++) {
            if (IsPathSeparator(*s))            // / == \ in targets
                if (IsPathSeparator(*t))
                    continue;
                else
                    break;
