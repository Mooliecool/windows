...
)
{
    ...
    // [....]:2008.09.21. Consider handling the following code differently.
...
;BuildEvent
    ...
    NOTE: this has to be kept in [....] with the associated decompilation logic in
...
)
{
        ...
        CurrentParameter = CurrentParameter->GetNext())
    {
        ...
        // [....] 9/13/2004: The numbers added together for the allocation are all values that we own, and are "small"
...
)
{
    ...
    else
    {
        if (ValueOfConstant.TypeCode == t_string)
        {
            ...
            // [....] 9/13/2004: Since the string already exists, and we own the calculation of its length, there's no chance
...
#if DEBUG
// DebugPrintf co-function for the above CanBeMyGroupCollectionMember(). Keep in [....]
void
...
)
{
    ...
    {
        VSFAIL("DebugPrintfCanBeMyGroupCollectionMember out of [....]");
    }
    if(ms)
    {
        VSDEBUGPRINTIF(
