...
)
{
    ...
    // THIS [....] NEEDS TO HAPPEN TILL THE POINT OF TIME THIS CODE IS RE-FACTORED
...
//          list_of_statements
//      finally
//          if not( nothing is temp )       // consider:([....]) use the new introduced 'isnot'operator
...
//         list_of_statements
//      finally
//          if not( nothing is v )       // consider:([....]) use the new introduced 'isnot'operator
...
//              list_of_statements
//          finally
//              if not( nothing is v2 )       // consider:([....]) use the new introduced 'isnot'operator
//                  CType(v2, IDisposable).Dispose()
//              end if
//          end try
//      finally
//          if not( nothing is v1 )       // consider:([....]) use the new introduced 'isnot'operator
...
)
{
    ...
    if (DelegateVariable)
    {
        ...
        // (Note: [....] - bug 34453. The use of EventVariableReference in the if first and later in the call is not thread
        ...
        if( m_IsGeneratingXML )
        {
            ...
            // Note ([....]) Not sure "UseTwiceShortLived" makes sense here; the event ref is just a name
