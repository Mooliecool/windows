...
Namespace Microsoft.VisualBasic.Devices
    ...
    Public Class Network
                ...
                SyncLock m_SyncObject 'we don't want our event firing before we've finished setting up the infrastructure.  Also, need to assure there are no races in here so we don't hook up the OS listener twice, etc.
                    ...
                    If m_NetworkAvailabilityEventHandlers.Count = 1 Then                      
                        ...
                        If AsyncOperationManager.SynchronizationContext IsNot Nothing Then
                            ...
                            Try ' UNDONE: Exceptions are thrown if the user isn't an admin. This try/catch is a temporary solution for beta1. For beta2 we need to find a way to enable these events for a non admin.
        ...
        Public Function Ping(ByVal address As Uri) As Boolean
            ' We're safe from Ping(Nothing, ...) due to overload failure (Ping(String,...) v.s Ping(Uri,...)).
        ...
        Public Function Ping(ByVal hostNameOrAddress As String, ByVal timeout As Integer) As Boolean
            ...
            If Not Me.IsAvailable Then
                Throw ExceptionUtils.GetInvalidOperationException(ResID.MyID.Network_NetworkNotAvailable)
            End If
        ...
        Public Sub DownloadFile(ByVal address As String, _
            ...
            ' We're safe from DownloadFile(Nothing, ...) due to overload failure (DownloadFile(String,...) v.s DownloadFile(Uri,...)).
        ...
        ''' <param name="overwrite">Indicates whether or not the file should be overwritten if local file already exists</param>
        ''' <remarks></remarks>
        Sub DownloadFile(ByVal address As Uri, _
                     ByVal destinationFileName As String, _
                     ByVal userName As String, _
                     ByVal password As String, _
                     ByVal showUI As Boolean, _
        ...
        Sub DownloadFile(ByVal address As Uri, _
            ...
            If connectionTimeout <= 0 Then
                Throw GetArgumentExceptionWithArgName("connectionTimeOut", ResID.MyID.Network_BadConnectionTimeout)
            End If
            ...
            Using client As New WebClientExtended
                ...
                Dim fullFilename As String = FileIO.FileSystem.NormalizeFilePath(destinationFileName, "destinationFileName")
                ...
                If System.IO.Directory.Exists(fullFilename) Then
                    Throw ExceptionUtils.GetInvalidOperationException(ResID.MyID.Network_DownloadNeedsFilename)
                End If

                'Throw if the file exists and the user doesn't want to overwrite
                If IO.File.Exists(fullFilename) And Not overwrite Then
                    Throw New IO.IOException(GetResourceString(ResID.MyID.IO_FileExists_Path, destinationFileName))
                End If
                ...
                If showUI AndAlso System.Environment.UserInteractive Then
                    ...
                    dialog.Text = GetResourceString(ResID.MyID.ProgressDialogDownloadingTitle, address.AbsolutePath)
                    dialog.LabelText = GetResourceString(ResID.MyID.ProgressDialogDownloadingLabel, address.AbsolutePath, fullFilename)
                End If
        ...
        ''' <param name="connectionTimeout">Time alloted before giving up on a connection</param>
        ''' <remarks></remarks>
        Public Sub UploadFile(ByVal sourceFileName As String, _
        ...
        Public Sub UploadFile(ByVal sourceFileName As String, _
            ...
            ' We're safe from UploadFile(Nothing, ...) due to overload failure (UploadFile(String,...) v.s UploadFile(Uri,...)).
            ...
            If System.IO.Path.GetFileName(addressUri.AbsolutePath) = "" Then
                Throw ExceptionUtils.GetInvalidOperationException(ResID.MyID.Network_UploadAddressNeedsFilename)
            End If
        ...
        Public Sub UploadFile(ByVal sourceFileName As String, _
                              ByVal address As Uri, _
                              ByVal networkCredentials As ICredentials, _
                              ByVal showUI As Boolean, _
                              ByVal connectionTimeout As Integer)
        ...
        Public Sub UploadFile(ByVal sourceFileName As String, _
            ...
            sourceFileName = FileIO.FileSystem.NormalizeFilePath(sourceFileName, "sourceFileName")

            'Make sure the file exists
            If Not IO.File.Exists(sourceFileName) Then
                Throw New IO.FileNotFoundException(GetResourceString(ResID.MyID.IO_FileNotFound_Path, sourceFileName))
            End If
            ...
            Using client As New WebClientExtended()
                ...
                If showUI AndAlso System.Environment.UserInteractive Then
                    Dialog = New ProgressDialog
                    Dialog.Text = GetResourceString(ResID.MyID.ProgressDialogUploadingTitle, sourceFileName)
                    Dialog.LabelText = GetResourceString(ResID.MyID.ProgressDialogUploadingLabel, sourceFileName, address.AbsolutePath)
                End If
        ...
        ''';PingBuffer
        ''' <summary>
        '''  A buffer for pinging. This immitates the buffer used by Ping.Exe
        ''' </summary>
        ...
        Private Function GetUri(ByVal address As String) As Uri
            ...
            Catch ex As UriFormatException
                ...
                Throw GetArgumentExceptionWithArgName("address", ResID.MyID.Network_InvalidUriString, address)
            End Try
        End Function
