...
#If DEBUG Then
    <AttributeUsage(AttributeTargets.Constructor Or AttributeTargets.Method Or AttributeTargets.Property, AllowMultiple:=False, Inherited:=False)> _
    Friend NotInheritable Class DebuggerHiddenAttribute
        Inherits Attribute
    End Class

    <AttributeUsage(AttributeTargets.Constructor Or AttributeTargets.Method Or AttributeTargets.Property, AllowMultiple:=False, Inherited:=False)> _
    Friend NotInheritable Class DebuggerStepThroughAttribute
        Inherits Attribute
    End Class
#End If
...
#Region " BACKWARDS COMPATIBILITY "
    ...
    Public NotInheritable Class LateBinding
        ...
        Public Shared Function LateGet(ByVal o As Object, _
            ...
            flags = BindingFlags.IgnoreCase Or _
            ...
            Else
                ...
                If (Not mi Is Nothing) AndAlso (mi.MemberType = MemberTypes.Field) Then
                    ...
                    If objType Is objIReflect OrElse CType(mi, FieldInfo).IsStatic OrElse _
            ...
            Catch ex As Exception When IsMissingMemberException(ex)
                ...
                If objType.IsCOMObject() OrElse ((Not args Is Nothing) AndAlso (args.Length > 0)) Then
                    ...
                    If oTmp Is Nothing Then
                        Throw New MissingMemberException(GetResourceString(ResID.MissingMember_MemberNotFoundOnType2, name, VBFriendlyName(objType, o)))
                    Else
                        Try
        ...
        Private Shared Function IsMissingMemberException(ByVal ex As Exception) As Boolean
            ...
            Else
                ...
                ElseIf TypeOf ex Is TargetInvocationException AndAlso _
                        (TypeOf ex.InnerException Is COMException AndAlso _
        ...
        <DebuggerHiddenAttribute(), DebuggerStepThroughAttribute()> _
        Public Shared Sub LateSetComplex(ByVal o As Object, ByVal objType As Type, ByVal name As String, _
                                  ByVal args() As Object, ByVal paramnames() As String, _
                                  ByVal OptimisticSet As Boolean, ByVal RValueBase As Boolean)

            'CONSIDER jimcan (4/12/2001): rewrite the binder to suppress the exception in this case
            Try
                ...
                If RValueBase AndAlso objType.IsValueType Then
                    ...
                    Throw New Exception(GetResourceString(ResID.RValueBaseForValueType, VBFriendlyName(objType, o), VBFriendlyName(objType, o)))
                End If
                ' UNDONE - harishk - replace below with 'when IsMissingException' - also why the 'when OptimisticSet = True' clause below ?
        ...
        <DebuggerHiddenAttribute(), DebuggerStepThroughAttribute()> _
        Public Shared Sub LateSet(ByVal o As Object, ByVal objType As Type, ByVal name As String, _
        ...
        <DebuggerHiddenAttribute(), DebuggerStepThroughAttribute()> _
        Friend Shared Sub InternalLateSet(ByVal o As Object, _
                                   ByRef objType As Type, _
                                   ByVal name As String, _
                                   ByVal args() As Object, _
                                   ByVal paramnames() As String, _
            ...
            Else
                ...
                If (Not mi Is Nothing) AndAlso (mi.MemberType = MemberTypes.Field) Then
                    ...
                    If fi.IsInitOnly Then
                        ...
                        Throw New MissingMemberException(GetResourceString(ResID.MissingMember_ReadOnlyField2, name, VBFriendlyName(objType, o)))
                    End If
                    ...
                    ElseIf args.Length = 1 Then
                        ...
                        If objType Is objIReflect OrElse fi.IsStatic OrElse _
            ...
            If (OptimisticSet AndAlso args.GetUpperBound(0) > 0) Then
                ...
                Dim propflags As BindingFlags = _
                                BindingFlags.GetProperty Or _
            ...
            Catch ex As TargetInvocationException
                ...
                ElseIf TypeOf ex.InnerException Is TargetParameterCountException Then
                    If (flags And BindingFlags.PutRefDispProperty) <> 0 Then
                        ...
                        Throw New MissingMemberException(GetResourceString(ResID.MissingMember_MemberSetNotFoundOnType2, name, VBFriendlyName(objType, o)))
                    Else
                        'Let was being called
                        Throw New MissingMemberException(GetResourceString(ResID.MissingMember_MemberLetNotFoundOnType2, name, VBFriendlyName(objType, o)))
                    End If
                Else
                    Throw ex.InnerException
                End If
            ...
            Else
                ...
                If Not objType.IsCOMObject() Then
                    ...
                    If members Is Nothing Or iNext = 0 Then
                        Throw New MissingMemberException(GetResourceString(ResID.MissingMember_NoDefaultMemberFound1, VBFriendlyName(objType, o)))
                    End If
                ...
                Try
                    ...
                    Else
                        ...
                        If objType Is objIReflect OrElse method.IsStatic OrElse _
        ...
        Private Shared Function GetDefaultMembers(ByVal typ As Type, ByVal objIReflect As IReflect, ByRef DefaultName As String) As MemberInfo()
            ...
            If typ Is objIReflect Then
                Do
                    ...
                    If (Not attributeList Is Nothing) AndAlso (attributeList.Length <> 0) Then
                        ...
                        members = typ.GetMember(DefaultName, BindingFlags.IgnoreCase Or _
                                                            BindingFlags.FlattenHierarchy Or _
                                                            BindingFlags.Instance Or _
                                                            BindingFlags.Static Or _
                                                            BindingFlags.Public)
            ...
            Else
                ...
                members = objIReflect.GetMember("", BindingFlags.IgnoreCase Or _
                                          BindingFlags.FlattenHierarchy Or _
                                          BindingFlags.Instance Or _
                                          BindingFlags.Static Or _
                                          BindingFlags.Public)
        ...
        <DebuggerHiddenAttribute(), DebuggerStepThroughAttribute()> _
        Public Shared Sub LateIndexSetComplex(ByVal o As Object, ByVal args() As Object, ByVal paramnames() As String, _
                                       ByVal OptimisticSet As Boolean, ByVal RValueBase As Boolean)

            'CONSIDER jimcan (4/12/2001): rewrite the binder to suppress the exception in this case
            Try
                'CONSIDER jimcan (5/9/2001):
                '   this really needs to be done in two steps:
                '         step 1:  can the Set succeed?
                '         step 2:  perform the Set
                '   the rvaluebase check would be done between 1 and 2
                ...
                If RValueBase AndAlso o.GetType().IsValueType Then
                    Throw New Exception(GetResourceString(ResID.RValueBaseForValueType, o.GetType().Name, o.GetType().Name))
                End If
            Catch ex As System.MissingMemberException When OptimisticSet = True
                'A missing member exception means it has no Set member.  Silently handle the exception.
            End Try
        ...
        Private Shared Function GetPropertyPutFlags(ByVal NewValue As Object) As BindingFlags
            ...
            ElseIf (TypeOf NewValue Is System.ValueType) OrElse _
                (TypeOf NewValue Is String) OrElse _
                (TypeOf NewValue Is DBNull) OrElse _
                (TypeOf NewValue Is Missing) OrElse _
                (TypeOf NewValue Is System.Array) OrElse _
        ...
        <DebuggerHiddenAttribute(), DebuggerStepThroughAttribute()> _
        Public Shared Sub LateCall(ByVal o As Object, ByVal objType As Type, ByVal name As String, _
        ...
        <DebuggerHiddenAttribute(), DebuggerStepThroughAttribute()> _
        Friend Shared Function InternalLateCall(ByVal o As Object, ByVal objType As Type, ByVal name As String, _
            ...
            If Not objType.IsCOMObject Then
                ...
                If MemberIsField(mi) Then
                    ...
                    Throw New ArgumentException(GetResourceString(ResID.ExpressionNotProcedure, name, VBFriendlyName(objType, o)))
                End If
        ...
        Private Shared Function FastCall(ByVal o As Object, ByVal method As MethodBase, ByVal Parameters As ParameterInfo(), ByVal args As Object(), ByVal objType As Type, ByVal objIReflect As IReflect) As Object
            ...
            Else
                '' UNDONE - passing the binder to user code ok ? seems ok, but check with jimcan
            ...
            For MemberIndex = 0 To mi.GetUpperBound(0)
                ...
                ElseIf ThisMember.MemberType = MemberTypes.Field Then
                    ...
                    For j = 0 To mi.GetUpperBound(0)
                        ...
                        If MemberIndex <> j AndAlso (Not mi(j) Is Nothing) AndAlso _
        ...
        Const VBLateBindingFlags As BindingFlags = BindingFlags.IgnoreCase Or _
                                                       BindingFlags.FlattenHierarchy Or _
                                                       BindingFlags.OptionalParamBinding Or _
                                                       BindingFlags.Static Or _
                                                       BindingFlags.Instance Or _
        ...
        Friend Shared Function InvokeMemberOnIReflect(ByVal objIReflect As IReflect, ByVal member As MemberInfo, ByVal flags As BindingFlags, ByVal target As Object, ByVal args As Object()) As Object
            ' UNDONE - Harishk - 06-06-2002 - consider if we can reuse the same binder instance used for the rest of this latebound call
        ...
        Private Shared Function GetCorrectIReflect(ByVal o As Object, ByVal objType As Type) As IReflect
            ...
            ' We don't support this For COM Objects because this is not a valid COM scenario and the perf. degrade is intolerable
            If (Not (o Is Nothing)) AndAlso (Not objType.IsCOMObject) AndAlso (Not RemotingServices.IsTransparentProxy(o)) AndAlso (Not (TypeOf o Is System.Type)) Then
                Dim IReflectObject As IReflect = TryCast(o, IReflect)

                If IReflectObject IsNot Nothing Then
                    Return IReflectObject
                End If
            End If
            ...
            If Value Is Nothing Then
                ...
                If Not IsStatic Then
                    ...
                    Throw New NullReferenceException( _
                        GetResourceString(ResID.NullReference_InstanceReqToAccessMember1, MemberToString(Member)))
                End If
            End If
        End Sub
