'*-----------------------------------------------------------------------
'* <copyright file="LikeOperator.vb" company="Microsoft">
...
Namespace Microsoft.VisualBasic.CompilerServices
    ...
    Public NotInheritable Class LikeOperator
        ...
        Private Shared Function LigatureIndex(ByVal ch As Char) As Byte
            ...
            If Asc(ch) < Ligatures.Min OrElse Asc(ch) > Ligatures.Max Then
                Return 0
            End If

            Return LigatureMap(Asc(ch) - Ligatures.Min)
        End Function

        Private Shared Function CanCharExpand _
        ( _
            ByVal ch As Char, _
            ByVal LocaleSpecificLigatureTable As Byte(), _
            ByVal Comparer As CompareInfo, _
        ...
        ( _
            ByRef Input As String, _
            ByRef Length As Integer, _
            ByRef InputLigatureInfo As LigatureInfo(), _
            ByVal LocaleSpecificLigatureTable As Byte(), _
            ByVal Comparer As CompareInfo, _
            ByVal Options As CompareOptions, _
            ByRef WidthChanged As Boolean, _
            ByVal UseFullWidth As Boolean _
        )
            Const CODEPAGE_JAPANESE As Integer = 932
            ...
            Dim Encoding As Text.Encoding = Text.Encoding.GetEncoding(Culture.TextInfo.ANSICodePage)
            Dim Flags As Integer = NativeTypes.LCMAP_LOWERCASE
            Dim MapDone As Boolean = False

            If Not Encoding.IsSingleByte Then
                Flags = NativeTypes.LCMAP_LOWERCASE Or NativeTypes.LCMAP_HALFWIDTH  

                If IsValidCodePage(CODEPAGE_JAPANESE) Then
                    If UseFullWidth Then
	                Flags = NativeTypes.LCMAP_LOWERCASE Or NativeTypes.LCMAP_FULLWIDTH Or NativeTypes.LCMAP_KATAKANA
                    Else
	                Flags = NativeTypes.LCMAP_LOWERCASE Or NativeTypes.LCMAP_HALFWIDTH Or NativeTypes.LCMAP_KATAKANA
                    End If

     	            ' Mapping certain wide Katakana characters to narrow can
	            ' increase the string length.  See if this is going to happen.
	            '
                    Input = vbLCMapString(Culture, Flags, Input)
                    MapDone = True
...
#End If
        ...
        Public Shared Function LikeString(ByVal Source As String, ByVal Pattern As String, ByVal CompareOption As CompareMethod) As Boolean
                ...
                Select Case p
                    ...
                    Case "["c, ChrW(&HFF3B)
                        ...
                        MatchRange( _
                            ...
                            SourceLength, _
                            SourceIndex, _
                            SourceLigatureInfo, _
                            Pattern, _
                            PatternLength, _
                            PatternIndex, _
                            PatternLigatureInfo, _
                            RangePatternEmpty, _
                            RangeMismatch, _
                            RangePatternError, _
                            Comparer, _
            ...
            While PatternIndex < PatternLength
                ...
                ElseIf PatternIndex + 1 < PatternLength AndAlso _
                        ((p = "["c AndAlso Pattern.Chars(PatternIndex + 1) = "]"c) OrElse _
        ...
        ( _
            ...
            ByVal LeftLength As Integer, _
            ByVal LeftStart As Integer, _
            ByRef LeftEnd As Integer, _
            ByVal LeftLigatureInfo As LigatureInfo(), _
            ByVal Right As String, _
            ByVal RightLength As Integer, _
            ByVal RightStart As Integer, _
            ByRef RightEnd As Integer, _
            ByVal RightLigatureInfo As LigatureInfo(), _
            ByVal Comparer As CompareInfo, _
            ByVal Options As CompareOptions, _
            Optional ByVal MatchBothCharsOfExpandedCharInRight As Boolean = False, _
            Optional ByVal UseUnexpandedCharForRight As Boolean = False _
        ) As Integer
        ...
        ( _
            ...
            ByRef SourceIndex As Integer, _
            ByVal SourceLigatureInfo As LigatureInfo(), _
            ByVal Pattern As String, _
            ByVal PatternLength As Integer, _
            ByRef PatternIndex As Integer, _
            ByVal PatternLigatureInfo As LigatureInfo(), _
            Byref RangePatternEmpty As Boolean, _
            ByRef Mismatch As Boolean, _
            ByRef PatternError As Boolean, _
            ByVal Comparer As CompareInfo, _
            ByVal Options As CompareOptions, _
            Optional ByRef SeenNot As Boolean = False, _
            Optional ByVal RangeList As List(Of Range) = Nothing, _
            Optional ByVal ValidatePatternWithoutMatching As Boolean = False _
        )

            Debug.Assert(PatternIndex <= PatternLength AndAlso _
                         (Pattern.Chars(PatternIndex) = "[" OrElse Pattern.Chars(PatternIndex) = ChrW(&HFF3B)), _
            ...
            Do
                ...
                If Not ValidatePatternWithoutMatching AndAlso _
                   PatternLigatureInfo IsNot Nothing AndAlso _
                ...
                If PatternNextIndex + 2 < PatternLength AndAlso _
                   (Pattern.Chars(PatternNextIndex + 1) = "-"c OrElse Pattern.Chars(PatternNextIndex + 1) = ChrW(&HFF0D)) AndAlso _
                   (Pattern.Chars(PatternNextIndex + 2) <> "]"c AndAlso Pattern.Chars(PatternNextIndex + 2) <> ChrW(&HFF3D)) Then
                    ...
                    If Not ValidatePatternWithoutMatching AndAlso _
                           CompareChars( _
                               ...
                               SourceLigatureInfo, _
                               Pattern, _
                               Range.Start + Range.StartLength, _
                               Range.Start, _
                               Nothing, _
                               PatternLigatureInfo, _
                               Comparer, _
                               Options, _
                               UseUnexpandedCharForRight:=True) >= 0 AndAlso _
                           CompareChars( _
                               Source, _
                               SourceLength, _
                               SourceIndex, _
...
OneCharMatch:
                        ...
                        Loop While Pattern.Chars(PatternIndex) <> "]"c AndAlso _
        ...
        ( _
            ...
            ByVal Comparer As CompareInfo, _
            ByVal Options As CompareOptions, _
            ByRef SeenNot As Boolean, _
            ByRef RangeList As List(Of Range) _
        ) As Boolean
...
#If DEBUG Then
            ...
            Friend Property StringPatternStart() As Integer
                Get
                    Debug.Assert(PatType = PatternType.STRING, "Unexpected pattern groun type!!!")
...
#If DEBUG Then
            ...
            Friend Property MinSourceIndex() As Integer
                Get
                    Debug.Assert(PatType = PatternType.STAR OrElse PatType = PatternType.NONE, "Unexpected pattern groun type!!!")
...
#If DEBUG Then
            ...
            Property RangeStarts() As String()
                Get
                    Debug.Assert(PatType = PatternType.EXCLIST OrElse PatType = PatternType.INCLIST, "Unexpected pattern groun type!!!")
...
#End If
        ...
        ( _
            ...
            ByVal PatternLigatureInfo As LigatureInfo(), _
            ByRef PatternError As Boolean, _
            ByRef PGIndexForLastAsterisk As Integer, _
            ByVal Comparer As CompareInfo, _
            ByVal Options As CompareOptions, _
            ByRef PatternGroups() As PatternGroup _
        )
            ' Pattern groups:
            '
            ...
            Do While PGIndex > 0
                Select Case PatternGroups(PGIndex).PatType
                    ...
                    Case Else
                        Debug.Fail("Unexpected pattern kind!!!")
        ...
        ( _
            ...
            ByVal Pattern As String, _
            ByVal PatternLength As Integer, _
            ByVal PatternIndex As Integer, _
            ByVal PattternLigatureInfo As LigatureInfo(), _
            ByRef Mismatch As Boolean, _
            ByRef PatternError As Boolean, _
            ByVal Comparer As CompareInfo, _
            ByVal Options As CompareOptions _
            ...
            Debug.Assert(PatternIndex <= PatternLength AndAlso _
                         (Pattern.Chars(PatternIndex) = "*"c OrElse Pattern.Chars(PatternIndex) = ChrW(&HFF0A)), _
            ...
            BuildPatternGroups( _
                ...
                PatternLength, _
                PatternIndex, _
                PattternLigatureInfo, _
                PatternError, _
                PGIndexForLastAsterisk, _
                Comparer, _
                Options, _
                PatternGroups)
            ...
            Debug.Assert(PatternGroups IsNot Nothing AndAlso _
                         PatternGroups.Length > 0 AndAlso _
        ...
        ( _
            ...
            ByVal Pattern As String, _
            ByVal PatternLigatureInfo As LigatureInfo(), _
            ByVal PatternGroups() As PatternGroup, _
            ByVal PGIndex As Integer, _
            ByRef Mismatch As Boolean, _
            ByRef PatternError As Boolean, _
            ByVal Comparer As CompareInfo, _
            ByVal Options As CompareOptions _
...
MatchString:
                        ...
                        Do
                            Dim CompareResult As Integer = _
                                CompareChars( _
                                    Source, _
                                    SourceLength, _
                                    SourceMatchIndex, _
                                    ...
                                    SourceLigatureInfo, _
                                    Pattern, _
                                    PGCurrent.StringPatternEnd + 1, _
                                    StringPatternIndex, _
                                    StringPatternIndex, _
                                    PatternLigatureInfo, _
                                    Comparer, _
                                    Options)
...
MatchList:
                        ...
                        If Not MatchRangeAfterAsterisk( _
                                    Source, _
                                    SourceLength, _
                                    SourceIndex, _
                                    SourceLigatureInfo, _
                    ...
                    Case PatternType.NONE
                        ...
                        If PatternGroups(PGPrevMismatchIndex).PatType <> PatternType.STAR AndAlso _
...
ShiftPosition:
        ...
        ( _
            ...
            'CONSIDER:harishk - improve performance, maybe store the ranges as a table/array lookup for some special cases.
            ...
            For Each Range As Range In RangeList
                ...
                If CompareResultStart = 0 OrElse _
        ...
        ( _
            ...
            ByVal CharsToSubtract As Integer, _
            ByVal InputLigatureInfo As LigatureInfo(), _
            ByVal Options As CompareOptions _
        )
        ...
        ( _
            ...
            If InputLigatureInfo IsNot Nothing AndAlso _
