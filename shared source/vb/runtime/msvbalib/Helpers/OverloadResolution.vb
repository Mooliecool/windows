...
#End If
        ...
        Private Shared Function LeastGenericProcedure( _
            ...
            If ParamIndex < LeftParamsLen orElse ParamIndex < RightParamsLen
...
#End If
        ...
        Private Shared Function InferTypeArgumentsFromArgument( _
            ...
            If (Inferred OrElse _
                ...
                (Not ParameterType.IsClass AndAlso Not ParameterType.IsInterface))
            ...
            Else If (Not ArgumentType.IsClass AndAlso _
            ...
            Else If (IsInstantiatedGeneric(ArgumentType) AndAlso _
        ...
        Friend Shared Function CanPassToParameter( _
            ...
            If Argument Is System.Reflection.Missing.Value Then
                If Parameter.IsOptional Then
                    Return True
                ElseIf Not IsRootObjectType(ParameterType) OrElse Not IsExpandedParamArray
...
#End If
            ...
            If TargetProcedure.HasParamArray Then
                ...
                Debug.Assert(ParamIndex = TargetProcedure.ParamArrayIndex, _
                             "current parameter must be parram array by this point")
            ...
            If MatchedParameters IsNot Nothing Then
                For Index As Integer = 0 To MatchedParameters.Length - 1
                    If MatchedParameters(Index) = False Then
                        If Parameters(Index).IsOptional Then
...
#End If
        ...
        Private Shared Function MoreSpecificProcedure( _
            ...
            ByVal ArgumentNames As String(), _
            ByVal CompareGenericity As ComparisonType, _
            Byref Optional BothLose As Boolean = False, _
            ByVal Optional ContinueWhenBothLose As Boolean = False) As Method
        ...
        Private Shared Function MostSpecificProcedure( _
            ...
            For Each CurrentCandidate As Method In Candidates
                ...
                For Each Contender As Method In Candidates
                    ...
                    If BestOfTheTwo Is Nothing AndAlso Not SignatureMismatch AndAlso
                        CurrentCandidate.UsedDefaultForAnOptionalParameter <> Contender.UsedDefaultForAnOptionalParameter Then
                        
                        If Contender.UsedDefaultForAnOptionalParameter
                            BestOfTheTwo = CurrentCandidate
                        Else
                            Debug.Assert(CurrentCandidate.UsedDefaultForAnOptionalParameter)
                            BestOfTheTwo = Contender
                        End If
                    End If
        ...
        Private Shared Function ReportOverloadResolutionFailure( _
            ...
            If CandidateReportCount = 1 Then
                ...
                'CONSIDER cambecc 3/1/2004: Using the same error message for the single-candidate case
                'is also misleading, but the benefit is not high enough for constructing a better message.
                'CONSIDER cambecc 2/26/2004: InvalidCastException is thrown only for back compat.  It would
