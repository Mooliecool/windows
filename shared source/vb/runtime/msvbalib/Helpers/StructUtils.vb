...
Namespace Microsoft.VisualBasic.CompilerServices
    ...
    Friend Class StructUtils
        ...
        ' Sort fields using their metadatatoken. If GetFields is called before any other Reflection call, then
        ' it returns the fields in the metadata order.
        Private Class FieldSorter
            ...
            Friend Function Compare(ByVal x As Object, ByVal y As Object) As Integer Implements System.Collections.IComparer.Compare
                Dim first As FieldInfo = DirectCast(x, FieldInfo)
                Dim second As FieldInfo = DirectCast(y, FieldInfo)

                Return Comparer.Default.Compare(first.MetadataToken, second.MetadataToken)
            End Function

            Friend Shared ReadOnly Instance As New FieldSorter()
        End Class
        ...
        Friend Shared Function EnumerateUDT(ByVal oStruct As ValueType, ByVal intfRecEnum As IRecordEnum, ByVal fGet As Boolean) As System.Object
            ...
            ' Type.GetFields() does not guarantee the order of the fields. Sort it so that
            ' we can read them back in the same order.
            Array.Sort(fi, FieldSorter.Instance)
