...
#End If
        ...
        '    '
        '    ' UNDONE:harishk - currently there is no way to compare generic method instantions
        ...
        Friend NotInheritable Class Container
            ...
            'CONSIDER: make this a generic sorter once generic array sort becomes available.
            Private Class InheritanceSorter : Implements IComparer

                Private Sub New()
                End Sub

                Private Function Compare(ByVal Left As Object, ByVal Right As Object) As Integer Implements IComparer.Compare
                    Dim LeftType As Type = DirectCast(Left, MemberInfo).DeclaringType
                    Dim RightType As Type = DirectCast(Right, MemberInfo).DeclaringType
...
#End If
                    ...
                    'is still okay. Returning 1 in this IReflect case will not cause qsort to never
                    'terminate.
                    Return 1
                End Function
            ...
            Private Shared s_IsInvokableDelegate As System.Func(Of MethodBase, Boolean) = GetIsInvokableDelegate()

            <System.Security.SecuritySafeCritical>
            <ReflectionPermission(SecurityAction.Assert, Flags:=ReflectionPermissionFlag.MemberAccess)>
            Shared Function GetIsInvokableDelegate() As System.Func(Of MethodBase, Boolean)

                ' CreateDelegate will do a MemberAccess demand here since MethodBase.IsDynamicallyInvokable is internal.
                ' Asserting that permission here to make sure the delegate creation always succeeds. 
                ' This is really a hack as we don't have time to make IsDynamicallyInvokable public in Dev11 and we don't
                ' want to make Microsoft.CSharp a friend of mscorlib. This code should be replaced with a static call
                ' to IsDynamicallyInvokable when we make it public in Dev12.

                Dim isInvokableDelegate As System.Func(Of MethodBase, Boolean) = Nothing

                Dim IsInvokable As MethodInfo = GetType(MethodBase).GetMethod(
                    "get_IsDynamicallyInvokable",
                ...
                If (IsInvokable <> Nothing) Then
                    ' AppDomains in a metro process are always Homogenous and fully trusted
                    If (currentDomain.IsHomogenous AndAlso currentDomain.IsFullyTrusted) Then
                        isInvokableDelegate = DirectCast(IsInvokable.CreateDelegate(GetType(Func(Of MethodBase, Boolean))), Func(Of MethodBase, Boolean))
                    End If
                End If

                Return isInvokableDelegate
            End Function
            ...
            Private Shared Function IsMethodDynamicallyInvokable(Method As MethodBase) As Boolean
                ' If MethodBase.IsDynamicallyInvokable doesn't exist we should fall back to the default behavior: everything is invokable.
                Return s_IsInvokableDelegate = Nothing OrElse
                    s_IsInvokableDelegate(Method)
            End Function
            ...
            Private Shared Function FilterInvalidMembers(Members As MemberInfo()) As MemberInfo()
                ...
                For MemberIndex = 0 To Members.Length - 1
                    ...
                    Dim IsDynamicallyInvokable As Boolean = True
                    ...
                    Select Case Members(MemberIndex).MemberType
                        ...
                        Case MemberTypes.Constructor, _
                            ...
                            IsDynamicallyInvokable = IsMethodDynamicallyInvokable(CurrentMethod)
                    ...
                    If AreParametersAndReturnTypesValid(Parameters, ReturnType) AndAlso IsDynamicallyInvokable Then
                        ValidMemberCount += 1
                    Else
...
#End If
        ...
        Friend NotInheritable Class Method
            ...
            Friend UsedDefaultForAnOptionalParameter As Boolean 'Indicates whether the argument matching task (CanMatchArguments) ran into an Optional Parameter without explicitly provided value.
            ...
            Friend Function BindGenericArguments() As Boolean
                ...
                'for this method.
                '
                'ISSUE cambecc (3/4/2004): Constructing the generic binding using Reflection peforms
