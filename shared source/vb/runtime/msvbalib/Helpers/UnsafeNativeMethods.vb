...
'* CALLING NOTES:
'*
'* Beware of garbage collection issues when calling these PINVOKES .  For instance,
'* calling foo(x as IntPtr).  If you pass a managed object that has a finalizer
'* in it as x, e.g. foo(SomeSafeHandle.GetDangerousHandle()), you want to make sure
'* that SomeSafeHandle is referenced after the call to foo().  The reason is that
'* SomeSafeHandle has a finalizer in it that closes the handle.  The moment
'* SomeSafeHandle is passed, it becomes eligible for garbage collection if it
'* isn't referenced somewhere after the call to foo().  This means you could
'* get in a race condition where foo() is executing and the handle behind
'* SomeSafeHandle gets closed because the finalizer runs.  This would lead to
'* intermittant strange and bizarre crashes.  One way to deal with this is to
'* use the HandleRef class.  It gaurantees that your managed object will stay
'* alive until the platform invoke returns.  If that doesn't work for you then
'* make sure you are keeping the object alive somehow after the call to foo.
'* Things like a using block involving SomeSafeHandle would do it, gc.KeepAlive(),
'* directly using SomeSafeHandle afterwards in a way that the JITter can't optimize
'* away.  
'* You can read up on HandleRef and some of the related interop issues with
'* garbage collection at: http://msdn2.microsoft.com/en-us/library/system.runtime.interopservices.handleref.aspx
'* You'll also want to read Chris Brumme's blogs on finalization
'*****************************************************************************/
...
Namespace Microsoft.VisualBasic.CompilerServices
    ...
    Friend NotInheritable Class UnsafeNativeMethods
        ...
        ' REVIEW : jimcan - c# version was class, does it make a difference?
        ...
        Public Interface IDispatch
            ...
            ' WARNING : jimcan - This api NOT COMPLETELY DEFINED, DO NOT CALL!
            <PreserveSig()> _
            <SecurityCritical()> _
