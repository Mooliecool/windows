...
'*
'* Purpose:
'*  Class that represent a VB file
'*
'*****************************************************************************/
...
#Region " BACKWARDS COMPATIBILITY "
            ...
            Else
                Select Case Type.GetTypeCode(FieldType)
                    ...
                    Case Else 'Case TypeCode.Object
                        ...
                        Else
                            Throw VbMakeException(New ArgumentException(GetResourceString(ResID.Argument_UnsupportedFieldType2, field_info.Name, FieldType.Name)), vbErrors.IllegalFuncCall)
                        End If
                End Select
            End If
    ...
    Friend MustInherit Class VB6File
        ...
        Protected Sub New(ByVal sPath As String, ByVal access As OpenAccess, ByVal share As OpenShare, ByVal lRecordLen As Integer)
            ...
            If access <> OpenAccess.Read AndAlso _
            ...
            If (share <> OpenShare.Shared AndAlso _
        ...
        Friend Overridable Sub OpenFile()
            Try
                ...
                '        If e1.ErrorCode = Win32Api.ERROR_SHARING_VIOLATION Then
            ...
            Catch e2 As SecurityException
                ...
                '    Catch e3 As IOException
                '        If e3.ErrorCode = Win32Api.ERROR_NOT_READY Then
                '            Throw VbMakeException(vbErrors.DiskNotReady)
                '        Else
                '            Throw VbMakeException(vbErrors.PathFileAccess)
                '            'Throw VbMakeException(vbErrors.PathNotFound)
                '        End If
        ...
        Friend Sub SetColumn(ByVal lColumn As Integer)
            If m_lWidth <> 0 AndAlso m_lCurrentColumn <> 0 AndAlso _
        ...
        Friend Overridable Function LOC() As Long
            ...
            If m_lRecordLen = 0 Then
                'Debug.Assert "HOW DID WE GET HERE"
        ...
        Friend Overridable Overloads Sub Lock()
            ...
            m_file.Lock(0, Int32.MaxValue)      'Win98 doesn't handle Int64
        End Sub
...
NewLine:
        ...
        Friend Sub SetPrintMode()
            ...
            If mode = OpenMode.Input OrElse _
                mode = OpenMode.Binary OrElse _
        ...
        Friend Sub PutFixedArray(ByVal RecordNumber As Long, ByVal arr As System.Array, ByVal ElementType As System.Type, _
            Optional ByVal FixedStringLength As Integer = -1, Optional ByVal FirstBound As Integer = -1, _
        ...
        Friend Sub PutDynamicArray(ByVal RecordNumber As Long, ByVal arr As System.Array, _
            ...
            ElseIf cDims <> 0 Then
                Throw New ArgumentException(GetResourceString(ResID.Argument_UnsupportedArrayDimensions))
            End If
        ...
        Friend Sub PutFixedLengthString(ByVal RecordNumber As Long, ByVal s As String, ByVal lengthToWrite As Integer)
            ...
            If ByteLength > lengthToWrite Then
                ...
                Else
                    ...
                    'VSWhidbey 521192: The assumption that if cuts off half a DBCS character, that character is replaced with Chr(0)
                    '   is nolonger true. See VSWhidbey 521213.
                    Dim Bytes() As Byte = m_Encoding.GetBytes(s)
                    s = m_Encoding.GetString(Bytes, 0, lengthToWrite)

                    Diagnostics.Debug.Assert(Not m_Encoding Is Nothing)
                    ByteLength = m_Encoding.GetByteCount(s)
                    'VSWhidbey 521192: In case of cutting off half a DBCS character, the encoding may return a default DBCS character,
                    '   making ByteLenght > Length. Replace the ending bytes with 0 until we got back a string with correct byte count.
                    If ByteLength > lengthToWrite Then
            ...
            SetRecord(RecordNumber)
            ' CONSIDER: We should call LengthCheck against m_ae.GetByteCount(s) instead of asserting as above?
        ...
        Friend Sub GetFixedArray(ByVal RecordNumber As Long, ByRef arr As System.Array, _
        ByVal FieldType As System.Type, Optional ByVal FirstBound As Integer = -1, _
        ...
        Friend Function GetArrayDesc(ByVal typ As System.Type) As System.Array
            ...
            ' whidbey 43829 - consider support for fixedstring attributes
        ...
        Friend Sub PutArrayData(ByVal arr As System.Array, ByVal typ As System.Type, ByVal FixedStringLength As Integer, _
            ...
            ElseIf (arr.GetUpperBound(0) > FirstBound) Then
                Throw New ArgumentException(GetResourceString(ResID.Argument_ArrayDimensionsDontMatch))
            End If
            ...
            If vtype = VT.String Then
                ...
                End If
                If FixedStringLength = 0 Then
                    Throw New ArgumentException(GetResourceString(ResID.Argument_InvalidFixedLengthString))
            ...
            ' VSWhidbey 32415: Improve performance for FilePut.
                    ...
                    Select Case vtype
                        ...
                        Case VT.String
                            ...
                            If ByteLength > System.Int16.MaxValue Then
                                ...
                                Throw VbMakeException(New ArgumentException(GetResourceString(ResID.FileIO_StringLengthExceeded)), vbErrors.IllegalFuncCall)
                            End If
        ...
        Friend Sub GetArrayData(ByVal arr As System.Array, ByVal typ As System.Type, Optional ByVal FirstBound As Integer = -1, _
            Optional ByVal SecondBound As Integer = -1, Optional ByVal FixedStringLength As Integer = -1)
            ...
            If arr Is Nothing Then
                Throw New ArgumentException(GetResourceString(ResID.Argument_ArrayNotInitialized))
            End If
                    ...
                    Select Case Type.GetTypeCode(typ)
                        ...
                        Case Else
                            ...
                            ElseIf typ Is GetType(System.Reflection.Missing) Then
                                s = "Error 448"
                            Else
                                Throw New ArgumentException(GetResourceString(ResID.Argument_UnsupportedIOType1, VBFriendlyName(typ)))
                            End If
                    End Select
                End If
...
#End If
        ...
        Friend Overridable Overloads Sub [Get](ByRef Value As System.Array, Optional ByVal RecordNumber As Long = 0, _
        ...
        Friend Overridable Overloads Sub Put(ByVal Value As System.Array, Optional ByVal RecordNumber As Long = 0, _
            Optional ByVal ArrayIsDynamic As Boolean = False, Optional ByVal StringIsFixedLength As Boolean = False)
...
SkipWhiteSpaceExit:
        ...
        'twhitney: The deal here is that this function was moved from vb6randomfile.vb to fix VSWhidbey 32408.  The problem is, it introduced a breaking change.  The bug this fix
        ...
        'what is going on.  But that is a breaking change from Everett behavior.  So I'm taking his function, throwing the original NullReference exception, but putting Huy's better
        ...
        'huyn: ValidateWriteable is not used anywhere - FxCop violation.
        'Private Sub ValidateWriteable()
        ...
        '        Throw New NullReferenceException(JustNeedTheMessage.Message, New IO.IOException(GetResourceString(ResID.FileOpenedNoWrite)))
        '    End If
        'End Sub
