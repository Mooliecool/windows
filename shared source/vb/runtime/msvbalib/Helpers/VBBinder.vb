...
#Region " BACKWARDS COMPATIBILITY "
    ...
    Friend NotInheritable Class VBBinder
        ...
        Private Sub ThrowInvalidCast(ByVal ArgType As System.Type, ByVal ParmType As System.Type, ByVal ParmIndex As Integer)
            Throw New InvalidCastException(GetResourceString(ResID.InvalidCast_FromToArg4, CalledMethodName(), CStr(ParmIndex + 1), VBFriendlyName(ArgType), VBFriendlyName(ParmType)))
        End Sub
...
NextMethod8:
            ...
            For ParmIndex = 0 To Math.Min(ArgCountUpperBound, Math.Max(LastNonSetValueIndex0, LastNonSetValueIndex1))
                ...
                ElseIf (ParamArrayExpanded1 AndAlso ParamArrayIndex1 <> PARAMARRAY_NONE AndAlso ParmIndex >= ParamArrayIndex1) Then
                    ...
                    If ParamArrayElementType1 Is Type0 Then
                        'Identical
                        If MostSpecific = -1 AndAlso ParamArrayIndex0 = PARAMARRAY_NONE AndAlso ParmIndex = LastNonSetValueIndex0 AndAlso _
                ...
                ElseIf (ParamArrayExpanded0 AndAlso ParamArrayIndex0 <> PARAMARRAY_NONE AndAlso ParmIndex >= ParamArrayIndex0) Then
                    ...
                    If ParamArrayElementType0 Is Type1 Then
                        'Identical
                        If MostSpecific = -1 AndAlso ParamArrayIndex1 = PARAMARRAY_NONE AndAlso ParmIndex = LastNonSetValueIndex1 AndAlso _
        ...
        Public Overrides Function BindToField(ByVal bindingAttr As BindingFlags, ByVal match() As FieldInfo, ByVal value As Object, ByVal culture As CultureInfo) As FieldInfo
            ...
            If (Not m_CachedMember Is Nothing) AndAlso (m_CachedMember.MemberType = MemberTypes.Field) AndAlso (Not match(0) Is Nothing) AndAlso _
                (match(0).Name = m_CachedMember.Name) Then
        ...
        '/**
        ' * Given a of propreties that match the base criteria, select one.
        ' */
        ...
        Public Overrides Function ChangeType(ByVal value As Object, ByVal typ As Type, ByVal culture As CultureInfo) As Object
            ...
            Catch ex As Exception
                Throw New InvalidCastException(GetResourceString(ResID.InvalidCast_FromTo, VBFriendlyName(value), VBFriendlyName(typ)))
            End Try
        End Function
        ...
        Private Function BindingScore(ByVal Parameters() As ParameterInfo, ByVal paramOrder() As Integer, ByVal ArgTypes() As Type, ByVal IsPropertySet As Boolean, ByVal ParamArrayIndex As Integer) As BindScore
            ...
            For ParmIndex = 0 To Math.Max(LastArgNonSetValueIndex, LastParamNonSetValueIndex)
                ...
                Else
                    ...
                    ElseIf ParmIndex = ParamArrayIndex AndAlso ArgType.IsArray() AndAlso _
                    ...
                    Else
                        ...
                        ElseIf ArgType.IsArray() AndAlso _
                            (m_state.m_OriginalArgs Is Nothing OrElse m_state.m_OriginalArgs(ArgIndex) Is Nothing OrElse ParmType.IsInstanceOfType(m_state.m_OriginalArgs(ArgIndex))) Then
                            If Score < BindScore.Widening1 Then
                                Score = BindScore.Widening1
                            End If
        ...
        Private Function CreateParamOrder(ByVal SetProp As Boolean, ByVal paramOrder() As Integer, ByVal pars() As ParameterInfo, ByVal args() As Object, ByVal names() As String) As Exception
            ...
            For i = 0 To names.GetUpperBound(0)
                ...
                For j = 0 To LastNonSetIndex
                    ...
                    If StrComp(names(i), pars(j).Name, CompareMethod.Text) = 0 Then
                        ...
                        If paramOrder(j) <> -1 Then
                            Return New ArgumentException(GetResourceString(ResID.NamedArgumentAlreadyUsed1, pars(j).Name))
                        End If
                ...
                If (j > LastNonSetIndex) Then
                    ...
                    Return New MissingMemberException(GetResourceString(ResID.Argument_InvalidNamedArg2, names(i), CalledMethodName()))
                End If
        ...
        Friend Function InvokeMember(ByVal name As String, _
            ByVal invokeAttr As BindingFlags, _
            ByVal objType As System.Type, _
            ByVal objIReflect As IReflect, _
            ...
            If objType.IsCOMObject() Then
                ...
                If (Not m_ByRefFlags Is Nothing) AndAlso (Not target Is Nothing) AndAlso _
                    (Not System.Runtime.Remoting.RemotingServices.IsTransparentProxy(target)) Then

                    Dim pmTemp As Reflection.ParameterModifier = New Reflection.ParameterModifier(args.Length)
                    modifiers = New Reflection.ParameterModifier() {pmTemp}
                    'Set all flags to ByRef
                    Dim MissingValue As Object = System.Reflection.Missing.Value
                    For i = 0 To args.GetUpperBound(0)
                        If args(i) Is MissingValue Then
                            'Missing type, don't set as byref
                        Else
                            pmTemp.Item(i) = m_ByRefFlags(i)
                        End If
                    Next i
                End If
                ...
                Catch ex As MissingMemberException
                    Throw New MissingMemberException(GetResourceString(ResID.MissingMember_MemberNotFoundOnType2, name, VBFriendlyName(objType)))
                End Try
            End If
...
#If DEBUG Then
            If (Not namedParameters Is Nothing) Then
                For i = 0 To namedParameters.GetUpperBound(0)
...
#End If
            ...
            If (name.Length = 0) Then
                If (objType Is objIReflect) Then
                    ...
                    If (name Is Nothing) Then
                        Throw New MissingMemberException(GetResourceString(ResID.MissingMember_NoDefaultMemberFound1, VBFriendlyName(objType)))
                    End If
                Else
            ...
            If (invokeMethod Is Nothing) Then
                Throw New MissingMemberException(GetResourceString(ResID.NoMethodTakingXArguments2, CalledMethodName(), CStr(GetPropArgCount(args, (invokeAttr And BindingFlags.SetProperty) <> 0))))
            End If
        ...
        Private Function GetMethodsByName(ByVal objType As System.Type, ByVal objIReflect As IReflect, ByVal name As String, ByVal invokeAttr As BindingFlags) As MethodBase()
            ...
            For MemberIndex = 0 To mi.GetUpperBound(0)
                ...
                ElseIf ThisMember.MemberType = MemberTypes.Method Then
                    ...
                    ElseIf Not ThisMethod.IsVirtual OrElse _
                        (ThisMethod.IsVirtual AndAlso ((ThisMethod.Attributes And MethodAttributes.NewSlot) <> 0)) OrElse _
                ...
                ElseIf ThisMember.MemberType = MemberTypes.NestedType Then
                    ...
                    If RemovedCount = mi.Length - 1 Then
                        Throw New ArgumentException(GetResourceString(ResID.Argument_IllegalNestedType2, name, VBFriendlyName(objType)))
                    End If
