...
Namespace Microsoft.VisualBasic.MyServices.Internal
    ...
    Friend Class WebClientCopy
        ...
        ''' <remarks></remarks>
        Public Sub New(ByVal client As WebClient, ByVal dialog As ProgressDialog)
        ...
        Public Sub DownloadFile(ByVal address As Uri, ByVal destinationFileName As String)
            ...
            Debug.Assert(destinationFileName <> "" AndAlso Directory.Exists(Path.GetDirectoryName(Path.GetFullPath(destinationFileName))), "Invalid path")
            ...
            If m_ProgressDialog IsNot Nothing Then
                ...
                m_ProgressDialog.ShowProgressDialog() 'returns when the download sequence is over, whether due to success, error, or being cancelled
            Else
            ...
            If m_ExceptionEncounteredDuringFileTransfer IsNot Nothing Then
                If m_ProgressDialog Is Nothing OrElse Not m_ProgressDialog.UserCanceledTheDialog Then
                    Throw m_ExceptionEncounteredDuringFileTransfer
                End If
            End If
        ...
        Public Sub UploadFile(ByVal sourceFileName As String, ByVal address As Uri)
            Debug.Assert(m_WebClient IsNot Nothing, "No WebClient")
            Debug.Assert(address IsNot Nothing, "No address")
            Debug.Assert(sourceFileName <> "" AndAlso File.Exists(sourceFileName), "Invalid file")
        ...
        ''' <remarks></remarks>
        Private Sub InvokeIncrement(ByVal progressPercentage As Integer)
            ...
            If m_ProgressDialog IsNot Nothing Then
                If m_ProgressDialog.IsHandleCreated Then
                    ...
                    Dim increment As Integer = progressPercentage - m_Percentage
        ...
        '''********************************************************************
        ''';m_WebClient_DownloadFileCompleted
        ''' <summary>
        ...
        Private Sub m_WebClient_DownloadFileCompleted(ByVal sender As Object, ByVal e As System.ComponentModel.AsyncCompletedEventArgs) Handles m_WebClient.DownloadFileCompleted
            Try
                ' If the download was interupted by an exception, keep track of the exception, which we'll throw from the main thread
                If e.Error IsNot Nothing Then
                    m_ExceptionEncounteredDuringFileTransfer = e.Error
                End If
                ...
                End If
            Finally
                'We don't close the dialog until we recieve the WebClient.DownloadFileCompleted event 
                CloseProgressDialog()
            End Try
        End Sub

        '''********************************************************************
        ''';m_WebClient_DownloadProgressChanged
        ''' <summary>
        ...
        ''' <param name="e"></param>
        ''' <remarks></remarks>
        Private Sub m_WebClient_DownloadProgressChanged(ByVal sender As Object, ByVal e As System.Net.DownloadProgressChangedEventArgs) Handles m_WebClient.DownloadProgressChanged
            InvokeIncrement(e.ProgressPercentage)
        End Sub

        '''********************************************************************
        ''';m_WebClient_UploadFileCompleted
        ''' <summary>
        ...
        ''' <param name="e"></param>
        ''' <remarks></remarks>
        Private Sub m_WebClient_UploadFileCompleted(ByVal sender As Object, ByVal e As System.Net.UploadFileCompletedEventArgs) Handles m_WebClient.UploadFileCompleted

            ' If the upload was interupted by an exception, keep track of the exception, which we'll throw from the main thread
            Try
                If e.Error IsNot Nothing Then
                    m_ExceptionEncounteredDuringFileTransfer = e.Error
                End If
        ...
        '**********************************************************************
        ''';m_WebClient_UploadProgressChanged
        ''' <summary>
        ...
        ''' <param name="e"></param>
        ''' <remarks></remarks>
        Private Sub m_WebClient_UploadProgressChanged(ByVal sender As Object, ByVal e As System.Net.UploadProgressChangedEventArgs) Handles m_WebClient.UploadProgressChanged
            Dim increment As Long = (e.BytesSent * 100) \ e.TotalBytesToSend
            InvokeIncrement(CInt(increment))
        End Sub

        '''*********************************************************************
        ''';m_ProgressDialog_UserCancelledEvent
        ''' <summary>
        ...
        'Keeps track of the error that happend during upload/download so we can throw it once we can guarantee we are back on the main thread
        Private m_ExceptionEncounteredDuringFileTransfer As Exception

        ' Used for invoking ProgressDialog.Increment
        Private Delegate Sub DoIncrement(ByVal Increment As Integer)
