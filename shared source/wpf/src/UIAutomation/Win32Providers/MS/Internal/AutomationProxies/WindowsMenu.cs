//---------------------------------------------------------------------------
//
// <copyright file="WindowsMenu.cs" company="Microsoft">
...
namespace MS.Internal.AutomationProxies
{
    ...
    class WindowsMenu: ProxyHwnd
    {
            ...
            : base (hwnd, parent, item)
        {
            ...
            // PS #1509982 (Vista Explorer FolerBandModuleInner menu items don't raise InvokedEvent).  During events, this ctor is
        ...
        internal override string LocalizedName
        {
            get
            {
                ...
                {
                    return ST.Get(GetLocalizedNameFromType());
                }
            }
        }
        #endregion
        ...
        //Returns the STID of the resource name string
        private STID GetLocalizedNameFromType()
        {
            switch (_type)
            {
                case MenuType.Toplevel:
                    return STID.LocalizedNameWindowsMenuBar;

                case MenuType.System:
                    return STID.LocalizedNameWindowsSystemMenuBar;

                case MenuType.SystemPopup:
                    return STID.LocalizedNameWindowsMenu;
...
#pragma warning suppress 6503
            ...
            internal override string LocalizedName
            {
                get
                {
                    ...
                    if (_menuType == WindowsMenu.MenuType.System)
                    {
                        return ST.Get(STID.LocalizedNameWindowsSystemMenuItem);
                    }
                    ...
                    string keyCtrl = ST.Get(STID.KeyCtrl);
                    string keyControl = ST.Get(STID.KeyControl);
                    string keyAlt = ST.Get(STID.KeyAlt);
                    string keyShift = ST.Get(STID.KeyShift);
                    string keyWin = ST.Get(STID.KeyWinKey);
            ...
            internal override string GetAccessKey()
            {
                ...
                switch (type)
                {
                    case MenuType.System:
                        {
                            return ST.Get(STID.KeyAlt) + " + " + ST.Get(STID.KeySpace);
                        }
                    case MenuType.Submenu:
                    case MenuType.SystemPopup:
                        {
                    ...
                    else
                    {
                        ...
                        if (Misc.GetMenuItemInfo(_hmenu, _item, true, ref menuItemInfo))
                        {
                            if (Misc.IsBitSet(menuItemInfo.fType, NativeMethods.MF_SEPARATOR) ||
                                ...
                                Misc.IsBitSet(menuItemInfo.fType, NativeMethods.MF_MENUBREAK))
                            {
                                return ST.Get(STID.LocalizedNameWindowsMenuSeparator);
                            }
