...
* Module Name:
*
*   Generic Text Print Render Target (RT)
*
* Created:
*
*   2/27/2003 [....]
...
/// <SecurityNote>
///     Critical    : This code calls critical SHGetSpecialFolderPathW function to get font directory
/// </SecurityNote>
...
/// <SecurityNote>
/// Critical - asserts registry permissions to read physical font list from LocalMachine.
/// we only read a value from LocalMachine. Info is stored in m_InstalledFonts list, but it's not exposed outside of this class.
/// </SecurityNote>
[SecurityCritical]
Hashtable^ BuildFontList(String ^ fontdir)
{
    ...
    RegistryPermission^ permission = gcnew RegistryPermission(
        RegistryPermissionAccess::Read,
        "HKEY_LOCAL_MACHINE\\Software\\Microsoft\\Windows NT\\CurrentVersion\\Fonts");

    permission->Assert(); // Blessed assert

    try
    {
        RegistryKey^ key = Registry::LocalMachine->OpenSubKey("Software\\Microsoft\\Windows NT\\CurrentVersion\\Fonts");

        // Permission is needed whenever key is accessed, so we can't move the look out of try block
